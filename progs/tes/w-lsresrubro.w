...
DEFINE VARIABLE h_v-valoresdetalle AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     "   Indique si desea el listado consolidado o solo la empresa actual" VIEW-AS TEXT
          SIZE 100 BY .86 AT ROW 10.67 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "   Indique la caja cuyos movimientos desean listarse" VIEW-AS TEXT
          SIZE 100 BY .86 AT ROW 1.81 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "   Indique el rango de fechas de los movimientos" VIEW-AS TEXT
          SIZE 100 BY .86 AT ROW 6.14 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Saldos por Rubro de Caja"
         HEIGHT             = 22.33
         WIDTH              = 128
         MAX-HEIGHT         = 22.33
         MAX-WIDTH          = 158.2
...
ON END-ERROR OF W-Win /* Saldos por Rubro de Caja */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Saldos por Rubro de Caja */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-consolidado ( 11.86 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 42.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-valoresdetalle.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-valoresdetalle ).
       RUN set-position IN h_v-valoresdetalle ( 11.86 , 70.60 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 42.00 ) */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-valoresdetalle ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/TES" "Saldos por Rubro de Caja"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE detalle          AS LOGICAL NO-UNDO.
  ...
  END.
    ...
    RUN dar_rango IN h_v-valoresdetalle  ( OUTPUT detalle,
                                      OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Detalle Valores" ).
       hay_error = error_rango.
  END.
  ...
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Ingrese el numero de caja a visualizar" ).
       hay_error = error_rango.
  END.
