...
&Scoped-Define ENABLED-OBJECTS v-has_fecha v-fecha_acreditacion ~
v-cantcheques v-totdeposito v-des_fecha btn_acreditar btn_rechazar ~
btn_seltodos btn_desmarcar btn_verlog btn_listar btn_comprobante br_table ~
RECT-2 RECT-3 RECT-4 RECT-5 
&Scoped-Define DISPLAYED-OBJECTS v-has_fecha v-fecha_acreditacion ~
v-cantcheques v-totdeposito v-des_fecha 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 119 BY 15.1
...
DEFINE FRAME F-Main
     v-has_fecha AT ROW 1.24 COL 42 COLON-ALIGNED
     v-fecha_acreditacion AT ROW 1.24 COL 71 COLON-ALIGNED
     v-cantcheques AT ROW 1.24 COL 95 COLON-ALIGNED
     v-totdeposito AT ROW 1.24 COL 103 COLON-ALIGNED
     v-des_fecha AT ROW 1.29 COL 19 COLON-ALIGNED
     btn_acreditar AT ROW 3.14 COL 2
     btn_rechazar AT ROW 3.14 COL 19
     btn_seltodos AT ROW 3.14 COL 37
     btn_desmarcar AT ROW 3.14 COL 54
     btn_verlog AT ROW 3.14 COL 71
     btn_listar AT ROW 3.14 COL 88
     btn_comprobante AT ROW 3.14 COL 105
     br_table AT ROW 5.05 COL 2
     RECT-2 AT ROW 1 COL 1
     RECT-3 AT ROW 2.86 COL 1
     RECT-4 AT ROW 2.86 COL 36
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "sic.Valor OF sic.Caja"
...
"Valor.observacion" "Observación!Cheque" "X(55)" "character" ? ? ? ? ? ? no ? no no "57.8" yes no no "U" "" ""
...
DO:
  DO TRANSACTION:
    ...
    THEN DO:
         Valor.user-id-sel = "ACD-" + USERID("sic").
...
DO:
   ...
   sino = NO.
   MESSAGE "Realmente desea registrar la acreditación de los valores seleccionados?" 
...
DO:
   ...
   sino = NO.
   MESSAGE "Realmente desea registrar el RECHAZO de los valores seleccionados?" 
           VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO
           TITLE "Se pide confirmación" UPDATE sino.
   IF sino 
   THEN DO:
...
------------------------------------------------------------------------------*/
    ...
    DO TRANSACTION WITH FRAME frm-listado:
         ...
         tot_importes = 0.
         FOR EACH Valor WHERE Valor.user-id-sel = "ACD-" + USERID("sic") EXCLUSIVE-LOCK,
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME frm-listado:
         ...
         tot_importes = 0.
         FOR EACH Valor WHERE Valor.user-id-sel = "ACD-" + USERID("sic"),
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
    ...
    btn_acreditar:SENSITIVE IN FRAME {&FRAME-NAME} = AVAILABLE Valor.
    btn_rechazar:SENSITIVE IN FRAME {&FRAME-NAME} = AVAILABLE Valor.
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION:
     ...
     FOR EACH B-Valor OF Cuenta_bancaria
         ...
         B-Valor.user-id-sel = "ACD-" + USERID("sic").
         v-cantcheques = v-cantcheques + 1.
...
------------------------------------------------------------------------------*/
    ...
    DO TRANSACTION WITH FRAME frm-listado:
         ...
         fecha_lis = TODAY.
         hora_lis = STRING(TIME,"HH:MM:SS").
         titulo_f = "Valores rechazados el " + STRING(v-fecha_acreditacion,"99/99/99").
