...
DEFINE VARIABLE h_v-par-veresumymov AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     "   Indique el rango de cuentas a listar" VIEW-AS TEXT
          SIZE 89 BY .88 AT ROW 2.88 COL 13
     ...
     "   Indique el rango de fechas de los movimientos" VIEW-AS TEXT
          SIZE 89 BY .88 AT ROW 6.65 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Detalle de Movimientos por Cuenta"
         HEIGHT             = 22.31
         WIDTH              = 114.29
         MAX-HEIGHT         = 22.35
         MAX-WIDTH          = 127.14
...
ON END-ERROR OF W-Win /* Detalle de Movimientos por Cuenta */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Detalle de Movimientos por Cuenta */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-cuenta ( 3.96 , 13.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 7.73 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-par-veresumymov.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-par-veresumymov ).
       RUN set-position IN h_v-par-veresumymov ( 11.77 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-par-veresumymov ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/TES" "Detalle de Movimientos por Cuenta Contable"}
...
------------------------------------------------------------------------------*/
  ...
  RUN dar_rango IN h_v-par-veresumymov  ( OUTPUT ver_movim,
                                          OUTPUT ver_resum,
                                          OUTPUT todos_mov,
                                          OUTPUT consolidado,
                                          OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Parametros de Listado" ).
       hay_error = error_rango.
  END.
  ...
  THEN DO:
    ...
    RUN lsmovcta.p ( INPUT  des_fecha,
                     INPUT  has_fecha,
                     INPUT  des_cuenta,  
                     INPUT  has_cuenta,  
                     INPUT  ver_movim,
