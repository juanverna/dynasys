...
DEFINE VARIABLE h_b-valoresxempresa AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-valor AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-id-empresa ( 1.00 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 1.00 , 54.00 ) NO-ERROR.
       RUN set-size IN h_p-navico ( 1.35 , 59.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-valoresxempresa.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-valoresxempresa ).
       RUN set-position IN h_b-valoresxempresa ( 2.62 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-valoresxempresa ( 18.31 , 14.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-valor ).
       RUN set-position IN h_v-valor ( 2.62 , 17.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.96 , 96.00 ) */
       ...
       RUN set-position IN h_p-soloedita ( 12.85 , 17.00 ) NO-ERROR.
       RUN set-size IN h_p-soloedita ( 1.62 , 96.00 ) NO-ERROR.
       ...
       RUN set-position IN h_b-caja_valor ( 14.73 , 17.00 ) NO-ERROR.
       RUN set-size IN h_b-caja_valor ( 6.19 , 96.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-empresa ).
       RUN set-position IN h_q-empresa ( 2.88 , 52.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.35 , 8.29 ) */

       /* Links to SmartViewer h_v-id-empresa. */
       RUN add-link IN adm-broker-hdl ( h_q-empresa , 'Record':U , h_v-id-empresa ).

       /* Links to SmartBrowser h_b-valoresxempresa. */
       RUN add-link IN adm-broker-hdl ( h_q-empresa , 'Record':U , h_b-valoresxempresa ).

       /* Links to SmartViewer h_v-valor. */
       RUN add-link IN adm-broker-hdl ( h_b-valoresxempresa , 'Record':U , h_v-valor ).
       RUN add-link IN adm-broker-hdl ( h_p-soloedita , 'TableIO':U , h_v-valor ).

       /* Links to SmartBrowser h_b-caja_valor. */
       RUN add-link IN adm-broker-hdl ( h_b-valoresxempresa , 'Record':U , h_b-caja_valor ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-valoresxempresa ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-valor ,
             h_b-valoresxempresa , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/TES" "Consulta de Valores en Cartera"}
