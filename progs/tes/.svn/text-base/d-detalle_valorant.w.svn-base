...
T-Cheque.cdg_cuenta_ban T-Valor.numero_cheque T-Valor.dias_clearing ~
T-Cheque.numero_cheque T-Valor.fecha_emision T-Valor.estado ~
...
T-Valor.numero_cheque T-Valor.dias_clearing T-Cheque.numero_cheque ~
T-Valor.fecha_emision T-Valor.estado T-Cheque.fecha_emision ~
...
T-Valor.dias_clearing T-Cheque.numero_cheque T-Valor.fecha_emision ~
...
&Scoped-Define ENABLED-OBJECTS Btn_OK Btn_Cancel RECT-13 RECT-14 RECT-15 ~
RECT-16 RECT-17 RECT-18 RECT-19 
...
"Egreso", "E":U
     ...
     T-Caj_detalle.importe AT ROW 5.52 COL 14 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 23 BY 1
          BGCOLOR 15 FGCOLOR 9 
     T-Caj_detalle.cambio AT ROW 5.52 COL 70 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 18 BY 1
          BGCOLOR 15 FGCOLOR 9 
     T-Caj_detalle.divisas AT ROW 5.52 COL 97 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 17 BY 1
          BGCOLOR 15 FGCOLOR 9 
     ...
     T-Valor.numero_cheque AT ROW 9.33 COL 14 COLON-ALIGNED
          ...
          SIZE 16 BY 1
          BGCOLOR 15 FGCOLOR 9 
     T-Valor.dias_clearing AT ROW 9.33 COL 41 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 16 BY 1
          BGCOLOR 15 FGCOLOR 9 
     ...
     T-Valor.fecha_emision AT ROW 10.52 COL 14 COLON-ALIGNED
          ...
          SIZE 16 BY 1
          BGCOLOR 15 FGCOLOR 9 
     T-Valor.estado AT ROW 10.52 COL 41 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 16 BY 1
          BGCOLOR 15 FGCOLOR 9 
     T-Cheque.fecha_emision AT ROW 10.52 COL 69.8 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 16 BY 1
          BGCOLOR 15 FGCOLOR 9 
     ...
     T-Valor.fecha_deposito AT ROW 11.71 COL 14 COLON-ALIGNED
          ...
          SIZE 16 BY 1
          BGCOLOR 15 FGCOLOR 9 
     T-Valor.fecha_acredita AT ROW 11.71 COL 41 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 16 BY 1
          BGCOLOR 15 FGCOLOR 9 
...
DEFINE FRAME Dialog-Frame
     ...
     Btn_Cancel AT ROW 13.38 COL 101
            ...
            THEN DO:
               ...
               THEN DO:
                    ...
                    ASSIGN T-Valor.cdg_empresa     = T-Caj_header.cdg_empresa
                           ...
                           T-Valor.importe         = T-Caj_detalle.importe.
               END.
...
ON LEAVE OF T-Valor.fecha_emision IN FRAME Dialog-Frame /* Emitido */
DO:
...
DO:
   ...
   ELSE DO:
      FIND Valor WHERE Valor.cdg_banco      = INPUT T-Valor.cdg_banco 
                   AND Valor.cdg_sucurbanco = INPUT T-Valor.cdg_sucurbanco 
                   AND Valor.numero_cheque  = INPUT T-Valor.numero_cheque NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE T-Valor THEN 
    DISPLAY T-Valor.cdg_banco T-Valor.cdg_sucurbanco T-Valor.numero_cheque 
          T-Valor.dias_clearing T-Valor.fecha_emision T-Valor.estado 
          T-Valor.fecha_deposito T-Valor.fecha_acredita 
      WITH FRAME Dialog-Frame.
  ENABLE T-Caj_detalle.observacion T-Caj_detalle.importe T-Caj_detalle.cambio 
         T-Caj_detalle.divisas T-Valor.cdg_banco T-Cheque.cdg_cuenta_ban 
         T-Valor.numero_cheque T-Valor.dias_clearing T-Cheque.numero_cheque 
         T-Valor.fecha_emision T-Valor.estado T-Cheque.fecha_emision 
         T-Cheque.dias_clearing T-Valor.fecha_deposito T-Valor.fecha_acredita 
         T-Cheque.fecha_deposito T-Cheque.fecha_acredita Btn_OK Btn_Cancel 
         RECT-13 RECT-14 RECT-15 RECT-16 RECT-17 RECT-18 RECT-19 
      WITH FRAME Dialog-Frame.
         ...
         CASE p-modo-detalle:
            ...
            THEN DO:
                ASSIGN
                     ...
                     T-Valor.fecha_acredita:SENSITIVE            = NO
                     T-Valor.estado:SENSITIVE                    = NO
                     T-Cheque.cdg_cuenta_ban:SENSITIVE           = Rubro.tipo = "P"
                     T-Cheque.numero_cheque:SENSITIVE            = NO
...
------------------------------------------------------------------------------*/
   ...
   DISPLAY 2 @ T-Cheque.dias_clearing
       WITH FRAME {&FRAME-NAME}.
...
------------------------------------------------------------------------------*/
   ...
   IF INPUT FRAME {&FRAME-NAME} T-Caj_detalle.importe = 0
   THEN DO:
      RUN PONMENSJ.P (INPUT "CAJA003").
      RETURN.
   END.   

   IF T-Cheque.cdg_cuenta_ban:SENSITIVE IN FRAME {&FRAME-NAME}
   THEN DO:
        IF NOT CAN-FIND(Cuenta_bancaria WHERE Cuenta_bancaria.cdg_cuenta_ban = 
                                    INPUT FRAME {&FRAME-NAME} T-Cheque.cdg_cuenta_ban)
   ...
   CASE Rubro.tipo:
     ...
     WHEN "V" THEN DO:
        ...
        IF NOT CAN-FIND(Banco WHERE Banco.cdg_banco = 
                                     INPUT FRAME {&FRAME-NAME} T-Valor.cdg_banco)
        ...
        THEN DO:
            ...
            IF CAN-FIND(T-Valor WHERE T-Valor.cdg_banco      = INPUT FRAME {&FRAME-NAME} T-Valor.cdg_banco 
                                  AND T-Valor.cdg_sucurbanco = INPUT FRAME {&FRAME-NAME} T-Valor.cdg_sucurbanco
                                  AND T-Valor.numero_cheque  = INPUT FRAME {&FRAME-NAME} T-Valor.numero_cheque)
            THEN DO:
               RUN PONMENSJ.P (INPUT "CAJA006").
               RETURN.
            END.           
            ...
            THEN DO:
                ...
                IF CAN-FIND(Valor WHERE Valor.cdg_banco      = INPUT FRAME {&FRAME-NAME} T-Valor.cdg_banco 
                                    AND Valor.cdg_sucurbanco = INPUT FRAME {&FRAME-NAME} T-Valor.cdg_sucurbanco
                                    AND Valor.numero_cheque  = INPUT FRAME {&FRAME-NAME} T-Valor.numero_cheque)
                THEN DO:
                ...
                IF INPUT FRAME {&FRAME-NAME} T-Valor.fecha_emision = ? 
                THEN DO:
            ...
            END.
            ELSE DO:
                /* en un EGRESO, el valor SI debe existir en la base de datos y no debe haber sido ingresado previamente */
                IF NOT CAN-FIND(Valor WHERE Valor.cdg_banco = INPUT FRAME {&FRAME-NAME} T-Valor.cdg_banco 
                                        AND Valor.cdg_sucurbanco = INPUT FRAME {&FRAME-NAME} T-Valor.cdg_sucurbanco
                                        AND Valor.numero_cheque = INPUT FRAME {&FRAME-NAME} T-Valor.numero_cheque)
                THEN DO:
