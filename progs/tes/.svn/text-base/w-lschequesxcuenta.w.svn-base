...
DEFINE VARIABLE h_v-lista_estadocheque AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-que-cuenta_bancaria AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     "   Indique el estado de los cheques a listar" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 11 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "   Indique la cuenta cuyos movimientos desean listarse" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 2.86 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "   Indique el rango de fechas de los movimientos" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 6.95 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Cheques x Cuenta Bancaria"
         HEIGHT             = 22.33
         WIDTH              = 127.2
         MAX-HEIGHT         = 22.33
         MAX-WIDTH          = 133.2
...
ON END-ERROR OF W-Win /* Cheques x Cuenta Bancaria */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Cheques x Cuenta Bancaria */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-que-cuenta_bancaria.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-que-cuenta_bancaria ).
       RUN set-position IN h_v-que-cuenta_bancaria ( 4.24 , 13.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 8.29 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-lista_estadocheque.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-lista_estadocheque ).
       RUN set-position IN h_v-lista_estadocheque ( 12.19 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-que-cuenta_bancaria ,
             v-estado:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-fechas ,
             h_v-que-cuenta_bancaria , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-lista_estadocheque ,
...
------------------------------------------------------------------------------*/
  ...
  RUN habilitar_botones IN h_p-ejecutar ( INPUT "verlistado=NO,imprimir=NO").
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/TES" "Cheques x Cuenta Bancaria"}
...
------------------------------------------------------------------------------*/
  ...
  RUN dar_rango IN h_v-que-cuenta_bancaria  ( OUTPUT que_cuenta,
                                              OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
  ...
  RUN dar_rango IN h_v-lista_estadocheque   ( OUTPUT v-lista_estados,
                                              OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Estado de Cheques" ).
       hay_error = error_rango.
  END.
  ...
  IF NOT hay_error THEN DO:
     ...
     RUN lschequesxcuenta.p ( INPUT  que_cuenta,
                              INPUT  des_fecha,
                              INPUT  has_fecha,
