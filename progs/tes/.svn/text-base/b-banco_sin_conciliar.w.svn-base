...
&Scoped-Define ENABLED-OBJECTS RECT-9 v-des_fecha v-has_fecha br_table 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Cta_cte_bco.leyenda FORMAT "X(15)":U WIDTH 22.6
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN NO-ROW-MARKERS SIZE 88 BY 17.95
...
DEFINE FRAME F-Main
     ...
     v-totbanco AT ROW 2.67 COL 72 COLON-ALIGNED
     br_table AT ROW 4.33 COL 2
     "                             Movimientos pendientes de conciliar" VIEW-AS TEXT
          SIZE 88 BY 1 AT ROW 1.29 COL 2
...
"Cta_cte_bco.fecha_movimto" "Fecha!Movmto" ? "date" ? ? ? ? ? ? no ? no no "10.2" yes no no "U" "" ""
     _FldNameList[2]   > sic.Cta_cte_bco.tip_comprob
"Cta_cte_bco.tip_comprob" "Ti-!po" ? "character" ? ? ? ? ? ? no ? no no "5.2" yes no no "U" "" ""
...
"Cta_cte_bco.leyenda" ? ? "character" ? ? ? ? ? ? no ? no no "22.6" yes no no "U" "" ""
...
DO:
  DO TRANSACTION:
    FIND CURRENT Cta_cte_bco EXCLUSIVE-LOCK.
    IF Cta_cte_bco.user-id-sel = ""
    THEN DO:
         Cta_cte_bco.user-id-sel = "CON-" + USERID("sic").
         v-totbanco = v-totbanco + ( Cta_cte_bco.credito - Cta_cte_bco.debito ).
  ...
  RUN get-link-handle IN adm-broker-hdl
       (THIS-PROCEDURE, 'DarBanco-Target':U, OUTPUT c).
  IF NUM-ENTRIES (c) eq 1 THEN DO:
    h = WIDGET-HANDLE (c).
    RUN recibir_banco IN h (INPUT v-totbanco).
  END.
...
DO TRANSACTION:
     FOR EACH Cta_cte_bco WHERE Cta_cte_bco.user-id-sel = "CON-" + USERID("sic") EXCLUSIVE-LOCK:
          Cta_cte_bco.user-id-sel = "".
     END.
  END.
