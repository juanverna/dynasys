...
DEFINE FRAME F-Main
     des_registro AT ROW 1.48 COL 17 COLON-ALIGNED
     des_nombre AT ROW 1.48 COL 33 COLON-ALIGNED NO-LABEL
...
DO:
    ...
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND FIRST Cuenta_bancaria 
             WHERE Cuenta_bancaria.denominacion_cta >= SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} 
      /*       AND CAN-DO(Cuenta_bancaria.lista_empresas,Empresa.cdg_empresa)*/
                   USE-INDEX alfabetico NO-LOCK NO-ERROR.
        ELSE FIND FIRST Cuenta_bancaria USE-INDEX alfabetico NO-LOCK NO-ERROR.
    
    IF AVAILABLE Cuenta_bancaria 
    THEN DO:
         des_nombre = Cuenta_bancaria.denominacion_cta.
         des_registro = Cuenta_bancaria.cdg_cuenta_ban.
    END.
    ELSE DO:
         des_nombre = "???".
    END.
...
ON + OF des_registro IN FRAME F-Main /* Cuenta Bancaria */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON MOUSE-MENU-DOWN OF des_registro IN FRAME F-Main /* Cuenta Bancaria */
DO:
  ...
  &SCOPED-DEFINE DSC_TABLA          denominacion_cta
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON RETURN OF des_registro IN FRAME F-Main /* Cuenta Bancaria */
DO:

    ASSIGN FRAME {&FRAME-NAME} des_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND FIRST Cuenta_bancaria WHERE Cuenta_bancaria.cdg_cuenta_ban >= SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
        ELSE FIND FIRST Cuenta_bancaria NO-LOCK NO-ERROR.
