...
DEFINE VARIABLE h_b-valor AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-valor ).
       RUN set-position IN h_v-valor ( 1.48 , 23.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.95 , 115.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-valor ).
       RUN set-position IN h_b-valor ( 1.52 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-valor ( 19.67 , 18.00 ) NO-ERROR.
       ...
       RUN set-position IN h_p-soloedita ( 11.71 , 22.00 ) NO-ERROR.
       RUN set-size IN h_p-soloedita ( 1.91 , 115.00 ) NO-ERROR.
       ...
       RUN set-position IN h_b-caja_valor ( 13.86 , 22.00 ) NO-ERROR.
       RUN set-size IN h_b-caja_valor ( 7.38 , 114.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-valor. */
       RUN add-link IN adm-broker-hdl ( h_b-valor , 'Record':U , h_v-valor ).
       RUN add-link IN adm-broker-hdl ( h_p-soloedita , 'TableIO':U , h_v-valor ).

       /* Links to SmartBrowser h_b-caja_valor. */
       RUN add-link IN adm-broker-hdl ( h_b-valor , 'Record':U , h_b-caja_valor ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-valor ,
             h_v-valor , 'AFTER':U ).
