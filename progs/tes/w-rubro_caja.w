...
DEFINE VARIABLE h_b-rubro AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-rubro AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Rubros de Caja */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Rubros de Caja */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Rubros de Caja */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-rubro ).
       RUN set-position IN h_b-rubro ( 1.48 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-rubro ( 19.52 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Rubros|Comprobantes' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 1.48 , 25.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 19.52 , 109.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rubro ).
       RUN set-position IN h_v-rubro ( 3.38 , 31.00 ) NO-ERROR.
       /* Size in UIB:  ( 14.52 , 89.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 18.05 , 31.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.38 , 89.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-rubro. */
       RUN add-link IN adm-broker-hdl ( h_b-rubro , 'Record':U , h_v-rubro ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-rubro ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rubro ,
             h_folder , 'AFTER':U ).
