...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 6 COL 36 WIDGET-ID 2
     v-estado AT ROW 7.91 COL 11 COLON-ALIGNED NO-LABEL
     "   Indique la caja que desea establecer un cierre" VIEW-AS TEXT
...
ON CHOOSE OF BUTTON-1 IN FRAME F-Main /* Efectuar el cierre */
DO:
  RUN lst-ejecutar.
END.
...
{findempresa.i}
  ...
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Caja inexistente" ).
...
IF NOT hay_error THEN DO:
    ...
    IF can-find( FIRST acumulado_caja WHERE acumulado_caja.cdg_caja = que_caja AND acumulado_caja.cdg_empresa = empresa.cdg_empresa AND
                 acumulado_caja.ano = year(today) AND acumulado_caja.mes = month(today) AND acumulado_caja.cerrado ) THEN DO:
        MESSAGE "Ya existe otro cierre anterior para la misma caja" SKIP
        "quiere eliminarlo y crear uno nuevo" VIEW-AS ALERT-BOX BUTTONS ok-cancel UPDATE hay_error.
        IF hay_error THEN
            FOR EACH acumulado_caja WHERE acumulado_caja.cdg_caja = que_caja 
                     AND acumulado_caja.cdg_empresa = empresa.cdg_empresa AND
                 acumulado_caja.ano = year(today) AND acumulado_caja.mes = month(today)  AND acumulado_caja.cerrado :
                DELETE acumulado_caja.
            END.
        ELSE DO:
            RETURN NO-APPLY.
        END.
    END.
    RUN cierracaja.p( INPUT  que_caja ).

   v-estado:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "Proceso Terminado. Se ha efectuado el cierre de la caja".
