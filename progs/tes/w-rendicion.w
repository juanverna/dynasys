...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rendicion ( 1.24 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.86 , 155.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Comprobantes|Valores' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 4.57 , 24.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 22.14 , 134.00 ) NO-ERROR.
       ...
       RUN set-position IN h_b-rendicion ( 4.81 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-rendicion ( 22.14 , 20.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1':U) NO-ERROR.

       /* Links to SmartViewer h_v-rendicion. */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_v-rendicion , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-rendicion ,
             h_folder , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN set-position IN h_b-comprobante_rendicion ( 6.71 , 26.00 ) NO-ERROR.
       RUN set-size IN h_b-comprobante_rendicion ( 19.52 , 114.20 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-position IN h_b-valores_rendicion ( 6.48 , 27.00 ) NO-ERROR.
       RUN set-size IN h_b-valores_rendicion ( 19.29 , 130.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-valores_rendicion. */
       RUN add-link IN adm-broker-hdl ( h_b-rendicion , 'Record':U , h_b-valores_rendicion ).
