...
DEFINE VARIABLE h_v-gen-asiento AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-rng-fechas AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-tip-movcaj AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     "   Indique el rango de fechas de los movimientos" VIEW-AS TEXT
          SIZE 43 BY .62 AT ROW 3.42 COL 38
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-fechas ( 4.23 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-tip-movcaj.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-tip-movcaj ).
       RUN set-position IN h_v-tip-movcaj ( 7.73 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-gen-asiento.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-gen-asiento ).
       RUN set-position IN h_v-gen-asiento ( 10.96 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-tip-movcaj ,
             h_v-rng-fechas , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-gen-asiento ,
             h_v-tip-movcaj , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE tipo_movm        AS CHARACTER.
  ...
  RUN dar_rango IN h_v-tip-movcaj  ( OUTPUT tipo_movm,
                                     OUTPUT ver_movim,
                                     OUTPUT ver_resum,
                                     OUTPUT error_rango ).
  IF error_rango 
  ...
  RUN dar_rango IN h_v-gen-asiento  ( OUTPUT fecha_contable,
                                      OUTPUT gen_asiento,
                                      OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
  ...
  IF NOT hay_error THEN DO:
    ...
    RUN LSASNCAJ.P ( INPUT des_fecha,
                     INPUT has_fecha,
                     INPUT ver_movim,
                     INPUT ver_resum,
                     INPUT tipo_movm,
