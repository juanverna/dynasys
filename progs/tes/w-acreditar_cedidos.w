...
DEFINE VARIABLE h_b-valores_cedidos AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-proveedor AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-id-proveedor AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Acreditación de Valores Cedidos en Pago"
         HEIGHT             = 24.38
         WIDTH              = 136.2
         MAX-HEIGHT         = 24.62
         MAX-WIDTH          = 136.2
...
ON END-ERROR OF W-Win /* Acreditación de Valores Cedidos en Pago */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Acreditación de Valores Cedidos en Pago */
DO:
  ...
  THEN DO:
    ...
    IF NOT txn_activa
    THEN DO: 
          RUN desmarcar_todos IN h_b-valores_cedidos.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Acreditación de Valores Cedidos en Pago */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-id-proveedor ).
       RUN set-position IN h_v-id-proveedor ( 1.24 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.67 , 104.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-valores_cedidos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-valores_cedidos ).
       RUN set-position IN h_b-valores_cedidos ( 3.14 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-valores_cedidos ( 19.38 , 122.00 ) NO-ERROR.
       ...
       RUN set-position IN h_q-proveedor ( 1.00 , 76.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.76 , 8.20 ) */

       /* Links to SmartBrowser h_b-valores_cedidos. */
       RUN add-link IN adm-broker-hdl ( h_q-proveedor , 'Record':U , h_b-valores_cedidos ).

       /* Links to SmartQuery h_q-proveedor. */
       RUN add-link IN adm-broker-hdl ( h_v-id-proveedor , 'Posicionar':U , h_q-proveedor ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-valores_cedidos ,
             h_v-id-proveedor , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/TES" "Acreditación de Valores Cedidos en Pago"}
