...
&Scoped-define FIELDS-IN-QUERY-br_table Valor.numero_cheque 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH Valor WHERE ~{&KEY-PHRASE} NO-LOCK ~
    BY Valor.numero_cheque INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Valor WHERE ~{&KEY-PHRASE} NO-LOCK ~
    BY Valor.numero_cheque INDEXED-REPOSITION.
...
&Scoped-Define ENABLED-OBJECTS RECT-4 v-codigo br_table 
...
<FOREIGN-KEYS>
nro_cliente|y|y|sic.Valor.nro_cliente
nro_proveedor|y|y|sic.Valor.nro_proveedor
nro_recibo|y|y|sic.Valor.nro_recibo
nro_titular-cob|y|y|sic.Valor.nro_titular-cob
nro_titular-pag|y|y|sic.Valor.nro_titular-pag
...
RUN set-attribute-list (
    'Keys-Accepted = "nro_cliente,nro_proveedor,nro_recibo,nro_titular-cob,nro_titular-pag",
     Keys-Supplied = "nro_cliente,nro_proveedor,nro_recibo,nro_titular-cob,nro_titular-pag,cdg_banco,cdg_caja,nro_transaccion,nro_cuenta,cdg_cuenta_ban,cdg_empresa,num_sucursal,nro_valor"':U).
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 15 BY 17.76
...
/* Query rebuild information for BROWSE br_table
     ...
     _OrdList          = "sic.Valor.numero_cheque|yes"
...
DO:
/*
  &SCOPED-DEFINE ROWID_TABLA        rid_valor
  &SCOPED-DEFINE SELECCION          SELVALOR.P
  &SCOPED-DEFINE TABLA              Valor
  &SCOPED-DEFINE CDG_TABLA          numero_cheque
...
DO:
  ...
  &SCOPED-DEFINE CODIGO numero_cheque
  ...
  THEN DO:
    ...
    FIND FIRST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} 
         WHERE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.{&CODIGO} >= v-codigo 
                USE-INDEX por_numero NO-LOCK NO-ERROR.
    IF NOT AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} 
       THEN FIND LAST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} USE-INDEX por_numero
                       NO-LOCK.
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'nro_cliente':U THEN DO:
       &Scope KEY-PHRASE Valor.nro_cliente eq INTEGER(key-value)
       {&OPEN-QUERY-{&BROWSE-NAME}}
    END. /* nro_cliente */
    WHEN 'nro_proveedor':U THEN DO:
       &Scope KEY-PHRASE Valor.nro_proveedor eq INTEGER(key-value)
       {&OPEN-QUERY-{&BROWSE-NAME}}
    END. /* nro_proveedor */
    WHEN 'nro_recibo':U THEN DO:
       &Scope KEY-PHRASE Valor.nro_recibo eq INTEGER(key-value)
       {&OPEN-QUERY-{&BROWSE-NAME}}
    END. /* nro_recibo */
    WHEN 'nro_titular-cob':U THEN DO:
       &Scope KEY-PHRASE Valor.nro_titular-cob eq INTEGER(key-value)
       {&OPEN-QUERY-{&BROWSE-NAME}}
    END. /* nro_titular-cob */
    WHEN 'nro_titular-pag':U THEN DO:
       &Scope KEY-PHRASE Valor.nro_titular-pag eq INTEGER(key-value)
       {&OPEN-QUERY-{&BROWSE-NAME}}
    END. /* nro_titular-pag */
    OTHERWISE DO:
