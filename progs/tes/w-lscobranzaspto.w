...
DEFINE FRAME F-Main
     v-estado AT ROW 13.38 COL 11 COLON-ALIGNED NO-LABEL
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-punto_venta.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-punto_venta ).
       RUN set-position IN h_v-rng-punto_venta ( 10.52 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 14.81 , 13.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  IF NOT hay_error THEN DO:
    ...
    THEN DO:
        RUN LSCOBROSpto.P ( INPUT  que_caja,
                     INPUT  des_fecha,
                     INPUT  has_fecha,
                     INPUT  des_punto,
                     INPUT-OUTPUT xfile).
         RUN graba_temp ( INPUT que_listado(), "", INPUT xfile ).
  END.
  ELSE DO:
...
------------------------------------------------------------------------------*/
  ...
  RETURN "LSCOBROSpto".   /* Function return value. LSCOBROSpto.rpt */
