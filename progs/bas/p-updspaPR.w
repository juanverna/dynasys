...
DEFINE VARIABLE botonera AS CHARACTER NO-UNDO.
...
Btn-Delete Btn-Commit Btn-Undo Btn-Print 
...
DEFINE BUTTON Btn-Add 
     IMAGE-UP FILE "add.gif":U
     LABEL "&Agrega" 
     SIZE 9 BY 1.33
     FONT 4.

DEFINE BUTTON Btn-Commit 
     IMAGE-UP FILE "save.gif":U
     LABEL "&Graba" 
     SIZE 9 BY 1.33
     FONT 4.

DEFINE BUTTON Btn-Copy 
     IMAGE-UP FILE "copy.gif":U
     LABEL "Co&pia" 
     SIZE 9 BY 1.33
     FONT 4.

DEFINE BUTTON Btn-Delete 
     IMAGE-UP FILE "delete.gif":U
     LABEL "&Elimina" 
     SIZE 9 BY 1.33
     FONT 4.

DEFINE BUTTON Btn-Print 
     IMAGE-UP FILE "printer.gif":U
     LABEL "&Imprime" 
     SIZE 9 BY 1.33
     FONT 4.

DEFINE BUTTON Btn-Reset 
     IMAGE-UP FILE "reset.gif":U
     LABEL "&Deshace" 
     SIZE 9 BY 1.33
     FONT 4.

DEFINE BUTTON Btn-Undo 
     IMAGE-UP FILE "cancel.gif":U
     LABEL "&Cancela" 
     SIZE 9 BY 1.33
     FONT 4.

DEFINE BUTTON Btn-Update 
     IMAGE-UP FILE "update.gif":U
     LABEL "&Modifica" 
     SIZE 9 BY 1.33
     FONT 4.
...
DEFINE FRAME Panel-Frame
     Btn-Update AT ROW 1.33 COL 2
     Btn-Reset AT ROW 1.33 COL 11
     Btn-Add AT ROW 1.33 COL 20
     Btn-Copy AT ROW 1.33 COL 29
     Btn-Delete AT ROW 1.33 COL 38
     Btn-Commit AT ROW 1.33 COL 47
     Btn-Undo AT ROW 1.33 COL 56
     Btn-Print AT ROW 1.33 COL 65.2 WIDGET-ID 2
...
ON CHOOSE OF Btn-Add IN FRAME Panel-Frame /* Agrega */
DO:
    add-active = yes.
...
ON CHOOSE OF Btn-Copy IN FRAME Panel-Frame /* Copia */
DO:
...
ON CHOOSE OF Btn-Delete IN FRAME Panel-Frame /* Elimina */
DO:
  ...
  IF sino THEN RUN notify ('delete-record':U).
...
ON CHOOSE OF Btn-Print IN FRAME Panel-Frame /* Imprime */
DO:
    DEFINE VAR tab-target-link AS CHARACTER NO-UNDO.
     DEFINE VAR h-target-link AS HANDLE NO-UNDO.
       RUN get-link-handle IN adm-broker-hdl
           (INPUT THIS-PROCEDURE, 'TABLEIO-TARGET':U, OUTPUT tab-target-link).
     h-target-link = WIDGET-HANDLE( tab-target-link ).
     RUN lstimprime in h-target-link.
END.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
RUN dispatch IN THIS-PROCEDURE ( INPUT 'local-initialize':U ) .
END PROCEDURE.

/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE habilitar_altas C-WIn 
PROCEDURE habilitar_altas :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  Btn-Add:SENSITIVE IN FRAME  {&FRAME-NAME}= p-habilitar.
  Btn-Copy:SENSITIVE IN FRAME  {&FRAME-NAME}= p-habilitar.
...
------------------------------------------------------------------------------*/
  ...
  Btn-Delete:SENSITIVE IN FRAME  {&FRAME-NAME}= p-habilitar.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE habilitar_solo_altas C-WIn 
PROCEDURE habilitar_solo_altas :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  Btn-Add:SENSITIVE IN FRAME  {&FRAME-NAME} = YES.
...
------------------------------------------------------------------------------*/
  ...
  THEN DO:
        DO WITH FRAME {&FRAME-NAME}:
              ...
              THEN DO:
                ASSIGN
                  Btn-Add:SENSITIVE = S-Btn-Add
                  Btn-Commit:SENSITIVE = S-Btn-Commit
                  Btn-Copy:SENSITIVE = S-Btn-Copy
                  Btn-Delete:SENSITIVE = S-Btn-Delete
              ...
              ELSE DO:  /* Deshabilitar */
                ASSIGN
                  S-Btn-Add       = Btn-Add:SENSITIVE
                  S-Btn-Commit    = Btn-Commit:SENSITIVE
                  S-Btn-Copy      = Btn-Copy:SENSITIVE
                  S-Btn-Delete    = Btn-Delete:SENSITIVE
...
-------------------------------------------------------------*/
  ...
  CASE p-state:
      ...
      WHEN "nueva-ficha"
      THEN DO:
           APPLY "CHOOSE" TO btn-add IN FRAME Panel-frame.
      END.     
      WHEN "cambio-ficha"
      THEN DO:
