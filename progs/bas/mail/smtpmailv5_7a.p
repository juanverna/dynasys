...
Input Param : 
     ...
     Attachments CHAR - Comma separated list of attachment descriptions 
                 Format looks like: 
       file[:type=<mimetype>][:charset=<charset>][:filetype=<filetype>][:disposition=>disposition type>
                 ...
                 directory.
                 opciones de disposition son attachment(default) o inline para que no aparezca como attach
...
DEF var Importance                  AS INT  INITIAL 2 NO-UNDO.
...
IF OS-GETENV("TMPDIR") <> ? AND TRIM(OS-GETENV("TMPDIR")) <> "" THEN DO:
  ...
  LogFile = EncodeDirectory + USERID("sic") +  "socketemail.log".
...
ELSE DO: /* decide on a temp directory */
    ...
    ASSIGN
      EncodeDirectory = "/tmp/".
      LogFile         = "/tmp/" + USERID("sic") + "socketemail.log".
    &ELSE
    ASSIGN
      EncodeDirectory = SESSION:TEMP-DIRECTORY
      LogFile         = userid("sic") + "socketemail.log". /* Write to default directory */
    &ENDIF
...
IF EmailFrom = "" OR EmailFrom = ? OR
  ...
  vmessage = "From o To esta en blanco".
  RETURN.
...
THEN DO:
  vmessage = "Usuario o Password en blanco o desconocida".
  RETURN.
END.
...
ELSE RUN gethostname.p(OUTPUT vLocalHostName).
...
PROCEDURE readhandler:
 ...
 /* Used to Build MIME Message Body */
  ...
  DEF VAR cDisposition AS CHARACTER NO-UNDO.
...
LK 12/11/2003 change "SUBSTRING(emailto" to "SUBSTRING(emailtoTMP" */
      ...
      WHEN 8 THEN DO: /* 05/19/2003 LK replace 5 to 8*/
         ...
         */
            ...
            smtpcmd = smtpcmd + "This is a multi-part message in MIME format"
          ...
          */
          ...
          /* IF the message was multipart the charset may have
             been overridden */
          IF cCharSet <> "" THEN 
            smtpcmd = smtpcmd + ";" + crlf + '        charset="' + cCharSet + '"'.
          smtpcmd = smtpcmd + crlf +  /*el original decia 7bit*/
          ...
          */
          DO idx = 1 TO NUM-ENTRIES(LocalFiles):
            ...
            IF cDisposition = 'inline' THEN  
              smtpcmd = smtpcmd + 'Content-ID:<' + cFile + '>' + crlf + crlf. 
            ELSE
              smtpcmd = smtpcmd + 'Content-Disposition: ' + cDisposition + ';' + crlf +
  ...
  IF vState = 10 THEN vMESSAGE = "Email ha sido aceptado para el envio".
&IF DEFINED(PublicVersion) = 0 &THEN
    IF vState < 0 THEN vMESSAGE = "Email ha sido abortado".
&ENDIF
...
PROCEDURE ParseParm:
  ...
  DEF OUTPUT PARAMETER cDisposition   AS CHARACTER NO-UNDO.
  ...
  DO i = 1 TO NUM-ENTRIES(cString,":"):
    ...
    CASE ENTRY(1,c,"="):
      ...
      WHEN "Disposition" THEN DO:
        cDisposition = ENTRY(2,c,"=").
      END.  /* WHEN "FileType" THEN DO: */
...
PROCEDURE dofiles:
  ...
  IF INDEX(FILE-INFO:FILE-TYPE,"F") = 0 THEN DO:
    vMessage = localfile + " No es un archivo valido".
    RETURN.
  END.
...
PROCEDURE DoneWithFiles.
  ...
  DEF VAR ccharset                         as CHARACTER NO-UNDO.
  DEF VAR cFileType                        AS CHARACTER NO-UNDO.
  DEFINE VAR cdisposition                   AS CHAR NO-UNDO.
