...
ON CHOOSE OF Btn-Cancel IN FRAME Panel-Frame /* Cancela */
DO:
  DO WITH FRAME Panel-Frame:
      add-active = no.
      RUN notify ('cancel-record':U).
      RUN set-adm-txn ( INPUT 'no' ).
   END.
END.
...
ON CHOOSE OF Btn-Save IN FRAME Panel-Frame /* Graba */
DO:
...
&ENDIF
  DO:
     ...
     DO WITH FRAME Panel-Frame:
        IF Btn-Save:LABEL = '&Modifica' THEN 
        DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE set-adm-txn C-WIn 
PROCEDURE set-adm-txn :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
