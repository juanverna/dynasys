...
DEFINE VARIABLE h_b-parametros_excepcion AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updsav AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-parametros_excepcion AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Mantenimiento de Excepciones de Parametros"
         HEIGHT             = 25.91
         WIDTH              = 158
         MAX-HEIGHT         = 26.76
         MAX-WIDTH          = 160
...
ON END-ERROR OF W-Win /* Mantenimiento de Excepciones de Parametros */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Mantenimiento de Excepciones de Parametros */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Mantenimiento de Excepciones de Parametros */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-parametros_excepcion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-parametros_excepcion ).
       RUN set-position IN h_b-parametros_excepcion ( 1.48 , 7.00 ) NO-ERROR.
       RUN set-size IN h_b-parametros_excepcion ( 16.91 , 125.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updsav ).
       RUN set-position IN h_p-updsav ( 1.71 , 136.00 ) NO-ERROR.
       RUN set-size IN h_p-updsav ( 24.05 , 14.20 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-parametros_excepcion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-parametros_excepcion ).
       RUN set-position IN h_v-parametros_excepcion ( 18.86 , 7.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.91 , 125.00 ) */

       /* Links to SmartViewer h_v-parametros_excepcion. */
       RUN add-link IN adm-broker-hdl ( h_b-parametros_excepcion , 'Record':U , h_v-parametros_excepcion ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_v-parametros_excepcion ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updsav ,
             h_b-parametros_excepcion , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-parametros_excepcion ,
             h_p-updsav , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/BAS" "Mantenimiento de Excepciones a Parámetros Generales"}
