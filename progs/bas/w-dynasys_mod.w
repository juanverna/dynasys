...
&Scoped-Define ENABLED-OBJECTS v-cdg_modulo v-cdg_empresa btn_ABA btn_DSP ~
btn_INV btn_DAC btn_AFI btn_PRD btn_BDU btn_FAC btn_COM btn_CXC btn_CXP ~
btn_GLA btn_RGV btn_CPS btn_EXP btn_TES btn_IMP btn_ADP btn_UTL btn_CPY 
&Scoped-Define DISPLAYED-OBJECTS v-titulo v-cdg_modulo v-cdg_empresa ~
v-nom_modulo v-accion EDITOR-1 
...
DEFINE VARIABLE v-cdg_modulo AS CHARACTER FORMAT "X(256)":U 
...
DEFINE VARIABLE v-accion AS CHARACTER FORMAT "X(256)":U 
     VIEW-AS FILL-IN 
     SIZE 44 BY 1
     BGCOLOR 15 FGCOLOR 9  NO-UNDO.
...
DEFINE VARIABLE v-titulo AS CHARACTER FORMAT "X(256)":U 
     VIEW-AS FILL-IN 
     SIZE 22 BY 1
     BGCOLOR 15 FGCOLOR 9  NO-UNDO.
...
DEFINE FRAME fContainer
     v-titulo AT ROW 1.24 COL 2 NO-LABEL WIDGET-ID 10
     v-cdg_modulo AT ROW 1.24 COL 23 COLON-ALIGNED NO-LABEL WIDGET-ID 14
     v-cdg_empresa AT ROW 1.24 COL 113 COLON-ALIGNED WIDGET-ID 12
     btn_ABA AT ROW 2.43 COL 3 WIDGET-ID 16
     btn_DSP AT ROW 2.43 COL 14 WIDGET-ID 36
     v-nom_modulo AT ROW 2.43 COL 23 COLON-ALIGNED NO-LABEL WIDGET-ID 58
     v-accion AT ROW 2.43 COL 113 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     btn_INV AT ROW 4.57 COL 3 WIDGET-ID 46
     btn_DAC AT ROW 4.57 COL 14 WIDGET-ID 34
     btn_AFI AT ROW 6.48 COL 14 WIDGET-ID 20
     ...
     EDITOR-1 AT ROW 24.1 COL 1 NO-LABEL WIDGET-ID 6
...
/* SETTINGS FOR EDITOR EDITOR-1 IN FRAME fContainer
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN v-accion IN FRAME fContainer
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN v-nom_modulo IN FRAME fContainer
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN v-titulo IN FRAME fContainer
   NO-ENABLE ALIGN-L                                                    */
...
&Scoped-define SELF-NAME v-cdg_modulo
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL v-cdg_modulo wWin
ON VALUE-CHANGED OF v-cdg_modulo IN FRAME fContainer /* Módulo */
DO:
    ASSIGN v-cdg_modulo.
    RUN emptyTree IN h_pure4gltv.  
    RUN carga_inicial.  
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             ...
             OUTPUT h_pure4gltv ).
       RUN repositionObject IN h_pure4gltv ( 3.62 , 25.00 ) NO-ERROR.
       RUN resizeObject IN h_pure4gltv ( 20.24 , 134.00 ) NO-ERROR.
       ...
       RUN adjustTabOrder ( h_pure4gltv ,
             v-accion:HANDLE IN FRAME fContainer , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  DISPLAY v-titulo v-cdg_modulo v-cdg_empresa v-nom_modulo v-accion EDITOR-1 
      WITH FRAME fContainer IN WINDOW wWin.
  ENABLE v-cdg_modulo v-cdg_empresa btn_ABA btn_DSP btn_INV btn_DAC btn_AFI 
         btn_PRD btn_BDU btn_FAC btn_COM btn_CXC btn_CXP btn_GLA btn_RGV 
         btn_CPS btn_EXP btn_TES btn_IMP btn_ADP btn_UTL btn_CPY 
      WITH FRAME fContainer IN WINDOW wWin.
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME {&FRAME-NAME}:
                    ...
                    WHERE Treemenu.cdg_padre = Modulo-sic.cdg_sigla-sic 
                      ...
                      AND CAN-FIND (FIRST Usuario_funcion OF Usuario 
                                          WHERE Usuario_funcion.cdg_empresa = Treemenu.cdg_empresa
                                            AND CAN-DO(Treemenu.permitidos,Usuario_funcion.cdg_funcion)))
            THEN DO:
                lista = lista + "|" + TRIM(Modulo-sic.descripcion) + "|" + STRING(Modulo-sic.cdg_sigla-sic).
        ...
        v-cdg_modulo:LIST-ITEM-PAIRS = lista.
        v-cdg_modulo = ENTRY(2,lista,"|").
        DISPLAY v-cdg_modulo.
...
FIND Treemenu 
    ...
    AND Treemenu.cdg_empresa = v-cdg_empresa
        NO-LOCK.
v-titulo = Treemenu.titulo.
IF Treemenu.accion <> "" AND Treemenu.accion <> ":X"
THEN DO:
  v-accion = Treemenu.accion.
  DISPLAY v-titulo v-accion
      WITH FRAME {&FRAME-NAME}.
END.
ELSE DO:
  v-accion = "".
