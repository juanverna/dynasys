...
DEFINE VARIABLE panel-type   AS CHARACTER NO-UNDO.
DEFINE VARIABLE add-active   AS LOGICAL NO-UNDO INIT no.
DEFINE VARIABLE sino         AS LOGICAL NO-UNDO INIT no.
...
Btn-Apoyo Btn-Traslado Btn-Commit Btn-Undo 
...
DEFINE FRAME Panel-Frame
     ...
     Btn-Apoyo AT ROW 1.29 COL 38
     Btn-Traslado AT ROW 1.29 COL 47
     Btn-Commit AT ROW 1.29 COL 56
     Btn-Undo AT ROW 1.29 COL 65
     RECT-1 AT ROW 1 COL 1
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1 SCROLLABLE 
         BGCOLOR 8 FGCOLOR 0 
         DEFAULT-BUTTON Btn-Commit.
...
ON CHOOSE OF Btn-Add IN FRAME Panel-Frame /* Nueva */
DO:
    add-active = yes.
...
&Scoped-define SELF-NAME Btn-Apoyo
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Btn-Apoyo C-WIn
ON CHOOSE OF Btn-Apoyo IN FRAME Panel-Frame /* Apoyo */
DO:
    RUN get-link-handle IN adm-broker-hdl
         (THIS-PROCEDURE, 'Duplicar-target':U, OUTPUT c).
    IF NUM-ENTRIES (c) eq 1 THEN DO:
      h = WIDGET-HANDLE (c).
      RUN tipo_ficha IN h ( INPUT 'apoyo' ).
    END.
...
DO:  
      ...
      END.
   /*   RUN new-state ( 'refrescar-browse' ).*/
  END.
...
ON CHOOSE OF Btn-Copy IN FRAME Panel-Frame /* Multiple */
DO:
    ...
    IF NUM-ENTRIES (c) eq 1 THEN DO:
      h = WIDGET-HANDLE (c).
      RUN tipo_ficha IN h ( INPUT 'multiple' ).
    END.
...
DO:
  sino = NO.
  MESSAGE "Desea deshacer los cambios?" VIEW-AS ALERT-BOX QUESTION
          BUTTONS YES-NO TITLE "Se pide confirmación..."  UPDATE sino.
  IF sino 
     THEN RUN notify ('reset-record':U).
END.
...
&Scoped-define SELF-NAME Btn-Traslado
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Btn-Traslado C-WIn
ON CHOOSE OF Btn-Traslado IN FRAME Panel-Frame /* Traslado */
DO:
    RUN w-traslados.w.
END.
...
DO:
  ...
  sino = NO.
  MESSAGE "Desea cancelar esta operación?" VIEW-AS ALERT-BOX QUESTION
          BUTTONS YES-NO TITLE "Se pide confirmación..."  UPDATE sino.
  IF sino 
  THEN DO:  
...
------------------------------------------------------------------------------*/
  ...
  panel-type = 'UPDATE':U.     /* Transaction panel runs in update mode only.*/
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON STOP UNDO, LEAVE ON ERROR UNDO, RETRY:
       ...
       RUN notify ( 'no-navegar, NO-MOVER-BROWSE' ). /* Deshabilitamos el browse de 
                                                        emergencias y la salida       */

       DISABLE Btn-Copy Btn-Apoyo
               WITH FRAME Panel-Frame.
