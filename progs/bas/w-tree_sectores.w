...
&Scoped-Define ENABLED-OBJECTS Btn_Done btn_cambiar btn_vertodo wLines ~
v-coniconos wButtons btn_addnodo btn_addsubnivel btn_deletenode ~
btn_refrescar btn_exportar btn_importar btn_exportar-2 RECT-1 RECT-2 RECT-4 
&Scoped-Define DISPLAYED-OBJECTS wLines v-coniconos wButtons 
...
DEFINE VARIABLE h_q-areas AS HANDLE NO-UNDO.
DEFINE VARIABLE h_treearea AS HANDLE NO-UNDO.
...
DEFINE VARIABLE v-coniconos AS LOGICAL INITIAL yes 
     LABEL "ver íconos" 
     VIEW-AS TOGGLE-BOX
     SIZE 13 BY .52 NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     wLines AT ROW 1.48 COL 43
     v-coniconos AT ROW 1.48 COL 57
     wButtons AT ROW 2.05 COL 43
...
/* ChTreeview OCXINFO:CREATE-CONTROL from: {6C00BE45-F188-11D2-8CE6-00A0D21A0A6B} type: TreeView4GL */
      ChTreeview:MOVE-AFTER(wButtons:HANDLE IN FRAME F-Main).
...
ON CHOOSE OF btn_addnodo IN FRAME F-Main /* Nueva Area */
DO:
...
ON CHOOSE OF btn_addsubnivel IN FRAME F-Main /* Nueva Sub Area */
DO:
...
ON CHOOSE OF btn_deletenode IN FRAME F-Main /* Eliminar Area */
DO:

  DEFINE VARIABLE a AS CHARACTER.
  DEFINE VARIABLE sino AS LOGICAL.
  DEFINE VARIABLE lError AS LOGICAL INITIAL FALSE.
  ...
  THEN DO:
      ...
      DO TRANSACTION:
         ...
         RUN vlb-area( INPUT  ROWID(Area), OUTPUT lError).
         IF lError THEN DO:
            RUN ponmensj (INPUT "AREA002").
            RETURN NO-APPLY.
         END.
         ELSE DO:
             DELETE Area.
...
------------------------------------------------------------------------------*/
  ...
  THEN DO:
       FIND Area WHERE Area.cdg_area = v-item 
                   AND Area.cdg_empresa = Empresa.cdg_empresa 
                       NO-LOCK.
       RUN posicionar_query IN h_q-areas ( INPUT ROWID(Area)).
       RETURN NO-APPLY.  
  END.
...
&Scoped-define SELF-NAME v-coniconos
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL v-coniconos W-Win
ON VALUE-CHANGED OF v-coniconos IN FRAME F-Main /* ver íconos */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wButtons W-Win
ON VALUE-CHANGED OF wButtons IN FRAME F-Main /* ver botones */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wLines W-Win
ON VALUE-CHANGED OF wLines IN FRAME F-Main /* ver líneas */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_p-soloedita ( 15.29 , 134.00 ) NO-ERROR.
       RUN set-size IN h_p-soloedita ( 6.91 , 25.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-treearea.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_treearea ).
       RUN set-position IN h_treearea ( 22.43 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.86 , 157.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-areas ).
       RUN set-position IN h_q-areas ( 1.48 , 78.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.86 , 10.80 ) */

       /* Links to SmartViewer h_treearea. */
       RUN add-link IN adm-broker-hdl ( h_p-soloedita , 'TableIO':U , h_treearea ).
       RUN add-link IN adm-broker-hdl ( h_q-areas , 'Record':U , h_treearea ).

       /* Links to SmartQuery h_q-areas. */
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'Mostrar':U , h_q-areas ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-soloedita ,
             btn_exportar-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_treearea ,
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE cargar_area W-Win 
PROCEDURE cargar_area :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  FOR EACH Area 
      WHERE Area.cdg_empresa =  Empresa.cdg_empresa
        AND Area.cdg_reporta = p-que_padre NO-LOCK BY Area.cdg_area:

      a = a  + "~n0~t" + Area.denominacion + "~t0~t1~t~tAccion=|" + Area.cdg_area.
      RUN cargar_hijos ( INPUT-OUTPUT a, INPUT Area.cdg_area, INPUT 1).
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Area 
      WHERE Area.cdg_empresa = Empresa.cdg_empresa 
        AND Area.cdg_reporta = p-cdg_reporta BY Area.cdg_area :
        
            IF CAN-FIND(FIRST B-Area WHERE B-Area.cdg_reporta = Area.cdg_area AND B-Area.cdg_empresa = Area.cdg_empresa)
            THEN DO:
                 a = a  + "~n" + STRING(p-nivel,">9") + "~t" + Area.denominacion + "~t0~t1~t~t" + 
                     "Accion=" + Area.reporte + "|" + Area.cdg_area.           
                 RUN cargar_hijos ( INPUT-OUTPUT a, INPUT Area.cdg_area, INPUT p-nivel + 1).
            END.
            ELSE DO:
                 a = a  + "~n" + STRING(p-nivel,">9") + "~t" + Area.denominacion + "~t0~t1~t~t" + 
                     "Accion=" + Area.reporte + "|" + Area.cdg_area.           
...
OCXFile = SEARCH( "w-tree_sectores.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-tree_sectores.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  DISPLAY wLines v-coniconos wButtons 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE Btn_Done btn_cambiar btn_vertodo wLines v-coniconos wButtons 
         btn_addnodo btn_addsubnivel btn_deletenode btn_refrescar btn_exportar 
         btn_importar btn_exportar-2 RECT-1 RECT-2 RECT-4 
      WITH FRAME F-Main IN WINDOW W-Win.
