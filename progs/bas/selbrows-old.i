...
&Scoped-define FIELDS-IN-QUERY-BROWSE-1 {&TABLA}.{&CODIGO} ~
{&TABLA}.{&NOMBRE} &IF DEFINED(OTROS-CAMPOS) &THEN {&OTROS-CAMPOS} &ENDIF
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-1 
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH {&TABLA} ~
      WHERE {&TABLA}.{&NOMBRE} BEGINS des_registro NO-LOCK ~
    BY {&TABLA}.{&NOMBRE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-1 OPEN QUERY BROWSE-1 FOR EACH {&TABLA} ~
      WHERE {&TABLA}.{&NOMBRE} BEGINS des_registro ~
           &IF DEFINED(CONDICION) &THEN AND {&CONDICION} &ENDIF ~
    NO-LOCK BY {&TABLA}.{&CODIGO} INDEXED-REPOSITION.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-1 gDialog _STRUCTURED
  QUERY BROWSE-1 NO-LOCK DISPLAY
      {&TABLA}.{&CODIGO} &IF DEFINED(FORMATO-CODIGO) &THEN {&FORMATO-CODIGO}  &ENDIF WIDTH 15.43
      {&TABLA}.{&NOMBRE} WIDTH 42.29
      &IF DEFINED(OTROS-CAMPOS) &THEN {&OTROS-CAMPOS} &ENDIF 
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 96 BY 7.81 EXPANDABLE.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME gDialog
     sel_registro AT ROW 1.29 COL 3 NO-LABEL
...
/* Query rebuild information for BROWSE BROWSE-1
     ...
     _FldNameList[1]   > sic.{&TABLA}.{&CODIGO}
"{&TABLA}.{&CODIGO}" ? ? "character" ? ? ? ? ? ? no ? no no "15.43" yes no no "U" "" ""
     _FldNameList[2]   > sic.{&TABLA}.{&NOMBRE}
"{&TABLA}.{&NOMBRE}" ? ? "character" ? ? ? ? ? ? no ? no no "42.29" yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
