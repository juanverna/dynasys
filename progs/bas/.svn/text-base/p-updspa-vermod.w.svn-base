...
DEFINE FRAME Panel-Frame
     Btn-Update AT ROW 1.27 COL 2
     Btn-Reset AT ROW 1.27 COL 11
     Btn-Add AT ROW 1.27 COL 20
     Btn-Copy AT ROW 1.27 COL 29
     Btn-Delete AT ROW 1.27 COL 38
     Btn-Commit AT ROW 1.27 COL 47
     Btn-Undo AT ROW 1.27 COL 56
...
DO:
  ...
  /* Ask the Record-Source for the current customer record.  Make sure
    ...
    RUN get-link-handle IN adm-broker-hdl
        (THIS-PROCEDURE, 'TableIO-Target':U, OUTPUT c).
    IF NUM-ENTRIES (c) eq 1 
    THEN DO:
         h = WIDGET-HANDLE (c).
         RUN puede_modificar IN h (OUTPUT ok).
    END.
    ...
    END.
    ELSE DO:
         MESSAGE "No puede modificar el registro actual" VIEW-AS ALERT-BOX WARNING
                 TITLE "Aviso!!!".
         RETURN NO-APPLY.               
    END.
END.
