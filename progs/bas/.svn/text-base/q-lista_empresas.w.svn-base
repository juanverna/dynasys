...
DEFINE VARIABLE v-lista_empresas LIKE Usuario.lista_empresa.
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH Empresa WHERE ~{&KEY-PHRASE} ~
      AND LOOKUP(Empresa.cdg_empresa,v-lista_empresas) <> 0 NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH Empresa WHERE ~{&KEY-PHRASE} ~
      AND LOOKUP(Empresa.cdg_empresa,v-lista_empresas) <> 0 NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main Empresa
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main Empresa
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Where[1]         = "LOOKUP(Empresa.cdg_empresa,v-lista_empresas) <> 0"
...
------------------------------------------------------------------------------*/
  ...
  {findempresa.i} 
  v-lista_empresas = Usuario.lista_empresas.
