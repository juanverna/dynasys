...
------------------------------------------------------------------------------*/
    ...
    DEFINE INPUT PARAMETER a        AS CHARACTER NO-UNDO.

    DEFINE VARIABLE v-accion        AS CHARACTER NO-UNDO.
    DEFINE VARIABLE x-programa      AS CHARACTER NO-UNDO.
    DEFINE VARIABLE i-modo          AS INTEGER   NO-UNDO.
    DEFINE VARIABLE x-modo          AS CHARACTER NO-UNDO.
    DEFINE VARIABLE x-comprobante   AS CHARACTER NO-UNDO.
    ...
    v-accion = GetAccion(a). /* v-accion puede ser "w-pirulo.w:X" o "w-pirulo.w:j" con j ente 0 y 9 */
    IF v-accion <> "" AND v-accion <> ":X"
    THEN DO:
         x-programa = ENTRY(1,v-accion,":").
         x-modo     = ENTRY(2,v-accion,":").
         IF NUM-ENTRIES(v-accion,":") > 2
         THEN DO:
             x-comprobante = ENTRY(3,v-accion,":").
         END.
         ELSE DO:
             x-comprobante = "".
         END.

         IF x-modo <> "X" /* si es de la forma "w-pirulo.w:j" */
         THEN DO:
              i-modo = INTEGER(x-modo).
         END.
         ELSE DO:
              i-modo = ?. /* no hay modo */
         END.     

         IF SEARCH(x-programa) <> ? /* Si esta definida la accion, la ejecutamos */
         THEN DO:
             
             RUN verificar_puedo_ejecutar ( INPUT x-programa, OUTPUT correr_ok ).
             IF correr_ok
             THEN DO:
                 IF Usuario.seguimiento THEN RUN ingresa_menu ( INPUT x-programa, INPUT-OUTPUT rid_logmenu ).
                 IF x-modo = "X"
                 THEN DO:
                     i-estado = {&WINDOW-NAME}:WINDOW-STATE.
                     {&WINDOW-NAME}:HIDDEN = YES.
                     RUN VALUE(x-programa). 
                 ...
                 ELSE DO:
                     ...
                     THEN DO:
                         i-estado = {&WINDOW-NAME}:WINDOW-STATE.             
                         {&WINDOW-NAME}:HIDDEN = YES.
                         RUN VALUE(x-programa) ( INPUT-OUTPUT h-tranxn, INPUT i-modo ).
                         {&WINDOW-NAME}:HIDDEN = NO.
                         {&WINDOW-NAME}:WINDOW-STATE = i-estado.
                         {&WINDOW-NAME}:MOVE-TO-TOP().
                     END.
                     ELSE DO:
                         i-estado = {&WINDOW-NAME}:WINDOW-STATE.
                         {&WINDOW-NAME}:HIDDEN = YES.
                         RUN VALUE(x-programa) ( INPUT-OUTPUT h-tranxn, INPUT i-modo, INPUT x-comprobante ).
                         {&WINDOW-NAME}:HIDDEN = NO.
                         {&WINDOW-NAME}:WINDOW-STATE = i-estado.
                         {&WINDOW-NAME}:MOVE-TO-TOP().
                     END.
             ...
             END.
             ELSE DO:
                 MESSAGE "Existen procesos concurrentes a " x-programa VIEW-AS ALERT-BOX ERROR TITLE "No puede ejecutarse la opción".
             END.     
             
         END.
         ELSE DO:
             MESSAGE "No se halló " x-programa VIEW-AS ALERT-BOX ERROR TITLE "No puede ejecutarse la opción".
         END.     
    END.
...
------------------------------------------------------------------------------*/
   ...
   DO TRANSACTION:
       ...
       ASSIGN Logmenu.fch_desde = TODAY
              ...
              Logmenu.cdg_item    = IF AVAILABLE MENU THEN Treemenu.cdg_item ELSE "".
