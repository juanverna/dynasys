...
ON + OF des_registro IN FRAME F-Main /* Desde Empresa */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON MOUSE-MENU-DOWN OF des_registro IN FRAME F-Main /* Desde Empresa */
DO:
  &SCOPED-DEFINE ROWID_TABLA        rid_deslocal
  &SCOPED-DEFINE SELECCION          SELEMPRE.P
  &SCOPED-DEFINE TABLA              Empresa
  &SCOPED-DEFINE CDG_TABLA          cdg_empresa
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON RETURN OF des_registro IN FRAME F-Main /* Desde Empresa */
DO:

    ASSIGN FRAME {&FRAME-NAME} des_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND Empresa WHERE Empresa.cdg_empresa = SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
        ELSE FIND FIRST Empresa NO-LOCK NO-ERROR.
    
    IF AVAILABLE Empresa 
    THEN DO:
         des_nombre = Empresa.nombre.
         des_registro = Empresa.cdg_empresa.
    END.
    ELSE DO:
         des_nombre = "???".
    END.
...
ON + OF has_registro IN FRAME F-Main /* Hasta Empresa */
DO:
  APPLY "MOUSE-MENU-DOWN" TO SELF.
  RETURN NO-APPLY.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL has_registro V-table-Win
ON MOUSE-MENU-DOWN OF has_registro IN FRAME F-Main /* Hasta Empresa */
DO:
  &SCOPED-DEFINE ROWID_TABLA        rid_haslocal
  &SCOPED-DEFINE SELECCION          SELEMPRE.P
  &SCOPED-DEFINE TABLA              Empresa
  &SCOPED-DEFINE CDG_TABLA          cdg_empresa
  &SCOPED-DEFINE DSC_TABLA          nombre
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL has_registro V-table-Win
ON RETURN OF has_registro IN FRAME F-Main /* Hasta Empresa */
DO:

    ASSIGN FRAME {&FRAME-NAME} has_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND Empresa WHERE Empresa.cdg_empresa = SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
        ELSE FIND LAST Empresa NO-LOCK NO-ERROR.
    
    IF AVAILABLE Empresa 
    THEN DO:
         has_nombre = Empresa.nombre.
         has_registro = Empresa.cdg_empresa.
    END.
    ELSE DO:
         has_nombre = "???".
    END.
