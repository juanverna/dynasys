...
/* Comment out the definition of the 2 following preprocessors to use the
 windows.p and winfunc.p API libraries that are provided with Dynamics.  These
 two libraries have a slightly different signature because of dummy RETURN
 parameters that are not defined (we actually never use them).
 BEWARE, ONCE YOU COMMENT OUT OR UNCOMMENT IN THE 2 NEXT LINES AND YOU RECOMPILE
 pure4gltv.w, THEN YOU SHALL EITHER RESTART YOUR SESSION OR MANUALLY KILL THE
 RUNNING INSTANCES OF windows.p and winfunc.p or af/sup/windows.p and af/sup/winfunc.p
 BEFORE USING pure4gltv AGAIN IN A CONTAINER  */
...
&IF "{&APIRtnParm}" = ""
&THEN /*{af/sup/windows.i}*/ {windows.i}
&ELSE {windows.i}
&ENDIF
...
&IF DEFINED (ADM-PROPERTY-DLG) = 0 &THEN
  &SCOP ADM-PROPERTY-DLG pure4glTvInstanceProp.w
&ENDIF
...
&SCOPED-DEFINE fsvWIDTH-PIXELS 16
...
CREATE WIDGET-POOL gWPNodeLabels.
CREATE WIDGET-POOL gWPNodeIcons.
...
DEF TEMP-TABLE node NO-UNDO
 ...
 INDEX parKe  par ke  /* used for sorting nodes by ke */ /* CR 22-05-2006 */
...
DEFINE VARIABLE gcDefaultIco       AS CHARACTER INITIAL "tvpics/Folder" NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD f-selectNode C-Win 
FUNCTION f-selectNode RETURNS LOGICAL
...
DEFINE BUTTON SbtnScrollDown 
     IMAGE-UP FILE "tvpics/scrollbuttondown.bmp":U NO-FOCUS
     LABEL "" 
     SIZE 3 BY .71.

DEFINE BUTTON SbtnScrollUp 
     IMAGE-UP FILE "tvpics/scrollbuttonup.bmp":U NO-FOCUS
     LABEL "" 
     SIZE 3 BY .71.

DEFINE IMAGE imgsv
     FILENAME "tvpics/vscrollbg.jpg":U
     SIZE 3 BY 1.19.
...
DEFINE FRAME fsv
     emptySquare AT ROW 3.33 COL 1
     Sbsv AT ROW 1.71 COL 1
     SbtnScrollDown AT ROW 2.67 COL 1
     SbtnScrollUp AT ROW 1 COL 1
     blackClickDownRectangle AT ROW 1.62 COL 1.2
     imgsv AT ROW 1.71 COL 1
...
IF lautoSort THEN DO:
    ...
    FIND LAST prevsibling WHERE prevsibling.par = bnode.par
     /*AND prevsibling.lab < bnode.lab */ /* CR Modificado para que ordene dentro de la clave no del label */
     AND prevsibling.ke < bnode.ke
     USE-INDEX parKe NO-ERROR.
...
RUN SendMessage{&A} IN hpApi (FRAME ftv:HWND, 517, 0, 0 {&APIRtnParmINT}).
...
IF NOT lMthRtn THEN DO:
    ...
    btvpic.hpic:LOAD-IMAGE("tvpics/missingPicFile.bmp") NO-ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE DoubleClick C-Win 
...
------------------------------------------------------------------------------*/
    ...
    DEFINE INPUT PARAMETER pcNodeKe AS CHARACTER.
    ...
    IF prevSelectedNodeId <> gCurNode THEN DO:
        ...
        PUBLISH "tvNodeEvent" ("DoubleClick", bnode.ke).
    END.
    RETURN "YES".
...
DEFINE VARIABLE itvDefaultFont            AS INTEGER    NO-UNDO.
...
DO WHILE TRUE:
    ...
    IF NOT lfollowPointerWidgetReady AND lDragStarted THEN DO:
        itvDefaultFont = gettvnodeDefaultFont().
        ...
        ASSIGN
         hpointerText:FONT = itvDefaultFont
...
RUN ShowScrollBar IN hpApi (FRAME ftv:HWND, {&SB_VERT},  0 {&APIRtnParmINT}).     
RUN ShowScrollBar IN hpApi (FRAME ftv:HWND, {&SB_HORZ},  0 {&APIRtnParmINT}).     
...
/* for some actions, it is important to have the selected node already in 
 the view port (sometimes not the case when srolled somewhere with scroll bar) */
IF AVAIL curNode
...
DO iiter = 1 TO gTvIterations:
    ...
    RUN createPic(/*X*/ 2 + 2 /*bnode.level*/ * gTvLevelWidth
                 ...
                 ,/*VISIBLE*/ NO
                 ,/*IMAGE file-name */ picFileName("collapsed","tvpics/Folder")).
...
------------------------------------------------------------------------------*/
    ...
    DO ON ERROR UNDO, LEAVE:
        ...
        RUN addNode ("n22"  ,"n2"  ,"node 22"  ,"tvpics/book02.bmp"   ,"expanded") NO-ERROR.
        RUN addNode ("n221" ,"n22" ,"node 221" ,"tvpics/book02.bmp"   ,"")         NO-ERROR.
        RUN addNode ("n222" ,"n22" ,"node 222" ,"tvpics/book02.bmp"   ,"")         NO-ERROR.
        RUN addNode ("n3"   ,""    ,"node 3"   ,"tvpics/present1.bmp" ,"expanded") NO-ERROR.
        RUN addNode ("n31"  ,"n3"  ,"node 31"  ,"tvpics/$.bmp"        ,"")         NO-ERROR.
        RUN addNode ("n4"   ,""    ,"node 4"   ,"tvpics/smile56.bmp"  ,"")         NO-ERROR.
    END.
    IF ERROR-STATUS:ERROR THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  IF gWindowsSkin = "XPStyle" THEN DO:
      ghbtnScrollDown:LOAD-IMAGE("tvpics/XPscrollButtonDoPressed.bmp").
  ...
  END.
  ELSE RUN SendMessageA IN hpApi ( ghbtnScrollDown:HWND, 
                                   {&BM_SETSTATE}, 
  ...
  IF gWindowsSkin = "XPStyle" THEN DO:
      ghbtnScrollDown:LOAD-IMAGE("tvpics/XPscrollButtonDo.bmp").
      /* strange, loading an image in ghbtnScrollDown makes the thumb go to the bottom... 
       The following fixes that*/
      ghbsv:MOVE-TO-TOP().
      IF VALID-HANDLE(ghbsvGraber) THEN ghbsvGraber:MOVE-TO-TOP().    
...
------------------------------------------------------------------------------*/
  ...
  IF gWindowsSkin = "XPStyle" THEN DO:
      ghbtnScrollUp:LOAD-IMAGE("tvpics/XPscrollButtonUpPressed.bmp").
      /* strange, loading an image in ghbtnScrollDown makes the thumb go to the bottom... 
       The following fixes that*/
      ghbsv:MOVE-TO-TOP().
      IF VALID-HANDLE(ghbsvGraber) THEN ghbsvGraber:MOVE-TO-TOP().    
  ...
  ELSE RUN SendMessageA IN hpApi ( ghbtnScrollUp:HWND, 
                                  {&BM_SETSTATE}, 
                                  1,  /* TRUE, set BST_PUSHED style */
                                  0
                                   {&APIRtnParmINT}).  
  ...
  IF gWindowsSkin = "XPStyle" THEN DO:
      ghbtnScrollUp:LOAD-IMAGE("tvpics/XPscrollButtonUp.bmp").
      /* strange, loading an image in ghbtnScrollDown makes the thumb go to the bottom... 
       The following fixes that*/
      ghbsv:MOVE-TO-TOP().
      IF VALID-HANDLE(ghbsvGraber) THEN ghbsvGraber:MOVE-TO-TOP().    
...
/*--------------------------------------------------------------------------------*/
/* MODIFICADO POR CR. 18/05/2006 PARA QUE RESPONDA AL DOBLE-CLICK ESPECIFICAMENTE */
/*--------------------------------------------------------------------------------*/
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN   
   ...
   FRAME fsv:X = gftvWP - {&fsvWIDTH-PIXELS} + 1
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE selectNode C-Win 
...
IF gWindowsSkin = "XPStyle" THEN ghbsv:LOAD-IMAGE("tvpics/XPscrollbuttonPressed.bmp").
...
IF gWindowsSkin = "XPStyle" THEN ghbsv:LOAD-IMAGE("tvpics/XPscrollbutton.bmp").
...
FIND bnode WHERE bnode.ke = pcNodeKe NO-ERROR.
IF NOT AVAIL bnode THEN RETURN ERROR NO.
...
------------------------------------------------------------------------------*/
   ...
   imgsv:LOAD-IMAGE("tvpics/XPVscrollbg.bmp") IN FRAME fsv.
  ...
  CREATE IMAGE ghbsv IN WIDGET-POOL cVertSBWP ASSIGN
   ...
   END TRIGGERS. 
  ghbsv:LOAD-IMAGE("tvpics/XPscrollbutton.bmp").
  ...
  CREATE IMAGE ghbsvGraber  IN WIDGET-POOL cVertSBWP ASSIGN
   ...
   FRAME = FRAME fsv:HANDLE
   X = Sbsv:X + 4 /* will remain the same forever*/
  ...
  ghbsvGraber:LOAD-IMAGE("tvpics/XPscrollButtonGraber.bmp").
  ...
  ASSIGN
   ...
   SbtnScrollUp:X = 0  /* move it there so it will not make problems... */
  ...
  ASSIGN
   ...
   SbtnScrollDown:X = 0  /* move it there so it will not make problems... */
...
------------------------------------------------------------------------------*/
    ...
    IF AVAIL bnode THEN PUBLISH "tvNodeEvent" ("deselect", bnode.ke).
...
IF NOT AVAIL nodeToExpand THEN RETURN ERROR NO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION f-selectNode C-Win 
FUNCTION f-selectNode RETURNS LOGICAL
  (pcNodeKe AS CHAR) :
/*------------------------------------------------------------------------------
  Purpose:  Select a the node of a given key
...
------------------------------------------------------------------------------*/
  ...
  FIND bnode WHERE bnode.ke = pcKe NO-ERROR.
  IF NOT AVAIL bnode THEN RETURN ERROR.
...
------------------------------------------------------------------------------*/
  ...
  IF NOT AVAIL bnode THEN RETURN ERROR NO. /*can happen when the node has been deleted*/
  DO WHILE TRUE:
      parNode = bnode.par.
...
------------------------------------------------------------------------------*/
  ...
  IF glOnlyPlusMinus THEN CASE cCode:
      WHEN "noChild"   THEN RETURN "tvpics/blank.bmp".
      WHEN "expanded"  THEN RETURN "tvpics/minus.bmp".
      WHEN "collapsed" THEN RETURN "tvpics/plus.bmp".
  END CASE.
  ...
  IF ico = "tvpics/Folder" THEN DO:
      IF glPicWithPlusMinus THEN CASE cCode:
         WHEN "noChild"   THEN RETURN "tvpics/FoldNoSign.bmp".
         WHEN "expanded"  THEN RETURN "tvpics/FoldMinus.bmp".
         WHEN "collapsed" THEN RETURN "tvpics/FoldPlus.bmp".
      END CASE.
      ELSE CASE cCode:
         WHEN "noChild"   THEN RETURN "tvpics/Fold.bmp".
         WHEN "expanded"  THEN RETURN "tvpics/FoldOpen.bmp".
         WHEN "collapsed" THEN RETURN "tvpics/Fold.bmp".
      END CASE.
  END. /* ico = "tvpics/Folder" */
...
------------------------------------------------------------------------------*/
  ...
  IF (gIterOffset = 0 AND gExpChildren <= gVisibleIterations)
   AND gVertScrollingActive THEN DO:
     ...
     IF gHorzScrollingActive THEN RUN ShowScrollBar IN hpApi (FRAME ftv:HWND, {&SB_VERT},  0  {&APIRtnParmINT}).     
...
IF gHorzScrollingActive THEN ASSIGN
 gFsvHP = gFtvHP - {&fsvWIDTH-PIXELS} /*note that the height of horzScrollBar =  {&fsvWIDTH-PIXELS}*/
 gFsvimgHP = gFsvHP - 2 * ghbtnScrollUp:HEIGHT-PIXELS
 imgsv:HEIGHT-PIXELS IN FRAME fsv = gFsvimgHP
 ghbtnScrollDown:Y = gFsvHP - ghbtnScrollDown:HEIGHT-PIXELS
...
------------------------------------------------------------------------------*/
  ...
  IF gWindowsSkin = "XPStyle" THEN RETURN createDynWidgetForVertScrollBar().
  ELSE ASSIGN
...
/* The following code executes only if cWindowsSkin = "Automatic" 
  the point is to determine whether gWindowsSkin should be set to classic or XPStyle */
...
IF NOT ERROR-STATUS:ERROR /* The RUN GetCurrentThemeName failed because uxtheme.dll is not available (old MS Windows, or wine without multi skin support)*/
 AND iRtn = 0 THEN DO: /* = 0 means that GetCurrentThemeName ran fine, if error, then value is negative*/
    ...
    IF cThemeName MATCHES "*\Luna.msstyles" THEN DO:
        IF cThemeColor = "NormalColor" THEN gWindowsSkin = "XPStyle".
        /* add other themes like Metallic here... 
        IF cThemeColor = "Metallic" THEN gWindowsSkin = "Metallic". */
    END.
END.
      ...
      ELSE DO:
          ...
          ELSE DO:
              ...
              CASE cThemeSelDisplayName:
                  WHEN "Windows XP style" THEN gWindowsSkin = "XPStyle".
...
IF gVWP > gFtvWP - {&VertSBAdjust}
 THEN DO:
   IF gVWP <> gFtvVWP - {&VertSBAdjust}
    ...
    IF NOT gVertScrollingActive THEN RUN ShowScrollBar IN hpApi (FRAME ftv:HWND, {&SB_VERT},  0  {&APIRtnParmINT}).
    /* else to not let the horz take all the width... */
    ELSE RUN ShowScrollBar IN hpApi (FRAME ftv:HWND, {&SB_VERT},  1  {&APIRtnParmINT}).
...
IF NOT glwineMode THEN DO:
    ...
    RUN SendMessage{&A} IN hpApi (FRAME ftv:HWND, {&WM_PAINT}, 0, 0  {&APIRtnParmINT}).    
END.
