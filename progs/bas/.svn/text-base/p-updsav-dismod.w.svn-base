...
&Scoped-Define ENABLED-OBJECTS Btn-Save Btn-Reset Btn-Add Btn-Cancel 
...
DEFINE FRAME Panel-Frame
     ...
     Btn-Reset AT ROW 1.27 COL 12
     Btn-Add AT ROW 1.27 COL 22
     Btn-Delete AT ROW 1.27 COL 32
     Btn-Cancel AT ROW 1.27 COL 42
...
/* SETTINGS FOR BUTTON Btn-Delete IN FRAME Panel-Frame
   NO-ENABLE                                                            */
...
&ENDIF
  DO:
     ...
     DO WITH FRAME Panel-Frame:
        IF Btn-Save:LABEL = '&Modificar' THEN 
        DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE disable-modify C-WIn 
PROCEDURE disable-modify :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/*         ELSE adm-panel-state = 'initial':U. */
         ELSE adm-panel-state = 'add-only':U.
       END.
     END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE enable-modify C-WIn 
PROCEDURE enable-modify :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
-------------------------------------------------------------*/
  ...
  IF (p-state BEGINS 'UPDATE':U) THEN 
     RUN new-state(p-state + ',CONTAINER-SOURCE':U). 
END PROCEDURE.
