...
DEFINE VARIABLE h_v-lst-estado AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 18.38 COL 12 COLON-ALIGNED NO-LABEL
     "  Estado de los pedidos a listar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 15.05 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de vendedores a listar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 1.81 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de fechas de los movimientos de ventas:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 6.24 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de fechas de los movimientos de cobranza:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 11 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
ON END-ERROR OF W-Win /* Libros del Viajante */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Libros del Viajante */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-vendedor ( 3.14 , 14.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas-2 ( 12.19 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-lst-estado.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-lst-estado ).
       RUN set-position IN h_v-lst-estado ( 16.24 , 14.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 19.81 , 14.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-lst-estado ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/DSP" "Libro del Viajante"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE v-lista_estados  AS CHARACTER.
  ...
  RUN dar_rango IN h_v-lst-estado    ( OUTPUT v-lista_estados, 
                                       OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Estados de Pedidos" ).
       hay_error = error_rango.
  END.     
  ...
  THEN DO:
       ...
       RUN lslibro_del_viajante-char.p ( INPUT des_vendedor,
                                    INPUT has_vendedor,
                                    INPUT des_fecha,
                                    INPUT has_fecha,
                                    INPUT v-lista_estados
                                   ).
