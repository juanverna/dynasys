...
Ped_header.cdg_estado Ped_header.tip_comprob Ped_header.nro_comprob ~
Ped_header.imp_total 
...
DEFINE VARIABLE v-estados AS CHARACTER INITIAL "IR" 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 108 BY 6.71
         BGCOLOR 11 FGCOLOR 9 FONT 4
         TITLE BGCOLOR 11 FGCOLOR 9 "Pedidos Rechazados Pendientes de Informar al Cliente".
...
DEFINE FRAME F-Main
     ...
     btn_renovar AT ROW 8.91 COL 3.6
     v-estados AT ROW 8.95 COL 31.6 NO-LABEL
     RECT-6 AT ROW 8.71 COL 1
...
"Ped_header.tip_comprob" "Tip!Ped" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[7]   > sic.Ped_header.nro_comprob
"Ped_header.nro_comprob" "Número!Pedido" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[8]   > sic.Ped_header.imp_total
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* Pedidos Rechazados Pendientes de Informar al Cliente */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* Pedidos Rechazados Pendientes de Informar al Cliente */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* Pedidos Rechazados Pendientes de Informar al Cliente */
DO:
