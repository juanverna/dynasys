...
&Scoped-define INTERNAL-TABLES Ped_header

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table Ped_header.tip_comprob ~
Ped_header.prf_comprob Ped_header.nro_comprob Ped_header.fecha ~
Ped_header.cdg_estado 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH Ped_header OF Cliente WHERE ~{&KEY-PHRASE} ~
      AND Ped_header.cdg_estado <> "ZZ" ~
 AND Ped_header.cdg_empresa = que_empresa NO-LOCK ~
    BY Ped_header.fecha DESCENDING
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Ped_header OF Cliente WHERE ~{&KEY-PHRASE} ~
      AND Ped_header.cdg_estado <> "ZZ" ~
 AND Ped_header.cdg_empresa = que_empresa NO-LOCK ~
    BY Ped_header.fecha DESCENDING.
&Scoped-define TABLES-IN-QUERY-br_table Ped_header
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 32 BY 6.69
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "sic.Ped_header OF sic.Cliente"
     _Options          = "NO-LOCK KEY-PHRASE"
     _OrdList          = "sic.Ped_header.fecha|no"
     _Where[1]         = "Ped_header.cdg_estado <> ""ZZ""
...
"Ped_header.nro_comprob" "Número!Pedido" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[4]   > sic.Ped_header.fecha
"Ped_header.fecha" "Fecha!Pedido" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   > sic.Ped_header.cdg_estado
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* Pedidos del Cliente */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* Pedidos del Cliente */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* Pedidos del Cliente */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE dar_numero_pedido B-table-Win 
PROCEDURE dar_numero_pedido :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  DEFINE OUTPUT PARAMETER p-nro_pedido LIKE Ped_header.nro_pedido.

  IF AVAILABLE Ped_header
     THEN p-nro_pedido = Ped_header.nro_pedido.
     ELSE p-nro_pedido = ?.
