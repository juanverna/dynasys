...
&Scoped-define INTERNAL-TABLES T-Ped_detalle-bon Bonificacion
...
&Scoped-define QUERY-STRING-BROWSE-7 FOR EACH T-Ped_detalle-bon ~
      WHERE T-Ped_detalle-bon.nro_linea = T-Ped_detalle.nro_linea NO-LOCK, ~
      EACH Bonificacion OF T-Ped_detalle-bon NO-LOCK
&Scoped-define OPEN-QUERY-BROWSE-7 OPEN QUERY BROWSE-7 FOR EACH T-Ped_detalle-bon ~
      WHERE T-Ped_detalle-bon.nro_linea = T-Ped_detalle.nro_linea NO-LOCK, ~
      EACH Bonificacion OF T-Ped_detalle-bon NO-LOCK.
...
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 84 BY 6.48
         TITLE "Bonificaciones del Item de pedido" EXPANDABLE.
...
DEFINE FRAME Dialog-Frame
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "Bonificaciones del detalle de pedidos".
...
/* Query rebuild information for BROWSE BROWSE-7
     _TblList          = "Temp-Tables.T-Ped_detalle-bon,sic.Bonificacion OF Temp-Tables.T-Ped_detalle-bon"
     _Options          = "NO-LOCK"
     _Where[1]         = "Temp-Tables.T-Ped_detalle-bon.nro_linea = T-Ped_detalle.nro_linea"
     _FldNameList[1]   > Temp-Tables.T-Ped_detalle-bon.cdg_bonificacion
"T-Ped_detalle-bon.cdg_bonificacion" "Có-!digo" ? "integer" ? ? ? ? ? ? no ? no no "8.2" yes no no "U" "" ""
...
"T-Ped_detalle-bon.porcentaje" "%.!Bon." ? "decimal" ? ? ? ? ? ? no ? no no "11.2" yes no no "U" "" ""
     _FldNameList[4]   > Temp-Tables.T-Ped_detalle-bon.importe
"T-Ped_detalle-bon.importe" "Importe!Bonificado" ? "decimal" ? ? ? ? ? ? no ? no no "14.4" yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-7 */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Bonificaciones del detalle de pedidos */
DO:
...
DO:
  ...
  THEN DO:
       FIND T-Ped_detalle-bon 
            WHERE T-Ped_detalle-bon.cdg_bonificacion = v-cdg_bonificacion 
              AND T-Ped_detalle-bon.nro_pedido       = T-Ped_detalle.nro_pedido
              AND T-Ped_detalle-bon.nro_linea        = T-Ped_detalle.nro_linea 
                  NO-LOCK NO-ERROR.
...
DO:
    ...
    FOR EACH T-Ped_detalle-bon WHERE T-Ped_detalle-bon.nro_linea = 0:
        ASSIGN T-Ped_detalle-bon.nro_linea   = T-Ped_detalle.nro_linea.
    END.        
