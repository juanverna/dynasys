...
DEFINE VARIABLE h_v-rng-tipoarticulo AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     "  Rango de tipos de articulos a listar:" VIEW-AS TEXT
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Total remitido x tipo de artículo"
         HEIGHT             = 22.15
         WIDTH              = 114.29
         MAX-HEIGHT         = 22.15
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF W-Win /* Total remitido x tipo de artículo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Total remitido x tipo de artículo */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng-tipoarticulo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-tipoarticulo ).
       RUN set-position IN h_v-rng-tipoarticulo ( 3.69 , 14.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/DSP" "Total Remitido x Tipo de Artículo"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE des_tipoart     AS CHARACTER.
  DEFINE VARIABLE has_tipoart     AS CHARACTER.
  ...
  RUN dar_rango IN h_v-rng-tipoarticulo ( OUTPUT des_tipoart, 
                                          OUTPUT has_tipoart,
                                          OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
  ...
  THEN DO:
       ...
       RUN lsremitido_xtipoarticulo.p ( INPUT des_tipoart,
                                        INPUT has_tipoart,
                                        INPUT des_fecha,
                                        INPUT has_fecha
                                       ).
