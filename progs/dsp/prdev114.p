/*=================================================================================*/
/*                IMPRESION DE FORMULARIO DE SOLICITUDES DE DEVOLUCION             */
/*=================================================================================*/
...
DO TRANSACTION:
    ...
    OUTPUT STREAM Formulario TO "c:\sic-temp\ped114.xpr" CONVERT TARGET "iso8859-1".
    ...
    FILE-INFO:File-NAME = "c:\sic-temp\dev114.xpr".
...
*/
    ...
    RUN escribir ( INPUT "07,70", INPUT "SOLICITUD DE DEVOLUCION", INPUT 14, INPUT YES). 
    ...
    PUT STREAM Formulario UNFORMATTED "<=#1><BGCOLOR=WHITE><AT=+18,+183><FROM><AT=+8,+8><FILLRECT)>". 
    ...
    RUN escribir ( ch_linea(det0 + 1,-1)  + ",118", INPUT "Color", INPUT 8, INPUT NO).
    RUN escribir ( ch_linea(det0 + 1,-1)  + ",149", INPUT "Pedido", INPUT 8, INPUT NO).
    RUN escribir ( ch_linea(det0 + 1,-1)  + ",175", INPUT "Preparado", INPUT 8, INPUT NO).

    RUN linea    ( ch_linea(det0 - 2,1)  + ",5", INPUT "190", "H" ).
    RUN linea    ( "57,26", INPUT "180", "V" ).
    RUN linea    ( "57,110", INPUT "180", "V" ).
    RUN linea    ( "57,138", INPUT "205", "V" ).
    RUN linea    ( "57,168", INPUT "205", "V" ).
    RUN linea    ( "242,138", INPUT "57", "H" ).
    RUN escribir ( INPUT "238,143", INPUT "Valor Declarado", INPUT 8, INPUT YES).
    RUN escribir ( INPUT "238,171", INPUT "Cantidad Bultos", INPUT 8, INPUT YES).
...
PROCEDURE forma:
    ...
    FOR EACH Ped_detalle OF Ped_header, Articulo OF Ped_detalle,
        ...
        THEN DO:
             ...
             OUTPUT STREAM Formulario TO "c:\sic-temp\dev114.xpr" CONVERT TARGET "iso8859-1".
             RUN inicia_hoja.             
        END.
        ...
        RUN escribenumero ( ch_linea(det0,linea0)  + ",144", INPUT STRING(Ped_detalle.cantidad,"ZZZZZZ9"), INPUT 8, INPUT NO).
    ...
    RUN escribir ( ch_linea(det0,linea0 + 1)  + ",30", INPUT "FIN DE LOS ITEMS DE DEVOLUCION", INPUT 12, INPUT YES).
