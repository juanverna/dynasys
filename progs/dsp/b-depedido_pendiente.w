...
DEFINE VARIABLE cantidad_pendiente LIKE Ped_detalle.cantidad COLUMN-LABEL "Cantidad!Pendiente".
DEFINE VARIABLE granel_pendiente LIKE Ped_detalle.granel COLUMN-LABEL "Granel!Pendiente".
...
&Scoped-define INTERNAL-TABLES Ped_detalle Articulo
...
(Ped_detalle.cantidad - Ped_detalle.cantidad_cum) @ cantidad_pendiente ~
Articulo.a_granel ~
(Ped_detalle.granel - Ped_detalle.granel_cum) @ granel_pendiente ~
Articulo.cdg_ugranel Ped_detalle.cdg_estado 
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Ped_detalle OF Ped_header WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Articulo OF Ped_detalle NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table Ped_detalle Articulo
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Articulo.cdg_articulo FORMAT "X(12)":U WIDTH 13.4
      (Ped_detalle.cantidad - Ped_detalle.cantidad_cum) @ cantidad_pendiente COLUMN-LABEL "Cantidad!Pendiente" FORMAT "->>>,>>>,>>9.99":U
      Articulo.a_granel COLUMN-LABEL "Gra!nel" FORMAT "Si/No":U
      (Ped_detalle.granel - Ped_detalle.granel_cum) @ granel_pendiente COLUMN-LABEL "Granel!Pendiente" FORMAT "->>>,>>>,>>9.99":U
      Articulo.cdg_ugranel FORMAT "X(12)":U
      Ped_detalle.cdg_estado COLUMN-LABEL "Es-!tado" FORMAT "X(2)":U
            WIDTH 7.4
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 76 BY 6.67 EXPANDABLE.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "sic.Ped_detalle OF sic.Ped_header,sic.Articulo OF sic.Ped_detalle"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > sic.Articulo.cdg_articulo
"Articulo.cdg_articulo" ? ? "character" ? ? ? ? ? ? no ? no no "13.4" yes no no "U" "" ""
     _FldNameList[2]   > "_<CALC>"
"(Ped_detalle.cantidad - Ped_detalle.cantidad_cum) @ cantidad_pendiente" "Cantidad!Pendiente" "->>>,>>>,>>9.99" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > sic.Articulo.a_granel
"Articulo.a_granel" "Gra!nel" ? "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[4]   > "_<CALC>"
"(Ped_detalle.granel - Ped_detalle.granel_cum) @ granel_pendiente" "Granel!Pendiente" "->>>,>>>,>>9.99" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   = sic.Articulo.cdg_ugranel
     _FldNameList[6]   > sic.Ped_detalle.cdg_estado
"Ped_detalle.cdg_estado" "Es-!tado" ? "character" ? ? ? ? ? ? no ? no no "7.4" yes no no "U" "" ""
