...
PROCEDURE cabecera_forma:
 ...
 /* RUN escribir ( INPUT "40,10", INPUT "I.V.A. RESPONSABLE INSCRIPTO", INPUT 8, INPUT NO, INPUT "BLACK"). */
    ...
    RUN escribir ( INPUT "84,110", INPUT "REMITO: ", INPUT 8, INPUT NO, INPUT "BLACK").
...
PROCEDURE detalle_forma:
    ...
    RUN escribir ( "97,93", INPUT "PARTIDA", INPUT 8, INPUT YES, INPUT "BLACK").
    ...
    RUN escribir ( "97,164", INPUT "OBSERVACIONES", INPUT 8, INPUT YES, INPUT "BLACK").
...
PROCEDURE cabecera_datos:
    ...
    RUN escribir ( INPUT "7,98", INPUT "X", INPUT 16, INPUT YES, INPUT "BLACK"). 

    RUN escribir ( INPUT "11,120", INPUT STRING(Rem_header.prf_comprob,"9999") + "-" + STRING(Rem_header.nro_comprob,"99999999"), INPUT 12, INPUT YES, INPUT "BLACK"). 
    ...
    RUN escribir ( INPUT "51,10", INPUT Cliente.nom_cliente, INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( INPUT "51,85", INPUT "[" + Cliente.cdg_cliente + "]", INPUT 8, INPUT NO, INPUT "BLACK").
    RUN escribir ( INPUT "55,10", INPUT Cliente.direccion, INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( INPUT "59,10", INPUT "(" + Cliente.cdg_postal + ") " + Cliente.localidad, INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( INPUT "63,10", INPUT Provlegal.nombre , INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( INPUT "63,85", INPUT "[" + Provlegal.cdg_provincia + "]", INPUT 8, INPUT NO, INPUT "BLACK").
    RUN escribir ( INPUT "67,20", INPUT Cliente.cuit, INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( INPUT "67,65", INPUT Cliente.ing_brutos, INPUT 8, INPUT YES, INPUT "BLACK").
    ...
    RUN escribir ( INPUT "55,105", INPUT Domicilio.direccion, INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( INPUT "59,105", INPUT "(" + Domicilio.cdg_postal + ") " + Domicilio.localidad, INPUT 8, INPUT NO, INPUT "BLACK").
    ...
    RUN escribir ( INPUT "67,119", INPUT Usuario.nombre, INPUT 8, INPUT NO, INPUT "BLACK").

    RUN escribir ( INPUT "80,42", INPUT Condicion_venta.descripcion, INPUT 8, INPUT NO, INPUT "BLACK").
    RUN escribir ( INPUT "84,170", INPUT Rem_header.nro_ocm, INPUT 8, INPUT NO, INPUT "BLACK").
    
    FIND Ped_header WHERE Ped_header.nro_pedido = Rem_header.nro_pedido NO-LOCK NO-ERROR.
    IF AVAILABLE Ped_header
    THEN DO:
         RUN escribir ( INPUT "80,130", INPUT Ped_header.tip_comprob + "-" + STRING(Ped_header.prf_comprob,"9999") + "-" + STRING(Ped_header.nro_comprob,"99999999"), INPUT 8, INPUT NO, INPUT "BLACK").
    END.
...
/*                            NO-ERROR.                                                           */
/*                 IF AVAILABLE Articulo_atributo THEN                                            */
/*                 FIND Atributo OF Articulo_atributo NO-LOCK NO-ERROR.                           */
/*                 IF AVAILABLE atributo THEN                                                     */
/*                 RUN escribir      ( ch_linea  + ",93",  INPUT Atributo.abreviatura, INPUT 8, INPUT NO, INPUT "BLACK").  */

               RUN escribir      ( ch_linea  + ",93",  INPUT Partida.cdg_partida, INPUT 8, INPUT NO, INPUT "BLACK").
                RUN escribenumero ( ch_linea  + ",127", INPUT STRING(v-cantidad,"ZZZZZZ9.99"), INPUT 8, INPUT NO, INPUT "BLACK").
                RUN escribir      ( ch_linea  + ",147", INPUT Unidad.abrevia, INPUT 8, INPUT NO, INPUT "BLACK").
                RUN escribir      ( ch_linea  + ",107", INPUT Articulo.cdg_articulo, INPUT 8, INPUT NO, INPUT "BLACK").
                RUN escribir      ( ch_linea  + ",10",  INPUT Articulo.descripcion, INPUT 8, INPUT NO, INPUT "BLACK").
            ...
            ELSE DO:
                ...
                DO j = 1 TO NUM-ENTRIES(v-detallada,"|"):
                    ch_linea = STRING(102 + linea0 * 4,">>9").                     
                    RUN escribir ( INPUT ch_linea  + ",10", INPUT ENTRY(j,v-detallada, "|"), INPUT 8, INPUT NO, INPUT "BLACK").
                    IF j = NUM-ENTRIES(v-detallada,"|")
                    THEN DO:
                        RUN verificar_fin_hoja.
                        v-cantidad = Rem_detalle.cantidad.
                        RUN escribir      ( ch_linea  + ",93",  INPUT Partida.cdg_partida, INPUT 8, INPUT NO, INPUT "BLACK").
                        RUN escribenumero ( ch_linea  + ",128", INPUT STRING(v-cantidad,"ZZZZZZ9.99  ") + Unidad.abrevia, INPUT 8, INPUT NO, INPUT "BLACK").
                        RUN escribenumero ( ch_linea  + ",145", INPUT STRING(Rem_detalle.precio,"-ZZZ,ZZ9.99"), INPUT 8, INPUT NO, INPUT "BLACK").
                        RUN escribenumero ( ch_linea  + ",166", INPUT STRING(Rem_detalle.subtotal_bruto,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT NO, INPUT "BLACK").
                    END.
                    linea0 = linea0 + 1.
                END.
...
PROCEDURE pie_forma:
    ...
    RUN escribir ( "5,10", INPUT "Observaciones:", INPUT 8, INPUT NO, INPUT "BLACK").
    /*
