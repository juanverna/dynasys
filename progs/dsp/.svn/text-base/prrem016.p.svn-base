...
DEFINE VARIABLE v-bruto             LIKE Rem_header.imp_neto FORMAT "Z,ZZZ,ZZ9.99-".
DEFINE VARIABLE v-desc              LIKE Rem_header.imp_neto FORMAT "Z,ZZZ,ZZ9.99-".
DEFINE VARIABLE v-neto              LIKE Rem_header.imp_neto FORMAT "Z,ZZZ,ZZ9.99-".
...
FORM
    ...
    /*v-comprobante          AT 56*/
    SKIP(2)
    Rem_header.fecha       AT 52
    SKIP(6)
    Rem_header.nombre      AT 12
    "ENTREGAR EN:"         AT 57
    SKIP
    Cliente.cdg_cliente    AT 1
    Cliente.direccion      AT 12
    Rem_header.direccion   AT 57
    SKIP
    Cliente.cdg_postal     AT 12
    Cliente.localidad      
    Rem_header.cdg_postal  AT 57 FORMAT "X(4)"
    ...
    Rem_header.cuit        AT 57 FORMAT "X(15)"
    SKIP(1)
    Ped_header.prf_comprob AT 60 FORMAT "9999"
    Ped_header.nro_comprob FORMAT "99999999" 
    SKIP(1)
    Condicion_venta.descripcion  AT 1 
    Rem_header.nro_ocm  AT 60
    SKIP(1)
    /*
...
FORM
    ...
    "Total Bruto" AT 95
    Rem_header.imp_bruto AT 132  FORMAT "Z,ZZZ,ZZ9.99-"
    SKIP
    ...
    */
    ...
    "Total Neto" AT 95
    Rem_header.imp_neto AT 132  FORMAT "Z,ZZZ,ZZ9.99-"
    SKIP
...
FIND Moneda OF Rem_header NO-LOCK.
...
FOR EACH Rem_header_impuesto OF Rem_header, Impuesto OF Rem_header_impuesto:
    IF Impuesto.es_iva
        THEN ASSIGN prciva = Rem_header_impuesto.tasa
                    importe_iva = Rem_header_impuesto.importe.
        ELSE ASSIGN prcnoi = Rem_header_impuesto.tasa
                    importe_noi = Rem_header_impuesto.importe.
END.
...
DISPLAY
    ...
    Ped_header.prf_comprob WHEN AVAILABLE Ped_header
...
FOR EACH Rem_detalle OF Rem_header, Articulo OF Rem_detalle NO-LOCK,
  ...
  FOR EACH Rem_detalle-bon OF Rem_detalle NO-LOCK:
      v-bonifs = v-bonifs + STRING(Rem_detalle-bon.porcentaje,"ZZ9.99") + " ".
  END.    
  ...
  IF Rem_detalle.detallada <> ""
  THEN DO:

      DO j-tomo = 1 TO NUM-ENTRIES(Rem_detalle.detallada,CHR(10)):

          v-tomo = ENTRY(j-tomo,Rem_detalle.detallada,CHR(10)).
  ...
  DISPLAY  Rem_detalle.precio
           Rem_detalle.subtotal_bruto
           WITH FRAME frm-detalle USE-TEXT STREAM-IO DOWN.
