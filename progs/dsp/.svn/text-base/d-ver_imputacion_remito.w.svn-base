...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-10 Dialog-Frame _STRUCTURED
  QUERY BROWSE-10 NO-LOCK DISPLAY
      Cuenta.cdg_cuenta FORMAT "X(8)":U COLUMN-FONT 2
      Cuenta.nombre_cta COLUMN-LABEL "Nombre!Cuenta" FORMAT "X(28)":U
            WIDTH 30.86
      Entidad.cdg_entidad FORMAT "X(8)":U COLUMN-FONT 2
      Obra.cdg_obra FORMAT "X(8)":U COLUMN-FONT 2
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 100 BY 8.08.
...
"Cuenta.cdg_cuenta" ? ? "character" ? ? 2 ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > sic.Cuenta.nombre_cta
"Cuenta.nombre_cta" "Nombre!Cuenta" "X(28)" "character" ? ? ? ? ? ? no ? no no "30.86" yes no no "U" "" ""
     _FldNameList[3]   > sic.Entidad.cdg_entidad
"Entidad.cdg_entidad" ? ? "character" ? ? 2 ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[4]   > sic.Obra.cdg_obra
"Obra.cdg_obra" ? ? "character" ? ? 2 ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   > "_<CALC>"
"fn-importe (T-Asn_detalle.debito) @ v-debito" "Importe!Débitos" "X(15)" ? ? ? 2 ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[6]   > "_<CALC>"
"fn-importe(T-Asn_detalle.credito) @ v-credito" "Importe!Créditos" "X(15)" ? ? ? 2 ? ? ? no ? no no ? yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-10 */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH T-Sub_detalle_inv BY T-Sub_detalle_inv.tipo:
        ...
        END.
        ELSE DO:
            ASSIGN T-Asn_detalle.credito     = T-Sub_detalle_inv.valor
