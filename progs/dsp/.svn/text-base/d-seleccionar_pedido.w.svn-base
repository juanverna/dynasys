...
&Scoped-define OPEN-QUERY-BROWSE-2     /*     MESSAGE "que_empresa"      que_empresa        SKIP             "lista_tipos"      lista_tipos        SKIP             "que_cliente"      que_cliente        SKIP             "v-des_fecha"      v-des_fecha        SKIP             "v-has_fecha"      v-has_fecha        SKIP             "lista_estados"    lista_estados      SKIP             "que_sector"       que_sector         SKIP         VIEW-AS ALERT-BOX INFO BUTTONS OK.    */    IF por_cliente    THEN DO:       OPEN QUERY {&SELF-NAME}            FOR EACH Ped_header                   WHERE Ped_header.cdg_empresa = que_empresa                     AND CAN-DO(lista_tipos, ~
      ...
      Ped_header.estado), ~
                               FIRST Cliente OF Ped_header                             WHERE Cliente.lista_sectores CONTAINS que_sector.    END.    ELSE DO:       OPEN QUERY {&SELF-NAME}            FOR EACH Ped_header                   WHERE Ped_header.cdg_empresa = que_empresa                     AND CAN-DO(lista_tipos, ~
      Ped_header.tip_comprob)                     AND Ped_header.fecha >= v-des_fecha                     AND Ped_header.fecha <= v-has_fecha                     AND CAN-DO(lista_estados, ~
      Ped_header.estado), ~
                               FIRST Cliente OF Ped_header                             WHERE Cliente.lista_sectores CONTAINS que_sector.    END.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 101 BY 9.95.
...
/* Query rebuild information for BROWSE BROWSE-2
     _START_FREEFORM
    /*
    MESSAGE "que_empresa"      que_empresa        SKIP
            "lista_tipos"      lista_tipos        SKIP
            "que_cliente"      que_cliente        SKIP
            "v-des_fecha"      v-des_fecha        SKIP
            "v-has_fecha"      v-has_fecha        SKIP
            "lista_estados"    lista_estados      SKIP
            "que_sector"       que_sector         SKIP
