...
&Scoped-define QUERY-STRING-br_table FOR EACH Rem_header OF Cliente WHERE ~{&KEY-PHRASE} ~
      AND Rem_header.proc_estad = FALSE ~
 AND Rem_header.cdg_empresa = que_empresa  ~
 AND Rem_header.tip_comprob = "RM" NO-LOCK, ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Rem_header.tip_comprob FORMAT "X(3)":U WIDTH 4.4
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 79 BY 6.67
...
/* Query rebuild information for BROWSE br_table
     ...
     _TblOptList       = ", OUTER"
     _Where[1]         = "Rem_header.proc_estad = FALSE
 AND Rem_header.cdg_empresa = que_empresa 
...
"Rem_header.tip_comprob" ? ? "character" ? ? ? ? ? ? no ? no no "4.4" yes no no "U" "" ""
...
"Rem_header.origen" "Ori-!gen" ? "character" ? ? ? ? ? ? no ? no no "5.6" yes no no "U" "" ""
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* Remitos Pendientes */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* Remitos Pendientes */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* Remitos Pendientes */
DO:
