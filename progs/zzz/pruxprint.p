...
DO TRANSACTION:
    ...
    PUT CONTROL "<FORMAT=A4>".
    FIND FIRST Cliente.
    DO j = 1 TO 8:
       PUT CONTROL "<AT=" TRIM(STRING(J * 30,">>>9")) ",0" /* TRIM(STRING(J * 10,">>>9"))*/ "><#1>".
       RUN rect10x10.
       FIND NEXT Cliente.
    END.
    PUT CONTROL "<#1>".
    PUT "<AT=+10,+10><FROM><RECT#1>".    
...
PROCEDURE rect10X10:
  PUT "<Farial><P8>".
  PUT "<AT=+25,+100><FROM><RECT#1>".
 
  PUT "<=#1><AT=+5,+5>" Cliente.cdg_cliente.
  PUT "<=#1><AT=+9,+5><B>" Cliente.nom_cliente "</B>".
  PUT "<=#1><AT=+15,+5><#2>".
  PUT "<=#2><AT=+5,+75><#3>".
  PUT "<=#2><FROM><BARCODE#3,TYPE=EAN128A,CHECKSUM=TRUE,VALUE=" 
      STRING(Cliente.nro_cliente,"99999999") 
      STRING(Cliente.nro_cliente,"99999999") ">".
...
PROCEDURE forma:
    ...
    FOR EACH Fac_detalle OF Fac_header:
        ...
        IF Fac_detalle.detallada <> ""
            THEN RUN RENGLONS.P (INPUT  Fac_detalle.detallada + IF AVAILABLE Plan THEN " Plan " + Plan.dsc_plan ELSE "", 
