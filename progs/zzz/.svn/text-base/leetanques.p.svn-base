INPUT FROM c:\tanques.csv.
DEFINE STREAM mal.
OUTPUT STREAM mal TO c:\tanquesmal.csv.
...
REPEAT :
    ...
    IMPORT DELIMITER ";" a1 ^ ^ a4 a5 a6 a7 a8 ^ a10 a11 a12 a13.
    ...
    END.
    nn = 0.
    FOR EACH contrato_hd OF cliente WHERE
        year(contrato_hd.rige_desde) * 100 + MONTH(contrato_hd.rige_desde) <= year(f5) * 100 + MONTH(f5) 
        AND NOT contrato_hd.anulado AND
        contrato_hd.nro_tipo_evento = 3 BY  contrato_hd.nro_contrato DESC:
        nn = contrato_hd.nro_contrato.
        LEAVE.
    END.
    IF f5 >= TODAY THEN DO: 
    ...
    END.
    IF nn = 0 THEN DO:
        PUT STREAM mal "Contrato no registrado;"  k ";" a1 ";" a6 ";" a5  SKIP.
        NEXT.
    END.
/*creando*/
    CREATE evento.
