...
DEF VAR minff AS DATE.
    DEF TEMP-TABLE tt-contrato_hd LIKE contrato_hd
        FIELD imp_anticipo LIKE contrato_hd.imp_total
        FIELD sumatoria_fac AS INT
        FIELD cantidad_facturas AS INT.
    DEF TEMP-TABLE tt-contrato_dt NO-UNDO LIKE contrato_dt.
    DEF TEMP-TABLE tt-fac_header NO-UNDO LIKE fac_header.
    DEF TEMP-TABLE tt-fac_detalle NO-UNDO LIKE fac_detalle.
    DEFINE DATASET dset FOR tt-contrato_hd , tt-fac_header .
FOR EACH contrato_hd WHERE contrato_hd.estado = "A" AND contrato_hd.cant_periodos <> 0 AND NOT contrato_hd.anulado AND contrato_hd.rige_desde > 01/01/2007 AND
    contrato_hd.rige_desde < TODAY AND 
    contrato_hd.fecha_baja = ?  NO-LOCK TRANSACTION :
    ...
    IF abs(it - contrato_hd.imp_total) > 1 AND  minmes > cf AND cf <> contrato_hd.cant_periodos - contrato_hd.resto_periodos THEN DO:
    
    DISPLAY minff contrato_hd.resto_periodo minmes contrato_hd.cant_periodos cf contrato_hd.nro_contrato  it  contrato_hd.imp_total.
...
/*DATASET dset:WRITE-XML ("FILE", "c:\contratos_factura.xml",  TRUE,
                                       ?,"",YES,YES).               */
FOR EACH tt-contrato_hd:
    DISPLAY tt-contrato_hd.nro_contrato.
END.
