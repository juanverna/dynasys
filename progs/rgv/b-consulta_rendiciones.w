...
&Scoped-define QUERY-STRING-br_table FOR EACH Rendgastos_hd WHERE ~{&KEY-PHRASE} ~
      AND Rendgastos_hd.abierta = que_abierta ~
 AND Rendgastos_hd.cdg_tiporendgastos = v-que_tipo ~
...
&Scoped-Define ENABLED-OBJECTS v-que_tipo que_abierta btn_imprimir br_table ~
RECT-1 
&Scoped-Define DISPLAYED-OBJECTS v-que_tipo que_abierta 
...
DEFINE BUTTON btn_imprimir 
     LABEL "&Imprimir Rendición" 
     SIZE 22 BY 1.14.

DEFINE VARIABLE v-que_tipo AS CHARACTER FORMAT "X(256)":U 
     LABEL "Rendiciones" 
...
DEFINE VARIABLE que_abierta AS LOGICAL 
...
DEFINE FRAME F-Main
     v-que_tipo AT ROW 1.24 COL 17 COLON-ALIGNED
     que_abierta AT ROW 1.24 COL 78 NO-LABEL
     btn_cerrar AT ROW 1.24 COL 109
     btn_imprimir AT ROW 1.24 COL 133
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "Rendgastos_hd.abierta = que_abierta
 AND Rendgastos_hd.cdg_tiporendgastos = v-que_tipo
...
ON MOUSE-SELECT-DBLCLICK OF br_table IN FRAME F-Main /* Rendiciones Pendientes de Aprobación */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON RETURN OF br_table IN FRAME F-Main /* Rendiciones Pendientes de Aprobación */
DO:
  DEFINE VARIABLE rid AS ROWID.
  rid = ROWID(Rendgastos_hd).
  RUN c-rendicion_gastos.w ( INPUT-OUTPUT rid, INPUT 2).
END.
...
ON CHOOSE OF btn_imprimir IN FRAME F-Main /* Imprimir Rendición */
DO:
    DEFINE VARIABLE sino-msg AS LOGICAL.
    IF AVAILABLE Rendgastos_hd
    THEN DO:
        sino-msg = NO.
        MESSAGE "Desea REIMPRIMIR esta rendición"
                VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO TITLE "Confirmación" UPDATE sino-msg.
        IF sino-msg
        THEN DO:
...
&Scoped-define SELF-NAME que_abierta
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL que_abierta B-table-Win
ON VALUE-CHANGED OF que_abierta IN FRAME F-Main
DO:
  ASSIGN que_abierta.
  btn_cerrar:SENSITIVE IN FRAME {&FRAME-NAME} = que_abierta.
  RUN dispatch IN THIS-PROCEDURE ('open-query').
END.
...
&Scoped-define SELF-NAME v-que_tipo
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL v-que_tipo B-table-Win
ON VALUE-CHANGED OF v-que_tipo IN FRAME F-Main /* Rendiciones */
DO:
  ASSIGN v-que_tipo.
  RUN dispatch IN THIS-PROCEDURE ('open-query').
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE cerrar_rendicion B-table-Win 
PROCEDURE cerrar_rendicion :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION:
      ...
      Rendgastos_hd.abierta = NO.
      Rendgastos_hd.cdg_estado = "CE".
...
------------------------------------------------------------------------------*/
   ...
   THEN DO:
       ...
       v-que_tipo:LIST-ITEM-PAIRS IN FRAME {&FRAME-NAME} = x-listas.
       v-que_tipo = ENTRY(2,x-listas,",").
   END.
   ELSE DO:
