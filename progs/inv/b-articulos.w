...
&Scoped-define QUERY-STRING-br_table FOR EACH Articulo WHERE ~{&KEY-PHRASE} ~
 ...
 AND can-do (Articulo.lista_sectores, que_sector) ~
...
&Scoped-Define ENABLED-OBJECTS RECT-5 v-estado v-codigo br_table 
&Scoped-Define DISPLAYED-OBJECTS v-estado v-codigo 
...
DEFINE VARIABLE v-codigo AS CHARACTER FORMAT "X(14)":U 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 20 BY 22.05
...
DEFINE FRAME F-Main
     v-estado AT ROW 1.29 COL 2 NO-LABEL
     v-codigo AT ROW 2.86 COL 2 NO-LABEL
     br_table AT ROW 3.95 COL 2
...
ASSIGN 
       br_table:MAX-DATA-GUESS IN FRAME F-Main         = 40.
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "Articulo.cdg_estado = v-estado
 AND can-do (Articulo.lista_sectores, que_sector)
 AND can-do (Articulo.lista_empresas, que_empresa)"
     _FldNameList[1]   > sic.Articulo.cdg_articulo
"Articulo.cdg_articulo" ? ? "character" ? ? ? ? ? ? no ? no no "15.2" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  ...
  &SCOPED-DEFINE CODIGO2 cdg_estado
  &SCOPED-DEFINE CODIGO3 lista_sectores
  &SCOPED-DEFINE CODIGO4 lista_empresas
  ...
  THEN DO:
    ...
    FIND FIRST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} 
         WHERE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.{&CODIGO}  >= v-codigo
           AND {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.{&CODIGO2}  = v-estado
           AND CAN-DO({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.{&CODIGO3},que_sector)
           AND CAN-DO({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.{&CODIGO4},que_empresa)
                NO-LOCK NO-ERROR.
    IF NOT AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} 
       THEN FIND LAST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}
                    WHERE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.{&CODIGO2} = v-estado
                      AND CAN-DO({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.{&CODIGO3},que_sector)
                      AND CAN-DO({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.{&CODIGO4},que_empresa)
                       NO-LOCK.
...
/*         FOR EACH Articulo                                                                    */
/*                WHERE Articulo.cdg_estado = v-estado                                          */
/*                  AND Articulo.cdg_articulo BEGINS v-codigo                                   */
/*                  AND CAN-DO(Articulo.lista_sectores,que_sector)                              */
/*                  AND CAN-DO(Articulo.lista_empresas,que_empresa) NO-LOCK INDEXED-REPOSITION. */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL v-estado B-table-Win
ON VALUE-CHANGED OF v-estado IN FRAME F-Main
DO:
  ASSIGN v-estado.
