...
DEFINE VARIABLE h_b-deposito AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-deposito AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     Btn_Done AT ROW 1.71 COL 115
     RECT-1 AT ROW 1.48 COL 114
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-deposito.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-deposito ).
       RUN set-position IN h_b-deposito ( 1.48 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-deposito ( 19.29 , 17.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-deposito.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-deposito ).
       RUN set-position IN h_v-deposito ( 1.48 , 22.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.62 , 87.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 14.33 , 22.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.14 , 87.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-deposito. */
       RUN add-link IN adm-broker-hdl ( h_b-deposito , 'Record':U , h_v-deposito ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-deposito ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-deposito ,
             Btn_Done:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-deposito ,
             h_b-deposito , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/INV" "Mantenimiento de Depósitos"}
