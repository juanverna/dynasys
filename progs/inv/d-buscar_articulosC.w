...
&Scoped-Define ENABLED-OBJECTS Btn_OK que_nombre v-estado Btn_Cancel ~
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 149 BY 18.71
         FONT 4
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   FRAME-NAME                                                           */
/* BROWSE-TAB br_articulos btn_todos Dialog-Frame */
ASSIGN 
...
/* Query rebuild information for BROWSE br_articulos
    ...
    CASE buscar_por:
         ...
         THEN DO:
              ...
              OPEN QUERY {&BROWSE-NAME}
                   FOR EACH Articulo NO-LOCK WHERE articulo.compras_sino
                                               AND    Articulo.cdg_articulo BEGINS que_numero
    ...
    /*          BROWSE {&BROWSE-NAME}:TITLE = "Artículos que satisfacen la condición de búsqueda: POR NRO.AFILIADO=" + que_numero.*/
         ...
         THEN DO:
              ...
              OPEN QUERY {&BROWSE-NAME}
                   FOR EACH Articulo NO-LOCK WHERE articulo.compras_sino
                                               AND Articulo.descripcion CONTAINS que_nombre
                                               AND Articulo.cdg_estado = v-estado
                                               AND Articulo.lista_sectores CONTAINS que_area
                                               AND Articulo.lista_empresas CONTAINS que_empresa.
...
------------------------------------------------------------------------------*/
    ...
    CASE buscar_por:
         ...
         THEN DO:
              OPEN QUERY {&BROWSE-NAME}
                   FOR EACH Articulo NO-LOCK WHERE Articulo.compras_sino 
                                               AND Articulo.cdg_articulo BEGINS clave_buscar
                                               AND Articulo.cdg_estado = v-estado
                                               AND Articulo.lista_sectores CONTAINS que_area
                                               AND Articulo.lista_empresas CONTAINS que_empresa.
         ...
         THEN DO:
              OPEN QUERY {&BROWSE-NAME}
                   FOR EACH Articulo NO-LOCK WHERE Articulo.compras_sino 
                                               AND Articulo.descripcion CONTAINS clave_buscar
                                               AND Articulo.cdg_estado = v-estado
                                               AND Articulo.lista_sectores CONTAINS que_area
                                               AND Articulo.lista_empresas CONTAINS que_empresa.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE Btn_OK que_nombre v-estado Btn_Cancel que_numero btn_todos 
         br_articulos 
      WITH FRAME Dialog-Frame.
