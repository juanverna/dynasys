/*====================================================================================*/
/*                       GENERACION AUTOMATICA DE REPOSICIONES                        */
/*====================================================================================*/
...
DEFINE INPUT PARAMETER cdg_deposito-salida LIKE Deposito.cdg_deposito. /* Deposito de salida */
DEFINE INPUT PARAMETER gen_requis          AS LOGICAL FORMAT "Si/No".
DEFINE INPUT PARAMETER ver_compras         AS LOGICAL INITIAL NO.
...
DEFINE BUFFER Deposito-salida  FOR Deposito.
...
DEFINE FRAME frm-titulo HEADER
  ...
  "Necesidades de Reposición por Depósito" AT 35
  ...
  hora_lis AT 102
  SKIP(1)
  WITH WIDTH 171 FRAME frm-titulo PAGE-TOP USE-TEXT STREAM-IO.
...
DEFINE FRAME frm-listado
       ...
       sal_granel COLUMN-LABEL "Existencia!Real"
       com_granel COLUMN-LABEL "Granel!en Repos."
       rep_granel COLUMN-LABEL "Necesidad!Reposición"
       WITH WIDTH 171 DOWN FRAME frm-listado USE-TEXT STREAM-IO.
...
PROCEDURE LISTAR:
  ...
  FIND Deposito-salida WHERE Deposito-salida.cdg_deposito = cdg_deposito-salida NO-LOCK .
  ...
  FOR EACH Deposito WHERE Deposito.cdg_deposito <= has_coddep
      ...
      titulo_det = "Depósito: " + STRING(Deposito.cdg_deposito) +  " - " + 
               Deposito.nombre + " Al " + STRING(has_fecha).
      ...
      FOR EACH Articulo-deposito OF Deposito
          WHERE Articulo-deposito.cdg_empresa = Empresa.cdg_empresa
             ...
             OR ( Articulo-deposito.remanente_granel - Articulo-deposito.reservado_granel <
               ...
               WHERE Articulo.Cdg_Articulo >= des_codigo 
         ...
         THEN DO:
              ...
              THEN DO:
                   ...
                   FIND FIRST T-Rqs_header 
                        WHERE T-Rqs_header.cdg_deposito-rep = Deposito.nro_deposito
                              EXCLUSIVE-LOCK NO-ERROR.
                   IF NOT AVAILABLE T-Rqs_header
                   THEN DO:
                        RUN CREAR_PEDIDO-HD.
                   END.
                   RUN CREAR_PEDIDO-DT.
...
PROCEDURE CREAR_PEDIDO-HD:

  FIND FIRST Sector-deposito OF Deposito WHERE Sector-deposito.modo_relacion = "A" NO-LOCK NO-ERROR.
  
  CREATE T-Rqs_header.
  ASSIGN T-Rqs_header.cdg_empresa      = Empresa.cdg_empresa
         T-Rqs_header.cdg_deposito-rep = Deposito.nro_deposito
         T-Rqs_header.nro_deposito     = Deposito-salida.nro_deposito
         T-Rqs_header.nro_area         = Sector-deposito.nro_area
         T-Rqs_header.fecha            = TODAY
         T-Rqs_header.nro_comprob      = NEXT-VALUE(proxima_transaccion)
         T-Rqs_header.nro_entidad      = Deposito-salida.nro_entidad
...
PROCEDURE BAJAR_DATOS:
  ...
  DO TRANSACTION:
     ...
     FIND Parametro WHERE Parametro.cdg_parametro = "PROXNRQS" 
