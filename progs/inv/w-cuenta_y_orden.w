...
DEFINE VARIABLE h_b-cuenta_y_orden AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-cyorden AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-que_articulo ( 1.00 , 26.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.67 , 94.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-cuenta_y_orden.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cuenta_y_orden ).
       RUN set-position IN h_b-cuenta_y_orden ( 2.91 , 26.00 ) NO-ERROR.
       RUN set-size IN h_b-cuenta_y_orden ( 20.00 , 80.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 2.91 , 107.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 23.57 , 13.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cyorden ).
       RUN set-position IN h_v-cyorden ( 23.14 , 26.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.33 , 78.00 ) */

       /* Links to SmartViewer h_v-que_articulo. */
       RUN add-link IN adm-broker-hdl ( h_b-articulo , 'Record':U , h_v-que_articulo ).

       /* Links to SmartBrowser h_b-cuenta_y_orden. */
       RUN add-link IN adm-broker-hdl ( h_b-articulo , 'Record':U , h_b-cuenta_y_orden ).

       /* Links to SmartViewer h_v-cyorden. */
       RUN add-link IN adm-broker-hdl ( h_b-cuenta_y_orden , 'Record':U , h_v-cyorden ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-cyorden ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-que_articulo ,
             h_b-articulo , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cuenta_y_orden ,
             h_v-que_articulo , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_b-cuenta_y_orden , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-cyorden ,
             h_p-updspa , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/INV" "Mantenimiento de Relaciones de Cuenta y Orden"}
