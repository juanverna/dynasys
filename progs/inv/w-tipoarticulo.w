...
DEFINE VARIABLE h_b-tipo_articulo AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-tipoarticulo AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-tipo_articulo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tipo_articulo ).
       RUN set-position IN h_b-tipo_articulo ( 1.71 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-tipo_articulo ( 19.67 , 15.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-tipoarticulo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-tipoarticulo ).
       RUN set-position IN h_v-tipoarticulo ( 1.81 , 21.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.38 , 83.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 8.81 , 21.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.14 , 72.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-tipoarticulo. */
       RUN add-link IN adm-broker-hdl ( h_b-tipo_articulo , 'Record':U , h_v-tipoarticulo ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-tipoarticulo ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-tipoarticulo ,
             h_b-tipo_articulo , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-tipoarticulo , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/INV" "Mantenimiento de Tipos de Artículos"}
