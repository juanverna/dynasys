...
&Scoped-define INTERNAL-TABLES Cct_stock Partida Deposito
...
Cct_stock.tipo_mov Deposito.cdg_deposito Cct_stock.cantidad ~
Cct_stock.granel Cct_stock.presupuestado Partida.cdg_partida ~
Partida.fecha_alta Partida.fecha_vencimiento 
...
&Scoped-define QUERY-STRING-br_table FOR EACH Cct_stock OF Articulo WHERE ~{&KEY-PHRASE} ~
 ...
 AND Deposito.cdg_deposito = que_deposito NO-LOCK, ~
      EACH Partida OF Cct_stock NO-LOCK, ~
      EACH Deposito OF Cct_stock NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Cct_stock OF Articulo WHERE ~{&KEY-PHRASE} ~
 ...
 AND Deposito.cdg_deposito = que_deposito NO-LOCK, ~
      ...
      EACH Deposito OF Cct_stock NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table Cct_stock Partida Deposito
&Scoped-define FIRST-TABLE-IN-QUERY-br_table Cct_stock
&Scoped-define SECOND-TABLE-IN-QUERY-br_table Partida
&Scoped-define THIRD-TABLE-IN-QUERY-br_table Deposito


/* Definitions for FRAME F-Main                                         */

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS que_deposito des_fecha has_fecha ~
btn_consolidado btn_imprimir br_table RECT-7 RECT-8 
&Scoped-Define DISPLAYED-OBJECTS que_deposito des_fecha has_fecha 
...
DEFINE VARIABLE que_deposito AS CHARACTER FORMAT "X(10)":U INITIAL "0" 
     LABEL "Depósito" 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Cct_stock.nro_comprob COLUMN-LABEL "Numero!Compbte." FORMAT "->,>>>,>>9":U
            WIDTH 11.2
      Cct_stock.tipo_mov COLUMN-LABEL "I!E" FORMAT "X(1)":U WIDTH 3.4
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 116 BY 17.62
...
DEFINE FRAME F-Main
     que_deposito AT ROW 1.24 COL 14 COLON-ALIGNED
     des_fecha AT ROW 2.91 COL 14 COLON-ALIGNED
     has_fecha AT ROW 2.91 COL 33 COLON-ALIGNED
     btn_consolidado AT ROW 2.91 COL 80
     btn_imprimir AT ROW 2.91 COL 99
     br_table AT ROW 4.33 COL 1
     RECT-7 AT ROW 1 COL 1
     RECT-8 AT ROW 2.67 COL 1
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "sic.Cct_stock OF sic.Articulo,sic.Partida OF sic.Cct_stock,sic.Deposito OF sic.Cct_stock"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "Cct_stock.cdg_empresa = que_empresa
 AND Cct_stock.fecha <= has_fecha
 AND Cct_stock.fecha >= des_fecha
 AND Deposito.cdg_deposito = que_deposito"
...
"Cct_stock.nro_comprob" "Numero!Compbte." ? "integer" ? ? ? ? ? ? no ? no no "11.2" yes no no "U" "" ""
     _FldNameList[5]   > sic.Cct_stock.tipo_mov
"Cct_stock.tipo_mov" "I!E" ? "character" ? ? ? ? ? ? no ? no no "3.4" yes no no "U" "" ""
     _FldNameList[6]   = sic.Deposito.cdg_deposito
     _FldNameList[7]   > sic.Cct_stock.cantidad
"Cct_stock.cantidad" "Cantidad!Movimiento" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[8]   > sic.Cct_stock.granel
"Cct_stock.granel" "Granel!Movimiento" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[9]   = sic.Cct_stock.presupuestado
     _FldNameList[10]   = sic.Partida.cdg_partida
     _FldNameList[11]   > sic.Partida.fecha_alta
"Partida.fecha_alta" "Fecha!Alta" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[12]   > sic.Partida.fecha_vencimiento
...
ON VALUE-CHANGED OF que_deposito IN FRAME F-Main /* Depósito */
DO:
  ASSIGN que_deposito.
  RUN dispatch IN THIS-PROCEDURE ('open-query':U).
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Deposito:
      xx = xx +  "," + Deposito.nombre + "," + STRING(Deposito.cdg_deposito).
  END.    
  que_deposito:LIST-ITEM-PAIRS IN FRAME {&FRAME-NAME} = SUBSTRING(xx,2).
  que_deposito = ENTRY(2,que_deposito:LIST-ITEM-PAIRS).
