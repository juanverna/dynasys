...
DEFINE VARIABLE h_v-par-asnventas AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 15.76 COL 12 COLON-ALIGNED NO-LABEL
     "  Rango de Fechas de Transacciones:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 1.48 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Parámetros Generales del Listado:" VIEW-AS TEXT
          SIZE 75 BY 1 AT ROW 5.52 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Asiento Contable:" VIEW-AS TEXT
          SIZE 24 BY 1 AT ROW 5.57 COL 90
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Moneda en la que se expresan los asientos:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 11.48 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
ON END-ERROR OF W-Win /* Asientos de Inventario */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Asientos de Inventario */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-fechas ( 2.76 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-par-asnventas.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-par-asnventas ).
       RUN set-position IN h_v-par-asnventas ( 6.81 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.29 , 100.00 ) */
       ...
       RUN set-position IN h_v-que-expresion ( 12.91 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 17.14 , 14.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-par-asnventas ,
             h_v-rng-fechas , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-que-expresion ,
             h_v-par-asnventas , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute PRIOR to standard behavior. */
   ...
   RUN fijar_lista IN h_v-par-asnventas ( INPUT "*").
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/INV" "Asientos de Inventario"}
...
------------------------------------------------------------------------------*/
  ...
  RUN dar_rango IN h_v-par-asnventas (  OUTPUT  des_ptovta,
                                        OUTPUT  has_ptovta,
                                        OUTPUT  lista_tipos,
                                        OUTPUT  lis_fecha,
                                        OUTPUT  gen_asiento,
  ...
  THEN DO:
       ...
       RUN lsasientos_inventario.p ( INPUT des_fecha,                                       
                                     INPUT has_fecha,                                       
                                     INPUT des_ptovta,                                      
                                     INPUT has_ptovta,                                      
                                     INPUT lista_tipos,                                     
