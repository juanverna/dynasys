...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME brw_articulos

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES Articulo Clase

/* Definitions for BROWSE brw_articulos                                 */
&Scoped-define FIELDS-IN-QUERY-brw_articulos Articulo.cdg_articulo ~
Articulo.descripcion 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brw_articulos 
&Scoped-define QUERY-STRING-brw_articulos FOR EACH Articulo NO-LOCK
&Scoped-define OPEN-QUERY-brw_articulos OPEN QUERY brw_articulos FOR EACH Articulo NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brw_articulos Articulo
&Scoped-define FIRST-TABLE-IN-QUERY-brw_articulos Articulo
...
&Scoped-Define ENABLED-OBJECTS que_subclase brw_clasificacion camino ~
brw_articulos Btn_OK Btn_Cancel RECT-1 
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY brw_articulos FOR 
      Articulo SCROLLING.
...
/* Browse definitions                                                   */
DEFINE BROWSE brw_articulos
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brw_articulos Dialog-Frame _STRUCTURED
  QUERY brw_articulos DISPLAY
      Articulo.cdg_articulo FORMAT "X(12)":U WIDTH 18.2
      Articulo.descripcion FORMAT "X(40)":U WIDTH 62.2
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 86 BY 9.14
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 86 BY 9.67
         BGCOLOR 11 FGCOLOR 9 FONT 4
...
DEFINE FRAME Dialog-Frame
     que_subclase AT ROW 2.43 COL 2 COLON-ALIGNED NO-LABEL
     que_nombre AT ROW 2.43 COL 22 COLON-ALIGNED NO-LABEL
     que_clase AT ROW 2.43 COL 91 COLON-ALIGNED NO-LABEL
     brw_clasificacion AT ROW 3.76 COL 4
     camino AT ROW 3.76 COL 93 NO-LABEL
     brw_articulos AT ROW 13.76 COL 4
     Btn_OK AT ROW 21.52 COL 94
     Btn_Cancel AT ROW 21.52 COL 120
     RECT-1 AT ROW 21.29 COL 93
     "   Indique la clase de articulo" VIEW-AS TEXT
          SIZE 86 BY 1 AT ROW 1.24 COL 4
          BGCOLOR 5 FGCOLOR 15 
     "   Clasificación seleccionada" VIEW-AS TEXT
          SIZE 52 BY 1 AT ROW 1.24 COL 93
          BGCOLOR 5 FGCOLOR 15 
     SPACE(1.59) SKIP(21.32)
...
/* BROWSE-TAB brw_articulos camino Dialog-Frame */
ASSIGN 
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE brw_articulos
/* Query rebuild information for BROWSE brw_articulos
     _TblList          = "sic.Articulo"
     _FldNameList[1]   > sic.Articulo.cdg_articulo
"Articulo.cdg_articulo" ? ? "character" ? ? ? ? ? ? no ? no no "18.2" yes no no "U" "" ""
     _FldNameList[2]   > sic.Articulo.descripcion
"Articulo.descripcion" ? ? "character" ? ? ? ? ? ? no ? no no "62.2" yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE brw_articulos */
&ANALYZE-RESUME
...
"SUBSTRING(Clase.cdg_subclase,LENGTH(que_clase) + 2)" "Código!Subclase" "X(10)" ? ? ? ? ? ? ? no ? no no "18.2" yes no no "U" "" ""
     _FldNameList[2]   > Temp-Tables.Clase.nombre_subclase
"Clase.nombre_subclase" "Denominacion!Subclase" "X(40)" "character" ? ? ? ? ? ? no ? no no "62.2" yes no no "U" "" ""
...
------------------------------------------------------------------------------*/
  ...
  ENABLE que_subclase brw_clasificacion camino brw_articulos Btn_OK Btn_Cancel 
         RECT-1 
      WITH FRAME Dialog-Frame.
