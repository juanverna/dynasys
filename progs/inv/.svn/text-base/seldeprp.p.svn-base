/*==========================================================================================*/
/*         SELECCION DE LOS DEPOSITOS A LOS CUALES SE PIDE PARA REPOSICION                  */
/*==========================================================================================*/

&GLOBAL-DEFINE TABLA           Deposito
&GLOBAL-DEFINE CODIGO          cdg_deposito
&GLOBAL-DEFINE NOMBRE          nombre
&GLOBAL-DEFINE PROCESO_INIT    FIND Area WHERE ROWID(Area) = act_area NO-LOCK.
&GLOBAL-DEFINE TITULO-BROWSE   Seleccione el depósito o ingrese nombre
&GLOBAL-DEFINE TITULO-FRAME    Selección de depósitos para PEDIDOS
&GLOBAL-DEFINE PROCESO         ACBRWDEP
&GLOBAL-DEFINE ULT_REGISTRO    ult_deposito
...
DEFINE QUERY brw_registro FOR {&TABLA}, Sector-deposito SCROLLING.
...
DEFINE BROWSE brw_registro 
       ...
       WITH NO-LABELS NO-UNDERLINE 10 DOWN 
              FGCOLOR h-fg_c BGCOLOR h-bg_c FONT 4
...
FORM
     ...
     WITH 1 DOWN OVERLAY SIDE-LABELS CENTERED THREE-D FRAME frm-sel
          BGCOLOR f-bg_c FGCOLOR f-fg_c FONT 4 TITLE "{&TITULO-FRAME}"
          VIEW-AS DIALOG-BOX.
...
DO WITH FRAME frm-sel:
   ...
   btn_elegir:WIDTH    = TRUNCATE ( ancho / 2 , 0).
   btn_salir:WIDTH     = TRUNCATE ( ancho / 2 , 0).
   btn_elegir:COLUMN   = 1.
   btn_salir:COLUMN    = FRAME frm-sel:WIDTH  - btn_salir:WIDTH - 0.5.
   btn_elegir:ROW      = FRAME frm-sel:HEIGHT - btn_elegir:HEIGHT - 0.2.
   btn_salir:ROW       = FRAME frm-sel:HEIGHT - btn_salir:HEIGHT - 0.2.

END.

FIND Area WHERE ROWID(Area) = act_area NO-LOCK.
...
PROCEDURE ABRE_QUERY:
     ...
     OPEN QUERY brw_registro
          FOR EACH {&TABLA} WHERE {&NOMBRE} BEGINS des_registro 
                              AND Deposito.recibe_pedidos = "S",
              EACH Sector-deposito OF Deposito WHERE Sector-deposito.nro_area = Area.nro_area
              BY {&NOMBRE}.
