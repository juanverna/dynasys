...
&Scoped-Define ENABLED-FIELDS Deposito.nombre 
...
&Scoped-Define DISPLAYED-FIELDS Deposito.nombre 
&Scoped-define DISPLAYED-TABLES Deposito
&Scoped-define FIRST-DISPLAYED-TABLE Deposito
&Scoped-Define DISPLAYED-OBJECTS v-cdg_deposito 
...
DEFINE VARIABLE v-cdg_deposito AS CHARACTER FORMAT "X(256)" INITIAL "0" 
     LABEL "Depósito" 
...
DEFINE FRAME F-Main
     v-cdg_deposito AT ROW 1.24 COL 8.6 COLON-ALIGNED
     Deposito.nombre AT ROW 1.24 COL 13 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN NATIVE 
          SIZE 22 BY .76
          BGCOLOR 11 FGCOLOR 9 
...
DO:
  FIND FIRST B-Deposito WHERE B-Deposito.cdg_deposito = INPUT FRAME {&FRAME-NAME} v-cdg_deposito NO-LOCK NO-ERROR.
  IF AVAILABLE B-Deposito
  THEN DO:
      ...
      /* Ask the Record-Source for the current customer record.  Make sure
       ...
       IF NUM-ENTRIES (c) eq 1 THEN DO:
          h = WIDGET-HANDLE (c).
          RUN posicionar_query IN h (INPUT ROWID(B-Deposito)).
       END.
  END.
  ELSE DO:
       MESSAGE "No existe el deposito indicado" VIEW-AS ALERT-BOX MESSAGE.
  END.
END.
