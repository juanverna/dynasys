...
&Scoped-define QUERY-STRING-br_table FOR EACH Cct_stock OF Articulo WHERE ~{&KEY-PHRASE} ~
 ...
 AND Cct_stock.fecha >= des_fecha NO-LOCK, ~
...
&Scoped-Define ENABLED-OBJECTS des_fecha has_fecha br_table RECT-7 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Cct_stock.nro_comprob FORMAT ">>>>>>9":U WIDTH 11.2
      Cct_stock.tipo_mov COLUMN-LABEL "IN!EG" FORMAT "X(1)":U WIDTH 3.2
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 115 BY 19.52
...
DEFINE FRAME F-Main
     des_fecha AT ROW 1.24 COL 10 COLON-ALIGNED
     has_fecha AT ROW 1.24 COL 30 COLON-ALIGNED
     btn_consolidado AT ROW 1.24 COL 75
...
/* SETTINGS FOR BUTTON btn_consolidado IN FRAME F-Main
   NO-ENABLE                                                            */
...
"Cct_stock.nro_comprob" ? ? "integer" ? ? ? ? ? ? no ? no no "11.2" yes no no "U" "" ""
     _FldNameList[5]   > sic.Cct_stock.tipo_mov
"Cct_stock.tipo_mov" "IN!EG" ? "character" ? ? ? ? ? ? no ? no no "3.2" yes no no "U" "" ""
