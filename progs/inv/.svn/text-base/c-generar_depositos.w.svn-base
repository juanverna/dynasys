...
DEFINE TEMP-TABLE T-Deposito LIKE Deposito.
DEFINE TEMP-TABLE O-Deposito LIKE Deposito.

DEFINE BUFFER Subclase FOR Clase_de_deposito.
DEFINE BUFFER Clase    FOR Clase_de_deposito.
DEFINE BUFFER Superior FOR Clase_de_deposito.
...
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH Clase_de_deposito ~
      WHERE Clase_de_deposito.cdg_clasedep  BEGINS que_subclase NO-LOCK INDEXED-REPOSITION
...
v-cdg_deposito v-modo_bajar BROWSE-1 
&Scoped-Define DISPLAYED-OBJECTS v-origen v-cdg_deposito v-dsc_deposito ~
...
DEFINE VARIABLE v-cdg_deposito AS CHARACTER FORMAT "X(15)":U 
     LABEL "Depósito Modelo" 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-1 C-Win _STRUCTURED
  QUERY BROWSE-1 NO-LOCK DISPLAY
      Clase_de_deposito.cdg_subclasedep FORMAT "X(8)":U WIDTH 37.2
      Clase_de_deposito.nombre_subclasedep FORMAT "X(30)":U WIDTH 59.6
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME DEFAULT-FRAME
     ...
     v-cdg_deposito AT ROW 2.43 COL 18 COLON-ALIGNED
     v-dsc_deposito AT ROW 2.43 COL 40 COLON-ALIGNED NO-LABEL
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Generación masiva de ubicaciones físicas"
         HEIGHT             = 25.91
         WIDTH              = 106.2
         MAX-HEIGHT         = 27.67
         MAX-WIDTH          = 160
...
/* Query rebuild information for BROWSE BROWSE-1
     ...
     _Where[1]         = "Clase_de_deposito.cdg_clasedep  BEGINS que_subclase"
     _FldNameList[1]   > sic.Clase_de_deposito.cdg_subclasedep
"Clase_de_deposito.cdg_subclasedep" ? ? "character" ? ? ? ? ? ? no ? no no "37.2" yes no no "U" "" ""
     _FldNameList[2]   > sic.Clase_de_deposito.nombre_subclasedep
"Clase_de_deposito.nombre_subclasedep" ? ? "character" ? ? ? ? ? ? no ? no no "59.6" yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
...
ON END-ERROR OF C-Win /* Generación masiva de ubicaciones físicas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Generación masiva de ubicaciones físicas */
DO:
...
DO:
    FIND Deposito WHERE Deposito.cdg_deposito = v-cdg_deposito:INPUT-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
...
ON MOUSE-SELECT-DBLCLICK OF v-cdg_deposito IN FRAME DEFAULT-FRAME /* Depósito Modelo */
OR MOUSE-MENU-DOWN,"." OF v-cdg_deposito IN FRAME {&FRAME-NAME}
DO:

  DEFINE VARIABLE rid_deposito AS ROWID.

  RUN seldepos.p ( INPUT-OUTPUT rid_deposito, 
                   INPUT YES ).
  ...
  THEN DO:
       ...
       APPLY "RETURN" TO v-cdg_deposito IN FRAME {&FRAME-NAME}.
  END.
  RETURN NO-APPLY.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL v-cdg_deposito C-Win
ON RETURN OF v-cdg_deposito IN FRAME DEFAULT-FRAME /* Depósito Modelo */
DO:
...
FIND Clase_de_deposito WHERE ROWID(Clase_de_deposito) = p-que_origen NO-LOCK.
que_clase = Clase_de_deposito.cdg_clase.
que_subclase = Clase_de_deposito.cdg_subclase.
v-origen = Clase_de_deposito.cdg_subclase.
...
------------------------------------------------------------------------------*/
  DISPLAY v-origen v-cdg_deposito v-dsc_deposito v-cdg_generacion 
          v-dsc_generacion v-cantidad v-tiempo v-artseg v-modo_bajar 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE btn_generar-2 btn_generar btn_salir v-cdg_deposito v-modo_bajar 
         BROWSE-1 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
------------------------------------------------------------------------------*/
    ...
    FIND Deposito WHERE Deposito.cdg_deposito = v-cdg_deposito:INPUT-VALUE IN FRAME {&FRAME-NAME} NO-LOCK.
    CREATE O-Deposito.
    BUFFER-COPY Deposito TO O-Deposito. 
    ...
    FOR EACH Clase WHERE NOT CAN-FIND(FIRST Subclase WHERE Subclase.cdg_clase = Clase.cdg_subclase) 
        ...
        CREATE T-Deposito.
        BUFFER-COPY O-Deposito TO T-Deposito
            ASSIGN c-registro = c-registro + 1
                   T-Deposito.cdg_deposito = v-cod
                   T-Deposito.nombre       = v-nom
                   T-Deposito.nro_deposito = c-registro
                   T-Deposito.cdg_subclase = Clase.cdg_subclase
    ...
    IF v-modo_bajar <> "N" 
    THEN DO:
        v-dsc_generacion = "BAJANDO DepositoS ...".
        v-cdg_generacion = "".
        DISPLAY v-cdg_generacion
                v-dsc_generacion
                WITH FRAME {&FRAME-NAME}.
        ...
        FOR EACH T-Deposito:
            ...
            FIND Deposito WHERE Deposito.cdg_deposito = T-Deposito.cdg_deposito EXCLUSIVE-LOCK NO-ERROR.
            IF AVAILABLE Deposito
            THEN DO:
                IF v-modo_bajar = "R" 
                    THEN BUFFER-COPY T-Deposito EXCEPT nro_deposito TO Deposito.
            END.
            ELSE DO:
                CREATE Deposito.
                BUFFER-COPY T-Deposito TO Deposito
                    ASSIGN Deposito.nro_deposito = NEXT-VALUE(proximo_deposito).
            END.
