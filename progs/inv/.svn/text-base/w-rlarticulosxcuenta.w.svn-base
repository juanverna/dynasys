...
DEFINE VARIABLE h_v-rng-tipocuenta AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     "  Indique el tipo de cuenta contable a utilizar:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 6.92 COL 12
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Artículos a considerar:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 2.88 COL 12
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-articulo ( 3.96 , 12.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng-tipocuenta.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-tipocuenta ).
       RUN set-position IN h_v-rng-tipocuenta ( 8.00 , 12.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-tipocuenta ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/INV" "Artículos por Tipo de Cuenta Contable"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE v-tipo_cuenta    AS CHARACTER.
  ...
  RUN dar_rango IN h_v-rng-tipocuenta  (  OUTPUT  v-tipo_cuenta,
                                          OUTPUT  error_rango).

  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Tipo de Cuenta" ).
       hay_error = error_rango.
  END.     
