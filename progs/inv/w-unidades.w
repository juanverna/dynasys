...
DEFINE VARIABLE h_b-unidades AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-unidad AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Mantenimiento de Unidades de Medida */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Mantenimiento de Unidades de Medida */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Mantenimiento de Unidades de Medida */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-unidad ).
       RUN set-position IN h_v-unidad ( 1.71 , 28.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.48 , 67.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-unidades.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-unidades ).
       RUN set-position IN h_b-unidades ( 1.81 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-unidades ( 8.00 , 17.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 7.43 , 28.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.14 , 67.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-unidad. */
       RUN add-link IN adm-broker-hdl ( h_b-unidades , 'Record':U , h_v-unidad ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-unidad ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-unidades ,
             h_v-unidad , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_b-unidades , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/INV" "Mantenimiento de Unidades de Medida"}
