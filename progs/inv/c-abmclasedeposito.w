...
DEFINE BUFFER B-Clase_de_deposito FOR Clase_de_deposito.
DEFINE BUFFER C-Clase_de_deposito FOR Clase_de_deposito.

DEFINE VARIABLE f-que_clase LIKE Clase_de_deposito.cdg_subclasedep.
...
FORM           
   SKIP(0.2)   
   Clase_de_deposito.nombre_subclasedep  COLON 16 FGCOLOR 9 BGCOLOR 15
   SKIP(0.2)   
   Clase_de_deposito.rotulo_siguiente COLON 16 FGCOLOR 9 BGCOLOR 15
   SKIP(0.2)   
   Clase_de_deposito.longitud_siguiente COLON 16 FGCOLOR 9 BGCOLOR 15
   Clase_de_deposito.tipo_siguiente COLON 16 FGCOLOR 9
   SKIP(0.2)   
...
SUBSTRING(Clase_de_deposito.cdg_subclasedep,LENGTH(que_clase) + 2) ~
Clase_de_deposito.nombre_subclasedep 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brw_clasificacion 
&Scoped-define QUERY-STRING-brw_clasificacion FOR EACH Clase_de_deposito ~
      WHERE Clase_de_deposito.cdg_clasedep = que_clase NO-LOCK
&Scoped-define OPEN-QUERY-brw_clasificacion OPEN QUERY brw_clasificacion FOR EACH Clase_de_deposito ~
      WHERE Clase_de_deposito.cdg_clasedep = que_clase NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brw_clasificacion Clase_de_deposito
&Scoped-define FIRST-TABLE-IN-QUERY-brw_clasificacion Clase_de_deposito
...
&Scoped-Define ENABLED-OBJECTS que_subclase v-que_nombre que_clase ~
brw_clasificacion camino btn_copist btn_modificar btn_eliminar btn_vaciar ~
btn_generar Btn_edquiv Btn_Imprimir Btn_Elegir Btn_Salir 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brw_clasificacion C-Win _STRUCTURED
  QUERY brw_clasificacion DISPLAY
      SUBSTRING(Clase_de_deposito.cdg_subclasedep,LENGTH(que_clase) + 2) COLUMN-LABEL "Código!Clase" FORMAT "X(8)":U
            WIDTH 13.2
      Clase_de_deposito.nombre_subclasedep COLUMN-LABEL "Denominacion!Clasificación" FORMAT "X(30)":U
            WIDTH 54.2
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME DEFAULT-FRAME
     ...
     v-que_nombre AT ROW 1.24 COL 16 COLON-ALIGNED NO-LABEL
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Mantenimiento de Clases de Depósitos"
         HEIGHT             = 25.71
         WIDTH              = 160
         MAX-HEIGHT         = 27.67
         MAX-WIDTH          = 160
...
"SUBSTRING(Clase_de_deposito.cdg_subclasedep,LENGTH(que_clase) + 2)" "Código!Clase" "X(8)" ? ? ? ? ? ? ? no ? no no "13.2" yes no no "U" "" ""
     _FldNameList[2]   > sic.Clase_de_deposito.nombre_subclasedep
"Clase_de_deposito.nombre_subclasedep" "Denominacion!Clasificación" ? "character" ? ? ? ? ? ? no ? no no "54.2" yes no no "U" "" ""
     _Query            is OPENED
...
ON END-ERROR OF C-Win /* Mantenimiento de Clases de Depósitos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Mantenimiento de Clases de Depósitos */
DO:
...
DO:
   ...
   que_subclase = SUBSTRING(Clase_de_deposito.cdg_subclasedep,LENGTH(que_clase) + 2).
...
DO:
    ...
    RUN getparametro_o.p ( INPUT "EQVDSDEP", OUTPUT s-reemplazo ).

    RUN c-edttexto.w ( INPUT-OUTPUT s-reemplazo,
                       INPUT "Abreviaturas en descripciones de depósitos",
                       INPUT 0, /* Modo modificaciones */
                       OUTPUT puso_ok).
    IF puso_ok
    THEN DO:
        RUN setparametro.p ( INPUT "EQVDSDEP",
                             INPUT "",
                             INPUT 0.0,
                             INPUT NO,
                             INPUT 0,
...
DO:
  ...
  RUN lsclasificacion_depositos.p ( INPUT Clase_de_deposito.cdg_clase ) .
...
DO:
   DO TRANSACTION:
        FIND CURRENT Clase_de_deposito EXCLUSIVE-LOCK.   
        RUN modificar_clasificacion.
        DISPLAY Clase_de_deposito.nombre_subclasedep
...
DO:
   ...
   THEN DO:
      ...
      ELSE DO:
         ...
         THEN DO:
            p_punto = p_punto - 1.
            que_clase = SUBSTRING(que_clase,1,p_punto).
            FIND FIRST Clase_de_deposito WHERE Clase_de_deposito.cdg_subclasedep = que_clase.
            v-que_nombre = Clase_de_deposito.nombre.
            RUN armar_rotulo.
         END.
         ELSE DO:
            que_clase = "".
   ...
   ELSE DO:
      ...
      FIND FIRST Clase_de_deposito WHERE Clase_de_deposito.cdg_clasedep = que_clase 
                         AND Clase_de_deposito.cdg_subclasedep = que_clase + "." + que_subclase NO-ERROR.

      IF NOT AVAILABLE Clase_de_deposito
      THEN DO:
         v-que_nombre = "".
         RUN crear_clasificacion.         
      END.
      ELSE DO:
         v-que_nombre = Clase_de_deposito.nombre_subclasedep.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  RUN enable_UI.
  {setwintit.i "SIC/INV" "Clasificación de Depósitos"}
  RUN habilitar_botones.
...
------------------------------------------------------------------------------*/
/*
  IF AVAILABLE Clase_de_deposito
     THEN rotulo = FILL(" ",l_rotulo - LENGTH(Clase_de_deposito.rotulo_siguiente) - 1) + Clase_de_deposito.rotulo_siguiente + ":".           ELSE rotulo = FILL(" ",l_rotulo - 1) + ":".
*/
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE v-nodo_destino LIKE Clase_de_deposito.cdg_subclase.
    DEFINE VARIABLE v-lista_nodos  AS CHARACTER.
    DEFINE VARIABLE j-nodo         AS INTEGER.
    DEFINE VARIABLE p-ok           AS LOGICAL.

    RUN d-seleccionar_deposito_destino.w ( INPUT Clase_de_deposito.cdg_subclase,
    ...
    THEN DO:
        ...
        DO TRANSACTION:
            ...
            DO j-nodo = 1 TO NUM-ENTRIES(v-lista_nodos,CHR(1)):
                ...
                /*
                MESSAGE "Donde dice:" Clase_de_deposito.cdg_subclase SKIP
                        "debe ponerse:" v-nodo_destino
                        VIEW-AS ALERT-BOX INFO BUTTONS OK.
                */        
        
                FOR EACH B-Clase_de_deposito 
                    WHERE B-Clase_de_deposito.cdg_clase BEGINS Clase_de_deposito.cdg_subclase NO-LOCK:   
                
                    CREATE C-Clase_de_deposito.
                    BUFFER-COPY B-Clase_de_deposito TO C-Clase_de_deposito
                        ASSIGN C-Clase_de_deposito.cdg_clase    = REPLACE(B-Clase_de_deposito.cdg_clase,Clase_de_deposito.cdg_subclase,v-nodo_destino)
                               C-Clase_de_deposito.cdg_subclase = REPLACE(B-Clase_de_deposito.cdg_subclase,Clase_de_deposito.cdg_subclase,v-nodo_destino).
                   /*
                    MESSAGE "padre" SKIP
                            Clase_de_deposito.cdg_clase SKIP 
                            Clase_de_deposito.cdg_subclase SKIP(1)
                            "origen y destino" SKIP
                            B-Clase_de_deposito.cdg_clase " va a reemplazar por" REPLACE(B-Clase_de_deposito.cdg_clase,Clase_de_deposito.cdg_subclase,v-nodo_destino)        SKIP 
                            B-Clase_de_deposito.cdg_subclase " va a reemplazar por" REPLACE(B-Clase_de_deposito.cdg_subclase,Clase_de_deposito.cdg_subclase,v-nodo_destino) SKIP(1)
                            VIEW-AS ALERT-BOX INFO BUTTONS OK.
                    */
                    RELEASE C-Clase_de_deposito.
                END.
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE:
      ...
      CREATE Clase_de_deposito.
      ASSIGN Clase_de_deposito.cdg_clasedep = que_clase                                 
             Clase_de_deposito.cdg_subclasedep = que_clase + "." + que_subclase.
      RUN modificar_clasificacion.       
...
------------------------------------------------------------------------------*/
   ...
   IF CAN-FIND(FIRST B-Clase_de_deposito WHERE B-Clase_de_deposito.cdg_clasedep = Clase_de_deposito.cdg_subclasedep)
   THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  ENABLE que_subclase v-que_nombre que_clase brw_clasificacion camino 
         btn_copist btn_modificar btn_eliminar btn_vaciar btn_generar 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE generar_depositos C-Win 
PROCEDURE generar_depositos :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    RUN c-generar_depositos.w ( INPUT ROWID(Clase_de_deposito), OUTPUT p-ok).

    IF p-ok
        THEN MESSAGE "Las ubicaciones fisicas han sido generadas"
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
...
------------------------------------------------------------------------------*/
   ...
   THEN DO:
       btn_generar:SENSITIVE IN FRAME {&FRAME-NAME} = 
             CAN-FIND(FIRST B-Clase_de_deposito 
                      WHERE B-Clase_de_deposito.cdg_clase = Clase_de_deposito.cdg_subclase ).
...
------------------------------------------------------------------------------*/
  ...
  RUN d-editar_nodo.w ( INPUT-OUTPUT Clase_de_deposito.nombre_subclasedep,
                        INPUT-OUTPUT Clase_de_deposito.rotulo_siguiente,
                        INPUT-OUTPUT Clase_de_deposito.longitud_siguiente,
                        INPUT-OUTPUT Clase_de_deposito.tipo_siguiente).
...
------------------------------------------------------------------------------*/
   ...
   THEN DO:        
       ...
       DO TRANSACTION:
          c = 0.
          FOR EACH B-Clase_de_deposito 
              WHERE B-Clase_de_deposito.cdg_clase BEGINS Clase_de_deposito.cdg_subclase EXCLUSIVE-LOCK:   

              DELETE B-Clase_de_deposito.
              c = c + 1.
