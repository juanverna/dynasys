...
DEFINE VARIABLE h_b-familia_impositiva AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-familia_impositiva AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Mantenimiento de Familias Impositivas"
         HEIGHT             = 22.1
         WIDTH              = 119.2
         MAX-HEIGHT         = 22.1
         MAX-WIDTH          = 149
...
ON END-ERROR OF W-Win /* Mantenimiento de Familias Impositivas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Mantenimiento de Familias Impositivas */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Mantenimiento de Familias Impositivas */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-familia_impositiva.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-familia_impositiva ).
       RUN set-position IN h_b-familia_impositiva ( 1.29 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-familia_impositiva ( 20.48 , 14.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-familia_impositiva.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-familia_impositiva ).
       RUN set-position IN h_v-familia_impositiva ( 1.29 , 21.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.05 , 63.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 6.38 , 21.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.14 , 63.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-familia_impositiva. */
       RUN add-link IN adm-broker-hdl ( h_b-familia_impositiva , 'Record':U , h_v-familia_impositiva ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-familia_impositiva ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-familia_impositiva ,
             h_b-familia_impositiva , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-familia_impositiva , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/INV" "Mantenimiento de Familias Impositivas de Articulos"}
...
*/
    ...
    IF VALID-HANDLE(h_b-familia_impositiva)  THEN RUN set-sensitivo IN h_b-familia_impositiva   ( INPUT p-operacion = "HABILITAR" ).
