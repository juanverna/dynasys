...
DEFINE VARIABLE h_b-tipofamilia AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-tipofamilia AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     Btn_Done AT ROW 1.54 COL 99
     RECT-1 AT ROW 1.27 COL 98
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 114.14 BY 21.23.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-tipofamilia.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tipofamilia ).
       RUN set-position IN h_b-tipofamilia ( 1.27 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-tipofamilia ( 19.12 , 15.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-tipofamilia.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-tipofamilia ).
       RUN set-position IN h_v-tipofamilia ( 1.27 , 19.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.12 , 66.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 6.92 , 19.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 1.88 , 66.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-tipofamilia. */
       RUN add-link IN adm-broker-hdl ( h_b-tipofamilia , 'Record':U , h_v-tipofamilia ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-tipofamilia ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-tipofamilia ,
             Btn_Done:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-tipofamilia ,
             h_b-tipofamilia , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/INV" "Mantenimiento de Tipos de Familias de Artìculos"}
