...
DEFINE VARIABLE h_b-sumador AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-sumador AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/b-sumador.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-sumador ).
       RUN set-position IN h_b-sumador ( 3.38 , 11.00 ) NO-ERROR.
       RUN set-size IN h_b-sumador ( 12.86 , 13.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/v-sumador.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-sumador ).
       RUN set-position IN h_v-sumador ( 3.62 , 35.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.33 , 75.80 ) */
       ...
       RUN set-size IN h_p-updspa ( 1.76 , 76.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-sumador. */
       RUN add-link IN adm-broker-hdl ( h_b-sumador , 'Record':U , h_v-sumador ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-sumador ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-sumador ,
             h_b-sumador , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/ADP" "Mantenimiento de Sumadores por Empleado -"}
