...
DEFINE VARIABLE h_b-totalizador AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-totalizador AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/b-totalizador.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-totalizador ).
       RUN set-position IN h_b-totalizador ( 4.10 , 14.00 ) NO-ERROR.
       RUN set-size IN h_b-totalizador ( 12.14 , 12.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/v-totalizador.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-totalizador ).
       RUN set-position IN h_v-totalizador ( 4.10 , 34.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.52 , 74.20 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 14.57 , 34.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 1.76 , 74.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-totalizador. */
       RUN add-link IN adm-broker-hdl ( h_b-totalizador , 'Record':U , h_v-totalizador ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-totalizador ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-totalizador ,
             h_b-totalizador , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-totalizador , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/ADP" "Mantenimiento de Totales de Liquidación -"}
