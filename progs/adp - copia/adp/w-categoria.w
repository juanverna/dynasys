...
DEFINE VARIABLE h_b-categoria AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-categoria AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/b-categoria.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-categoria ).
       RUN set-position IN h_b-categoria ( 3.86 , 14.00 ) NO-ERROR.
       RUN set-size IN h_b-categoria ( 12.38 , 13.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/v-categoria.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-categoria ).
       RUN set-position IN h_v-categoria ( 3.86 , 35.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.10 , 75.40 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 14.57 , 35.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 1.76 , 75.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-categoria. */
       RUN add-link IN adm-broker-hdl ( h_b-categoria , 'Record':U , h_v-categoria ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-categoria ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-categoria ,
             h_b-categoria , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-categoria , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/ADP" "Mantenimiento de Categorías -"}
