...
DEFINE VARIABLE h_b-seccion AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-seccion AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/b-seccion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-seccion ).
       RUN set-position IN h_b-seccion ( 3.38 , 11.00 ) NO-ERROR.
       RUN set-size IN h_b-seccion ( 13.10 , 13.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/v-seccion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-seccion ).
       RUN set-position IN h_v-seccion ( 3.38 , 31.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.62 , 80.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 14.81 , 31.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 1.76 , 80.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-seccion. */
       RUN add-link IN adm-broker-hdl ( h_b-seccion , 'Record':U , h_v-seccion ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-seccion ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-seccion ,
             h_b-seccion , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/ADP" "Mantenimiento de Secciones -"}
