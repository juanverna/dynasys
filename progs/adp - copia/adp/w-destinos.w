...
DEFINE VARIABLE h_b-destinos AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-destinos AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/b-destinos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-destinos ).
       RUN set-position IN h_b-destinos ( 2.43 , 9.00 ) NO-ERROR.
       RUN set-size IN h_b-destinos ( 18.81 , 12.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/v-destinos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-destinos ).
       RUN set-position IN h_v-destinos ( 2.43 , 30.00 ) NO-ERROR.
       /* Size in UIB:  ( 16.19 , 84.40 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 19.33 , 31.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 1.76 , 84.20 ) NO-ERROR.

       /* Links to SmartViewer h_v-destinos. */
       RUN add-link IN adm-broker-hdl ( h_b-destinos , 'Record':U , h_v-destinos ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-destinos ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-destinos ,
             h_b-destinos , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-destinos , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/ADP" "Mantenimiento de Destinos -"}
