...
DEFINE TEMP-TABLE Acumulado
   ...
   FIELD tot_valor    LIKE Rcb_detalle.importe
   FIELD nro_columna  AS INTEGER
   INDEX por_empleado IS PRIMARY nro_legajo cdg_concepto ASCENDING.
...
PROCEDURE LISTAR_TODO:
  ...
  DO j = 1 TO nt_cols:
     ...
     FIND Concepto WHERE Concepto.cdg_concepto = INTEGER(titulo1) NO-LOCK.
     titulo2 = Concepto.abrevia.
  ...
  OUTPUT TO VALUE(dire_tmp + "lscodliq.txt") PAGED.
  ...
  DO WHILE AVAILABLE Empleado WITH FRAME frm-listado:
     ...
     FIND Rcb_header OF Empleado 
          WHERE Rcb_header.sec_liquidacion = 
                Liquidacion.sec_liquidacion NO-LOCK NO-ERROR.

     IF AVAILABLE Rcb_header
     THEN DO:
          str_linea = "".
          DO j = 1 TO nt_cols:
            
             FIND Concepto WHERE Concepto.cdg_concepto = INTEGER(ENTRY(j,sel_codigos)) NO-LOCK.
             FIND Rcb_detalle OF Rcb_header 
                  WHERE Rcb_detalle.nro_concepto = Concepto.nro_concepto
                        NO-LOCK NO-ERROR.
    
             IF AVAILABLE Rcb_detalle
             THEN DO:
                  g-acumdlq [ j ] = g-acumdlq [ j ] + Rcb_detalle.importe.
                  str_linea = str_linea + STRING(Rcb_detalle.importe,"ZZZZZZ9.99-") + " ".
             END.
             ELSE DO:
                  str_linea = str_linea + "            ".
             END.
