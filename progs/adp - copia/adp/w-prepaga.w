...
DEFINE VARIABLE h_b-prepaga AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-prepaga AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-prepagas.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-prepaga ).
       RUN set-position IN h_b-prepaga ( 1.95 , 10.00 ) NO-ERROR.
       RUN set-size IN h_b-prepaga ( 17.14 , 12.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/v-prepaga.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-prepaga ).
       RUN set-position IN h_v-prepaga ( 1.95 , 28.00 ) NO-ERROR.
       /* Size in UIB:  ( 13.71 , 85.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 17.43 , 30.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 1.76 , 85.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-prepaga. */
       RUN add-link IN adm-broker-hdl ( h_b-prepaga , 'Record':U , h_v-prepaga ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-prepaga ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-prepaga ,
             h_b-prepaga , 'AFTER':U ).
