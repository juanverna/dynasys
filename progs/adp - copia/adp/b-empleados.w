...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Empleado WHERE ~{&KEY-PHRASE} ~
      AND Empleado.cdg_empresa = que_empresa use-index por_legajo NO-LOCK ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Empleado.nro_legajo COLUMN-LABEL "Número!Legajo" FORMAT "ZZZZZ9":U
            WIDTH 9.2
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 14 BY 23.1
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "Empleado.cdg_empresa = que_empresa use-index por_legajo"
     _FldNameList[1]   > sic.Empleado.nro_legajo
"nro_legajo" "Número!Legajo" ? "integer" ? ? ? ? ? ? no ? no no "9.2" yes no no "U" "" ""
...
/*   DEFINE VARIABLE x-nro_legajo LIKE Empleado.nro_legajo.            */
/*  DEFINE VARIABLE x-ok         AS LOGICAL.                           */
/*  RUN d-empleado.w (OUTPUT x-nro_legajo, OUTPUT x-ok).               */
/*  IF x-ok                                                            */
/* THEN DO:                                                            */
/*     FIND Empleado WHERE Empleado.nro_legajo = x-nro_legajo NO-LOCK. */
/*     DISPLAY Empleado.nro_legajo @ v-codigo                          */
