...
DEFINE VARIABLE h_b-sindicatos AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-sindicato AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/b-sindicatos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-sindicatos ).
       RUN set-position IN h_b-sindicatos ( 3.38 , 10.00 ) NO-ERROR.
       RUN set-size IN h_b-sindicatos ( 12.38 , 12.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/v-sindicato.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-sindicato ).
       RUN set-position IN h_v-sindicato ( 3.38 , 24.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.81 , 72.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 13.38 , 24.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.38 , 72.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-sindicato. */
       RUN add-link IN adm-broker-hdl ( h_b-sindicatos , 'Record':U , h_v-sindicato ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-sindicato ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-sindicato ,
             h_b-sindicatos , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-sindicato , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/ADP" "Mantenimiento de Sindicatos -"}
