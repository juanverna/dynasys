...
DEFINE VARIABLE h_b-convenio AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-convenio_2 AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/b-convenio.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-convenio ).
       RUN set-position IN h_b-convenio ( 3.86 , 14.00 ) NO-ERROR.
       RUN set-size IN h_b-convenio ( 12.14 , 12.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/v-convenio_2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-convenio_2 ).
       RUN set-position IN h_v-convenio_2 ( 3.86 , 36.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.81 , 74.20 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/p-updspa.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Edge-Pixels = 2,
       ...
       /* Links to SmartViewer h_v-convenio_2. */
       RUN add-link IN adm-broker-hdl ( h_b-convenio , 'Record':U , h_v-convenio_2 ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-convenio_2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-convenio_2 ,
             h_b-convenio , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-convenio_2 , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/ADP" "Mantenimiento de Convenios -"}
