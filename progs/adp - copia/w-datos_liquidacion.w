...
DEFINE VARIABLE h_b-tit_dat_liq AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-tit_dat_liq AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Datos de Liquidación */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Datos de Liquidación */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Datos de Liquidación */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-tit_dat_liq.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tit_dat_liq ).
       RUN set-position IN h_b-tit_dat_liq ( 1.24 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-tit_dat_liq ( 19.76 , 20.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-tit_dat_liq.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-tit_dat_liq ).
       RUN set-position IN h_v-tit_dat_liq ( 1.24 , 25.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.48 , 69.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 7.19 , 25.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.14 , 69.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-tit_dat_liq. */
       RUN add-link IN adm-broker-hdl ( h_b-tit_dat_liq , 'Record':U , h_v-tit_dat_liq ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-tit_dat_liq ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-tit_dat_liq ,
             h_b-tit_dat_liq , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-tit_dat_liq , 'AFTER':U ).
    END. /* Page 0 */
