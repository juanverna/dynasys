...
DEFINE FRAME F-Main
     v-estado AT ROW 17.67 COL 12 COLON-ALIGNED NO-LABEL
     "  Rango de articulos a listar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 2.33 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de fechas de los movimientos:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 12.19 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de depósitos a listar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 6.95 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Total remitido x artículo"
         HEIGHT             = 22.14
         WIDTH              = 123.4
         MAX-HEIGHT         = 22.14
         MAX-WIDTH          = 123.4
...
ON END-ERROR OF W-Win /* Total remitido x artículo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Total remitido x artículo */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-articulo ( 3.67 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.86 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'inv/v-rng-deposito.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-deposito ).
       RUN set-position IN h_v-rng-deposito ( 8.38 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.86 , 100.00 ) */
       ...
       RUN set-position IN h_v-rng-fechas ( 13.62 , 14.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 19.05 , 14.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/COM" "Total Recibido por Artículo/Depósito"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE des_articulo     LIKE Articulo.cdg_articulo.
  DEFINE VARIABLE has_articulo     LIKE Articulo.cdg_articulo.
  ...
  DEFINE VARIABLE des_deposito     LIKE Deposito.cdg_deposito.
  DEFINE VARIABLE has_deposito     LIKE Deposito.cdg_deposito.
  ...
  RUN dar_rango IN h_v-rng-deposito ( OUTPUT des_deposito, 
                                      OUTPUT has_deposito, 
                                      OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
  ...
  THEN DO:
       ...
       RUN lsrecibido_xarticulo.p ( INPUT des_articulo,
                                    INPUT has_articulo,
                                    INPUT des_deposito,
                                    INPUT has_deposito,
                                    INPUT des_fecha,
