...
FOR EACH Ocm_detalle OF Ocm_header, Articulo OF Ocm_detalle NO-LOCK,
  ...
  THEN DO:
     total_chars = LENGTH(Articulo.detallada).
     ...
     DO j = 1 to nt_lineas:
        ...
        que_articulo = ( IF j = 1 THEN STRING(Articulo.cdg_articulo,"ZZZZZZZ9")
                                  ELSE "").
        que_descripcion = SUBSTRING(Articulo.detallada,( j - 1) * ancho_linea + 1 ,
                                                                     ancho_linea ).
        que_cantidad = STRING(Ocm_detalle.cantidad, "ZZZZZZZ9").
        que_unidad   = Unidad.abrevia.
        que_precio   = STRING(Ocm_detalle.precio, "ZZZZZZ9.99").
        que_subtotal = ( IF Ocm_detalle.cantidad = 0 THEN STRING(Ocm_detalle.precio, "ZZZZZZ9.99")
                                                     ELSE STRING(subtotal, "ZZZZZZ9.99")).
        /*
        DISPLAY  que_cantidad    WHEN Ocm_detalle.cantidad <> 0  AND j = nt_lineas
                 que_descripcion
...
run veresult.w ( input "procm501.txt", input 2).
