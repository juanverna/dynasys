...
DEFINE VARIABLE h_b-compradores AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-comprador AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     Btn_Done AT ROW 1.52 COL 99
     RECT-1 AT ROW 1.29 COL 98
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-compradores.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-compradores ).
       RUN set-position IN h_b-compradores ( 1.00 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-compradores ( 19.14 , 16.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-comprador.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-comprador ).
       RUN set-position IN h_v-comprador ( 1.48 , 21.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.05 , 71.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 6.48 , 21.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 1.86 , 71.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-comprador. */
       RUN add-link IN adm-broker-hdl ( h_b-compradores , 'Record':U , h_v-comprador ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-comprador ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-compradores ,
             Btn_Done:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-comprador ,
             h_b-compradores , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/ABA" "Mantenimiento de Compradores"}
