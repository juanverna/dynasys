...
&Scoped-define INTERNAL-TABLES T-Ocm_header-bon Bonificacion

/* Definitions for BROWSE BROWSE-8                                      */
&Scoped-define FIELDS-IN-QUERY-BROWSE-8 T-Ocm_header-bon.cdg_bonificacion ~
Bonificacion.descripcion T-Ocm_header-bon.porcentaje ~
T-Ocm_header-bon.importe 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-8 
&Scoped-define QUERY-STRING-BROWSE-8 FOR EACH T-Ocm_header-bon NO-LOCK, ~
      EACH Bonificacion OF T-Ocm_header-bon NO-LOCK
&Scoped-define OPEN-QUERY-BROWSE-8 OPEN QUERY BROWSE-8 FOR EACH T-Ocm_header-bon NO-LOCK, ~
      EACH Bonificacion OF T-Ocm_header-bon NO-LOCK.
&Scoped-define TABLES-IN-QUERY-BROWSE-8 T-Ocm_header-bon Bonificacion
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-8 T-Ocm_header-bon
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-8 Dialog-Frame _STRUCTURED
  QUERY BROWSE-8 NO-LOCK DISPLAY
      T-Ocm_header-bon.cdg_bonificacion COLUMN-LABEL "Có-!digo" FORMAT "ZZ9":U
            WIDTH 8.2
      Bonificacion.descripcion COLUMN-LABEL "Descripción!Bonificación" FORMAT "X(30)":U
            WIDTH 43.2
      T-Ocm_header-bon.porcentaje COLUMN-LABEL "%.!Bonificado" FORMAT "->>9.99":U
      T-Ocm_header-bon.importe COLUMN-LABEL "Importe!Bonificado" FORMAT "->,>>>,>>9.99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 83 BY 8 EXPANDABLE.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME Dialog-Frame
     v-cdg_bonificacion AT ROW 1.71 COL 2 COLON-ALIGNED NO-LABEL
     v-dsc_bonificacion AT ROW 1.71 COL 14 COLON-ALIGNED NO-LABEL
     v-porcentaje AT ROW 1.71 COL 74 COLON-ALIGNED
     BROWSE-8 AT ROW 2.91 COL 4
     btn_crear AT ROW 11.19 COL 4
     btn_eliminar AT ROW 11.24 COL 30
     btn_salir AT ROW 11.24 COL 64
     RECT-10 AT ROW 1.29 COL 2
     SPACE(0.99) SKIP(0.32)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Bonificaciones de la presente O/Compra".
...
/* Query rebuild information for BROWSE BROWSE-8
     _TblList          = "Temp-Tables.T-Ocm_header-bon,sic.Bonificacion OF Temp-Tables.T-Ocm_header-bon"
     _Options          = "NO-LOCK"
     _FldNameList[1]   > Temp-Tables.T-Ocm_header-bon.cdg_bonificacion
"T-Ocm_header-bon.cdg_bonificacion" "Có-!digo" ? "integer" ? ? ? ? ? ? no ? no no "8.2" yes no no "U" "" ""
     _FldNameList[2]   > sic.Bonificacion.descripcion
"Bonificacion.descripcion" "Descripción!Bonificación" ? "character" ? ? ? ? ? ? no ? no no "43.2" yes no no "U" "" ""
     _FldNameList[3]   > Temp-Tables.T-Ocm_header-bon.porcentaje
"T-Ocm_header-bon.porcentaje" "%.!Bonificado" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[4]   > Temp-Tables.T-Ocm_header-bon.importe
"T-Ocm_header-bon.importe" "Importe!Bonificado" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-8 */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Bonificaciones del presente pedido */
DO:
...
DO:
  ...
  THEN DO:
       FIND T-Ocm_header-bon 
            WHERE T-Ocm_header-bon.cdg_bonificacion = v-cdg_bonificacion NO-LOCK NO-ERROR.
       IF AVAILABLE T-Ocm_header-bon 
       THEN DO:
       ...
       ELSE DO:
            DO TRANSACTION:
               ...
               ASSIGN T-Ocm_header-bon.nro_ocompra       = T-Ocm_header.nro_ocompra
                      T-Ocm_header-bon.cdg_bonificacion = v-cdg_bonificacion
                      T-Ocm_header-bon.porcentaje       = v-porcentaje. 
            END.
...
DO:
    ...
    IF NOT AVAILABLE T-Ocm_header-bon
    THEN DO:
