...
&Scoped-define INTERNAL-TABLES Ocm_header Proveedor

/* Definitions for BROWSE BROWSE-2                                      */
&Scoped-define FIELDS-IN-QUERY-BROWSE-2 Ocm_header.tip_comprob Ocm_header.prf_comprob Ocm_header.nro_comprob Ocm_header.estado Ocm_header.fecha Ocm_header.imp_total Proveedor.cdg_proveedor Proveedor.nombre   
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-2   
&Scoped-define FIELD-PAIRS-IN-QUERY-BROWSE-2
&Scoped-define SELF-NAME BROWSE-2
&Scoped-define OPEN-QUERY-BROWSE-2    IF por_proveedor    THEN DO:       OPEN QUERY {&SELF-NAME}            FOR EACH Ocm_header                           WHERE Ocm_header.cdg_empresa = que_empresa                             AND Ocm_header.tip_comprob = "OC"                             AND Ocm_header.nro_proveedor = que_proveedor                             AND Ocm_header.fecha >= v-des_fecha                             AND Ocm_header.fecha <= v-has_fecha                             AND LOOKUP(Ocm_header.estado, ~
      lista_estados) <> 0, ~
                                       FIRST Proveedor OF Ocm_header.    END.    ELSE DO:       OPEN QUERY {&SELF-NAME}            FOR EACH Ocm_header                           WHERE Ocm_header.cdg_empresa = que_empresa                             AND Ocm_header.tip_comprob = "OC"                             AND Ocm_header.fecha >= v-des_fecha                             AND Ocm_header.fecha <= v-has_fecha                             AND LOOKUP(Ocm_header.estado, ~
      lista_estados) <> 0, ~
                                       FIRST Proveedor OF Ocm_header                                       WHERE Proveedor.titular_oxp_sino = FALSE.    END.
&Scoped-define TABLES-IN-QUERY-BROWSE-2 Ocm_header Proveedor
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-2 Ocm_header
...
&Scoped-Define ENABLED-OBJECTS RECT-1 RECT-9 Btn_OK Btn_Cancel ~
por_proveedor v-des_fecha v-has_fecha v-cdg_proveedor BROWSE-2 
&Scoped-Define DISPLAYED-OBJECTS por_proveedor v-des_fecha v-has_fecha ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-2 D-Dialog _FREEFORM
  QUERY BROWSE-2 DISPLAY
      ...
      Proveedor.nombre         COLUMN-LABEL "Razón!Social" FORMAT "X(45)"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME D-Dialog
     ...
     por_proveedor AT ROW 1.54 COL 19 NO-LABEL
     v-des_fecha AT ROW 1.54 COL 39 COLON-ALIGNED
     v-has_fecha AT ROW 1.54 COL 56 COLON-ALIGNED
     v-cdg_proveedor AT ROW 3.15 COL 13 COLON-ALIGNED
     ...
     "Por Proveedor:" VIEW-AS TEXT
          SIZE 14 BY .81 AT ROW 1.54 COL 5
      ...
      OPEN QUERY {&SELF-NAME}
           FOR EACH Ocm_header
                          WHERE Ocm_header.cdg_empresa = que_empresa
                            ...
                            AND LOOKUP(Ocm_header.estado,lista_estados) <> 0,
                                FIRST Proveedor OF Ocm_header.
   END.
   ELSE DO:
      OPEN QUERY {&SELF-NAME}
           FOR EACH Ocm_header
...
------------------------------------------------------------------------------*/
  DISPLAY por_proveedor v-des_fecha v-has_fecha v-cdg_proveedor v-nombre 
      WITH FRAME D-Dialog.
  ENABLE RECT-1 RECT-9 Btn_OK Btn_Cancel por_proveedor v-des_fecha v-has_fecha 
