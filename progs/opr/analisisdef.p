...
DEFINE TEMP-TABLE lstorden1 LIKE lstorden
        FIELD reporte AS CHAR.


DEFINE TEMP-TABLE lstdetalle LIKE resultados
        FIELD ind LIKE lstorden1.ind.

{crystal_dyna.p}

DEFINE INPUT PARAMETER TABLE FOR aimp.
DEFINE INPUT PARAMETER impnro AS int.
...
DEFINE VAR ftipo AS CHAR INITIAL "".
...
DEFINE VAR recursonom AS CHAR NO-UNDO.
vind = 0.
...
FOR EACH aimp :
    ...
    REPEAT kk = 1 TO NUM-ENTRIES(evento.recursos):
      IF kk > 2 THEN NEXT.
      FIND recurso WHERE recurso.cdg_recurso = ENTRY(kk,evento.recursos) NO-LOCK NO-ERROR.
      IF AVAILABLE recurso THEN 
          recursonom = recursonom + "," + recurso.nom_recurso + " DNI:" + recurso.dni.
    END.
    recursonom = substring(recursonom,2).

    CREATE lstorden1.
        ASSIGN lstorden1.nro_evento = evento.nro_evento
               lstorden1.direccion = cliente.direccion
               lstorden1.Frealizado = evento.frealizado
               lstorden1.fecha_analisis = evento_protocolo.fecha_analisis
               lstorden1.laboratorio = evento_protocolo.laboratorio
               lstorden1.Fecha_toma = evento_protocolo.Fecha_toma 
               lstorden1.extrajo = evento_protocolo.extrajo 
               lstorden1.nro_protocolo = evento_protocolo.nro_protocolo
               lstorden1.fvencimiento = sumarmeses(evento.frealizado,6)
               lstorden1.referencia = administracion.nom_cli
               lstorden1.ind = vind.
               lstorden1.nro_certificado = string(nrocerti,"9999999").
               lstorden1.letraprefijo = letraprefijo.
               lstorden1.fa_dia = DAY(evento_protocolo.fecha_analisis).
               lstorden1.fa_mes = nom_mes[ month(evento_protocolo.fecha_analisis) ].
               lstorden1.fa_ano = string(year(evento_protocolo.fecha_analisis),">>>9").
               lstorden1.recurso = recursonom.
               lstorden1.estado_tanque = entry(evento.entrega + 1 ,estado_var).
               evento.nro_certif = nrocerti.
               evento.letraprefijo = letraprefijo.
               evento.tipo_certif = ftipo.
               ...
               END.
               lstorden1.reporte = "definit_" + STRING( evento.nro_tipo_evento).
...
EMPTY TEMP-TABLE lstorden.
IF NOT AVAILABLE evento THEN RETURN.
FOR EACH lstorden1 BREAK BY lstorden1.reporte:
  CREATE lstorden.
  BUFFER-COPY lstorden1 TO lstorden .

  IF LAST-OF(lstorden1.reporte) THEN DO:
    ReportePath = lstorden1.reporte.
    ...
    RUN crearReporte(chReport,"rpt", /*ViewReport*/ FALSE , /*PrinterName*/ impresora.nombre,
