...
&Scoped-Define ENABLED-OBJECTS BUTTON-14 
&Scoped-Define DISPLAYED-OBJECTS v-Recurso 
...
DEFINE VARIABLE v-Recurso AS CHARACTER FORMAT "X(8)" 
     LABEL "Recursos" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME F-Main
     ...
     v-Recurso AT ROW 2.19 COL 13 COLON-ALIGNED WIDGET-ID 26
     BRECURSOS AT ROW 2.19 COL 56 WIDGET-ID 36
     BUTTON-14 AT ROW 6 COL 54.8 WIDGET-ID 38
...
/* SETTINGS FOR BUTTON BRECURSOS IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN v-Recurso IN FRAME F-Main
   NO-ENABLE                                                            */
...
IF AVAILABLE contrato_restriccion  THEN DO:
    v-recurso:SCREEN-VALUE IN FRAME {&FRAME-NAME} = valor.
END.
...
DO:
  ...
  lista = v-recurso:SCREEN-VALUE.
  RUN d-recursos.w (INPUT-OUTPUT lista,"*").
  v-recurso:SCREEN-VALUE = lista.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-14 V-table-Win
ON CHOOSE OF BUTTON-14 IN FRAME F-Main /* Ayuda */
DO:
  MESSAGE "1-Separar los operarios por coma" SKIP
           "2-Cuando la agenda de un operario este completa se eligira otro operario que tenga la habilidad suficiente con la menor carga de trabajo asignada".
END.
...
------------------------------------------------------------------------------*/
  ...
  v-recurso:SENSITIVE IN FRAME {&FRAME-NAME} = FALSE.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE VAR cc AS CHARACTER NO-UNDO.
  ...
  IF AVAILABLE contrato_restriccion THEN DO:
            v-recurso:SCREEN-VALUE IN FRAME {&FRAME-NAME} = contrato_restriccion.valor NO-ERROR.

  END.
  cc = v-recurso:SCREEN-VALUE IN FRAME {&FRAME-NAME} .
  DO i = 1 TO LENGTH(cc):
      IF INDEX( "0123456789, " , SUBSTRING(cc,i,1)) = 0 THEN DO:
          muestra-error("Caracter invalido , Corrija los valores").
          RETURN ERROR.
      END.
  END.
  /* Code placed here will execute AFTER standard behavior.    */
...
------------------------------------------------------------------------------*/
  ...
  v-recurso:SENSITIVE IN FRAME {&FRAME-NAME} = TRUE.
...
------------------------------------------------------------------------------*/
  ...
  RETURN v-recurso:SCREEN-VALUE IN FRAME {&FRAME-NAME} .   /* Function return value. */
