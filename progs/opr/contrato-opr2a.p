...
PROCEDURE calculo:
    ...
    FIND evento WHERE evento.nro_evento = ppnro_evento  NO-ERROR.
    ...
    DO:
        ...
        RETURN ERROR.
    END.
    IF evento.fasignado<>? OR evento.anulado THEN LEAVE.
...
DO: 
    ...
    DO:
        OUTPUT TO value("c:\sic-temp\erroresplan" + STRING(nplan) + ".txt" ) APPEND.
        PUT UNFORMATTED NOW " Error interno.No se encuentra el evento padre para evento " pnro_evento SKIP.
        OUTPUT CLOSE.
        RETURN ERROR.
    END.    
    analizado.muestras=pmuestras * 1000 / analizado.hduracion.
