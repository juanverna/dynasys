...
&Scoped-Define ENABLED-OBJECTS rtipo precursos soloabono 
&Scoped-Define DISPLAYED-OBJECTS rtipo precursos soloabono v-estado 
...
DEFINE VARIABLE precursos AS CHARACTER FORMAT "X(256)":U INITIAL "*" 
     LABEL "Recursos" 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1 TOOLTIP "Recursos a analizar o * para todos" NO-UNDO.
...
DEFINE VARIABLE rtipo AS INTEGER INITIAL 2 
     ...
     RADIO-BUTTONS 
          "Administracion->Tipo", 1,
"Tipo->Administracion", 2
     SIZE 51 BY 1.19 NO-UNDO.

DEFINE VARIABLE soloabono AS LOGICAL INITIAL yes 
     LABEL "Solo Abonos" 
     VIEW-AS TOGGLE-BOX
     SIZE 22.8 BY .81 TOOLTIP "Solo Abonos cant cuotas 0." NO-UNDO.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME F-Main
     rtipo AT ROW 5.29 COL 4 NO-LABEL WIDGET-ID 2
     precursos AT ROW 5.29 COL 63 COLON-ALIGNED WIDGET-ID 8
     soloabono AT ROW 5.48 COL 81.2 WIDGET-ID 6
     v-estado AT ROW 10.29 COL 1.8 COLON-ALIGNED NO-LABEL
     "  Rango de fechas de los movimientos:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 1.24 COL 4 WIDGET-ID 10
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de fechas de evento:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 6.48 COL 4.4 WIDGET-ID 12
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de fechas de los movimientos:" VIEW-AS TEXT
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Total de Ventas por Cliente y Articulo"
         HEIGHT             = 13.43
         WIDTH              = 106.2
         MAX-HEIGHT         = 27.67
         MAX-WIDTH          = 160
...
ON END-ERROR OF W-Win /* Total de Ventas por Cliente y Articulo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Total de Ventas por Cliente y Articulo */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-fechas-2 ( 7.67 , 3.60 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
...
------------------------------------------------------------------------------*/
  DISPLAY rtipo precursos soloabono v-estado 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE rtipo precursos soloabono 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
  ...
  DYNAMIC-FUNCTION("setfechas" IN h_v-rng-fechas-2, TODAY - 365 , TODAY ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Altas y Bajas Contratos en Período"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE des_feve                  AS DATE NO-UNDO.
  DEFINE VARIABLE has_feve                  AS DATE NO-UNDO.
  ...
  DEFINE VAR que_parametro AS CHAR  no-undo.
  ...
  THEN DO:
      ...
      IF error_rango 
      THEN DO:
           RUN ver-error-rango ( "Fechas Contratos" ).
           hay_error = hay_error OR error_rango.
      END.     
      RUN dar_rango IN h_v-rng-fechas-2    ( OUTPUT des_feve, 
                                           OUTPUT has_feve, 
                                           OUTPUT error_rango ).
      IF error_rango 
      THEN DO:
           RUN ver-error-rango ( "Fechas Eventos" ).
           hay_error = hay_error OR error_rango.
      END.     
  END.
  IF NOT hay_error
  THEN DO:
       ASSIGN FRAME {&FRAME-NAME} rtipo soloabono precursos.

       RUN setear-botones ( NO ).        
       v-estado:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "Ejecutando Proceso ...".
       IF v-fechalist = "Ejecutar" 
       THEN DO:
              RUN altabajacontrato.p  ( INPUT des_fecha,
                                        INPUT has_fecha,
                                        INPUT des_feve,
                                        INPUT has_feve,
                                        input soloabono,
           ...
           que_parametro = "des_fecha"     + CHR(1) + STRING(des_fecha)  + CHR(1) + 
                           "has_fecha"     + CHR(1) + STRING(has_fecha).
...
------------------------------------------------------------------------------*/
  ...
  IF rtipo = 1 THEN
     RETURN "altabajacontratos.rpt".
  ELSE
    RETURN "altabajacontratos_tipo".
  END FUNCTION.
