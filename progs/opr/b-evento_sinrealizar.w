...
&Scoped-define FIELDS-IN-QUERY-br_table Tipo_evento.cdg_tipo_evento recurso_agenda.cdg_recurso Evento.nro_evento Evento.nro_evento_padre Cliente.direccion evento.fasignado Evento.Duracion Cliente.cdg_cliente Evento.Origen Evento.nro_identificacion Evento.Periodo   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table   
&Scoped-define SELF-NAME br_table
&Scoped-define OPEN-QUERY-br_table  IF NOT tpendiente:INPUT-VALUE IN FRAME {&FRAME-NAME} THEN     OPEN QUERY {&SELF-NAME} FOR EACH recurso_agenda WHERE ~{&KEY-PHRASE}           AND recurso_agenda.fecha <= f_asignado and can-do(p_recursos, ~
      recurso_agenda.cdg_recurso) NO-LOCK, ~
                 EACH Evento OF recurso_agenda           WHERE NOT evento.anulado AND evento.frealizado = ? and ( evento.nro_tipo_evento = c_nro_tipo_evento or c_nro_tipo_evento = 0 ) and     nro_cobranza_tipo_evento <> evento.nro_tipo_evento NO-LOCK, ~
                 FIRST Cliente OF Evento NO-LOCK, ~
                 FIRST Tipo_evento OF Evento NO-LOCK         ~{&SORTBY-PHRASE}. ELSE     OPEN QUERY {&SELF-NAME} FOR EACH recurso_agenda WHERE ~{&KEY-PHRASE}           AND recurso_agenda.fecha = f_asignado NO-LOCK, ~
                 EACH Evento OF recurso_agenda           WHERE NOT evento.anulado AND evento.frealizado = ? and ( evento.nro_tipo_evento = c_nro_tipo_evento or c_nro_tipo_evento = 0 ) and                     nro_cobranza_tipo_evento <> evento.nro_tipo_evento NO-LOCK, ~
                 FIRST Cliente OF Evento NO-LOCK, ~
                 FIRST Tipo_evento OF Evento NO-LOCK         ~{&SORTBY-PHRASE}.
...
Bagenda_recurso c_nro_tipo_evento Brefresh TPendiente 
&Scoped-Define DISPLAYED-OBJECTS p_Recursos f_Asignado c_nro_tipo_evento ~
TPendiente 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD desa1 B-table-Win 
FUNCTION desa1 RETURNS LOGICAL
  ( k AS INT )  FORWARD.

/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD oc B-table-Win 
FUNCTION oc RETURNS CHARACTER
  ( /* parameter-definitions */ )  FORWARD.

/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD openq B-table-Win 
FUNCTION openq RETURNS CHARACTER
  ( /* parameter-definitions */ )  FORWARD.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD que_fecha B-table-Win 
FUNCTION que_fecha RETURNS DATE
  ( /* parameter-definitions */ )  FORWARD.
...
DEFINE VARIABLE c_nro_tipo_evento AS INTEGER FORMAT ">>>>>>>>9" INITIAL 0 
     ...
     LIST-ITEM-PAIRS "0",1
     DROP-DOWN-LIST
     SIZE 15.2 BY 1 TOOLTIP "Tipo de Restriccion o Tipo de Evento".
...
DEFINE VARIABLE TPendiente AS LOGICAL INITIAL no 
     LABEL "Pendiente" 
     VIEW-AS TOGGLE-BOX
     SIZE 16 BY .81 TOOLTIP "Muestra pendiente de hoy para todos los usuarios" NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _FREEFORM
  QUERY br_table NO-LOCK DISPLAY
      ...
      evento.fasignado FORMAT "99/99/9999"
      Evento.Duracion FORMAT ">,>>9":U
      Cliente.cdg_cliente FORMAT "X(8)":U
      Evento.Origen FORMAT "X(12)":U
...
DEFINE FRAME F-Main
     ...
     Bagenda_recurso AT ROW 3.57 COL 12 WIDGET-ID 38
     c_nro_tipo_evento AT ROW 4.81 COL 9.8 COLON-ALIGNED WIDGET-ID 40
     Brefresh AT ROW 5.91 COL 12 WIDGET-ID 80
     TPendiente AT ROW 6.95 COL 12 WIDGET-ID 82
...
IF NOT tpendiente:INPUT-VALUE IN FRAME {&FRAME-NAME} THEN
    OPEN QUERY {&SELF-NAME} FOR EACH recurso_agenda WHERE ~{&KEY-PHRASE}
          AND recurso_agenda.fecha <= f_asignado and can-do(p_recursos,recurso_agenda.cdg_recurso) NO-LOCK,
          EACH Evento OF recurso_agenda
          WHERE NOT evento.anulado AND evento.frealizado = ? and ( evento.nro_tipo_evento = c_nro_tipo_evento or c_nro_tipo_evento = 0 ) and
    nro_cobranza_tipo_evento <> evento.nro_tipo_evento NO-LOCK,
          FIRST Cliente OF Evento NO-LOCK,
          FIRST Tipo_evento OF Evento NO-LOCK
        ~{&SORTBY-PHRASE}.
ELSE
    OPEN QUERY {&SELF-NAME} FOR EACH recurso_agenda WHERE ~{&KEY-PHRASE}
          AND recurso_agenda.fecha = f_asignado NO-LOCK,
          EACH Evento OF recurso_agenda
          WHERE NOT evento.anulado AND evento.frealizado = ? and ( evento.nro_tipo_evento = c_nro_tipo_evento or c_nro_tipo_evento = 0 ) and
                    nro_cobranza_tipo_evento <> evento.nro_tipo_evento NO-LOCK,
          FIRST Cliente OF Evento NO-LOCK,
     ...
     _Where[1]         = "recurso_agenda.fecha = f_asignado and can-do(p_recursos,recurso_agenda.cdg_recurso)"
     _JoinCode[2]      = "recurso_agenda.fecha = f_asignado and recurso_agenda.cdg_recurso = p_recursos"
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Brefresh B-table-Win
...
DEFINE  VAR fondo LIKE Tipo_evento.color_fondo NO-UNDO.
      ...
      IF  evento.fasignado <> f_asignado THEN
          cliente.direccion:BGCOLOR = 14.
          ELSE
          cliente.direccion:BGCOLOR = ?.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL p_Recursos B-table-Win
ON RETURN OF p_Recursos IN FRAME F-Main /* Recursos */
DO:
   ASSIGN p_recursos.
  RUN adm-open-query.
END.
...
DEFINE VAR lista AS CHAR NO-UNDO.
  &Scoped-define CONDICION tipo_evento.nro_tipo_evento > 0 AND  nro_cobranza_tipo_evento <> tipo_evento.nro_tipo_evento
  DO WITH FRAME {&FRAME-NAME}:
     {levantacombo.i &TABLA=Tipo_evento &NOMBRE=cdg_tipo_evento &CODIGO=nro_tipo_evento &OBJETO=c_nro_tipo_evento}
  END. 
  c_nro_tipo_evento:ADD-FIRST("TODOS",0).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION desa1 B-table-Win 
FUNCTION desa1 RETURNS LOGICAL
  ( k AS INT ) :
/*------------------------------------------------------------------------------
  Purpose:  
...
FOR each brecurso_agenda WHERE brecurso_agenda.nro_evento = k:
    DELETE brecurso_agenda.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION oc B-table-Win 
FUNCTION oc RETURNS CHARACTER
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION openq B-table-Win 
FUNCTION openq RETURNS CHARACTER
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION que_fecha B-table-Win 
FUNCTION que_fecha RETURNS DATE
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
...
------------------------------------------------------------------------------*/
  ...
  RETURN f_asignado:INPUT-VALUE IN FRAME {&FRAME-NAME}.   /* Function return value. */
