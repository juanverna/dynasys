...
&Scoped-Define DISPLAYED-OBJECTS Cb Fd Fh 
...
DEFINE FRAME F-Main
     ...
     Cb AT ROW 2.43 COL 7 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     Fd AT ROW 3.86 COL 20 COLON-ALIGNED
     Fh AT ROW 3.86 COL 40 COLON-ALIGNED
     b_ayuda AT ROW 5.76 COL 53 WIDGET-ID 2
     "Medio de Confirmacion de Cobranza" VIEW-AS TEXT
          SIZE 49 BY 1.19 AT ROW 1.24 COL 10
...
ON LEAVE OF Fh IN FRAME F-Main /* Hasta */
DO:
  ASSIGN fh fd.
...
------------------------------------------------------------------------------*/
  ...
  fh:SENSITIVE IN FRAME {&FRAME-NAME} = FALSE.
...
IF AVAILABLE cliente_restriccion  THEN DO:
    ...
    fd:SCREEN-VALUE IN FRAME {&FRAME-NAME} = IF NUM-ENTRIES(cliente_restriccion.valor,"|") >= 2 THEN entry(2,cliente_restriccion.valor,"|") ELSE "".
    fh:SCREEN-VALUE IN FRAME {&FRAME-NAME} = IF NUM-ENTRIES(cliente_restriccion.valor,"|") >= 3 THEN entry(3,cliente_restriccion.valor,"|") ELSE "".
...
------------------------------------------------------------------------------*/
  ...
  fh:SENSITIVE IN FRAME {&FRAME-NAME} = TRUE.
  /* Code placed here will execute AFTER standard behavior.    */
...
DO WITH FRAME {&FRAME-NAME}:
  RETURN cb:SCREEN-VALUE + "|" + STRING(fd:SCREEN-VALUE) + "|" +  STRING(fh:SCREEN-VALUE).   
