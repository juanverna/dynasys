...
&Scoped-Define ENABLED-OBJECTS fdesde fhasta BUTTON-1 
...
DEFINE FRAME F-Main
     fdesde AT ROW 3.86 COL 20 COLON-ALIGNED WIDGET-ID 4
     fhasta AT ROW 3.86 COL 53 COLON-ALIGNED WIDGET-ID 2
     BUTTON-1 AT ROW 5.76 COL 29 WIDGET-ID 8
     "   Impresion de analisis de Agua para MCBA" VIEW-AS TEXT
...
DO:
  ASSIGN FRAME {&FRAME-NAME} fdesde fhasta.
  FOR EACH evento WHERE evento.frealizado >= fdesde AND evento.frealizado <= fhasta
     AND NOT evento.anulado AND evento.nro_tipo_evento = 3, evento_protocolo OF evento BY evento_protocolo.fecha_analisis:
  ...
  END.
  RUN analisisagua.p( INPUT TABLE aimp ).
END.
