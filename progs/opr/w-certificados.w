...
&Scoped-Define ENABLED-OBJECTS Tsoloactivos 
&Scoped-Define DISPLAYED-OBJECTS Tsoloactivos 
...
DEFINE VARIABLE h_b-certificados AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-certificados AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE VARIABLE Tsoloactivos AS LOGICAL INITIAL yes 
     LABEL "Ver solotalonarios activos" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME F-Main
     Tsoloactivos AT ROW 10.05 COL 86 WIDGET-ID 2
...
&Scoped-define SELF-NAME Tsoloactivos
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Tsoloactivos W-Win
ON VALUE-CHANGED OF Tsoloactivos IN FRAME F-Main /* Ver solotalonarios activos */
DO:
  dynamic-function("soloactivos" IN h_b-certificados,tsoloactivos:CHECKED ).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-certificados.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-certificados ).
       RUN set-position IN h_b-certificados ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-certificados ( 12.14 , 80.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-certificados.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-certificados ).
       RUN set-position IN h_v-certificados ( 1.00 , 84.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.33 , 52.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 11.24 , 86.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 1.76 , 51.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-certificados. */
       RUN add-link IN adm-broker-hdl ( h_b-certificados , 'Record':U , h_v-certificados ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-certificados ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-certificados ,
             Tsoloactivos:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-certificados ,
             h_b-certificados , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             Tsoloactivos:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
