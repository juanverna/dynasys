...
DEFINE VAR hcproc AS CHARACTER NO-UNDO.
DEFINE VAR grabobs AS LOGICAL NO-UNDO.
DEFINE VAR k AS INT NO-UNDO.
DEFINE VAR recnombre AS CHAR NO-UNDO. 
...
END.
  ...
  END.
  recnombre="".
  REPEAT k = 1 TO num-entries(evento.recursos):
      FIND recurso WHERE recurso.cdg_recurso = ENTRY(k,evento.recursos) NO-LOCK NO-ERROR.
      recnombre = recnombre + "," + Recurso.nom_recurso.
  END.
  recnombre = SUBSTRING(recnombre,2).
  evento.observaciones = agregaAdvTexto( recnombre, evento.observaciones).
