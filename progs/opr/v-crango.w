...
DEFINE FRAME F-Main
     ...
     Fd AT ROW 3.38 COL 13 COLON-ALIGNED
     Fh AT ROW 3.38 COL 29 COLON-ALIGNED
     b_ayuda AT ROW 5.76 COL 53 WIDGET-ID 2
     "Rango Dias del mes permitidos" VIEW-AS TEXT
          SIZE 32 BY 1.19 AT ROW 1.24 COL 10
...
DO:
  MESSAGE "Explicitar un rango en dias del mes" VIEW-AS ALERT-BOX INFORMATION.
END.
...
IF AVAILABLE contrato_restriccion  THEN DO:
    fd:SCREEN-VALUE IN FRAME {&FRAME-NAME} = entry(1,contrato_restriccion.valor,"|") NO-ERROR.
    fh:SCREEN-VALUE IN FRAME {&FRAME-NAME} = entry(2,contrato_restriccion.valor,"|") NO-ERROR.
END.
...
------------------------------------------------------------------------------*/
  ...
  RETURN STRING(fd:SCREEN-VALUE IN FRAME {&FRAME-NAME}) + "|" +  STRING(fh:SCREEN-VALUE IN FRAME {&FRAME-NAME}).   /* Function return value. */
