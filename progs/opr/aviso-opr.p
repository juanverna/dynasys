/*AVISO-OPR RESUELVE EL GRADO DE SATISFACCION DE UN CONTRATO EN CADA DIA INDICADO EN RVALOR*/
/*recibe como entrada la tabla analizado con los eventos a analizar 
y debe buscar los recursos que realizan algun evento en el radio , y asignar uno solo de esos recursos
*/
...
find evento where evento.nro_evento = ppnro_evento AND NOT evento.fasignado<>? AND NOT anulado NO-ERROR.
...
IF evento.RefEvento <> 0 THEN DO:
    FIND bevento WHERE bevento.nro_evento = evento.RefEvento AND bevento.fasignado<>? NO-LOCK.
    IF NOT AVAILABLE bevento THEN DO:
        MESSAGE "Evento relacionado " evento.RefEvento " No encontrado o no asignado" VIEW-AS ALERT-BOX ERROR.
        LEAVE.
    END.
END.
...
FOR EACH rvalor WHERE rvalor.nro_evento = evento.nro_evento:
           ...
           IF evento.RefEvento = 0  THEN DO:
               FOR EACH contrato_restriccion WHERE contrato_restriccion.nro_contrato = evento.nro_identificacion 
                   AND contrato_restriccion.sub_evento = evento.sub_evento
                        , restriccion no-lock OF contrato_restriccion where restriccion.evaluar AND restriccion.esrestriccion:
    ...
    /*acumulamos la suma para priorizar*/
           ELSE DO:
                FIND restriccion no-lock WHERE cdg_restriccion = "DfEV".
           ...
           END.
           ASSIGN   rvalor.valor = if not flg AND weekday(fecha) <> 7 then v else rvalor.valor . /*cuando no hay restriccion de dia perdon perdon*/
...
IF pnro_evento = ? THEN 
    do:
    /*son solo eventos del tipo sub_evento = 1 o que RefEvento es 0 , otro nro de sub_evento mayor se hace despues pasando en numero de evento ya que la asignacion depende de este*/
    /*valorizamos primero los sub_evento 1 sin dependencia, despues los 2 sin dependencia despues con dependencia 1 y dos*/
        FOR EACH analizado use-index idx2:
            ...
            if evento.sub_evento > 1 OR evento.RefEvento <> 0 then next. /*porque aun no estan asignados*/
            ...
            FOR EACH bevento WHERE bevento.origen = evento.origen AND 
                                          ...
                                          bevento.fmax <= evento.fmin AND
                                          ROWID(bevento) <> ROWID(evento) BY bevento.nro_evento DESC:
