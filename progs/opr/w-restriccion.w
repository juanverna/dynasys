...
DEFINE VARIABLE h_b-recorridos AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-recorrido AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* ABM Restricciones */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* ABM Restricciones */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* ABM Restricciones */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-restriccion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-recorridos ).
       RUN set-position IN h_b-recorridos ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-recorridos ( 12.62 , 18.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-restriccion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-recorrido ).
       RUN set-position IN h_v-recorrido ( 1.24 , 21.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.52 , 82.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 11.71 , 20.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 1.91 , 82.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-recorrido. */
       RUN add-link IN adm-broker-hdl ( h_b-recorridos , 'Record':U , h_v-recorrido ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-recorrido ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-recorrido ,
             h_b-recorridos , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-recorrido , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/DSP" "Mantenimiento de Recorridos de Entrega"}
