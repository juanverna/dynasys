...
DEFINE VARIABLE h_b-analisis_protocolo AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-excel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-analisis_protocolo AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 22.67 COL 111 WIDGET-ID 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Administracion de Analisis"
         HEIGHT             = 27.57
         WIDTH              = 156.2
         MAX-HEIGHT         = 27.57
         MAX-WIDTH          = 168
...
ON END-ERROR OF W-Win /* Administracion de Analisis */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Administracion de Analisis */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Administracion de Analisis */
DO:
...
ON CHOOSE OF BUTTON-1 IN FRAME F-Main /* ITR */
DO:
DEFINE VAR dd AS DATE.
DEFINE VAR ldato AS LONGCHAR.
DEFINE VAR retok AS LOGICAL.

  FOR EACH evento_protocolo WHERE evento_protocolo.estado = "N" , evento OF evento_protocolo WHERE evento.frealizado < TODAY - 5 :
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-analisis_protocolo_control.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-analisis_protocolo ).
       RUN set-position IN h_b-analisis_protocolo ( 1.24 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-analisis_protocolo ( 10.95 , 153.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-analisis_protocolo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-analisis_protocolo ).
       RUN set-position IN h_v-analisis_protocolo ( 12.67 , 11.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.05 , 131.20 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 22.43 , 20.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_p-excel ).
       RUN set-position IN h_p-excel ( 22.43 , 92.00 ) NO-ERROR.
       RUN set-size IN h_p-excel ( 1.76 , 11.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-analisis_protocolo. */
       RUN add-link IN adm-broker-hdl ( h_p-excel , 'excel':U , h_b-analisis_protocolo ).

       /* Links to SmartV8Viewer h_v-analisis_protocolo. */
       RUN add-link IN adm-broker-hdl ( h_b-analisis_protocolo , 'Record':U , h_v-analisis_protocolo ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-analisis_protocolo ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-analisis_protocolo ,
             BUTTON-1:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-analisis_protocolo ,
             h_b-analisis_protocolo , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-analisis_protocolo , 'AFTER':U ).
