...
Cliente.direccion comprest(evento.nro_evento) @ comprest Evento.Recursos ~
Evento.Turno Evento.Duracion ~
duref(evento.hora_desde,evento.hora_hasta) @ duref ~
rdur(evento.nro_evento)  @ rdur unidades(evento.nro_cliente) @ unidades ~
Evento.FAsignado Evento.Anulado stavisado(Evento.nro_evento) @ stasignado ~
evavisado(evento.nro_evento) @ evavisado Evento.Bloqueado ~
evento.fasignado<>? @ asignado perant(evento.nro_evento) @ perant ~
Evento.nro_evento Evento.nro_evento_padre Evento.nro_identificacion ~
Evento.sub_evento evento.frealizado <> ? @ realizado Evento.FRealizado ~
tienevortex() @ tienevortex Evento.hora_desde Evento.hora_hasta Evento.FMin ~
Evento.FMax Evento.Evaluar Evento.Mobs Evento.nro_planasignar ~
Evento.FCreado Cliente.nom_cliente Evento.Origen Evento.Observaciones ~
Evento.Leyenda 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH recurso_agenda WHERE ~{&KEY-PHRASE} ~
      AND Evento.Anulado = FALSE NO-LOCK, ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH recurso_agenda WHERE ~{&KEY-PHRASE} ~
      AND Evento.Anulado = FALSE NO-LOCK, ~
      EACH Evento OF recurso_agenda ~
      WHERE Evento.Anulado = FALSE NO-LOCK, ~
      EACH Cliente OF Evento NO-LOCK, ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      comprest(evento.nro_evento) @ comprest COLUMN-LABEL "Restricciones"
            WIDTH 46.2
      ...
      Evento.Bloqueado COLUMN-LABEL "Blk" FORMAT "Si/No":U VIEW-AS TOGGLE-BOX
      ...
      Evento.Evaluar FORMAT "si/no":U VIEW-AS TOGGLE-BOX
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 109 BY 6.67 ROW-HEIGHT-CHARS .62 FIT-LAST-COLUMN TOOLTIP "Boton secundario para activar RDUR".
...
ASSIGN 
       ...
       br_table:MAX-DATA-GUESS IN FRAME F-Main         = 5000
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "sic.recurso_agenda,sic.Evento OF sic.recurso_agenda,sic.Cliente OF sic.Evento,sic.Tipo_evento WHERE sic.Evento ..."
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE"
     _TblOptList       = ",,,"
     _OrdList          = "sic.Evento.Turno|yes"
     _Where[1]         = "sic.Evento.Anulado = FALSE"
...
"Tipo_evento.cdg_tipo_evento" "Ti!Ev" "X(3)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > sic.Cliente.direccion
"Cliente.direccion" ? ? "character" ? ? ? ? ? ? no ? no no "35.4" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > "_<CALC>"
"comprest(evento.nro_evento) @ comprest" "Restricciones" ? ? ? ? ? ? ? ? no ? no no "46.2" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > sic.Evento.Recursos
"Evento.Recursos" "Rec." ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > sic.Evento.Turno
"Evento.Turno" "Tu" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > sic.Evento.Duracion
"Evento.Duracion" "Durac" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > "_<CALC>"
"duref(evento.hora_desde,evento.hora_hasta) @ duref" "DurRef" ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > "_<CALC>"
"rdur(evento.nro_evento)  @ rdur" "Rdur" ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > "_<CALC>"
"unidades(evento.nro_cliente) @ unidades" "Unid" ">>9" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   = sic.Evento.FAsignado
     _FldNameList[11]   > sic.Evento.Anulado
"Evento.Anulado" ? ? "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "TOGGLE-BOX" "," ? ? 5 no 0 no no
     _FldNameList[12]   > "_<CALC>"
"stavisado(Evento.nro_evento) @ stasignado" "AV" "X" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[13]   > "_<CALC>"
"evavisado(evento.nro_evento) @ evavisado" "AVISO" ">>>>>>>9" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[14]   > sic.Evento.Bloqueado
"Evento.Bloqueado" "Blk" ? "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "TOGGLE-BOX" "?" ? ? 5 no 0 no no
     _FldNameList[15]   > "_<CALC>"
"evento.fasignado<>? @ asignado" "AS" "S/N" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[16]   > "_<CALC>"
"perant(evento.nro_evento) @ perant" "As.Anterior" ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[17]   = sic.Evento.nro_evento
     _FldNameList[18]   = sic.Evento.nro_evento_padre
     _FldNameList[19]   > sic.Evento.nro_identificacion
"Evento.nro_identificacion" "Ident." ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[20]   = sic.Evento.sub_evento
     _FldNameList[21]   > "_<CALC>"
"evento.frealizado <> ? @ realizado" "RE" "S/N" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[22]   = sic.Evento.FRealizado
     _FldNameList[23]   > "_<CALC>"
"tienevortex() @ tienevortex" "VTX" "S/N" ? ? ? ? ? ? ? no ? no no "4.8" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[24]   = sic.Evento.hora_desde
     _FldNameList[25]   = sic.Evento.hora_hasta
     _FldNameList[26]   = sic.Evento.FMin
     _FldNameList[27]   = sic.Evento.FMax
     _FldNameList[28]   > sic.Evento.Evaluar
"Evento.Evaluar" ? ? "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "TOGGLE-BOX" "," ? ? 5 no 0 no no
     _FldNameList[29]   = sic.Evento.Mobs
     _FldNameList[30]   = sic.Evento.nro_planasignar
     _FldNameList[31]   = sic.Evento.FCreado
     _FldNameList[32]   = sic.Cliente.nom_cliente
     _FldNameList[33]   = sic.Evento.Origen
     _FldNameList[34]   = sic.Evento.Observaciones
     _FldNameList[35]   = sic.Evento.Leyenda
...
/*------------------------------------------------------------------------------
  Purpose:  Duracion media
    Notes:  Se tiene en cuenta de descartar valores  alejados +- 20% de la media
------------------------------------------------------------------------------*/
IF NOT MENU-ITEM m_RDUR:CHECKED IN MENU POPUP-MENU-br_table THEN RETURN 0.
DEFINE VAR kk AS INT NO-UNDO.
DEFINE VAR pp1 AS INT NO-UNDO.
DEFINE VAR media AS INT NO-UNDO.
DEFINE VAR suma AS INT NO-UNDO.
