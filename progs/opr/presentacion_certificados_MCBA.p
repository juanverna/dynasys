...
DEFINE VAR impresora AS CHAR.
DEFINE INPUT PARAMETER cdesde AS INT LABEL "Certificado".
DEFINE INPUT PARAMETER chasta AS INT LABEL "".
DEFINE INPUT PARAMETER cletraprefijo AS CHAR LABEL "LetraPrefijo".
...
/*UPDATE cdesde chasta WITH SIDE-LABELS.*/

impresora = "?EX". /*impreport(3).*/
vind = 0.

FOR EACH evento_protocolo WHERE evento_protocolo.nro_tipo_evento = 3 AND evento_protocolo.letraprefijo = cletraprefijo AND evento_protocolo.nro_certificado >= cdesde AND evento_protocolo.nro_certificado <= chasta, evento OF evento_protocolo,tipo_evento OF evento,cliente OF evento BY evento_protocolo.nro_certificado:
    vind = vind + 1.
    /*IF NUM-ENTRIES(evento.recursos) <> 2 THEN NEXT.*/
    recursonom = "".
    
    REPEAT i = 1 TO NUM-ENTRIES(evento.recursos):
      IF i > 2 THEN NEXT.
    ...
    ASSIGN lstorden.nro_evento = evento.nro_evento
               ...
               lstorden.referencia = cliente.nom_cli
               lstorden.ind = vind.
               lstorden.estado_tanque = entry(evento.entrega + 1 ,estado_var).
               lstorden.nro_certificado = string(evento_protocolo.nro_certificado,"9999999").
               lstorden.letraprefijo = evento_protocolo.letraprefijo.
               ...
               lstorden.recurso = recursonom.
END.


ReportePath = "definit_3Entrega".
...
RUN crearReporte(chReport,"rpt", /*ViewReport*/ FALSE , /*PrinterName*/ impresora,
