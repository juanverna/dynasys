...
DEFINE VAR tienevortex AS CHAR NO-UNDO COLUMN-LABEL "VTX".
DEFINE VAR selecionados AS CHAR NO-UNDO.
DEFINE VAR originados AS CHAR INITIAL "*".
DEFINE VAR solorealizados AS LOGICAL NO-UNDO INITIAL TRUE.
{stavisado.i}
...
DEFINE VAR h_diaoper AS HANDLE.
DEFINE VAR sin-certif AS LOGICAL NO-UNDO INITIAL TRUE.
DEFINE VAR minper AS INT64 NO-UNDO.
minper = YEAR( TODAY - 60 ) * 100 + MONTH( TODAY - 60 ).
...
Evento.Bloqueado Evento.FRealizado tienevortex() @ tienevortex ~
...
&Scoped-define QUERY-STRING-br_table FOR EACH Evento OF Cliente WHERE ~{&KEY-PHRASE} ~
      AND can-do( selecionados, string(evento.nro_tipo_evento)) and ~
can-do( originados, evento.origen ) and not ( evento.nro_tipo_evento = 10 and evento.periodo < minper ) ~
and ( solorealizados or evento.frealizado <> ?) and ~
( sin-certif or evento.nro_certif = 0 ) NO-LOCK, ~
      EACH Tipo_evento WHERE Tipo_evento.nro_tipo_evento = Evento.nro_tipo_evento NO-LOCK ~
    BY Evento.Periodo DESCENDING ~
       BY Evento.nro_evento DESCENDING INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Evento OF Cliente WHERE ~{&KEY-PHRASE} ~
      AND can-do( selecionados, string(evento.nro_tipo_evento)) and ~
can-do( originados, evento.origen ) and not ( evento.nro_tipo_evento = 10 and evento.periodo < minper ) ~
and ( solorealizados or evento.frealizado <> ?) and ~
( sin-certif or evento.nro_certif = 0 ) NO-LOCK, ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD originados B-table-Win 
FUNCTION originados RETURNS LOGICAL
  ( pselecionados AS CHAR )  FORWARD.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD realizados B-table-Win 
FUNCTION realizados RETURNS LOGICAL
  ( prealizados AS CHAR )  FORWARD.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD seleccionados B-table-Win 
FUNCTION seleccionados RETURNS LOGICAL
  ( pselecionados AS CHAR )  FORWARD.

/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD sin-certif B-table-Win 
FUNCTION sin-certif RETURNS LOGICAL
  (psincertif AS CHAR )  FORWARD.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      stavisado(evento.nro_evento) @ stavisado COLUMN-LABEL "AV" FORMAT "X":U
      Evento.Bloqueado FORMAT "Si/No":U VIEW-AS TOGGLE-BOX
      Evento.FRealizado COLUMN-LABEL "FRealiz." FORMAT "99/99/99":U
      tienevortex() @ tienevortex COLUMN-LABEL "VTX" FORMAT "X":U
            WIDTH 4.8
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN NO-COLUMN-SCROLLING SEPARATORS SIZE 122 BY 6.71
         FGCOLOR 0  TOOLTIP "Con dbl-click visualiza la agenda del operario del dia del evento".
...
/* Query rebuild information for BROWSE br_table
     ...
     _OrdList          = "sic.Evento.Periodo|no,sic.Evento.nro_evento|no"
     _Where[1]         = "can-do( selecionados, string(evento.nro_tipo_evento)) and
can-do( originados, evento.origen ) and not ( evento.nro_tipo_evento = 10 and evento.periodo < minper )
and ( solorealizados or evento.frealizado <> ?) and
( sin-certif or evento.nro_certif = 0 )"
     _JoinCode[2]      = "sic.Tipo_evento.nro_tipo_evento = sic.Evento.nro_tipo_evento"
     _FldNameList[1]   > sic.Tipo_evento.cdg_tipo_evento
"Tipo_evento.cdg_tipo_evento" "Ti!Ev" "X(3)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  /*solo si soy yo*/
IF USERID("sic") = "fernando" THEN do:
...
DO:
   IF VALID-HANDLE(h_diaoper) THEN 
       RUN dispatch IN h_diaoper ( INPUT 'destroy':U ) .
END.
...
IF AVAILABLE evento THEN
  RUN d-traza.w(evento.nro_evento).
END.
...
DO:
  IF AVAILABLE evento THEN DO:  
      ...
      IF NOT VALID-HANDLE(h_diaoper) THEN DO:
         RUN w-diaoper.w PERSISTENT set h_diaoper  ( rowid(evento) ).
         RUN dispatch IN h_diaoper ( INPUT 'initialize':U ) .
      END.
  END.
END.
      ...
      IF evento.origen = "CONTRATO" 
          THEN do:
            ...
            FOR EACH contrato_restriccion WHERE contrato_restriccion.nro_contrato = evento.nro_identificacion AND
              ...
              FIND restriccion no-lock OF contrato_restriccion NO-ERROR.
              IF NOT AVAILABLE restriccion THEN DO:
                    MESSAGE "No se encuentra la restriccion " contrato_restriccion.nro_restriccion SKIP
                        "del contrato " contrato_restriccion.nro_contrato evento.sub_evento VIEW-AS ALERT-BOX ERROR.
                    RETURN NO-APPLY.
              END.
...
DO:
 DEFINE VAR pp AS CHAR NO-UNDO.
 pp = selecionados.
 RUN d-filtroevento.w ( INPUT-OUTPUT selecionados ).
 IF pp <> selecionados THEN
       {&OPEN-QUERY-{&BROWSE-NAME}}
 APPLY "END-SEARCH" TO br_table.
END.
...
------------------------------------------------------------------------------*/
  /* Code placed here will execute PRIOR to standard behavior. */
 FOR EACH tipo_evento WHERE cdg_tipo_evento <> "--":
      selecionados = selecionados + "," +  string(nro_tipo_evento).
  END.
  selecionados = SUBSTR(selecionados,2).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION originados B-table-Win 
FUNCTION originados RETURNS LOGICAL
  ( pselecionados AS CHAR ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
 IF originados <> pselecionados THEN DO:
  originados = pselecionados.
 END.
...
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
IF AVAILABLE evento THEN RETURN evento.nro_evento.
  ELSE RETURN ?.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION realizados B-table-Win 
FUNCTION realizados RETURNS LOGICAL
  ( prealizados AS CHAR ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
  solorealizados = NOT ( prealizados = "SOLO").
  RETURN solorealizados.   /* Function return value. */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION seleccionados B-table-Win 
FUNCTION seleccionados RETURNS LOGICAL
  ( pselecionados AS CHAR ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
 IF selecionados <> pselecionados THEN DO:
  selecionados = pselecionados.
 END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION sin-certif B-table-Win 
FUNCTION sin-certif RETURNS LOGICAL
  (psincertif AS CHAR ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
  sin-certif = NOT psincertif = "SOLO".
  RETURN sin-certif.   /* Function return value. */
