...
&Scoped-Define ENABLED-OBJECTS v-certi BUTTON-14 
&Scoped-Define DISPLAYED-OBJECTS v-certi 
...
DEFINE VARIABLE v-certi AS LOGICAL INITIAL yes 
     LABEL "No Chequea Administracion" 
     VIEW-AS TOGGLE-BOX
     SIZE 39 BY .81 TOOLTIP "Marquelo si no desea checkeo" NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     v-certi AT ROW 2.91 COL 18 WIDGET-ID 44
     BUTTON-14 AT ROW 6 COL 54.8 WIDGET-ID 38
     "        Si requiere del libro puede eliminar esta restriiccion" VIEW-AS TEXT
          SIZE 67 BY 1.19 AT ROW 4.33 COL 2.4 WIDGET-ID 42
...
IF AVAILABLE cliente_restriccion  THEN DO:
    v-certi:SCREEN-VALUE IN FRAME {&FRAME-NAME} = valor.
END.
...
DO:
  MESSAGE "Elija el momento de la impresion del certificado" VIEW-AS ALERT-BOX INFORMATION.
END.
...
------------------------------------------------------------------------------*/
  ...
  v-certi:SENSITIVE IN FRAME {&FRAME-NAME} = FALSE.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE contrato_restriccion THEN DO:
            v-certi:SCREEN-VALUE IN FRAME {&FRAME-NAME} = contrato_restriccion.valor NO-ERROR.
  END.
  /* Code placed here will execute AFTER standard behavior.    */
...
------------------------------------------------------------------------------*/
  ...
  v-certi:SENSITIVE IN FRAME {&FRAME-NAME} = TRUE.
...
------------------------------------------------------------------------------*/
  ...
  RETURN v-certi:SCREEN-VALUE IN FRAME {&FRAME-NAME} .   /* Function return value. */
