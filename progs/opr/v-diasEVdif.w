...
DEFINE FRAME F-Main
     v-Valor AT ROW 1 COL 11 COLON-ALIGNED HELP
          "Valor de la restriccion"
          LABEL "Contrato" FORMAT "x(10)"
          BGCOLOR 15 FGCOLOR 9 
     v-sub_evento AT ROW 1 COL 38 COLON-ALIGNED WIDGET-ID 4
...
/* Cs OCXINFO:CREATE-CONTROL from: {EAF26C8F-9586-101B-9306-0020AF234C9D} type: CSSpin */
      Cs:MOVE-AFTER(v-sub_evento:HANDLE IN FRAME F-Main).
...
ON LEAVE OF v-Valor IN FRAME F-Main /* Contrato */
DO:
...
OCXFile = SEARCH( "v-diasEVdif.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "v-diasEVdif.wrx":U SKIP(1)
...
IF AVAILABLE contrato_restriccion  THEN DO:
        ...
        err = err OR error-status:ERROR.
        chcs:csspin:VALUE  = int(ENTRY(3,Contrato_restriccion.valor,"|")) NO-ERROR.
        err = err OR error-status:ERROR.
        IF NOT err  THEN DO:
            ...
            IF AVAILABLE b_contrato_hd THEN DO:
                FIND cliente OF b_contrato_hd NO-ERROR.
                  ...
                  {&OPEN-QUERY-{&BROWSE-NAME}}
            END.
            ELSE muestra-error("Ya hay una restriccion igual").  
        END.
...
------------------------------------------------------------------------------*/
  ...
  RETURN trim(v-valor) + "|" + STRING(v-sub_evento) + "|" +  string(chcs:CSSpin:VALUE).   /* Function return value. */
