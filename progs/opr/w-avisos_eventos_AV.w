...
DEFINE TEMP-TABLE aimp
    ...
    DEFINE TEMP-TABLE aimp3 LIKE aimp
...
&Scoped-Define ENABLED-OBJECTS Ccondicion bProceso 
&Scoped-Define DISPLAYED-OBJECTS Ccondicion 
...
DEFINE VARIABLE Ccondicion AS CHARACTER FORMAT "X(256)":U INITIAL "1" 
     ...
     LIST-ITEM-PAIRS "No impreso con anterioridad","1",
                     "Reimprimir solo los de la fecha indicada","2"
     DROP-DOWN-LIST
...
DEFINE FRAME F-Main
     Ccondicion AT ROW 2.14 COL 15.2 COLON-ALIGNED WIDGET-ID 46
...
DO:
    DEFINE VAR p-des_fecha AS DATE INITIAL 01/01/1000.
   ...
   /*buscando todos los avisos asignados para el dia de evaluacion*/
    ...
    RUN dar_rango IN h_v-rng-fechas ( OUTPUT p-has_fecha , OUTPUT  error_rango) .
    IF ERROR_rango THEN DO:
        MESSAGE "Error de fechas".
        RETURN NO-APPLY.
    END.
    EMPTY TEMP-TABLE aimp.
    IF  ccondicion = "1" THEN p-des_fecha = 01/01/2010.
    IF  ccondicion = "2" THEN p-des_fecha = p-has_fecha.

    FOR EACH evento NO-LOCK WHERE  
           NOT evento.anulado AND
           evento.nro_tipo_evento = tipo_evento.nro_tipo_evento AND
           evento.fasignado >= p-des_fecha AND
           evento.fasignado <= p-has_fecha :

          IF  ccondicion = "1" AND evento.impreso  THEN NEXT.
          IF  ccondicion = "2" AND NOT evento.impreso AND evento.fasignado <> p-has_fecha THEN NEXT.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-fechas ( 4.57 , 33.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 42.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-fechas ,
             Ccondicion:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
