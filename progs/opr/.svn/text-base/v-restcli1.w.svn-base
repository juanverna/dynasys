...
&Scoped-Define ENABLED-FIELDS Contrato_Restriccion.Prioridad 
...
&Scoped-Define DISPLAYED-FIELDS Contrato_Restriccion.Prioridad 
...
RUN set-attribute-list (
    'Keys-Accepted = ,
     Keys-Supplied = "nro_Contrato,nro_restriccion,nro_contrato"':U).
/**************************
</EXECUTING-CODE> */
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD que_contrato V-table-Win 
...
DEFINE FRAME F-Main
     ...
     v-descripcion AT ROW 1.24 COL 19 COLON-ALIGNED HELP
          "" NO-LABEL
     Contrato_Restriccion.Prioridad AT ROW 2.91 COL 5 HELP
...
/* SETTINGS FOR SLIDER Contrato_Restriccion.Prioridad IN FRAME F-Main
   EXP-HELP                                                             */
...
DO:
    ...
    END.
    contrato_restriccion.prioridad:screen-value = string(restriccion.prioridad,"99").  
...
------------------------------------------------------------------------------*/
  ...
  DEF VAR v_nro_contrato LIKE contrato_restriccion.nro_contrato NO-UNDO.
  DEF VAR v_sub_evento LIKE contrato_restriccion.sub_evento NO-UNDO.
  DEF VAR rr AS ROWID NO-UNDO.
  DEF VAR v_valor AS CHARACTER NO-UNDO.
  ASSIGN FRAME {&FRAME-NAME} v_nro_restriccion.

  v_nro_contrato = que_contrato().
  v_sub_evento= que_sub_evento().
  v_valor = que_valor().
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'assign-statement':U ) .
  IF NEW contrato_restriccion THEN
  ASSIGN contrato_restriccion.nro_Contrato = v_nro_contrato
         contrato_restriccion.sub_evento = v_sub_evento.

  ASSIGN contrato_restriccion.valor = v_valor
         contrato_restriccion.nro_restriccion = v_nro_restriccion.
  ...
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'end-update':U ) .
    ...
    RUN get-link-handle IN adm-broker-hdl
      ( INPUT THIS-PROCEDURE,
        ...
        OUTPUT hcproc ).
    hproc = WIDGET-HANDLE(hcproc).
    IF VALID-HANDLE(hproc) THEN RUN re-reposicionar IN hproc ( ROWID(contrato_restriccion) ).
...
IF AVAILABLE contrato_restriccion THEN DO:
         v_nro_restriccion = contrato_restriccion.nro_restriccion.
         IF ant_restriccion <> contrato_restriccion.nro_restriccion THEN DO:
           ant_restriccion = contrato_restriccion.nro_restriccion.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION que_contrato V-table-Win 
FUNCTION que_contrato RETURNS INTEGER
...
RETURN IF VALID-HANDLE( hp) THEN DYNAMIC-FUNCTION( "que_contrato" IN hp ) ELSE ?.
