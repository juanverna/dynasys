...
&Scoped-Define ENABLED-OBJECTS conf b_ayuda 
&Scoped-Define DISPLAYED-OBJECTS conf 
...
DEFINE FRAME F-Main
     ...
     conf AT ROW 2.91 COL 24 NO-LABEL WIDGET-ID 6
     b_ayuda AT ROW 5.76 COL 53 WIDGET-ID 2
     "Envio de Facturas a la emision" VIEW-AS TEXT
          SIZE 49 BY 1.19 TOOLTIP "Se enviara la factura" AT ROW 1.24 COL 10
...
DO:
  MESSAGE "Se entregara al destinatario la factura para su pago" 
...
------------------------------------------------------------------------------*/
  ...
  conf:SENSITIVE IN FRAME {&FRAME-NAME} = FALSE.
...
IF AVAILABLE cliente_restriccion  THEN DO:
    conf:SCREEN-VALUE IN FRAME {&FRAME-NAME} = cliente_restriccion.valor NO-ERROR.
END.
...
------------------------------------------------------------------------------*/
  ...
  conf:SENSITIVE IN FRAME {&FRAME-NAME} = TRUE.
