...
&Scoped-Define DISPLAYED-OBJECTS durac 
...
DEFINE VARIABLE durac AS INTEGER FORMAT ">,>>9":U INITIAL 0 
     LABEL "Duracion" 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1 TOOLTIP "Duracion de la tarea an minutos" NO-UNDO.
...
DEFINE FRAME F-Main
     Contrato_Restriccion.Valor AT ROW 1.24 COL 20 NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 4 BY .29
     durac AT ROW 3.86 COL 28 COLON-ALIGNED WIDGET-ID 6
     b_ayuda AT ROW 5.76 COL 54 WIDGET-ID 4
     "Duracion en minutos del evento" VIEW-AS TEXT
          SIZE 32 BY 1.43 AT ROW 1.95 COL 19
...
/* SETTINGS FOR FILL-IN durac IN FRAME F-Main
   NO-ENABLE                                                            */
...
DO:
  MESSAGE "Es la duracion en minutos del evento" skip
          "en caso de eventos con OT el tiempo se ajustara" skip
          "al ingresar la OT" SKIP
      "El valor por default se obtiene del parametro OPRDDTA" VIEW-AS ALERT-BOX INFORMATION.
END.
...
DEF VAR vv AS CHAR NO-UNDO.
  ...
  RUN getparametro_n.p( INPUT "OPRDURAC", OUTPUT vv).
  durac:input-value in frame {&FRAME-NAME} = string(int(vv)) NO-ERROR.
...
durac:sensitive in frame {&FRAME-NAME}= false.
...
IF AVAILABLE contrato_restriccion THEN DO:
    durac = int(contrato_restriccion.valor) NO-ERROR.
    display durac with frame {&FRAME-NAME}.
    IF error-status:ERROR THEN DO:
        muestra-error("El valor de:" + Contrato_restriccion.valor + " no es valido se ignorara").  
    END.
...
durac:sensitive in frame {&FRAME-NAME}= true.
  /* Code placed here will execute AFTER standard behavior.    */
...
/*------------------------------------------------------------------------------
  Purpose: Devuelve el valor a ser asignado
    Notes:  
------------------------------------------------------------------------------*/
assign frame {&FRAME-NAME} durac.
  RETURN  string(durac:input-value in frame {&FRAME-NAME}).   /* Function return value. */
