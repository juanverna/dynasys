...
DEFINE VARIABLE Cs AS WIDGET-HANDLE NO-UNDO.
DEFINE VARIABLE chCs AS COMPONENT-HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     "Dias de Diferencia con el SUBEVENTO anterior" VIEW-AS TEXT
          SIZE 48 BY 1.43 AT ROW 1.95 COL 12
     SPACE(0.00) SKIP(1.81)
...
CREATE CONTROL-FRAME Cs ASSIGN
...
PROCEDURE adm-create-controls:
      Cs:NAME = "Cs":U .
/* Cs OCXINFO:CREATE-CONTROL from: {EAF26C8F-9586-101B-9306-0020AF234C9D} type: CSSpin */
      Cs:MOVE-AFTER(Contrato_Restriccion.Valor:HANDLE IN FRAME F-Main).
...
DO:
  MESSAGE "Es la diferencia de dias con el " SKIP 
          "subevento inmediatamente inferior" skip
...
OCXFile = SEARCH( "v-diasSEdif.wrx":U ).
IF OCXFile = ? THEN
...
DO:
  ASSIGN
    chCs = Cs:COM-HANDLE
    UIB_S = chCs:LoadControls( OCXFile, "Cs":U)
  .
  RUN DISPATCH IN THIS-PROCEDURE("initialize-controls":U) NO-ERROR.
END.
ELSE MESSAGE "v-diasSEdif.wrx":U SKIP(1)
...
DEF VAR vv AS CHAR NO-UNDO.
  ...
  RUN getparametro_n.p( INPUT "OPRDDTA", OUTPUT vv).
  chcs:CSSpin:VALUE = int(vv) NO-ERROR.
...
IF AVAILABLE contrato_restriccion THEN DO:
    ...
    chcs:csspin:VALUE  = int(contrato_restriccion.valor) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  RETURN  string(chcs:CSSpin:VALUE).   /* Function return value. */
