...
DEFINE VARIABLE h_v-que_periodo AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 5.29 COL 18 COLON-ALIGNED NO-LABEL
     "Periodo del Listado" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 1.19 COL 20
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-que-periodo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-que_periodo ).
       RUN set-position IN h_v-que_periodo ( 2.43 , 20.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 6.71 , 20.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.14 , 103.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-que_periodo ,
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE v-que_periodo        AS CHAR NO-UNDO.
  ...
  DEFINE VAR xfile AS CHAR NO-UNDO INITIAL "c:\sic-temp\contratos_restriccion.xml".
    
  DEF VAR ERROR_nro AS INT NO-UNDO.
  DEFINE VARIABLE ReportePath AS CHARACTER NO-UNDO. 


  RUN dar_rango IN h_v-que_periodo    ( OUTPUT v-que_periodo, 
                                       OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Periodo" ).
  ...
  THEN DO:
       ...
       RUN contrato_restriccion.p ( INPUT  v-que_periodo , INPUT-OUTPUT xfile ).
       v-estado:SCREEN-VALUE in FRAME {&FRAME-NAME} = "Proceso Terminado. Ingresando Parametros ...".
       
       RUN fullPath ("contrato_restr", '.rpt':U, OUTPUT cFullPath).
IF cFullPath = ? 
THEN DO:
    RUN mensajepar.p (INPUT "contrato_restr.rpt", INPUT "CREP000").
    RETURN ERROR.
END.
...
setParametro( chReport , "p-periodo" , v-que_periodo ).
