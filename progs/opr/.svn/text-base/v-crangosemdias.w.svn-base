...
t-3 m-6 t-4 m-7 ctt1 ctt2 m-8 s-3 t-5 m-9 t-6 m-10 m-11 s-4 t-7 m-12 
...
DEFINE VARIABLE ctt1 AS CHARACTER FORMAT "X":U 
     LABEL "T" 
...
DEFINE VARIABLE ctt2 AS CHARACTER FORMAT "X":U 
     VIEW-AS COMBO-BOX INNER-LINES 5
...
DEFINE FRAME F-Main
     ...
     ctt1 AT ROW 4.81 COL 53 COLON-ALIGNED WIDGET-ID 50
     ctt2 AT ROW 4.81 COL 63 COLON-ALIGNED NO-LABEL WIDGET-ID 52
...
/* SETTINGS FOR COMBO-BOX ctt1 IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR COMBO-BOX ctt2 IN FRAME F-Main
   NO-ENABLE                                                            */
...
/* Code placed here will execute PRIOR to standard behavior. */
  ...
  fh:SENSITIVE = FALSE.
  ctt1:SENSITIVE = FALSE.
  ctt2:SENSITIVE = FALSE.
...
IF AVAILABLE cliente_restriccion THEN DO:
    ...
    ctt1 = substring(entry(4,cliente_restriccion.valor,"|"),1,1) NO-ERROR.
    err = err OR ERROR-STATUS:ERROR.
    ctt2 = substring(entry(4,cliente_restriccion.valor,"|"),2,1) NO-ERROR.    
    err = err OR ERROR-STATUS:ERROR.
    fd = int(entry(5,cliente_restriccion.valor,"|")) NO-ERROR.
    err = err OR ERROR-STATUS:ERROR.
    fh = int(entry(6,cliente_restriccion.valor,"|")) NO-ERROR.
    err = err OR ERROR-STATUS:ERROR.
    ...
    IF err THEN 
       ...
       ELSE DO:
           ...
           END.
           DISPLAY fd fh ctt1 ctt2 WITH FRAME {&FRAME-NAME}.
       END.
END.
...
sal = sal + "|" + STRING(ctt1:SCREEN-VALUE IN FRAME {&FRAME-NAME}) + STRING(ctt2:SCREEN-VALUE) + "|" + STRING(fd:SCREEN-VALUE IN FRAME {&FRAME-NAME}) + "|" +  STRING(fh:SCREEN-VALUE). 
RETURN sal .
