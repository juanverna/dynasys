...
DEFINE VAR diassel AS date NO-UNDO.
DEFINE VAR ppselected AS CHAR NO-UNDO.
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY {&SELF-NAME} FOR EACH recurso_agenda WHERE ~{&KEY-PHRASE}       AND can-do(ppselected, ~
      recurso_agenda.cdg_recurso) and recurso_agenda.fecha = diassel NO-LOCK, ~
             EACH Evento OF recurso_agenda       WHERE NOT Evento.Anulado AND can-do( nrotipo , ~
       STRING(evento.nro_tipo_evento))  NO-LOCK, ~
             EACH Cliente OF Evento NO-LOCK, ~
...
OPEN QUERY {&SELF-NAME} FOR EACH recurso_agenda WHERE ~{&KEY-PHRASE}
      AND can-do(ppselected,recurso_agenda.cdg_recurso) and recurso_agenda.fecha = diassel NO-LOCK,
      EACH Evento OF recurso_agenda
      WHERE NOT Evento.Anulado AND can-do( nrotipo , STRING(evento.nro_tipo_evento))  NO-LOCK,
      EACH Cliente OF Evento NO-LOCK,
...
IF VALID-HANDLE(hProc) THEN DO:
   ...
   RUN seleccionados IN hproc ( OUTPUT ppselected ) NO-ERROR.
   /*{&OPEN-QUERY-{&BROWSE-NAME}}*/
END.
