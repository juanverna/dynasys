...
DEFINE VARIABLE Fd AS INTEGER FORMAT ">9":U INITIAL 1 
     LABEL "Fecha del mes optima" 
     VIEW-AS FILL-IN NATIVE 
     SIZE 6 BY 1 TOOLTIP "Dia del mes 1,2,3,4,.....31"
...
DEFINE FRAME F-Main
     ...
     Fd AT ROW 2.67 COL 32 COLON-ALIGNED
...
DO:
  MESSAGE "Dynasys utilizara esta fecha como optima para realizar el evento" skip
          "si no se lograga asignar este dia , cada dia en el entorno tendra" SKIP 
          "una satisfaccion decreciente a ambos lado del dia elegido"
...
ON LEAVE OF Fd IN FRAME F-Main /* Fecha del mes optima */
DO:
    ASSIGN fd.
    IF fd < 1 OR fd > 3 THEN DO:
      muestra-error("Valor fuera del rango [1-3]").
      RETURN NO-APPLY.
  END.
END.
...
IF AVAILABLE contrato_restriccion THEN DO:
          fd:SCREEN-VALUE IN FRAME {&FRAME-NAME} = contrato_restriccion.valor NO-ERROR.
