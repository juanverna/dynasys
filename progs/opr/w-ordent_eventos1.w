...
&Scoped-Define ENABLED-OBJECTS c_nro_tipo_evento dsc_tipo_evento ~
letraprefijo Treimprime bProceso tcerti tot 
&Scoped-Define DISPLAYED-OBJECTS c_nro_tipo_evento dsc_tipo_evento ~
letraprefijo dnro_certificado hnro_certificado Treimprime tcerti tot 
...
DEFINE VARIABLE tcerti AS INTEGER FORMAT "->,>>>,>>9":U INITIAL 0 
     LABEL "Certificados" 
     VIEW-AS COMBO-BOX INNER-LINES 5
...
DEFINE VARIABLE letraprefijo AS CHARACTER FORMAT "X(256)":U 
     LABEL "Certif." 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1 TOOLTIP "LetraPrefijo o serie del certificado" NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     letraprefijo AT ROW 6.24 COL 28.2 COLON-ALIGNED WIDGET-ID 72
     dnro_certificado AT ROW 6.24 COL 50.4 COLON-ALIGNED WIDGET-ID 52
     hnro_certificado AT ROW 6.24 COL 65.2 COLON-ALIGNED NO-LABEL WIDGET-ID 54
     Treimprime AT ROW 6.33 COL 5.2 WIDGET-ID 56
     bProceso AT ROW 6.71 COL 83 WIDGET-ID 2
     tcerti AT ROW 7.43 COL 20 COLON-ALIGNED WIDGET-ID 68
     tot AT ROW 7.43 COL 49 COLON-ALIGNED WIDGET-ID 70
...
DO:
    ...
    ASSIGN letraprefijo c_nro_tipo_evento dnro_certificado hnro_certificado treimprime tcerti tot.
    IF tcerti = tot AND tot <> 0 THEN DO:
        IF ERROR_rango THEN DO:
            MESSAGE "No puede seleccionar la misma impresora para los dos formularios".
            RETURN NO-APPLY.
        END.
    END.
    ...
    IF cFullPath = ? 
      ...
      END.
      IF tcerti <> 0 THEN DO:
      FOR EACH aimp BY aimp.recurso BY aimp.turno:
          FIND evento WHERE evento.nro_evento = aimp.nro_evento NO-LOCK.
            RUN impcertif.p ( evento.nro_evento , "O" ,tcerti).
          END.
      END.
END.
...
DO:
  ASSIGN treimprime.
         ...
         letraprefijo:VISIBLE = treimprime:CHECKED.
...
------------------------------------------------------------------------------*/
  DISPLAY c_nro_tipo_evento dsc_tipo_evento letraprefijo dnro_certificado 
          hnro_certificado Treimprime tcerti tot 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE c_nro_tipo_evento dsc_tipo_evento letraprefijo Treimprime bProceso 
         tcerti tot 
      WITH FRAME F-Main IN WINDOW W-Win.
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  tcerti:LIST-ITEM-PAIRS =  tcerti:LIST-ITEM-PAIRS + "," + imprelista().
