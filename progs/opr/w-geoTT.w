...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD mostrar W-Win 
FUNCTION mostrar RETURNS LOGICAL
  (  INPUT TABLE FOR ttgeo )  FORWARD.
...
END.
ELSE MESSAGE "w-geoTT.wrx":U SKIP(1)
...
/*------------------------------------------------------------------------------
  Purpose:  Genera el html para googlemaps
    ...
    Pone un mashup distinto por cada tipo de evento
------------------------------------------------------------------------------*/
...
DEFINE VAR colores AS CHAR INITIAL "red,blue,yellow,green,orange,brown,white,black" NO-UNDO.
...
almenos = FALSE.
FOR EACH ttgeo BY ttgeo.ttind:
            IF NOT almenos THEN DO:
                PUT UNFORMATTED
                'map.setCenter(new GLatLng(' ttgeo.ttgeolat ',' ttgeo.ttgeolong '), 15);' skip
                'map.addControl(new GSmallMapControl());' SKIP
                'var customIcons = [];' SKIP.
                DO kk = 1 TO NUM-ENTRIES(colores):
                    PUT UNFORMATTED
                    ...
                    'customIcons["' kk '"] = icon' entry(kk,colores) ';' SKIP.
                END.
                PUT UNFORMATTED
            ...
            END.
            PUT UNFORMATTED
            /*'var point = new GLatLng (' ttgeo.ttgeolat ',' ttgeo.ttgeolong ' );' skip */
            'map.addOverlay(createMarker (new GLatLng (' ttgeo.ttgeolat ',' ttgeo.ttgeolong ' ),"' tturl 
                 '","' ( IF ttgeo.tttipo > NUM-ENTRIES(colores) THEN NUM-ENTRIES(colores) ELSE ttgeo.tttipo ) '"));' skip.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION mostrar W-Win 
FUNCTION mostrar RETURNS LOGICAL
  (  INPUT TABLE FOR ttgeo ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
