...
DEFINE VARIABLE h_b-clientes_nombre AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-exencion_impuestos AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-excencion_impuestos AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Mantenimiento de Excenciones Impositivas"
         HEIGHT             = 21.23
         WIDTH              = 114.29
         MAX-HEIGHT         = 26.42
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF W-Win /* Mantenimiento de Excenciones Impositivas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Mantenimiento de Excenciones Impositivas */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Mantenimiento de Excenciones Impositivas */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-clientes_nombre.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-clientes_nombre ).
       RUN set-position IN h_b-clientes_nombre ( 1.00 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-clientes_nombre ( 20.46 , 57.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-exencion_impuestos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-exencion_impuestos ).
       RUN set-position IN h_b-exencion_impuestos ( 1.00 , 60.00 ) NO-ERROR.
       RUN set-size IN h_b-exencion_impuestos ( 11.58 , 54.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-excencion_impuestos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-excencion_impuestos ).
       RUN set-position IN h_v-excencion_impuestos ( 13.12 , 60.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.85 , 54.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 18.50 , 60.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.96 , 54.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-exencion_impuestos. */
       RUN add-link IN adm-broker-hdl ( h_b-clientes_nombre , 'Record':U , h_b-exencion_impuestos ).

       /* Links to SmartViewer h_v-excencion_impuestos. */
       RUN add-link IN adm-broker-hdl ( h_b-exencion_impuestos , 'Record':U , h_v-excencion_impuestos ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-excencion_impuestos ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-exencion_impuestos ,
             h_b-clientes_nombre , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-excencion_impuestos ,
             h_b-exencion_impuestos , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-excencion_impuestos , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Mantenimiento de Excenciones Impositivas"}
...
*/
    IF VALID-HANDLE(h_b-clientes_nombre)   THEN RUN set-sensitivo IN h_b-clientes_nombre   ( INPUT p-operacion = "HABILITAR" ).
