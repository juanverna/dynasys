...
{tt2xls.i}
DEFINE VAR archivo AS CHAR FORMAT "x(40)" INITIAL "c:\cli1.xls".
DEFINE VAR flag AS LOGICAL NO-UNDO.
    ...
    DEFINE VAR p-des_punto        AS INTEGER INITIAL 2.  
...
aumento = 18.
    ...
    UPDATE p-fecha LABEL "Creados hasta la fecha indicada" aumento LABEL "Aumento en porcentage" archivo aplica
    p-des_fecha = p-fecha.
    p-has_fecha = 01/01/3000.
    aumento = 1 + aumento / 100.

DEFINE VAR ii AS INT.
    FOR EACH Contrato_hd EXCLUSIVE-LOCK 
...
ii = ii + 1.
        flag = FALSE.
        FOR EACH contrato_dt OF contrato_hd EXCLUSIVE-LOCK:
            FIND articulo OF contrato_dt.
            flag = flag OR LOOKUP(articulo.cdg_articulo,"01,02,03,04,05,06,09,10,35,351,23F") <> 0.
        ...
        /*emails*/
               ...
               IF AVAILABLE domicilio THEN DO:
                   ...
                   IF listado.email = "" THEN
                       FOR EACH Cliente-contacto OF Domicilio WHERE cliente-contacto.cdg_cargo BEGINS "ADM" and  can-do(Cliente-contacto.canal-email,"ADM"), first Persona OF Cliente-contacto WHERE persona.email <> "" NO-LOCK :
                            listado.nombre = persona.nombre.
                            listado.email = persona.email.
                            LEAVE.
                       END.
