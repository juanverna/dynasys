...
DEFINE VARIABLE h_v-par-ivaventas AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Resumen de ventas por condicion impositiva"
         HEIGHT             = 22.14
         WIDTH              = 123
         MAX-HEIGHT         = 22.14
         MAX-WIDTH          = 123
...
ON END-ERROR OF W-Win /* Resumen de ventas por condicion impositiva */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Resumen de ventas por condicion impositiva */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-par-ivaventas.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-par-ivaventas ).
       RUN set-position IN h_v-par-ivaventas ( 8.29 , 14.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-par-ivaventas ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Resumen de ventas por condicion impositiva"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE des_ptovta       AS INTEGER NO-undo.
  DEFINE VARIABLE has_ptovta       AS INTEGER NO-undo.
  DEFINE VARIABLE lista_tipos      AS CHARACTER NO-undo.
  DEFINE VARIABLE lis_fecha        AS LOGICAL NO-undo.
  DEFINE VARIABLE error_rango      AS LOGICAL NO-undo.
  DEFINE VARIABLE hay_error        AS LOGICAL NO-undo.
  DEFINE VAR totcond AS LOGICAL NO-undo.
  ...
  RUN dar_rango IN h_v-par-ivaventas (  OUTPUT  des_ptovta,
                                        OUTPUT  has_ptovta,
                                        OUTPUT  lista_tipos,
                                        OUTPUT  lis_fecha,
                                        OUTPUT totcond,
  ...
  THEN DO:
       ...
       RUN lsresumen_condicion.p ( INPUT des_fecha, 
                                   INPUT has_fecha, 
                                   INPUT des_ptovta,
                                   INPUT has_ptovta,
                                   INPUT lista_tipos,
