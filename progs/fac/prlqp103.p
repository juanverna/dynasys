...
PROCEDURE cabecera_forma:
    ...
    v-formulario [ n-tomo ] = v-formulario [ n-tomo ] + "<=#1><BGCOLOR=WHITE><AT=+77,+5><FROM><AT=+10,+190><RECT>". 
    /*
    v-formulario [ n-tomo ] = v-formulario [ n-tomo ] + "<=#1><BGCOLOR=WHITE><AT=+95,+5><FROM><AT=+7,+190><RECT>". 
    */
    RUN escribir ( "45,45", INPUT "COMITENTE", INPUT 10, INPUT YES, INPUT "BLACK").    
...
PROCEDURE detalle_forma:
    ...
    v-formulario [ n-tomo ] = v-formulario [ n-tomo ] + "<=#1><BGCOLOR=WHITE><AT=+88,+5><FROM><AT=+7,+190><RECT>". 
    v-formulario [ n-tomo ] = v-formulario [ n-tomo ] + "<=#1><BGCOLOR=WHITE><AT=+95,+5><FROM><AT=+7,+190><RECT>". 
    ...
    RUN escribir ( "97,32", INPUT "VENTA BRUTA", INPUT 6, INPUT YES, INPUT "BLACK").
    RUN linea    ( "95,50", INPUT "7", "V" ).
    RUN escribir ( "97,52", INPUT "DEVOLUCIONES", INPUT 6, INPUT YES, INPUT "BLACK").

    RUN linea    ( "95,70", INPUT "7", "V" ).
    RUN escribir ( "97,72", INPUT "VENTA NETA", INPUT 6, INPUT YES, INPUT "BLACK").
    RUN linea    ( "88,90", INPUT "14", "V" ).
    RUN escribir ( "97,92", INPUT "VENTA BRUTA", INPUT 6, INPUT YES, INPUT "BLACK").
    RUN linea    ( "95,110", INPUT "7", "V" ).
    RUN escribir ( "97,112", INPUT "DEVOLUCION", INPUT 6, INPUT YES, INPUT "BLACK").
    RUN linea    ( "95,130", INPUT "7", "V" ).
    RUN escribir ( "97,132", INPUT "BONIFICACION", INPUT 6, INPUT YES, INPUT "BLACK").
    RUN linea    ( "95,150", INPUT "7", "V" ).
    RUN escribir ( "97,152", INPUT "BON.POR PROM.", INPUT 6, INPUT YES, INPUT "BLACK").
    RUN linea    ( "95,170", INPUT "7", "V" ).
    RUN escribir ( "97,173", INPUT "NETO LIQUIDO", INPUT 6, INPUT YES, INPUT "BLACK").

    v-formulario [ n-tomo ] = v-formulario [ n-tomo ] + "<=#1><BGCOLOR=WHITE><AT=+102,+5><FROM><AT=+113,+190><RECT>". 

    RUN linea    ( "215,90", INPUT "7", "V" ).
    RUN linea    ( "215,110", INPUT "7", "V" ).
    RUN linea    ( "215,130", INPUT "7", "V" ).
    RUN linea    ( "215,150", INPUT "7", "V" ).
    RUN linea    ( "215,170", INPUT "7", "V" ).
...
PROCEDURE cabecera_datos:
    ...
    /*
    RUN escribir ( INPUT "67,65", INPUT Proveedor.ing_brutos, INPUT 8, INPUT NO, INPUT "BLUE").
    */
...
PROCEDURE detalle_datos:
    ...
    FOR EACH Fac_detalle_prv OF Fac_header_prv 
             WHERE Fac_detalle_prv.cantidad <> 0 OR Fac_detalle_prv.granel <> 0,
