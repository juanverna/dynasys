...
DEFINE INPUT PARAMETER r-cli AS ROWID no-undo.
...
END.
ELSE MESSAGE "w-geocli.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  FIND cliente WHERE ROWID(cliente) = r-cli NO-LOCK NO-ERROR.
  muestracli(r-cli).
...
PUT UNFORMATTED '<!DOCTYPE html>'
'<html>'
'<meta http-equiv="X-UA-Compatible" content="IE=edge">'
'<body>'
'<div id="mapa" style="width:' + string( ancho - 20 ) + 'px;height:' + string( alto - 2 ) + 'px;position:absolute; top: 0px; left:0px;"></div>' 
'<script>'
'function myMap() ~{'
'var mipos=new google.maps.LatLng(' cliente.geolat ',' cliente.geolong ');'
...
'~}'
'</script>'
'<script src="https://maps.googleapis.com/maps/api/js?key=' + clave + '&callback=myMap"></script>'
'</body>'
'</html>' SKIP(1).
OUTPUT CLOSE.
SESSION:NUMERIC-FORMAT= sistema.
  RETURN archivo.   /* Function return value. */
  /*'<script src="https://maps.googleapis.com/maps/api/js?callback=myMap&key=' + clave + '"></script>'*/
END FUNCTION.
...
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
  FIND cliente WHERE ROWID(cliente) = r-cli NO-LOCK.
  chMundo:WebBrowser:Navigate( genehtml(rowid(cliente)) ).
