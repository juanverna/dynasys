...
&Scoped-Define ENABLED-OBJECTS BUTTON-1 BUTTON-2 RECT-3 
...
DEFINE FRAME F-Main
     ...
     BUTTON-2 AT ROW 1.24 COL 23
     v-ocupado AT ROW 1.24 COL 42 COLON-ALIGNED NO-LABEL
...
ON CHOOSE OF BUTTON-1 IN FRAME F-Main /* Objetivos */
DO:
    v-ocupado:VISIBLE IN FRAME {&FRAME-NAME} = YES.
    RUN ejecutar ( INPUT "listado" ).
...
ON CHOOSE OF BUTTON-2 IN FRAME F-Main /* Cumplimiento */
DO:
    v-ocupado:VISIBLE IN FRAME {&FRAME-NAME} = YES.
    RUN ejecutar ( INPUT "cumplimiento" ).
    v-ocupado:VISIBLE IN FRAME {&FRAME-NAME} = NO.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE ejecutar V-table-Win 
PROCEDURE ejecutar :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
