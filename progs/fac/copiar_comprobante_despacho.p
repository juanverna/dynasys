...
DEFINE INPUT PARAMETE p-rid_remito      AS ROWID.
...
PROCEDURE proceso:
  ...
  FIND Rem_header WHERE ROWID(Rem_header) = p-rid_remito NO-LOCK.
  ...
  BUFFER-COPY Rem_header 
       EXCEPT tip_comprob prf_comprob nro_comprob cdg_imputacion fecha fecha_precios cdg_comprobante
              TO T-Fac_header 
       ASSIGN T-Fac_header.origen               = "R"
              T-Fac_header.estado               = "P"
              T-Fac_header.cta_cte              = Cliente.tiene_ctacte
              T-Fac_header.nro_administrador    = rem_header.nro_administrador
              T-Fac_header.nro_deposito         = Rem_header.nro_deposito
              T-Fac_header.nro_factura          = 0
              T-Fac_header.fecha_precios        = Rem_header.fecha
              T-Fac_header.fecha_iva            = Rem_header.fecha
  ...
  IF CAN-FIND(Tipo_punto 
              WHERE Tipo_punto.cdg_puntovta = Rem_header.prf_comprob 
                AND Tipo_punto.cdg_comprobante = T-Fac_header.cdg_comprobante)
  THEN DO:
      ASSIGN T-Fac_header.prf_comprob = Rem_header.prf_comprob.
  END.
  ...
  ASSIGN T-Fac_header.nro_entidad = Cliente.nro_entidad
   ...
   FOR EACH Rem_detalle OF Rem_header NO-LOCK:
      ...
      IF NOT AVAILABLE T-Fac_detalle 
      THEN DO:
      ...
      FOR EACH Registrable-remito OF Rem_detalle NO-LOCK:

           CREATE T-Registrable-factura.
           BUFFER-COPY Registrable-remito TO T-Registrable-factura
               ASSIGN T-Registrable-factura.nro_factura      = T-Fac_detalle.nro_factura
