...
DEFINE VARIABLE h_v-rng-refacturacion AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 15.52 COL 13 COLON-ALIGNED NO-LABEL
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Proceso de Refacturacion"
         HEIGHT             = 22.67
         WIDTH              = 130.2
         MAX-HEIGHT         = 22.67
         MAX-WIDTH          = 130.2
...
ON END-ERROR OF W-Win /* Proceso de Refacturacion */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Proceso de Refacturacion */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng-refacturacion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-refacturacion ).
       RUN set-position IN h_v-rng-refacturacion ( 4.24 , 15.20 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-impresora ( 10.14 , 15.20 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 17.14 , 15.20 ) NO-ERROR.
       /* Size in UIB:  ( 2.62 , 100.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-refacturacion ,
             v-estado:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-impresora ,
             h_v-rng-refacturacion , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/AFI" "Reemplazo de Facturas"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE v-tip_nuevo        LIKE Fac_header.tip_comprob.
  DEFINE VARIABLE v-impresora        AS CHARACTER.

  DEFINE VARIABLE error_rango      AS LOGICAL.
  DEFINE VARIABLE hay_error        AS LOGICAL.

  RUN dar_rango IN h_v-rng-refacturacion  ( OUTPUT v-tip_comprob,
                                            OUTPUT v-tip_nuevo,
  ...
  THEN DO:
       ...
       RUN reemplazar_facturas.p ( INPUT v-tip_comprob,
                                   INPUT v-tip_nuevo,
                                   INPUT v-prf_comprob,
