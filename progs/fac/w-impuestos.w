...
DEFINE VARIABLE h_b-impuestos AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-impuesto AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-impuesto.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-impuesto ).
       RUN set-position IN h_v-impuesto ( 1.24 , 19.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.71 , 77.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-impuestos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-impuestos ).
       RUN set-position IN h_b-impuestos ( 1.29 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-impuestos ( 20.48 , 15.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 13.14 , 19.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.14 , 77.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-impuesto. */
       RUN add-link IN adm-broker-hdl ( h_b-impuestos , 'Record':U , h_v-impuesto ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-impuesto ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-impuestos ,
             h_v-impuesto , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_b-impuestos , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Mantenimiento de Impuestos"}
