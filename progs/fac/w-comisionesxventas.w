...
DEFINE VARIABLE h_v-par-asiento AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 17.69 COL 12 COLON-ALIGNED NO-LABEL
     "  Parámetros de generación del asiento contable:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 13.12 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de fechas de los movimientos:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 9.35 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Moneda de los movimientos a considerar:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 5.58 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Vendedores a Liquidar:" VIEW-AS TEXT
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Liquidación de comisiones x ventas"
         HEIGHT             = 22.15
         WIDTH              = 114.29
         MAX-HEIGHT         = 22.15
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF W-Win /* Liquidación de comisiones x ventas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Liquidación de comisiones x ventas */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-vendedor ( 2.62 , 14.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-esp-moneda ( 6.65 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */
       ...
       RUN set-position IN h_v-rng-fechas ( 10.42 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-par-asiento.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-par-asiento ).
       RUN set-position IN h_v-par-asiento ( 14.19 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 19.04 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-par-asiento ,
...
------------------------------------------------------------------------------*/
  ...
  RUN habilitar_campos IN h_v-par-asiento ( INPUT NO).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Liquidación de comisiones x ventas"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE gen_asiento        AS LOGICAL.
  ...
  RUN dar_rango IN h_v-par-asiento  ( OUTPUT ver_movim,
                                      OUTPUT ver_resum,
                                      OUTPUT gen_asiento,
  ...
  THEN DO:
       ...
       RUN lscomisionesxventas.p ( INPUT des_vendedor,
                                   INPUT has_vendedor,
                                   INPUT des_fecha,
                                   INPUT has_fecha,
                                   INPUT que_moneda,
