...
DEFINE VARIABLE cont-remitos   AS INTEGER.
...
DO TRANSACTION:
    ...
    FOR EACH Rem_header WHERE Rem_header.nro_factura = Fac_header.nro_factura NO-LOCK:
        cont-remitos = cont-remitos + 1.
        IF cont-remitos = 1 THEN
        v-remitos = fnComprobante(Rem_header.tip_comprob,Rem_header.prf_comprob,Rem_header.nro_comprob).
        ELSE
           v-remitos = "Se adjunta detalle".
    ...
    FOR EACH Fac_detalle OF Fac_header,
        ...
        THEN DO:
            ...
            THEN DO:
                ...
                THEN DO:
                    IF Imputacion.afecta_stock 
                        THEN RUN detalle-numr ( ch_linea  + ",147", INPUT STRING(Fac_detalle.precio_cf,"-ZZZ,ZZ9.9999"), INPUT 8, INPUT NO, INPUT "BLUE").
                    RUN detalle-numr ( ch_linea  + ",166", INPUT STRING(Fac_detalle.subtotal_bruto_cf,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT NO, INPUT "BLUE").
                END.
                ELSE DO:
                ...
                THEN DO:
                    ...
                    THEN DO:
                        ...
                        THEN DO:
                            ...
                            RUN detalle-numr ( ch_linea  + ",145", INPUT STRING(Fac_detalle.precio_cf,"-ZZZ,ZZ9.99"), INPUT 8, INPUT NO, INPUT "BLUE").
                        END.
                        RUN detalle-numr ( ch_linea  + ",166", INPUT STRING(Fac_detalle.subtotal_bruto_cf,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT NO, INPUT "BLUE").
                    END.
                END.
...
PROCEDURE pie_datos:
    ...
    RUN escribir ( "18,16", INPUT STRING(Fac_header.imp_total,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT YES,  INPUT "BLUE").
    RUN escribir ( "18,38", INPUT STRING(0,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT YES,  INPUT "BLUE").
    RUN escribir ( "18,76", INPUT STRING(Fac_header.imp_total,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT YES,  INPUT "BLUE").
    RUN escribir ( "18,113", INPUT STRING(0,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT YES,  INPUT "BLUE").
