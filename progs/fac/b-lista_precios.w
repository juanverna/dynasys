...
&Scoped-define QUERY-STRING-br_table FOR EACH Lista_Precios WHERE ~{&KEY-PHRASE} ~
      AND CAN-DO(Lista_precios.lista_empresas,que_empresa) NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Lista_Precios WHERE ~{&KEY-PHRASE} ~
      AND CAN-DO(Lista_precios.lista_empresas,que_empresa) NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
RUN set-attribute-list (
    'Keys-Accepted = "descripcion",
     Keys-Supplied = "descripcion,cdg_lista"':U).
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 12 BY 21.05
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "CAN-DO(Lista_precios.lista_empresas,que_empresa)"
     _FldNameList[1]   > sic.Lista_Precios.cdg_lista
"Lista_Precios.cdg_lista" "Código!Lista" ">>>>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
DO:
  ...
  &SCOPED-DEFINE SELECCION          SELLISTA.P
  &SCOPED-DEFINE TABLA              Lista_precios
  &SCOPED-DEFINE CDG_TABLA          cdg_lista 
  &SCOPED-DEFINE DSC_TABLA          dsc_lista
...
DO:
  ...
  &SCOPED-DEFINE CODIGO cdg_lista
