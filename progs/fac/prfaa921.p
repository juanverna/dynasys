...
v-filtro = "Fac_header.cdg_empresa = '" + Empresa.cdg_empresa + "' AND " + 
           "Fac_header.tip_comprob = '" + Fac_header.tip_comprob + "' AND " + 
           "Fac_header.prf_comprob = " + STRING(Fac_header.prf_comprob) + " AND " +
           "Fac_header.nro_comprob = " + STRING(Fac_header.nro_comprob).

RUN exreport.p (  INPUT  ".\prl\forms918.prl",            /* Librería desde la que se ejecuta */
                  INPUT "FA-918",                         /* Nombre del reporte a ejecutar    */
