...
DEFINE FRAME F-Main
     des_registro AT ROW 1.24 COL 19 COLON-ALIGNED
     des_nombre AT ROW 1.24 COL 40 COLON-ALIGNED NO-LABEL
     has_registro AT ROW 2.43 COL 19 COLON-ALIGNED
     has_nombre AT ROW 2.43 COL 40 COLON-ALIGNED NO-LABEL
...
ON + OF des_registro IN FRAME F-Main /* Desde Vendedor */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON MOUSE-MENU-DOWN OF des_registro IN FRAME F-Main /* Desde Vendedor */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON RETURN OF des_registro IN FRAME F-Main /* Desde Vendedor */
DO:

    ASSIGN FRAME {&FRAME-NAME} des_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND Vendedor WHERE Vendedor.cdg_vendedor = SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
        ELSE FIND FIRST Vendedor NO-LOCK NO-ERROR.
    
    IF AVAILABLE Vendedor 
    THEN DO:
         des_nombre = Vendedor.nombre.
         des_registro = Vendedor.cdg_vendedor.
    END.
    ELSE DO:
         des_nombre = "???".
    END.
...
ON + OF has_registro IN FRAME F-Main /* Hasta Vendedor */
DO:
  APPLY "MOUSE-MENU-DOWN" TO SELF.
  RETURN NO-APPLY.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL has_registro V-table-Win
ON MOUSE-MENU-DOWN OF has_registro IN FRAME F-Main /* Hasta Vendedor */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL has_registro V-table-Win
ON RETURN OF has_registro IN FRAME F-Main /* Hasta Vendedor */
DO:

    ASSIGN FRAME {&FRAME-NAME} has_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND Vendedor WHERE Vendedor.cdg_vendedor = SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
        ELSE FIND LAST Vendedor NO-LOCK NO-ERROR.
    
    IF AVAILABLE Vendedor 
    THEN DO:
         has_nombre = Vendedor.nombre.
         has_registro = Vendedor.cdg_vendedor.
    END.
    ELSE DO:
         has_nombre = "???".
    END.
