...
DEFINE BUFFER B-Clase_de_vendedor FOR Clase_de_vendedor.

DEFINE VARIABLE f-que_clase LIKE Clase_de_vendedor.cdg_subclasevnd.
...
FORM           
   SKIP(0.2)   
   Clase_de_vendedor.nombre_subclasevnd  COLON 16 FGCOLOR 9 BGCOLOR 15
   SKIP(0.2)   
   Clase_de_vendedor.rotulo_siguiente COLON 16 FGCOLOR 9 BGCOLOR 15
   SKIP(0.2)   
   Clase_de_vendedor.longitud_siguiente COLON 16 FGCOLOR 9 BGCOLOR 15
   Clase_de_vendedor.tipo_siguiente COLON 16 FGCOLOR 9
   SKIP(0.2)   
...
&Scoped-define INTERNAL-TABLES Clase_de_Vendedor

/* Definitions for BROWSE brw_clasificacion                             */
&Scoped-define FIELDS-IN-QUERY-brw_clasificacion ~
SUBSTRING(Clase_de_vendedor.cdg_subclasevnd,LENGTH(que_clase) + 2) ~
Clase_de_Vendedor.nombre_subclasevnd 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brw_clasificacion 
&Scoped-define FIELD-PAIRS-IN-QUERY-brw_clasificacion
&Scoped-define OPEN-QUERY-brw_clasificacion OPEN QUERY brw_clasificacion FOR EACH Clase_de_Vendedor ~
      WHERE Clase_de_Vendedor.cdg_clasevnd = que_clase NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brw_clasificacion Clase_de_Vendedor
&Scoped-define FIRST-TABLE-IN-QUERY-brw_clasificacion Clase_de_Vendedor
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brw_clasificacion C-Win _STRUCTURED
  QUERY brw_clasificacion DISPLAY
      SUBSTRING(Clase_de_vendedor.cdg_subclasevnd,LENGTH(que_clase) + 2) COLUMN-LABEL "Código!Clase" FORMAT "X(8)"
      Clase_de_Vendedor.nombre_subclasevnd COLUMN-LABEL "Denominacion!Clase" FORMAT "X(40)"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Mantenimiento de Estructuras de Ventas"
         HEIGHT             = 22.15
         WIDTH              = 114.29
         MAX-HEIGHT         = 22.19
         MAX-WIDTH          = 114.29
...
/* Query rebuild information for BROWSE brw_clasificacion
     _TblList          = "sic.Clase_de_Vendedor"
     _Where[1]         = "Clase_de_Vendedor.cdg_clasevnd = que_clase"
     _FldNameList[1]   > "_<CALC>"
"SUBSTRING(Clase_de_vendedor.cdg_subclasevnd,LENGTH(que_clase) + 2)" "Código!Clase" "X(8)" ? ? ? ? ? ? ? no ?
     _FldNameList[2]   > sic.Clase_de_Vendedor.nombre_subclasevnd
"Clase_de_Vendedor.nombre_subclasevnd" "Denominacion!Clase" "X(40)" "character" ? ? ? ? ? ? no ?
     _Query            is OPENED
...
ON END-ERROR OF C-Win /* Mantenimiento de Estructuras de Ventas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Mantenimiento de Estructuras de Ventas */
DO:
...
DO:
   ...
   que_subclase = SUBSTRING(Clase_de_vendedor.cdg_subclasevnd,LENGTH(que_clase) + 2).
...
DO:
   ...
   IF CAN-FIND(FIRST B-Clase_de_vendedor WHERE B-Clase_de_vendedor.cdg_clasevnd = Clase_de_vendedor.cdg_subclasevnd)
   THEN DO:
       RUN PONMENSJ.P (INPUT "CLAS001" ).
   END.    
   ELSE DO:                                                 
       DO TRANSACTION:
          FIND CURRENT Clase_de_vendedor EXCLUSIVE-LOCK.   
          DELETE Clase_de_vendedor.
       END.
       RUN ABRE_QUERY.
   END.    
...
DO:
   DO TRANSACTION:
        ...
        DISPLAY Clase_de_vendedor.nombre_subclasevnd
...
DO:
   ...
   THEN DO:
      ...
      ELSE DO:
         ...
         THEN DO:
            p_punto = p_punto - 1.
            que_clase = SUBSTRING(que_clase,1,p_punto).
            FIND FIRST Clase_de_vendedor WHERE Clase_de_vendedor.cdg_subclasevnd = que_clase.
            que_nombre = Clase_de_vendedor.nombre.
            RUN armar_rotulo.
         END.
         ELSE DO:
            que_clase = "".
   ...
   ELSE DO:
      ...
      FIND FIRST Clase_de_vendedor WHERE Clase_de_vendedor.cdg_clasevnd = que_clase 
                         AND Clase_de_vendedor.cdg_subclasevnd = que_clase + "." + que_subclase NO-ERROR.

      IF NOT AVAILABLE Clase_de_vendedor
      THEN DO:
         que_nombre = "".
         RUN crear_clasificacion.         
      END.
      ELSE DO:
         que_nombre = Clase_de_vendedor.nombre_subclasevnd.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  RUN enable_UI.
  {setwintit.i "SIC/FAC" "Mantenimiento de Estructuras de Ventas"}
...
------------------------------------------------------------------------------*/
/*
  IF AVAILABLE Clase_de_vendedor
     THEN rotulo = FILL(" ",l_rotulo - LENGTH(Clase_de_vendedor.rotulo_siguiente) - 1) + Clase_de_vendedor.rotulo_siguiente + ":".           ELSE rotulo = FILL(" ",l_rotulo - 1) + ":".
*/
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE:
         ...
         CREATE Clase_de_vendedor.
         ASSIGN 
                Clase_de_vendedor.cdg_clasevnd = que_clase                                 
                Clase_de_vendedor.cdg_subclasevnd = que_clase + "." + que_subclase.
         RUN MODIFICAR_CLASIFICACION.       
...
------------------------------------------------------------------------------*/
   ...
   UPDATE Clase_de_vendedor.nombre_subclasevnd
          Clase_de_vendedor.rotulo_siguiente
          Clase_de_vendedor.longitud_siguiente
          Clase_de_vendedor.tipo_siguiente
