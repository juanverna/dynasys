...
DEFINE VARIABLE x-impuesto LIKE Fac_header_impuesto.importe COLUMN-LABEL "Importe!Percibido".
...
DEFINE FRAME frm-titulo HEADER
    ...
    "DDJJ " AT 55
    tit_percepcion
    "Página:" AT 133 PAGE-NUMBER FORMAT ">>>9" AT 140
    SKIP
    fecha_lis
    "Período" AT 55
    des_fecha " - " has_fecha
...
FORM
    Cliente.cdg_cliente
    Fac_header.nro_domicilio COLUMN-LABEL "Sucur-!sal"
    Cliente.nom_cliente
    Cliente.ing_brutos
    Cliente.cuit
    que_comprobante
    Fac_header.fecha COLUMN-LABEL "Fecha de!Emisión"
    x-monto
    x-impuesto
...
PROCEDURE LISTAR:
  ...
  FOR EACH Fac_header_impuesto WHERE Fac_header_impuesto.cdg_impuesto = Impuesto.cdg_impuesto NO-LOCK,
      ...
      IF Tipocomprobante.debita
          THEN signo = 1.
          ELSE signo = ( -1 ).
      x-monto   = Fac_header_impuesto.monto_imponible * signo * Fac_header.cambio.
      x-impuesto = Fac_header_impuesto.importe * signo * Fac_header.cambio.

      DISPLAY   Cliente.cdg_cliente WHEN FIRST-OF(Cliente.cdg_cliente)
                Cliente.nom_cliente WHEN FIRST-OF(Cliente.cdg_cliente)
                Cliente.ing_brutos  WHEN FIRST-OF(Cliente.cdg_cliente)
                Cliente.cuit        WHEN FIRST-OF(Cliente.cdg_cliente)
                Fac_header.nro_domicilio
                que_comprobante
                Fac_header.fecha
                x-monto
                x-impuesto
