...
DEFINE FRAME F-Main
     v-estado AT ROW 19.62 COL 11 COLON-ALIGNED NO-LABEL
     "  Rango de Vendedores a listar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 5.76 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Artículos a considerar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 10.19 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Listar movimientos del período:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 15.05 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de Zonas a listar:" VIEW-AS TEXT
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'cxc/v-rng-zona.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-zona ).
       RUN set-position IN h_v-rng-zona ( 2.67 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.86 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'fac/v-rng-vendedor.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-vendedor ).
       RUN set-position IN h_v-rng-vendedor ( 6.95 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.86 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'inv/v-rng-articulo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-articulo ).
       RUN set-position IN h_v-rng-articulo ( 11.29 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.86 , 100.00 ) */
       ...
       RUN set-position IN h_v-rng-fechas ( 16.33 , 13.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 20.95 , 13.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
   ...
   RUN habilitar_verpor IN h_v-rng-articulo ( INPUT NO ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Total de Ventas por Vendedor/Artículo/Cliente"}
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE VARIABLE des_zona     LIKE Zona_geografica.cdg_zonag.
  DEFINE VARIABLE has_zona     LIKE Zona_geografica.cdg_zonag.
  ...
  RUN dar_rango IN h_v-rng-zona ( OUTPUT des_zona, 
                                      OUTPUT has_zona, 
                                      OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
  ...
  THEN DO:
       ...
       RUN lsventasxvendedorarticulozona.p ( INPUT des_zona,
                                             INPUT has_zona,
                                             INPUT des_vendedor, 
                                             INPUT has_vendedor, 
                                             INPUT des_codart, 
                                             ...
                                             INPUT des_fecha,
                                             INPUT has_fecha,
                                             INPUT YES /* Forzamos impresión detallada */).
