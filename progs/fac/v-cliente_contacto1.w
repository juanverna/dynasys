...
Cliente-contacto.Canal-email t-Persona.observacion t-Persona.fecha_grab 
...
&Scoped-Define ENABLED-OBJECTS RECT-2 te-1 vte-1 te-2 vte-2 te-3 vte-3 te-4 ~
vte-4 
...
DEFINE FRAME F-Main
     ...
     Cliente-contacto.cdg_cargo AT ROW 6.48 COL 74 COLON-ALIGNED
          LABEL "Rel"
          VIEW-AS COMBO-BOX INNER-LINES 5
          LIST-ITEM-PAIRS "Item 1","Item 1"
          DROP-DOWN-LIST
     ...
     Cliente-contacto.Canal-email AT ROW 6.95 COL 105 COLON-ALIGNED WIDGET-ID 22
          VIEW-AS FILL-IN NATIVE 
          SIZE 22 BY 1
     t-Persona.observacion AT ROW 8.14 COL 8 COLON-ALIGNED
          LABEL "Obs." FORMAT "X(256)"
          VIEW-AS FILL-IN NATIVE 
          SIZE 118 BY 3.57
          BGCOLOR 15 FGCOLOR 9 
     t-Persona.fecha_grab AT ROW 6.05 COL 109.4 COLON-ALIGNED
          LABEL "Grab."
           VIEW-AS TEXT 
          SIZE 17 BY .62
...
------------------------------------------------------------------------------*/
  ...
  IF NEW cliente-contacto THEN DO:
    MESSAGE "@@@es nuevo" VIEW-AS ALERT-BOX ERROR.
...
IF NEW cliente-contacto THEN DO:
    MESSAGE "@@@es nuevo" VIEW-AS ALERT-BOX ERROR.
     FIND FIRST Persona WHERE Persona.nombre = T-Persona.nombre NO-LOCK NO-ERROR.
      IF AVAILABLE Persona
      THEN DO:
          MESSAGE "@@@al d-persona_ambigua.w" VIEW-AS ALERT-BOX ERROR.
...
END.
ELSE DO:
    MESSAGE "@@@no es nuevo" VIEW-AS ALERT-BOX ERROR.
...
END.
  ...
  ASSIGN Cliente-contacto.nro_cliente   = Domicilio.nro_cliente
         Cliente-contacto.nro_Domicilio = Domicilio.nro_Domicilio
         Cliente-contacto.nro_persona = Persona.nro_persona
         Cliente-contacto.cdg_cargo = cdg_cargo.
