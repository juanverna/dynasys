...
DEFINE VARIABLE h_v-que-cicloventas AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 18.71 COL 23 COLON-ALIGNED NO-LABEL
     "  Rango de vendedores a listar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 2.91 COL 25
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Ciclo de ventas al que se refiere el listado:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 7.95 COL 25
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Empresas a considerar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 12.67 COL 25
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Total facturado x artículo"
         HEIGHT             = 25.95
         WIDTH              = 151.8
         MAX-HEIGHT         = 27.33
         MAX-WIDTH          = 160
...
ON END-ERROR OF W-Win /* Total facturado x artículo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Total facturado x artículo */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-vendedor ( 4.33 , 25.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.86 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-que-cicloventas.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-que-cicloventas ).
       RUN set-position IN h_v-que-cicloventas ( 9.33 , 25.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-lst-empresas ( 13.86 , 25.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 20.10 , 25.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-que-cicloventas ,
             h_v-rng-vendedor , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-lst-empresas ,
             h_v-que-cicloventas , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Cumplimiento de Objetivos de Ventas por Vendedor"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE que_ciclo           LIKE Ciclo_ventas.cdg_cicloventas.
  ...
  RUN dar_rango IN h_v-que-cicloventas    ( OUTPUT que_ciclo, 
                                            OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Ciclo de Ventas" ).
       hay_error = error_rango.
  END.     
  ...
  THEN DO:
       ...
       RUN lscumplimiento_xvendedor.p ( INPUT des_vendedor,
                                        INPUT has_vendedor,
                                        INPUT que_ciclo,
