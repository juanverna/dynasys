...
DEFINE VARIABLE h_v-asignar_abasto AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Asignación de Tasa de Abasto"
         HEIGHT             = 21.23
         WIDTH              = 114.29
         MAX-HEIGHT         = 21.23
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF W-Win /* Asignación de Tasa de Abasto */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Asignación de Tasa de Abasto */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Asignación de Tasa de Abasto */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_b-articulos ( 1.00 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-articulos ( 20.19 , 18.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-asignar_abasto.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-asignar_abasto ).
       RUN set-position IN h_v-asignar_abasto ( 1.00 , 23.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.31 , 79.00 ) */
       ...
       RUN set-position IN h_p-soloedita ( 5.58 , 23.00 ) NO-ERROR.
       RUN set-size IN h_p-soloedita ( 1.77 , 79.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-asignar_abasto. */
       RUN add-link IN adm-broker-hdl ( h_b-articulos , 'Record':U , h_v-asignar_abasto ).
       RUN add-link IN adm-broker-hdl ( h_p-soloedita , 'TableIO':U , h_v-asignar_abasto ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-asignar_abasto ,
             h_b-articulos , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-soloedita ,
             h_v-asignar_abasto , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Asignación de Tasa de Abasto"}
