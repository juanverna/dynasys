...
DEFINE VARIABLE f-cantidad AS DECIMAL EXTENT 4 FORMAT "->>>,>>>,>>9.99".
...
DEFINE VARIABLE f-pesos    AS DECIMAL EXTENT 5 FORMAT "->>>,>>>,>>9.99".
...
DEFINE VARIABLE a-fecha         LIKE T-Liquido_producto.fecha.
...
DEFINE FRAME frm-titulo HEADER
   que_empresa
   "Detalle de Liquido Producto por Proveedor/Fecha" AT 55
   "Página:" AT 148 PAGE-NUMBER FORMAT "9999" AT 158
   SKIP
   fecha_lis
   "Período" AT 55
   des_fecha " - " has_fecha
...
PROCEDURE LISTAR:
    ...
    FOR EACH T-Liquido_producto, Proveedor OF T-Liquido_producto, Articulo OF T-Liquido_producto, Imputacion OF T-Liquido_producto
                   BREAK BY Proveedor.cdg_proveedor BY T-Liquido_producto.fecha BY Articulo.cdg_articulo BY Imputacion.cdg_imputacion:
        ...
        THEN DO:
            ...
            DISPLAY Proveedor.cdg_proveedor WHEN Proveedor.cdg_proveedor <> a-cdg_proveedor
                    T-Liquido_producto.fecha  WHEN T-Liquido_producto.fecha  <> a-fecha
                    Articulo.cdg_articulo   
                    t-cantidad [ 1 ] 
                    t-cantidad [ 2 ] 
                    t-cantidad [ 3 ] 
            ...
            a-fecha = T-Liquido_producto.fecha.

            DO j = 1 TO 5:
               f-pesos [ j ] = f-pesos [ j ] + t-pesos [ j ].
            END.

            DO j = 1 TO 4:
               f-cantidad [ j ] = f-cantidad [ j ] + t-cantidad [ j ].
            END.
        ...
        IF LAST-OF(T-Liquido_producto.fecha)
        THEN DO:
            ...
            DO j = 1 TO 5:
               p-pesos [ j ] = p-pesos [ j ] + f-pesos [ j ].
            END.

            DO j = 1 TO 4:
               p-cantidad [ j ] = p-cantidad [ j ] + f-cantidad [ j ].
            END.
