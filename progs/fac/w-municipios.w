...
DEFINE VARIABLE h_b-municipios AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-tasa_abasto AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-municipios AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-tasa_abasto AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-municipios.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-municipios ).
       RUN set-position IN h_b-municipios ( 1.27 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-municipios ( 20.46 , 14.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-municipios.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-municipios ).
       RUN set-position IN h_v-municipios ( 1.27 , 17.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.88 , 96.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 3.42 , 17.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.42 , 96.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-tasa_abasto.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tasa_abasto ).
       RUN set-position IN h_b-tasa_abasto ( 6.38 , 17.00 ) NO-ERROR.
       RUN set-size IN h_b-tasa_abasto ( 10.23 , 76.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updspa-2 ).
       RUN set-position IN h_p-updspa-2 ( 6.38 , 97.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa-2 ( 15.35 , 16.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-tasa_abasto.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-tasa_abasto ).
       RUN set-position IN h_v-tasa_abasto ( 17.15 , 17.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.58 , 76.00 ) */

       /* Links to SmartViewer h_v-municipios. */
       RUN add-link IN adm-broker-hdl ( h_b-municipios , 'Record':U , h_v-municipios ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-municipios ).

       /* Links to SmartBrowser h_b-tasa_abasto. */
       RUN add-link IN adm-broker-hdl ( h_b-municipios , 'Record':U , h_b-tasa_abasto ).

       /* Links to SmartViewer h_v-tasa_abasto. */
       RUN add-link IN adm-broker-hdl ( h_b-tasa_abasto , 'Record':U , h_v-tasa_abasto ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa-2 , 'TableIO':U , h_v-tasa_abasto ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-municipios ,
             h_b-municipios , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-municipios , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-tasa_abasto ,
             h_p-updspa , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa-2 ,
             h_b-tasa_abasto , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-tasa_abasto ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Mantenimiento de Municipios"}
...
*/
    ...
    END.
*/
    IF VALID-HANDLE(h_b-municipios)  THEN RUN set-sensitivo IN h_b-municipios ( INPUT p-operacion = "HABILITAR" ).
    IF VALID-HANDLE(h_b-tasa_abasto) THEN RUN set-sensitivo IN h_b-tasa_abasto ( INPUT p-operacion = "HABILITAR" ).
