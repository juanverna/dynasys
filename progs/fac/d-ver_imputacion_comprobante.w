...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-10 Dialog-Frame _STRUCTURED
  QUERY BROWSE-10 NO-LOCK DISPLAY
      ...
      Cuenta.nombre_cta COLUMN-LABEL "Nombre!Cuenta" FORMAT "X(45)":U
            WIDTH 39.4
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 124 BY 8.1
...
DEFINE FRAME Dialog-Frame
     BROWSE-10 AT ROW 1.81 COL 6
     RECT-12 AT ROW 1.29 COL 3
     SPACE(1.79) SKIP(0.32)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Imputación contable del comprobante actual".
...
"Cuenta.nombre_cta" "Nombre!Cuenta" "X(45)" "character" ? ? ? ? ? ? no ? no no "39.4" yes no no "U" "" ""
     _FldNameList[3]   = sic.Entidad.cdg_entidad
     _FldNameList[4]   = sic.Obra.cdg_obra
     _FldNameList[5]   > "_<CALC>"
"fn-importe (T-Asn_detalle.debito) @ v-debito" "Importe!Débitos" "X(15)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[6]   > "_<CALC>"
"fn-importe(T-Asn_detalle.credito) @ v-credito" "Importe!Créditos" "X(15)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-10 */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Imputación contable del comprobante actual */
DO:
...
nl = 1.
FIND FIRST T-Sub_header_vta.
FIND Tipocomprobante OF T-Sub_header_vta NO-LOCK.

IF Tipocomprobante.debita
THEN DO:
    CREATE T-Asn_detalle.
    ASSIGN T-Asn_detalle.credito     = 0
           T-Asn_detalle.debito      = T-Sub_header_vta.imp_total
           T-Asn_detalle.nro_cuenta  = T-Sub_header_vta.nro_cuenta
           T-Asn_detalle.nro_entidad = T-Sub_header_vta.nro_entidad
           T-Asn_detalle.nro_obra    = T-Sub_header_vta.nro_obra
...
FOR EACH T-Sub_detalle_vta:
    CREATE T-Asn_detalle.
    ASSIGN T-Asn_detalle.credito     = T-Sub_detalle_vta.valor
           T-Asn_detalle.debito      = 0
           T-Asn_detalle.nro_cuenta  = T-Sub_detalle_vta.nro_cuenta
           T-Asn_detalle.nro_entidad = T-Sub_detalle_vta.nro_entidad
           T-Asn_detalle.nro_obra    = T-Sub_detalle_vta.nro_obra
           T-Asn_detalle.nro_linea   = nl
           nl                        = nl + 1.
    
    IF Tipocomprobante.debita
       THEN ASSIGN T-Asn_detalle.credito     = T-Sub_detalle_vta.valor
                   T-Asn_detalle.debito      = 0.
       ELSE ASSIGN T-Asn_detalle.credito     = 0
                   T-Asn_detalle.debito      = T-Sub_detalle_vta.valor.
...
THEN DO:
    CREATE T-Asn_detalle.
    ASSIGN T-Asn_detalle.credito     = T-Sub_header_vta.imp_total
           T-Asn_detalle.debito      = 0
           T-Asn_detalle.nro_cuenta  = T-Sub_header_vta.nro_cuenta
           T-Asn_detalle.nro_entidad = T-Sub_header_vta.nro_entidad
           T-Asn_detalle.nro_obra    = T-Sub_header_vta.nro_obra
