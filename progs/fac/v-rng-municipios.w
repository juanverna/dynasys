...
ON + OF des_registro IN FRAME F-Main /* Desde Municipio */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON MOUSE-MENU-DOWN OF des_registro IN FRAME F-Main /* Desde Municipio */
DO:
  &SCOPED-DEFINE ROWID_TABLA        rid_desregistro
  &SCOPED-DEFINE SELECCION          selmunicipio.p
  &SCOPED-DEFINE TABLA              Municipio
  &SCOPED-DEFINE CDG_TABLA          cdg_municipio
  &SCOPED-DEFINE DSC_TABLA          dsc_municipio
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON RETURN OF des_registro IN FRAME F-Main /* Desde Municipio */
DO:

    ASSIGN FRAME {&FRAME-NAME} des_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND FIRST Municipio WHERE Municipio.cdg_municipio >= SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME}
                  NO-LOCK NO-ERROR.
        ELSE FIND FIRST Municipio NO-LOCK NO-ERROR.
    
    IF AVAILABLE Municipio 
    THEN DO:
         des_nombre = Municipio.dsc_municipio.
         des_registro = Municipio.cdg_municipio.
    END.
    ELSE DO:
         des_nombre = "???".
    END.
...
ON + OF has_registro IN FRAME F-Main /* Hasta Municipio */
DO:
  APPLY "MOUSE-MENU-DOWN" TO SELF.
  RETURN NO-APPLY.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL has_registro V-table-Win
ON MOUSE-MENU-DOWN OF has_registro IN FRAME F-Main /* Hasta Municipio */
DO:
  &SCOPED-DEFINE ROWID_TABLA        rid_hasMunicipio
  &SCOPED-DEFINE SELECCION          selmunicipio.p
  &SCOPED-DEFINE TABLA              Municipio
  &SCOPED-DEFINE CDG_TABLA          cdg_municipio
  &SCOPED-DEFINE DSC_TABLA          dsc_municipio
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL has_registro V-table-Win
ON RETURN OF has_registro IN FRAME F-Main /* Hasta Municipio */
DO:

    ASSIGN FRAME {&FRAME-NAME} has_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND LAST Municipio WHERE Municipio.cdg_municipio <= SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} 
                                      NO-LOCK NO-ERROR.
        ELSE FIND LAST Municipio NO-LOCK NO-ERROR.
    
    IF AVAILABLE Municipio 
    THEN DO:
         has_nombre = Municipio.dsc_municipio.
         has_registro = Municipio.cdg_municipio.
    END.
    ELSE DO:
         has_nombre = "???".
    END.
