...
DEFINE VARIABLE h_v-rng_ventasocmarginal AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 19.05 COL 11 COLON-ALIGNED NO-LABEL
     "  Valor que deben reflejar los importes:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 15.81 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de Vendedores a listar:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 1.29 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Artículos a considerar:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 5.05 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Listar movimientos del período:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 8.81 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Lista de empresas a considerar:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 12.57 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Total de Ventas por Vendedor/Artículo/Cliente"
         HEIGHT             = 22.14
         WIDTH              = 114.2
         MAX-HEIGHT         = 22.14
         MAX-WIDTH          = 114.2
...
ON END-ERROR OF W-Win /* Total de Ventas por Vendedor/Artículo/Cliente */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Total de Ventas por Vendedor/Artículo/Cliente */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-vendedor ( 2.14 , 7.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-famarticulo ( 6.14 , 13.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 9.86 , 13.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-lst-empresas ( 13.67 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng_ventasocmarginal.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng_ventasocmarginal ).
       RUN set-position IN h_v-rng_ventasocmarginal ( 16.86 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.91 , 89.00 ) */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng_ventasocmarginal ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Ranking de Ventas por Vendedor/Artículo/Cliente"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE des_codart       LIKE Familia_articulo.cdg_familia.
  DEFINE VARIABLE has_codart       LIKE Familia_articulo.cdg_familia.

  DEFINE VARIABLE ver_por          AS INTEGER.
  DEFINE VARIABLE lista_empresas   AS CHARACTER.
  DEFINE VARIABLE modo_importe     AS CHARACTER.
  ...
  RUN dar_rango IN h_v-rng-famarticulo (  OUTPUT  des_codart,
                                          OUTPUT  has_codart,
                                          OUTPUT  error_rango).
  ...
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Lista de Empresas" ).
       hay_error = error_rango.
  END.     

  RUN dar_rango IN h_v-rng_ventasocmarginal (  OUTPUT  modo_importe,
                                               OUTPUT  error_rango).

  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Modos de Importes" ).
       hay_error = error_rango.
  END.     
  ...
  THEN DO:
       ...
       RUN lsrankingxvendedorarticulo.p ( INPUT des_vendedor, 
                                          INPUT has_vendedor, 
                                          INPUT des_codart, 
                                          INPUT has_codart, 
                                          INPUT des_fecha,
