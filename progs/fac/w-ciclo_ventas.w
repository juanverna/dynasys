...
DEFINE VARIABLE h_b-ciclo_ventas AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-ciclo_ventas AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Ciclos de Ventas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Ciclos de Ventas */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Ciclos de Ventas */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-ciclo_ventas.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-ciclo_ventas ).
       RUN set-position IN h_b-ciclo_ventas ( 2.19 , 13.00 ) NO-ERROR.
       RUN set-size IN h_b-ciclo_ventas ( 17.86 , 83.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-ciclo_ventas.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-ciclo_ventas ).
       RUN set-position IN h_v-ciclo_ventas ( 20.29 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.00 , 83.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 25.52 , 13.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 1.76 , 83.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-ciclo_ventas. */
       RUN add-link IN adm-broker-hdl ( h_b-ciclo_ventas , 'Record':U , h_v-ciclo_ventas ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-ciclo_ventas ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-ciclo_ventas ,
             h_b-ciclo_ventas , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-ciclo_ventas , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Definición de Ciclos de Ventas"}
...
*/
 ...
 IF VALID-HANDLE(h_b-ciclo_ventas) THEN RUN set-sensitivo  IN h_b-ciclo_ventas ( INPUT p-operacion = "HABILITAR" ).
