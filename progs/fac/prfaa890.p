...
DEFINE VARIABLE v-leng_leyenda AS INTEGER INITIAL 60. /* Ancho en chars de la leyenda  */
...
DO TRANSACTION:
    ...
    FIND Grupofam WHERE Grupofam.cdg_grupofam = SUBSTRING(Cliente.cdg_cliente,2)
                    AND Grupofam.cdg_empresa  = SUBSTRING(Cliente.cdg_cliente,1,1)
                        NO-ERROR.
                        
    IF AVAILABLE Grupofam
    THEN DO:
        IF Grupofam.cdg_plan <> ""
           THEN FIND Plan OF Grupofam NO-LOCK.
...
PROCEDURE forma:
    ...
    */
    ...
    RUN escribir ( INPUT "54,142", INPUT que_dia, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "54,153", INPUT nom_mes [ MONTH(Fac_header.fecha) ], INPUT 8, INPUT NO).
    RUN escribir ( INPUT "54,182", INPUT que_ano, INPUT 8, INPUT NO).
    
    RUN escribir ( INPUT "94,35", INPUT Cliente.nom_cliente, INPUT 8, INPUT YES).
    RUN escribir ( INPUT "94,77", INPUT "[" + Cliente.cdg_cliente + "]", INPUT 8, INPUT NO).

    RUN escribir ( INPUT "94,110", INPUT Domicilio.direccion, INPUT 8, INPUT YES).
    RUN escribir ( INPUT "102,110", INPUT "(" + Domicilio.cdg_postal + ") " +  
                 Domicilio.localidad, INPUT 8, INPUT YES).
    RUN escribir ( INPUT "102,35", INPUT Cliente.cuit, INPUT 8, INPUT YES).

    /*
    IF AVAILABLE Grupo-domicilio
    THEN DO:
         RUN escribir ( INPUT "48,70", INPUT "entre " + Grupo-domicilio.entre1 + " y " + Grupo-domicilio.entre2, INPUT 8, INPUT YES).
         RUN escribir ( INPUT "60,133", INPUT "T.E.: " + Grupo-domicilio.prefijo + "-" + Grupo-domicilio.telefono, INPUT 8, INPUT YES).
    END.
    ...
    */
        ...
        DO j = 1 TO NUM-ENTRIES(v-detallada, "|"):
           ...
           RUN escribir ( INPUT TRIM(STRING(linea0,">>9")) + ",30", INPUT ENTRY(j,v-detallada, "|"), INPUT 8, INPUT NO).
            IF j = NUM-ENTRIES(v-detallada, "|")
               THEN RUN escribenumero ( INPUT TRIM(STRING(linea0,">>9")) + ",175", INPUT STRING(Fac_detalle.subtotal_neto,"-ZZZ,ZZ9.99"), INPUT 8, INPUT NO).
    ...
    THEN DO:    
        ...
        DO j = 1 TO v-reng_leyenda:
            IF j <= NUM-ENTRIES(v-leyenda, "|")
               THEN RUN escribir ( INPUT TRIM(STRING(linea0,">>9")) + ",17", INPUT ENTRY(j,v-leyenda, "|"), INPUT 8, INPUT NO).
            linea0 = linea0 + 3.
        END.
    ...
    RUN escribenumero ( INPUT "222,175", INPUT STRING(Fac_header.imp_neto,"-ZZZ,ZZ9.99"), INPUT 8, INPUT NO).
      
    RUN escribenumero ( INPUT "256,173", INPUT STRING(Fac_header.imp_total,"-ZZZ,ZZ9.99"), INPUT 10, INPUT YES).

    RUN escribenumero ( INPUT "239,128", INPUT STRING(prciva,"ZZ9.99"), INPUT 8, INPUT NO).
    RUN escribenumero ( INPUT "239,175", INPUT STRING(Fac_header.imp_iva,"-ZZZ,ZZ9.99"), INPUT 8, INPUT NO).

    RUN escribenumero ( INPUT "247,128", INPUT STRING(prcnoi,"ZZ9.99"), INPUT 8, INPUT NO).
    RUN escribenumero ( INPUT "247,175", INPUT STRING(importe_noi,"-ZZZ,ZZ9.99"), INPUT 8, INPUT NO).

    RUN escribir ( INPUT "271,67", INPUT Fac_header.tip_comprob + " " + STRING(Fac_header.prf_comprob,"9999") + " " + STRING(Fac_header.nro_comprob,"99999999"), INPUT 8, INPUT NO).
...
PROCEDURE escribir:
    ...
    linea = '<FGCOLOR=' + textColor + '><AT=' + 
            TRIM(STRING(( ncopia - 1 ) * 177 + i-linea,">>9")) + ',' + 
...
PROCEDURE escribenumero:
    ...
    linea = '<FGCOLOR=' + textColor + '><AT=' + 
            TRIM(STRING(( ncopia - 1 ) * 177 + i-linea,">>9")) + ',' + 
            TRIM(STRING(i-columna,">>9")) + '><Flineprinter><P' + puntos + '>'.
