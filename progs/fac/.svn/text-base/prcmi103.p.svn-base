...
PROCEDURE cabecera_forma:
    ...
    RUN grabar_linea ( "<=#1><BGCOLOR=WHITE><AT=+102,+5><FROM><AT=+127,+190><RECT>"). 
    ...
    RUN escribir ( "97,108", INPUT "ARTICULO", INPUT 8, INPUT YES, INPUT "BLACK").
    ...
    */
    ...
    RUN grabar_linea ("<=#1><BGCOLOR=WHITE><AT=+05,+75><FROM><AT=+10,+50><RECT>"). 
    RUN linea    ( "15,100", INPUT "35", "V" ).

    RUN escribir ( INPUT "7,82", INPUT Tipocomprobante.denominacion_impresa, INPUT 12, INPUT YES, INPUT "BLACK"). 
    RUN escribir ( INPUT "6,134", INPUT "NRO.: ", INPUT 12, INPUT YES, INPUT "BLACK"). 
    ...
    RUN escribir ( INPUT "21,107", INPUT "DOCUMENTO NO VALIDO COMO FACTURA", INPUT 12, INPUT YES, INPUT "BLACK").
...
PROCEDURE cabecera_datos:
    ...
    RUN escribir ( INPUT "6,157", INPUT Fac_header.tip_comprob + "-" + STRING(Fac_header.prf_comprob,"9999") + "-" + STRING(Fac_header.nro_comprob,"99999999"), INPUT 12, INPUT YES, INPUT "BLACK"). 
...
PROCEDURE detalle_datos:
    ...
    FOR EACH Fac_detalle OF Fac_header,
        ...
        THEN DO:
            ...
            RUN detalle-char ( ch_linea  + ",93",  INPUT Atributo.abreviatura, INPUT 8, INPUT NO, INPUT "BLUE").
            ...
            THEN DO:
                ...
                THEN DO:
                    IF Imputacion.afecta_stock 
                        THEN RUN detalle-numr ( ch_linea  + ",147", INPUT STRING(Fac_detalle.precio,"-ZZZ,ZZ9.9999"), INPUT 8, INPUT NO, INPUT "BLUE").
...
PROCEDURE pie_forma:
    ...
    RUN grabar_linea ( "<=#1><BGCOLOR=WHITE><AT=+9,+5><FROM><AT=+14,+190><RECT>" ). 
...
PROCEDURE pie_datos:
    ...
    DO j = 1 TO v-reng_monto:
        IF j <= NUM-ENTRIES(v-monto_letras, "|")
           THEN RUN escribir ( INPUT TRIM(STRING(linea0,">>9")) + ",10", INPUT "SON PESOS " + ENTRY(j,v-monto_letras, "|"), INPUT 8, INPUT NO, INPUT "BLACK").
        linea0 = linea0 + 4.
    END.

    RUN escribir ( "17,170", INPUT STRING(Fac_header.imp_total,"-ZZ,ZZZ,ZZ9.99"), INPUT 10, INPUT YES, INPUT "BLACK").
