...
DO TRANSACTION:
    ...
    PUT "NCLI," + "[" + Cliente.cdg_cliente + "]" FORMAT "X(13)" SKIP.
        ...
        DO j = 1 TO NUM-ENTRIES(v-detallada, "|"):
            ...
            IF j = NUM-ENTRIES(v-detallada, "|")
               THEN ltexto = nomcampo( INPUT "I", INPUT linea0 ) + "," +
                        STRING(Fac_detalle.subtotal_neto_cf,"->>>,>>9.99").
    ...
    OS-COMMAND {&MUDO}  "..\prl\proform ..\prl\dab918.prn < ..\prl\dab918.txt > ..\prl\dab918.lst".
    OS-COMMAND {&MUDO}  "copy ..\prl\dab918.lst prn /b".
