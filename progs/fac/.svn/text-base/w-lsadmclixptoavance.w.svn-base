...
{nrorelea.i} 
    ...
    DEFINE TEMP-TABLE ff
       ...
       FIELD pperiodo   AS INT FORMAT "999999".
 ...
 FOR EACH fac_header WHERE NOT anulado AND
    ...
    FOR EACH fac_detalle OF fac_header:
        ...
        ASSIGN  ff.imp_total    = Fac_detalle.subtotal_gral
                ...
                ff.pperiodo = year(fac_header.fecha) * 100 + MONTH(fac_header.FECHA).            
    END.
 END.
...
/*VentasxadmptovtaTBL.rpt*/ 
       ...
       RUN fullPath ("VentasxadmptovtaTBL2", '.rpt':U, OUTPUT cFullPath).
       IF cFullPath = ? 
       THEN DO:
           RUN mensajepar.p (INPUT "VentasxadmptovtaTBL2.rpt", INPUT "CREP000").
           RETURN ERROR.
       END.
