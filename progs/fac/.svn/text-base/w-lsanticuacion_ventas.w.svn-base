...
DEFINE VARIABLE h_v-par-anticventas AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-vendedor ( 2.57 , 30.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-esp-moneda ( 6.71 , 30.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-par-anticventas.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-par-anticventas ).
       RUN set-position IN h_v-par-anticventas ( 10.76 , 30.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 14.95 , 30.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 20.33 , 30.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-par-anticventas ,
             h_v-esp-moneda , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-fechas ,
             h_v-par-anticventas , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Ventas por vendedor y por período"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE dias_columna            AS INTEGER INITIAL 3.

  DEFINE VARIABLE todos_mov               AS LOGICAL.
  DEFINE VARIABLE consolidado             AS LOGICAL.
  DEFINE VARIABLE inhabilitado            AS LOGICAL.
...
MESSAGE "entro" VIEW-AS ALERT-BOX.
  RUN dar_rango IN h_v-par-anticventas (  OUTPUT ncol_vencidas,
                                          OUTPUT dias_columna,
                                          OUTPUT consolidado,
                                          OUTPUT error_rango,
                                          OUTPUT inhabilitado).
  ...
  THEN DO:
       ...
       RUN lsanticuacion_ventas.p ( INPUT  des_vendedor,
                                    INPUT  has_vendedor,
                                    INPUT  que_moneda,
                                    INPUT  ncol_vencidas,
                                    INPUT  dias_columna,
