...
DEFINE VARIABLE h_v-par-asncomprobantes AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Asientos de Ventas por Comprobante"
         HEIGHT             = 22.35
         WIDTH              = 114.29
         MAX-HEIGHT         = 22.35
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF W-Win /* Asientos de Ventas por Comprobante */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Asientos de Ventas por Comprobante */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-fechas ( 3.96 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-par-asncomprobantes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-par-asncomprobantes ).
       RUN set-position IN h_v-par-asncomprobantes ( 8.27 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.77 , 89.00 ) */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-par-asncomprobantes ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Asientos de Ventas por Tipo de Comprobante"}
...
------------------------------------------------------------------------------*/
  ...
  RUN dar_rango IN h_v-par-asncomprobantes (  OUTPUT  des_ptovta,
                                              OUTPUT  has_ptovta,
                                              OUTPUT  lista_tipos,
                                              OUTPUT  lis_fecha,
                                              OUTPUT  gen_asiento,
  ...
  THEN DO:
       ...
       RUN lsasientos_comprobantes.p ( INPUT des_fecha, 
                                       INPUT has_fecha, 
                                       INPUT des_ptovta,
                                       INPUT has_ptovta,
                                       INPUT lista_tipos,
