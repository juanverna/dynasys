...
DO TRANSACTION:
    ...
    OUTPUT STREAM Formulario TO "c:\sic-temp\prfaa114.xpr" CONVERT TARGET "iso8859-1".
 ...
 /* PUT STREAM Formulario CONTROL '<TOOLBAR=!PRINT>'. */
    ...
    FILE-INFO:File-NAME = "c:\sic-temp\prfaa114.xpr".
...
PROCEDURE forma:
    ...
    PUT STREAM Formulario UNFORMATTED "<=#1><BGCOLOR=WHITE><AT=+57,+5><FROM><AT=+180,+190><FILLRECT)>". 
    PUT STREAM Formulario UNFORMATTED "<=#1><BGCOLOR=WHITE><AT=+237,+5><FROM><AT=+25,+190><FILLRECT)>". 


    RUN escribir ( INPUT "6,115", INPUT "FACTURA", INPUT 12, INPUT YES). 
...
*/    
    RUN escribir ( INPUT "31,10", INPUT "RAZON SOCIAL:", INPUT 8, INPUT YES). 
    RUN escribir ( INPUT "31,32", INPUT Cliente.nom_cliente, INPUT 8, INPUT YES).
    RUN escribir ( INPUT "31,110", INPUT "CLIENTE NRO.:", INPUT 8, INPUT YES). 
    RUN escribir ( INPUT "31,129", INPUT "[" + Cliente.cdg_cliente + "]", INPUT 8, INPUT NO).
    ...
    FOR EACH Fac_detalle OF Fac_header 
        ...
        v-subtotal = v-subtotal + Fac_detalle.subtotal_bruto.
    ...
    RUN escribir      ( INPUT "230,100", INPUT "Subtotal", INPUT 10, INPUT NO).
    RUN escribenumero ( INPUT "230,162", INPUT STRING(Fac_header.imp_bruto,"-ZZZ,ZZ9.99"), INPUT 10, INPUT NO).
    ...
    RUN escribir      ( INPUT "238,100", INPUT "Neto", INPUT 10, INPUT NO).
    RUN escribenumero ( INPUT "238,162", INPUT STRING(Fac_header.imp_neto,"-ZZZ,ZZ9.99"), INPUT 10, INPUT NO).
      
    RUN escribir      ( INPUT "242,100", INPUT "IVA % " + STRING(prciva,">9.99"), INPUT 10, INPUT NO).
    RUN escribenumero ( INPUT "242,162", INPUT STRING(Fac_header.imp_iva,"-ZZZ,ZZ9.99"), INPUT 10, INPUT NO).

    RUN escribir      ( INPUT "246,100", INPUT "Alicuota NO Inscripto % " + STRING(prcnoi,">9.99"), INPUT 10, INPUT NO).
    RUN escribenumero ( INPUT "246,162", INPUT STRING(importe_noi,"-ZZZ,ZZ9.99"), INPUT 10, INPUT NO).
