...
&Scoped-define QUERY-STRING-br_table FOR EACH Cliente WHERE ~{&KEY-PHRASE} ~
      AND CAN-DO(Cliente.lista_empresas,que_empresa) ~
AND LOOKUP(que_sector, Cliente.lista_sectores) <> 0 NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Cliente WHERE ~{&KEY-PHRASE} ~
      AND CAN-DO(Cliente.lista_empresas,que_empresa) ~
AND LOOKUP(que_sector, Cliente.lista_sectores) <> 0 NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Cliente.cdg_cliente FORMAT "X(10)":U WIDTH 12.2
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 17 BY 24.29
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "CAN-DO(Cliente.lista_empresas,que_empresa)
AND LOOKUP(que_sector, Cliente.lista_sectores) <> 0"
     _FldNameList[1]   > sic.Cliente.cdg_cliente
"Cliente.cdg_cliente" ? "X(10)" "character" ? ? ? ? ? ? no ? no no "12.2" yes no no "U" "" ""
...
DO:
  ...
  &SCOPED-DEFINE CODIGO  cdg_cliente
  &SCOPED-DEFINE CODIGO2 lista_empresas
  ...
  THEN DO:
    ...
    FIND FIRST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} 
         WHERE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.{&CODIGO} >= v-codigo
           AND CAN-DO({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.{&CODIGO2}, que_empresa) 
                NO-LOCK NO-ERROR.
    IF NOT AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} 
       THEN FIND LAST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}
                WHERE CAN-DO({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.{&CODIGO2}, que_empresa)
                       NO-LOCK.
