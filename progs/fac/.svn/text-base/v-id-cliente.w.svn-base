...
DEFINE VARIABLE que_sector AS CHARACTER.
...
DEFINE FRAME F-Main
     v-cdg_cliente AT ROW 1.24 COL 10 COLON-ALIGNED
     v-nom_cliente AT ROW 1.24 COL 30 NO-LABEL
...
DO:
  ...
  THEN DO:
       v-nom_cliente:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
       RUN PONMENSJ.P ( '1036' ).
       RETURN NO-APPLY.
  END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE aplica-return V-table-Win 
PROCEDURE aplica-return :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
------------------------------------------------------------------------------*/
  ...
  IF NOT AVAILABLE Cliente
  THEN DO:
       FIND FIRST Cliente WHERE CAN-DO(Cliente.lista_empresas, que_empresa) NO-LOCK.
       v-cdg_cliente = Cliente.cdg_cliente.
       v-nom_cliente = Cliente.nom_cliente.
       DISPLAY v-cdg_cliente
               v-nom_cliente
