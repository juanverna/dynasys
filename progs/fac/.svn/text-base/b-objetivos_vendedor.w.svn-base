...
&Scoped-define FIELDS-IN-QUERY-br_table Vendedor_objetivo.cdg_subclaseart ~
Clase_de_Articulo.nombre_subclaseart Vendedor_objetivo.cantidad ~
Vendedor_objetivo.granel Vendedor_objetivo.subtotal 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH Vendedor_objetivo OF Vendedor WHERE ~{&KEY-PHRASE} ~
      AND Vendedor_objetivo.cdg_empresa = que_empresa ~
 AND Vendedor_objetivo.nro_cicloventas = Ciclo_ventas.nro_cicloventas  NO-LOCK, ~
      FIRST Clase_de_Articulo WHERE TRUE /* Join to Vendedor_objetivo incomplete */ ~
...
RUN set-attribute-list (
    'Keys-Accepted = ,
     Keys-Supplied = "cdg_empresa,nro_moneda,nro_vendedor"':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Vendedor_objetivo.cdg_subclaseart FORMAT "X(25)":U
      Clase_de_Articulo.nombre_subclaseart COLUMN-LABEL "Denominacion!Subclase" FORMAT "X(40)":U
            WIDTH 47.4
      Vendedor_objetivo.cantidad FORMAT "->>>,>>>,>>9.99":U
      Vendedor_objetivo.granel FORMAT "->>>>>9.99":U
      Vendedor_objetivo.subtotal FORMAT "->,>>>,>>9.99":U WIDTH 16.4
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 123 BY 6.71
         TITLE "Objetivos del Actual Vendedor".
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "Vendedor_objetivo.cdg_empresa = que_empresa
 AND Vendedor_objetivo.nro_cicloventas = Ciclo_ventas.nro_cicloventas "
     ...
     _FldNameList[1]   = sic.Vendedor_objetivo.cdg_subclaseart
     _FldNameList[2]   > sic.Clase_de_Articulo.nombre_subclaseart
"Clase_de_Articulo.nombre_subclaseart" "Denominacion!Subclase" ? "character" ? ? ? ? ? ? no ? no no "47.4" yes no no "U" "" ""
     _FldNameList[3]   = sic.Vendedor_objetivo.cantidad
     _FldNameList[4]   = sic.Vendedor_objetivo.granel
     _FldNameList[5]   > sic.Vendedor_objetivo.subtotal
"Vendedor_objetivo.subtotal" ? ? "decimal" ? ? ? ? ? ? no ? no no "16.4" yes no no "U" "" ""
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* Objetivos del Actual Vendedor */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* Objetivos del Actual Vendedor */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* Objetivos del Actual Vendedor */
DO:
