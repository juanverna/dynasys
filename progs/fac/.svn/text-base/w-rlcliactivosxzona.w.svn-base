...
DEFINE VARIABLE h_v-que-fecha-2 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 17.42 COL 12 COLON-ALIGNED NO-LABEL
     "  Rango de Zonas a Considerar:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 7.19 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de Vendedores a Considerar:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 2.35 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Fecha de referencia del listado:" VIEW-AS TEXT
          SIZE 42 BY .81 AT ROW 11.77 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Fecha del último movimiento:" VIEW-AS TEXT
          SIZE 42 BY .81 AT ROW 11.77 COL 61
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Clientes Activos x Vendedor/Zona"
         HEIGHT             = 22.15
         WIDTH              = 114.29
         MAX-HEIGHT         = 22.15
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF W-Win /* Clientes Activos x Vendedor/Zona */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Clientes Activos x Vendedor/Zona */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-vendedor ( 3.69 , 14.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-zona ).
       RUN set-position IN h_v-rng-zona ( 8.27 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */
       ...
       RUN set-position IN h_v-que-fecha ( 13.12 , 14.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-que-fecha-2 ).
       RUN set-position IN h_v-que-fecha-2 ( 13.12 , 61.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 42.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 18.77 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-que-fecha-2 ,
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute PRIOR to standard behavior. */
   ...
   RUN fijar_fecha IN h_v-que-fecha   ( INPUT TODAY).
   RUN fijar_fecha IN h_v-que-fecha-2 ( INPUT TODAY - 90).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Clientes Activos x Vendedor/Zona"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE a-que-fecha         AS DATE.
  DEFINE VARIABLE fecha-ultimo        AS DATE.
  ...
  RUN dar_rango IN h_v-que-fecha ( OUTPUT a-que-fecha, 
                                   OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Fecha de Referencia" ).
       hay_error = error_rango.
  END.     

  RUN dar_rango IN h_v-que-fecha-2 ( OUTPUT fecha-ultimo, 
                                     OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Fecha Ultimo Movimiento" ).
       hay_error = error_rango.
  END.     
  ...
  THEN DO:
       ...
       RUN lscliactivosxzona.p (INPUT des_vendedor,
                                INPUT has_vendedor,
                                INPUT des_zonag,
                                INPUT has_zonag,
                                INPUT a-que-fecha,
