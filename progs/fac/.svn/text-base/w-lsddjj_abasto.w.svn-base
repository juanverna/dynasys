...
DEFINE VARIABLE h_v-rng-municipios AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     "  Municipios a considerar:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 1.81 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Listar movimientos del período:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 5.85 COL 13
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Declaracion Jurada de Abasto Municipal"
         HEIGHT             = 22.15
         WIDTH              = 114.29
         MAX-HEIGHT         = 27.35
         MAX-WIDTH          = 146.29
         VIRTUAL-HEIGHT     = 27.35
...
ON END-ERROR OF W-Win /* Declaracion Jurada de Abasto Municipal */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Declaracion Jurada de Abasto Municipal */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng-municipios.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-municipios ).
       RUN set-position IN h_v-rng-municipios ( 3.15 , 13.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 6.92 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Declaración Jurada de Abasto Municipal"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE des_codmun       LIKE Municipio.cdg_municipio.
  DEFINE VARIABLE has_codmun       LIKE Municipio.cdg_municipio.
  ...
  RUN dar_rango IN h_v-rng-municipios (  OUTPUT  des_codmun,
                                         OUTPUT  has_codmun,
                                         OUTPUT  error_rango).
      
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Municipios" ).
       hay_error = error_rango.
  END.     
  ...
  THEN DO:
       ...
       RUN lsddjj_abasto.p ( INPUT des_codmun, 
                             INPUT has_codmun, 
                             INPUT des_fecha,
                             INPUT has_fecha,
