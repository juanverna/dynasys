...
DEFINE VARIABLE vnro_contrato AS INTEGER FORMAT "->,>>>,>>9":U INITIAL 0 
     LABEL "Contrato" 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1 NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     Fac_header.imp_total AT ROW 7.67 COL 15 COLON-ALIGNED WIDGET-ID 16
          VIEW-AS FILL-IN 
          SIZE 25.8 BY 1
     vnro_contrato AT ROW 9.1 COL 15 COLON-ALIGNED WIDGET-ID 8
     basig AT ROW 9.1 COL 32 WIDGET-ID 10
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Asignacion de facturas a contratos"
         HEIGHT             = 9.76
         WIDTH              = 84.2
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 84.2
...
ON END-ERROR OF W-Win /* Asignacion de facturas a contratos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Asignacion de facturas a contratos */
DO:
...
DO:
  ASSIGN vtip_comprob vnro_comprob vprf_comprob vnro_contrato.
  ...
  FIND contrato_hd NO-LOCK WHERE contrato_hd.nro_contrato = vnro_contrato.

  IF NOT AVAILABLE fac_header THEN DO:
      STATUS INPUT "Verifique los datos".
      RETURN NO-APPLY.
  END.
  fac_header.nro_contrato = vnro_contrato.
...
DO:
  ...
  DISPLAY fac_header.codigo_cliente
      ...
      fac_header.nombre
      fac_header.imp_total 
      ( IF fac_header.nro_contrato = ? THEN 0 ELSE fac_header.nro_contrato ) @ vnro_contrato WITH FRAME {&FRAME-NAME}.
END.
...
&Scoped-define SELF-NAME vnro_contrato
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL vnro_contrato W-Win
ON LEAVE OF vnro_contrato IN FRAME F-Main /* Contrato */
DO:
  ASSIGN vnro_contrato.
  FIND contrato_hd WHERE contrato_hd.nro_contrato = vnro_contrato AND 
                         contrato_hd.nro_cliente = fac_header.nro_cliente NO-LOCK NO-ERROR.
  IF NOT AVAILABLE contrato_hd AND vnro_contrato <> 0 THEN DO:
      STATUS INPUT "El contrato no existe o no es de ese cliente".
      RETURN NO-APPLY.
  END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL vnro_contrato W-Win
ON MOUSE-SELECT-DBLCLICK OF vnro_contrato IN FRAME F-Main /* Contrato */
OR MOUSE-MENU-DOWN OF vnro_contrato IN FRAME {&FRAME-NAME}
DO :
  DEFINE VARIABLE rid_contrato AS ROWID.
  FIND contrato_hd WHERE contrato_hd.nro_contrato = vnro_contrato:INPUT-VALUE IN FRAME {&FRAME-NAME} NO-ERROR.
  ...
  THEN DO:
      ...
      DISPLAY contrato_hd.nro_contrato @ vnro_contrato
...
------------------------------------------------------------------------------*/
  DISPLAY vtip_comprob vprf_comprob vnro_comprob vnro_contrato 
  ...
  ENABLE vtip_comprob vprf_comprob vnro_comprob Fac_header.codigo_cliente 
         Fac_header.nombre Fac_header.direccion Fac_header.nom_Administrador 
         Fac_header.imp_total vnro_contrato basig 
      WITH FRAME F-Main IN WINDOW W-Win.
