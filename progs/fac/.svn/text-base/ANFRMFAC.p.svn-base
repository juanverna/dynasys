
/*=================================================================================*/
/*                           VARIABLES, FRAMES, Y SUBMENUES                        */
/*=================================================================================*/

/**/
{VRSHARED.I}
{VPERSINM.I}
/**/

DEFINE VARIABLE que_empresa     LIKE Empresa.nombre.
DEFINE VARIABLE que_parametro   LIKE Parametro.cdg_parametro.
DEFINE VARIABLE credebfc        LIKE Parametro.valor_l INITIAL NO.
DEFINE VARIABLE tipos_validos   AS CHARACTER LABEL "Tipos de formulario".
DEFINE VARIABLE tip_forma       AS CHARACTER LABEL "Tipo de formulario".
DEFINE VARIABLE mensaje         AS CHARACTER FORMAT "X(40)".
DEFINE VARIABLE des_forma       AS INTEGER LABEL "Desde formulario".
DEFINE VARIABLE has_forma       AS INTEGER LABEL "Hasta formulario".

DEFINE VARIABLE cantidad_formas AS INTEGER LABEL "Cantidad de formularios" INITIAL 1.
DEFINE VARIABLE j               AS INTEGER.
DEFINE VARIABLE fecha_anulacion AS DATE LABEL "Fecha de Anulación".


DEFINE BUTTON btn_proceso
     LABEL "&Procesar":L
     SIZE 15 BY 1 FONT 6.

DEFINE BUTTON btn_salir
     LABEL "&Salir":L
     SIZE 15 BY 1 FONT 6.

FORM
     mensaje NO-LABEL
     WITH FRAME frm-espere OVERLAY
          TITLE FGCOLOR 14 BGCOLOR 4 "Aguarde un momento por favor"
          CENTERED ROW 7 FGCOLOR 14 BGCOLOR 4.

FORM
   pto_venta AT ROW 3.00 COL 30 COLON-ALIGNED
      LABEL "Punto de venta"
      FORMAT "9999"
      VIEW-AS FILL-IN 
      SIZE 12 BY 1
      BGCOLOR 15 FGCOLOR 9 
   
   tip_forma AT ROW 4.50 COL 30 COLON-ALIGNED
      VIEW-AS FILL-IN 
      SIZE 15 BY 1
      BGCOLOR 15 FGCOLOR 9 

   des_forma  AT ROW 6.00 COL 30 COLON-ALIGNED
      VIEW-AS FILL-IN 
      SIZE 15 BY 1
      BGCOLOR 15 FGCOLOR 9 

   has_forma   AT ROW 7.50 COL 30 COLON-ALIGNED
      VIEW-AS FILL-IN 
      SIZE 12 BY 1
      BGCOLOR 15 FGCOLOR 9 

   fecha_anulacion   AT ROW 9.00 COL 30 COLON-ALIGNED
      VIEW-AS FILL-IN 
      SIZE 12 BY 1
      BGCOLOR 15 FGCOLOR 9 
   SKIP(1)
   SPACE(5) BTN_PROCESO  SPACE(20) BTN_SALIR SPACE(5) SKIP(1)
   WITH FRAME frm-rango SIDE-LABELS CENTERED ROW 3 FGCOLOR 0 /*f-fg_c*/ BGCOLOR 8 /*f-bg_c*/
        VIEW-AS DIALOG-BOX THREE-D TITLE "Anulacion de formularios de facturas" FONT 6.

/*=================================================================================*/
/*                      T R I G G E R S    D E    U S U A R I O                    */
/*=================================================================================*/

ON RETURN OF tip_forma IN FRAME frm-rango
DO:

  ASSIGN tip_forma.
  tip_forma = CAPS(tip_forma).
  DISPLAY tip_forma WITH FRAME frm-rango.
  IF credebfc
     THEN tipos_validos = "FA,FB,FC".
     ELSE tipos_validos = "FA,FB,FC,CA,CB,CC,DA,DB,DC".

  IF LOOKUP(tip_forma,tipos_validos) = 0
  THEN DO:
     RUN PONMENSJ.P (INPUT "ANUL000").
     RETURN NO-APPLY.
  END.
  ELSE DO:
     CASE tip_forma:
          WHEN "FA" THEN que_parametro = "PFAA".
          WHEN "FB" THEN que_parametro = "PFAB".
          WHEN "FC" THEN que_parametro = "PFAC".
          WHEN "FE" THEN que_parametro = "PFAE".
          WHEN "CA" THEN que_parametro = "PCRA".
          WHEN "CB" THEN que_parametro = "PCRB".
          WHEN "CC" THEN que_parametro = "PCRC".
          WHEN "CE" THEN que_parametro = "PCRE".
          WHEN "DA" THEN que_parametro = "PDBA".
          WHEN "DB" THEN que_parametro = "PDBB".
          WHEN "DC" THEN que_parametro = "PDBC".
     END CASE.
     que_parametro = que_parametro + STRING(pto_venta,"9999").
     FIND Parametro 
          WHERE Parametro.cdg_parametro = que_parametro 
            AND Parametro.cdg_empresa   = Empresa.cdg_empresa
                EXCLUSIVE-LOCK NO-WAIT NO-ERROR.
     IF LOCKED Parametro
     THEN DO:
        RUN PONMENSJ.P (INPUT "ANUL001").
        RETURN NO-APPLY.
     END.
     ELSE DO:
        des_forma = Parametro.valor_n.
        has_forma = des_forma.
        DISPLAY des_forma has_forma with FRAME frm-rango.
     END.
  END.
END.


ON CHOOSE OF btn_proceso
DO:

  ASSIGN tip_forma des_forma has_forma fecha_anulacion.
  mensaje = "Procesando ...".
  DISPLAY mensaje WITH FRAME frm-espere.
 cantidad_formas = has_forma - des_forma.
  DO j = 0 TO cantidad_formas:

     CREATE Fac_header.
     ASSIGN Fac_header.fecha       = fecha_anulacion
            Fac_header.cdg_empresa = Empresa.cdg_empresa
            Fac_header.tip_comprob = tip_forma
            Fac_header.prf_comprob = pto_venta
            Fac_header.nro_comprob = des_forma + j 
            Fac_header.origen      = "A"
            Fac_header.anulado     = YES
            Fac_header.nro_factura = NEXT-VALUE(proxima_transaccion)
            Fac_header.impreso     = "S".

     CREATE Sub_header_vta.
     ASSIGN
            Sub_header_vta.cdg_empresa = Fac_header.cdg_empresa
            Sub_header_vta.tip_comprob = Fac_header.tip_comprob
            Sub_header_vta.prf_comprob = Fac_header.prf_comprob
            Sub_header_vta.nro_comprob = Fac_header.nro_comprob
            Sub_header_vta.fecha       = Fac_header.fecha
            Sub_header_vta.anulado     = YES.

  END.

  PAUSE 0.
  HIDE FRAME frm-espere.
  DISPLAY tip_forma des_forma has_forma cantidad_formas
          WITH FRAME frm-rango.

  DISABLE BTN_PROCESO WITH FRAME frm-rango.

END.

ON LEAVE OF pto_venta IN FRAME frm-rango
DO:
   FIND Punto-venta 
        WHERE Punto-venta.cdg_puntovta = INPUT FRAME frm-rango pto_venta
          AND Punto-venta.cdg_empresa = Empresa.cdg_empresa
              NO-LOCK NO-ERROR.
   IF AVAILABLE Punto-venta
   THEN DO:
        ASSIGN pto_venta.
   END.
   ELSE DO:
        MESSAGE "No existe el punto de venta indicado!!!" VIEW-AS ALERT-BOX ERROR.
        RETURN NO-APPLY.
   END.
END.   

/*=================================================================================*/
/*                          B L O Q U E   P R I N C I P A L                        */
/*=================================================================================*/

{findempresa.i}

nom_funcion = "Anulacion de Formularios".
nom_menu = "SUBDIARIO".

   RUN getparametro.p (  INPUT  "CREDEBFC",
                         OUTPUT v-valor_c,
                         OUTPUT v-valor_d,
                         OUTPUT v-valor_l,
                         OUTPUT v-valor_n,
                         OUTPUT v-observacion ).
   credebfc = v-valor_l.


titulo_w = Empresa.nombre + "   " + nom_menu + " -- " + nom_funcion.
que_empresa = Empresa.nombre.

fecha_anulacion = TODAY.

RUN PONER_SESION.

DISPLAY
       pto_venta tip_forma des_forma  fecha_anulacion
       WITH FRAME frm-rango.

ENABLE ALL WITH FRAME frm-rango.

WAIT-FOR CHOOSE OF btn_salir.
HIDE FRAME frm-rango NO-PAUSE.

PROCEDURE PONER_SESION.

  CURRENT-WINDOW:TITLE   = titulo_w.
  STATUS INPUT "Ingrese datos o presione Esc para salir del programa.".

END PROCEDURE.

