...
&Scoped-define FIELDS-IN-QUERY-BROWSE-1 Cliente.cdg_cliente ~
Cliente.nom_cliente 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-1 
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH Cliente ~
      WHERE Cliente.nom_cliente BEGINS des_registro NO-LOCK ~
    BY Cliente.nom_cliente INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-1 OPEN QUERY BROWSE-1 FOR EACH Cliente ~
      WHERE Cliente.nom_cliente BEGINS des_registro NO-LOCK ~
    BY Cliente.nom_cliente INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-1 Cliente
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-1 gDialog _STRUCTURED
  QUERY BROWSE-1 NO-LOCK DISPLAY
      Cliente.cdg_cliente FORMAT "X(8)":U WIDTH 15.43
      Cliente.nom_cliente FORMAT "X(40)":U WIDTH 42.29
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 62 BY 7.81 EXPANDABLE.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME gDialog
     sel_registro AT ROW 1.27 COL 3 NO-LABEL
     BROWSE-1 AT ROW 2.62 COL 3
     Btn_elegir AT ROW 10.69 COL 3
     Btn_salir AT ROW 10.69 COL 50
     SPACE(2.42) SKIP(0.46)
...
/* Query rebuild information for BROWSE BROWSE-1
     ...
     _Where[1]         = "Cliente.nom_cliente BEGINS des_registro"
     _FldNameList[1]   > sic.Cliente.cdg_cliente
"Cliente.cdg_cliente" ? ? "character" ? ? ? ? ? ? no ? no no "15.43" yes no no "U" "" ""
     _FldNameList[2]   > sic.Cliente.nom_cliente
"Cliente.nom_cliente" ? ? "character" ? ? ? ? ? ? no ? no no "42.29" yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
