...
DEFINE BUFFER B-Clase FOR sic.Clase_de_Vendedor.
DEFINE BUFFER Clase FOR sic.Clase_de_Vendedor.
...
&Scoped-define INTERNAL-TABLES Clase Vendedor
...
/* Definitions for BROWSE brw_vendedores                                */
&Scoped-define FIELDS-IN-QUERY-brw_vendedores Vendedor.cdg_vendedor ~
Vendedor.nombre 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brw_vendedores 
&Scoped-define FIELD-PAIRS-IN-QUERY-brw_vendedores
&Scoped-define OPEN-QUERY-brw_vendedores OPEN QUERY brw_vendedores FOR EACH Vendedor NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brw_vendedores Vendedor
&Scoped-define FIRST-TABLE-IN-QUERY-brw_vendedores Vendedor


/* Definitions for DIALOG-BOX Dialog-Frame                              */
&Scoped-define OPEN-BROWSERS-IN-QUERY-Dialog-Frame ~
    ~{&OPEN-QUERY-brw_vendedores}

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS RECT-1 que_subclase brw_clasificacion camino ~
brw_vendedores Btn_OK Btn_Cancel 
...
DEFINE QUERY brw_vendedores FOR 
      Vendedor SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brw_clasificacion Dialog-Frame _STRUCTURED
  QUERY brw_clasificacion NO-LOCK DISPLAY
      SUBSTRING(Clase.cdg_subclase,LENGTH(que_clase) + 2) COLUMN-LABEL "Código!Subclase" FORMAT "X(10)"
      Clase.nombre_subclase COLUMN-LABEL "Denominacion!Subclase" FORMAT "X(40)"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 55 BY 9.69
         BGCOLOR 11 FGCOLOR 9 FONT 4
         TITLE BGCOLOR 11 FGCOLOR 9 "Subclases Definidas para la Clase Actual".

DEFINE BROWSE brw_vendedores
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brw_vendedores Dialog-Frame _STRUCTURED
  QUERY brw_vendedores DISPLAY
      Vendedor.cdg_vendedor FORMAT "X(10)"
      Vendedor.nombre COLUMN-LABEL "Descripción!Artículo" FORMAT "X(40)"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 55 BY 9.15
         BGCOLOR 11 FGCOLOR 9 FONT 4
         TITLE BGCOLOR 11 FGCOLOR 9 "Artículos comprendidos en la Clase.".


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME Dialog-Frame
     que_subclase AT ROW 1.81 COL 12 COLON-ALIGNED
     que_nombre AT ROW 1.81 COL 27 COLON-ALIGNED NO-LABEL
     que_clase AT ROW 1.81 COL 59 COLON-ALIGNED NO-LABEL
     brw_clasificacion AT ROW 3.15 COL 4
     camino AT ROW 3.15 COL 61 NO-LABEL
     brw_vendedores AT ROW 13.12 COL 4
     Btn_OK AT ROW 20.92 COL 62
     Btn_Cancel AT ROW 20.92 COL 88
     RECT-1 AT ROW 20.65 COL 61
     SPACE(2.28) SKIP(0.87)
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: B-Clase B "?" ? sic Clase_de_Vendedor
      TABLE: Clase B "?" ? sic Clase_de_Vendedor
   END-TABLES.
 */
...
/* BROWSE-TAB brw_vendedores camino Dialog-Frame */
ASSIGN 
...
"SUBSTRING(Clase.cdg_subclase,LENGTH(que_clase) + 2)" "Código!Subclase" "X(10)" ? ? ? ? ? ? ? no ?
     _FldNameList[2]   > Temp-Tables.Clase.nombre_subclase
"Clase.nombre_subclase" "Denominacion!Subclase" "X(40)" "character" ? ? ? ? ? ? no ?
     _Query            is NOT OPENED
*/  /* BROWSE brw_clasificacion */
&ANALYZE-RESUME

&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE brw_vendedores
/* Query rebuild information for BROWSE brw_vendedores
     _TblList          = "sic.Vendedor"
     _FldNameList[1]   > sic.Vendedor.cdg_vendedor
"cdg_vendedor" ? "X(10)" "character" ? ? ? ? ? ? no ?
     _FldNameList[2]   > sic.Vendedor.nombre
"nombre" "Descripción!Artículo" "X(40)" "character" ? ? ? ? ? ? no ?
     _Query            is OPENED
*/  /* BROWSE brw_vendedores */
&ANALYZE-RESUME
...
DO:
   ...
   THEN DO:
        ...
        sel_clase = que_clase.
        rid_vendedor = ROWID(Vendedor).
   END.
   ELSE DO:
        MESSAGE "No hay disponible artículo alguno para elegir"
...
DO:
   ...
   ELSE DO:
      ...
      THEN DO:
         ...
         CREATE Clase.
         ASSIGN 
                Clase.cdg_clase = que_clase                                 
                Clase.cdg_subclase = que_clase + "." + que_subclase.
...
------------------------------------------------------------------------------*/
        ...
        OPEN QUERY brw_clasificacion 
             FOR EACH Clase WHERE Clase.cdg_clase = que_clase. 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE abre_query_vendedors Dialog-Frame 
PROCEDURE abre_query_vendedors :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
        ...
        OPEN QUERY brw_vendedores 
             FOR EACH Vendedor WHERE Vendedor.cdg_subclase BEGINS que_clase
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 que_subclase brw_clasificacion camino brw_vendedores Btn_OK 
         Btn_Cancel 
      WITH FRAME Dialog-Frame.
