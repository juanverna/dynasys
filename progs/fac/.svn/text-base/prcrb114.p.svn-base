...
DO TRANSACTION:
    ...
    v-desc  = Fac_header.imp_bruto - Fac_header.imp_total.
    v-descuentos = "".
    FOR EACH Fac_header-bon OF Fac_header NO-LOCK:
        v-descuentos = v-descuentos + " " + STRING(Fac_header-bon.porcentaje,"ZZ9.99").
    END.    
    IF LENGTH(v-descuentos) <> 0 
       THEN v-descuentos = "Descuentos " + SUBSTRING(v-descuentos,2).
    ...
    OUTPUT STREAM Formulario TO "c:\sic-temp\prcrb114.xpr" CONVERT TARGET "iso8859-1".
 ...
 /* PUT STREAM Formulario CONTROL '<TOOLBAR=!PRINT>'. */
    ...
    FILE-INFO:File-NAME = "c:\sic-temp\prcrb114.xpr".
...
*/    
    ...
    FOR EACH Fac_detalle OF Fac_header, Articulo OF Fac_detalle BREAK BY Articulo.cdg_articulo:
        ...
        THEN DO:
            ...
            IF Fac_detalle.precio_cf <> 0
            THEN DO:
                RUN escribenumero ( ch_linea  + ",140", INPUT STRING(Fac_detalle.precio_cf,"-ZZZ,ZZ9.99"), INPUT 8, INPUT NO).
    ...
    IF LENGTH(v-descuentos) <> 0
    THEN DO:
         RUN escribir      ( INPUT "246,110", INPUT v-descuentos, INPUT 10, INPUT NO).
         RUN escribenumero ( INPUT "246,170", INPUT STRING(v-desc,"-ZZZ,ZZ9.99"), INPUT 10, INPUT NO).
    END.
