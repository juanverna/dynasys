...
DO TRANSACTION:
    ...
    FIND Impuesto_condicion OF Condicion_impos
         WHERE Impuesto_condicion.cdg_impuesto = 1 
           AND Impuesto_condicion.cdg_empresa = Fac_header.cdg_empresa
           AND Impuesto_condicion.fch_desde <= Fac_header.fecha_iva
           AND Impuesto_condicion.fch_hasta >= Fac_header.fecha_iva
               NO-LOCK NO-ERROR.
    IF AVAILABLE Impuesto_condicion
    THEN DO:
        prciva = Impuesto_condicion.tasa.
        prcnoi = prciva / 2.
    ...
    OUTPUT STREAM Formulario TO "c:\sic-temp\prcra114.xpr" CONVERT TARGET "iso8859-1".
      
 /* PUT STREAM Formulario CONTROL '<PREVIEW=70>' /*=ZoomToWidth>'*/. */
 /* PUT STREAM Formulario CONTROL '<TOOLBAR=!PRINT>'. */
    
    PUT STREAM Formulario CONTROL "<OPORTRAIT><Title=Impresion de notas de crédito><UNITS=mm><|2>".
    ...
    FILE-INFO:File-NAME = "c:\sic-temp\prcra114.xpr".
...
PROCEDURE forma:
    ...
    RUN escribir ( INPUT "20,125", INPUT "NOTA DE CREDITO", INPUT 12, INPUT YES). 
...
*/    
    ...
    RUN escribir ( INPUT "57,30", INPUT "[" + Vendedor.cdg_vendedor + "]", INPUT 8, INPUT NO).
    ...
    FOR EACH Fac_detalle OF Fac_header WHERE Fac_detalle.cantidad <> 0 OR Fac_detalle.granel <> 0,
             Articulo OF Fac_detalle BREAK BY Articulo.cdg_articulo:
        ...
        THEN DO:
            ...
            RUN escribenumero ( ch_linea  + ",2",   INPUT STRING(v-cantidad,"ZZZZ9"), INPUT 8, INPUT NO).
            RUN escribir      ( ch_linea  + ",30",  INPUT Articulo.cdg_articulo, INPUT 8, INPUT NO).
            RUN escribir      ( ch_linea  + ",40",  INPUT Articulo.descripcion, INPUT 8, INPUT NO).
            IF Fac_detalle.precio <> 0
            THEN DO:
                RUN escribenumero ( ch_linea  + ",140", INPUT STRING(Fac_detalle.precio,"-ZZZ,ZZ9.99"), INPUT 8, INPUT NO).
                RUN escribenumero ( ch_linea  + ",170", INPUT STRING(v-subtotal,"-ZZZ,ZZ9.99"), INPUT 8, INPUT NO).
            END.
            ELSE DO:
                RUN escribir ( ch_linea  + ",144", INPUT "Bonificado", INPUT 8, INPUT NO).
            END.
    ...
    DO j = 1 TO v-reng_monto:
        IF j <= NUM-ENTRIES(v-monto_letras, "|")
           THEN RUN escribir ( INPUT TRIM(STRING(linea0,">>9")) + ",6", INPUT ENTRY(j,v-monto_letras, "|"), INPUT 8, INPUT NO).
        linea0 = linea0 + 4.
    END.
    ...
    RUN escribir      ( INPUT "242,110", INPUT "Subtotal", INPUT 10, INPUT NO).
    RUN escribenumero ( INPUT "242,170", INPUT STRING(Fac_header.imp_bruto,"-ZZZ,ZZ9.99"), INPUT 10, INPUT NO).

    RUN escribir      ( INPUT "246,110", INPUT "Descuento " + v-descuentos, INPUT 10, INPUT NO).
    RUN escribenumero ( INPUT "246,170", INPUT STRING(Fac_header.imp_descuentos,"-ZZZ,ZZ9.99"), INPUT 10, INPUT NO).

    RUN escribir      ( INPUT "250,110", INPUT "Neto", INPUT 10, INPUT NO).
    RUN escribenumero ( INPUT "250,170", INPUT STRING(Fac_header.imp_neto,"-ZZZ,ZZ9.99"), INPUT 10, INPUT NO).
      
    RUN escribir      ( INPUT "254,110", INPUT "IVA % " + STRING(prciva,">9.99"), INPUT 10, INPUT NO).
    RUN escribenumero ( INPUT "254,170", INPUT STRING(Fac_header.imp_iva,"-ZZZ,ZZ9.99"), INPUT 10, INPUT NO).

    RUN escribir      ( INPUT "258,110", INPUT "Alicuota NO Inscripto % " + STRING(prcnoi,">9.99"), INPUT 10, INPUT NO).
    RUN escribenumero ( INPUT "258,170", INPUT STRING(importe_noi,"-ZZZ,ZZ9.99"), INPUT 10, INPUT NO).
    
    RUN escribir      ( INPUT "263,110", INPUT "Total a Pagar", INPUT 10, INPUT NO).
    RUN escribenumero ( INPUT "263,170", INPUT STRING(Fac_header.imp_total,"-ZZZ,ZZ9.99"), INPUT 10, INPUT YES).

/*

    RUN escribir      ( INPUT "242,110", INPUT "Neto", INPUT 10, INPUT NO).
    RUN escribenumero ( INPUT "242,170", INPUT STRING(Fac_header.imp_neto,"-ZZZ,ZZ9.99"), INPUT 10, INPUT NO).
      
    RUN escribir      ( INPUT "246,110", INPUT "IVA % 21.00", INPUT 10, INPUT NO).
    RUN escribenumero ( INPUT "246,170", INPUT STRING(Fac_header.imp_iva,"-ZZZ,ZZ9.99"), INPUT 10, INPUT NO).

    RUN escribir      ( INPUT "250,110", INPUT "Alicuota NO Inscripto % 10.50", INPUT 10, INPUT NO).
    RUN escribenumero ( INPUT "250,170", INPUT STRING(importe_noi,"-ZZZ,ZZ9.99"), INPUT 10, INPUT NO).

    RUN escribir      ( INPUT "263,110", INPUT "Total", INPUT 10, INPUT NO).
    RUN escribenumero ( INPUT "263,170", INPUT STRING(Fac_header.imp_total,"-ZZZ,ZZ9.99"), INPUT 10, INPUT YES).
*/

    RUN escribir      ( INPUT "271,120", INPUT STRING(Fac_header.prf_comprob,"9999") + " " + STRING(Fac_header.nro_comprob,"99999999"), INPUT 8, INPUT NO).
