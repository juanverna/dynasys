...
PROCEDURE cabecera_datos:
    ...
    */
    RUN escribir ( INPUT "6,105", INPUT Tipocomprobante.denominacion_impresa, INPUT 12, INPUT YES, INPUT "BLACK").     
  ...
  /*RUN escribir ( INPUT "16,180", INPUT TRIM(STRING(n-hoja,">>9")) + "/" + TRIM(STRING(nt-hojas,">>9")), INPUT 12, INPUT YES, INPUT "BLACK").*/
    ...
    RUN escribir ( INPUT "51,10", INPUT Fac_header.nombre, INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( INPUT "51,85", INPUT "[" + Cliente.cdg_cliente + "]", INPUT 8, INPUT NO, INPUT "BLACK").
    RUN escribir ( INPUT "55,10", INPUT Fac_header.direccion_leg, INPUT 8, INPUT NO, INPUT "BLACK").
    RUN escribir ( INPUT "59,10", INPUT "(" + Fac_header.cdg_postal_leg + ") " + Fac_header.localidad_leg, INPUT 8, INPUT NO, INPUT "BLACK").
    RUN escribir ( INPUT "63,10", INPUT Provlegal.nombre , INPUT 8, INPUT NO, INPUT "BLACK").
    RUN escribir ( INPUT "63,85", INPUT "[" + Provlegal.cdg_provincia + "]", INPUT 8, INPUT NO, INPUT "BLACK").
    RUN escribir ( INPUT "67,21", INPUT Fac_header.cuit, INPUT 8, INPUT NO, INPUT "BLACK").
    RUN escribir ( INPUT "67,70", INPUT Cliente.ing_brutos, INPUT 8, INPUT NO, INPUT "BLACK").
    RUN escribir ( INPUT "71,10", INPUT "I.V.A." + Condicion_impos.texto, INPUT 8, INPUT NO, INPUT "BLACK").

    RUN escribir ( INPUT "51,105", INPUT Domicilio.nombre, INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( INPUT "51,175", INPUT "[" + STRING(Domicilio.nro_domicilio,"9999") + "]", INPUT 8, INPUT NO, INPUT "BLACK").
    RUN escribir ( INPUT "55,105", INPUT Fac_header.direccion, INPUT 8, INPUT NO, INPUT "BLACK").
    RUN escribir ( INPUT "59,105", INPUT "(" + Fac_header.cdg_postal + ") " + Fac_header.localidad, INPUT 8, INPUT NO, INPUT "BLACK").
    RUN escribir ( INPUT "63,105", INPUT Provincia.nombre, INPUT 8, INPUT NO, INPUT "BLACK").
    RUN escribir ( INPUT "63,175", INPUT "[" + Provincia.cdg_provincia + "]", INPUT 8, INPUT NO, INPUT "BLACK").

    RUN escribir ( INPUT "67,119", INPUT Usuario.cdg_usuario, INPUT 8, INPUT NO, INPUT "BLACK").

    RUN escribir ( INPUT "80,42", INPUT Condicion_venta.descripcion, INPUT 8, INPUT NO, INPUT "BLACK").
    RUN escribir ( INPUT "84,170", INPUT Fac_header.nro_ocm, INPUT 8, INPUT NO, INPUT "BLACK").
    
    IF AVAILABLE Ped_header
    THEN DO:
         RUN escribir ( INPUT "80,130", INPUT STRING(Ped_header.nro_comprob,">>>>>9"), INPUT 8, INPUT NO, INPUT "BLACK").
    END.
     
    RUN escribir ( INPUT "88,25", INPUT v-remitos, INPUT 8, INPUT NO, INPUT "BLACK").

    IF AVAILABLE Cta_cte
       THEN RUN escribir ( INPUT "84,32", INPUT STRING(Cta_cte.fecha_vencimiento,"99/99/9999"), INPUT 8, INPUT NO, INPUT "BLACK").
...
PROCEDURE detalle_datos:
    ...
    FOR EACH Fac_detalle OF Fac_header,
        ...
        THEN DO:
            ...
            FIND FIRST Articulo_atributo OF Articulo 
                 WHERE Articulo_atributo.cdg_tipoatributo = "SABOR"
                   AND Articulo_atributo.cdg_atributo = ENTRY(3,Articulo.cdg_articulo,"-") NO-LOCK.
            FIND Atributo OF Articulo_atributo NO-LOCK.

            IF Imputacion.afecta_stock 
            THEN DO:
                RUN detalle-numr ( ch_linea  + ",127", INPUT STRING(v-cantidad,"ZZZZZZ9.99"), INPUT 8, INPUT NO, INPUT "BLACK").
                RUN detalle-char ( ch_linea  + ",147", INPUT Unidad.abrevia, INPUT 8, INPUT NO, INPUT "BLACK").
            END.

            RUN detalle-char ( ch_linea  + ",107", INPUT Articulo.cdg_articulo, INPUT 8, INPUT NO, INPUT "BLACK").
            RUN detalle-char ( ch_linea  + ",93",  INPUT Atributo.abreviatura, INPUT 8, INPUT NO, INPUT "BLACK").
            RUN detalle-char ( ch_linea  + ",10",  INPUT Articulo.descripcion, INPUT 8, INPUT NO, INPUT "BLACK").
            IF es_cuenta_y_orden
                THEN RUN detalle-char ( ch_linea  + ",06",  INPUT "(" + STRING(LOOKUP(ventas_terceros,Proveedor.cdg_proveedor,","),"9") + ")", INPUT 7, INPUT NO, INPUT "BLACK").
            ...
            THEN DO:
                ...
                THEN DO:
                    IF Imputacion.afecta_stock 
                        THEN RUN detalle-numr ( ch_linea  + ",147", INPUT STRING(Fac_detalle.precio_cf,"-ZZZ,ZZ9.9999"), INPUT 8, INPUT NO, INPUT "BLACK").
                    RUN detalle-numr ( ch_linea  + ",166", INPUT STRING(Fac_detalle.subtotal_bruto_cf,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT NO, INPUT "BLACK").
                END.
                ELSE DO:
                    RUN detalle-char ( ch_linea  + ",154", INPUT "Bonificado", INPUT 8, INPUT NO, INPUT "BLACK").
                END.
            END.
        ...
        ELSE DO:
            ...
            DO j = 1 TO NUM-ENTRIES(v-detallada,"|"):
                ch_linea = STRING(linea0,">>9").                     
                RUN detalle-char ( INPUT ch_linea  + ",10", INPUT ENTRY(j,v-detallada, "|"), INPUT 8, INPUT NO, INPUT "BLACK").
                IF Articulo.sumaneto > 0
                THEN DO:
                    ...
                    THEN DO:
                        ...
                        THEN DO:
                            RUN detalle-numr ( ch_linea  + ",128", INPUT STRING(v-cantidad,"ZZZZZZ9.99  ") + Unidad.abrevia, INPUT 8, INPUT NO, INPUT "BLACK").
                            RUN detalle-numr ( ch_linea  + ",145", INPUT STRING(Fac_detalle.precio_cf,"-ZZZ,ZZ9.99"), INPUT 8, INPUT NO, INPUT "BLACK").
                        END.
                        RUN detalle-numr ( ch_linea  + ",166", INPUT STRING(Fac_detalle.subtotal_bruto_cf,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT NO, INPUT "BLACK").
                    END.
                END.
    ...
    THEN DO:
        ...
        DO j = nreng TO comienzo_impuestos - 1 BY 1:
            c-linea_detalle = c-linea_detalle + 1.
            RUN detalle-char ( INPUT TRIM(STRING(linea0,">>9")) + ",100", INPUT "", INPUT 8, INPUT NO, INPUT "BLACK").
        END.

        c-linea_detalle = c-linea_detalle + 1.

        FOR EACH Fac_header_impuesto OF Fac_header, FIRST Impuesto OF Fac_header_impuesto WHERE NOT Impuesto.es_iva BY Impuesto.cdg_impuesto:
            RUN detalle-char ( INPUT TRIM(STRING(linea0,">>9")) + ",100", INPUT Impuesto.nombre, INPUT 8, INPUT NO, INPUT "BLACK").
            RUN detalle-numr ( INPUT TRIM(STRING(linea0,">>9")) + ",166", INPUT STRING(Fac_header_impuesto.importe,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT NO, INPUT "BLACK").        
            c-linea_detalle = c-linea_detalle + 1.
        END.
...
PROCEDURE pie_datos:
    ...
    RUN escribir ( "4,48",  INPUT STRING(v-imp_gravado,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( "4,114", INPUT STRING(v-imp_exento,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( "4,158", INPUT STRING(v-con_impuestos,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( "18,16", INPUT STRING(Fac_header.imp_total,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT YES,  INPUT "BLACK").
    RUN escribir ( "18,38", INPUT STRING(0,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT YES,  INPUT "BLACK").
    RUN escribir ( "18,76", INPUT STRING(Fac_header.imp_total,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT YES,  INPUT "BLACK").
    RUN escribir ( "18,113", INPUT STRING(0,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT YES,  INPUT "BLACK").
    RUN escribir ( "18,138", INPUT STRING(0,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT YES,  INPUT "BLACK").
    RUN escribir ( "18,172", INPUT STRING(Fac_header.imp_total,"-ZZ,ZZZ,ZZ9.99"), INPUT 10, INPUT YES,  INPUT "BLACK").
    RUN escribir ( INPUT "32,18", INPUT SUBSTRING(Fac_header.tip_comprob,2,1) + " " +
                                        STRING(Fac_header.prf_comprob,"9999") + "-" + 
                                        STRING(Fac_header.nro_comprob,"99999999"), INPUT 8, INPUT NO, INPUT "BLACK"). 
...
PROCEDURE pie_relleno:
    ...
    RUN escribir ( "4,50",  INPUT "***************", INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( "4,114", INPUT "***************", INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( "4,158", INPUT "***************", INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( "18,16", INPUT "***************", INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( "18,38", INPUT "***************", INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( "18,76", INPUT "***************", INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( "18,113", INPUT "***************", INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( "18,138", INPUT "***************", INPUT 8, INPUT YES, INPUT "BLACK").
    RUN escribir ( "18,172", INPUT "***************", INPUT 10, INPUT YES, INPUT "BLACK").
...
PROCEDURE poner_monto_letras:
    ...
    DO j = 1 TO v-reng_monto:
        IF j <= NUM-ENTRIES(v-monto_letras, "|")
           THEN RUN escribir ( INPUT TRIM(STRING(linea0 + j * 4,">>9")) + ",10", INPUT ENTRY(j,v-monto_letras, "|"), INPUT 6, INPUT NO, INPUT "BLACK").
    END.
...
PROCEDURE bajar_copias:
   ...
   /*
   OUTPUT STREAM Formulario TO "c:\sic-temp\prfab103.txt" CONVERT TARGET "iso8859-1".
   FOR EACH T-Detalle:
   ...
   END.
   OUTPUT CLOSE.
   RUN VERESULT.W ( INPUT "c:\sic-temp\prfab103.txt", INPUT 22 ).
   */
   ...
   DO ncopia = 1 TO nt-copias:    
        ...
        OUTPUT STREAM Formulario TO "c:\sic-temp\prfab103.xpr" CONVERT TARGET "iso8859-1".
        ...
        FOR EACH T-Detalle BREAK BY T-Detalle.n-linea:
            ...
            THEN DO:
                ...
                THEN DO:
                    ...
                    FILE-INFO:File-NAME = "c:\sic-temp\prfab103.xpr".
