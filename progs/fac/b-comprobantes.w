...
DEF VAR aplicado AS CHAR FORMAT "X".

qstr = qstr0. /*valor inicial*/
DEFINE VAR signo AS INT FORMAT "-9" INITIAL 1 NO-UNDO.
DEFINE VAR mes AS INT FORMAT "99" NO-UNDO.
{strdebitan.i}
...
Fac_header.impreso Fac_header.estado_2_impresion aplicado() @ aplicado ~
pagado() @ paga Fac_header.fecha Fac_header.tip_comprob ~
Fac_header.prf_comprob Fac_header.nro_comprob ~
imp_total * signo(fac_header.tip_comprob) @ imp_total listaP() @ listaP ~
Fac_header.nro_contrato Fac_header.codigo_cliente Fac_header.nombre ~
Fac_header.direccion month(fecha) @ mes ~
signo(fac_header.tip_comprob) @ signo Fac_header.nom_Administrador ~
Fac_header.fecha_2_impresion Fac_header.nro_usuario_2_impresion 
...
&Scoped-Define ENABLED-OBJECTS br_table btn_cancelaciones fblogico b-query ~
fbdesde fbusqueda b-nueva fbhasta Btn-excel 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD Aplicado B-table-Win 
FUNCTION Aplicado RETURNS CHARACTER
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD signo B-table-Win 
FUNCTION signo RETURNS INTEGER
...
DEFINE BUTTON Btn-excel 
     ...
     LABEL "&Modifica" 
     SIZE 9 BY 1.19 TOOLTIP "Modifica el registro actual"
     FONT 4.

DEFINE BUTTON btn_cancelaciones 
     LABEL "Cancelación" 
     SIZE 13 BY 1.14.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Fac_header.anulado FORMAT "Si/No":U
      Fac_header.impreso COLUMN-LABEL "I" FORMAT "X":U WIDTH 1.4
      Fac_header.estado_2_impresion COLUMN-LABEL "R" FORMAT "X(1)":U
            WIDTH 2.8
      aplicado() @ aplicado COLUMN-LABEL "AP" FORMAT "X":U
      pagado() @ paga COLUMN-LABEL "P" FORMAT "X":U
      ...
      imp_total * signo(fac_header.tip_comprob) @ imp_total COLUMN-LABEL "Importe!Total" FORMAT "->,>>>,>>>,>>9.99":U
      listaP() @ listaP COLUMN-LABEL "Lista" FORMAT "X(8)":U WIDTH 8.6
      Fac_header.nro_contrato FORMAT "ZZZZZ9":U
      Fac_header.codigo_cliente FORMAT "X(8)":U
      Fac_header.nombre FORMAT "X(40)":U WIDTH 39.6
      Fac_header.direccion FORMAT "X(45)":U WIDTH 25.2
      month(fecha) @ mes COLUMN-LABEL "ME" FORMAT "99":U
      signo(fac_header.tip_comprob) @ signo COLUMN-LABEL "SG" FORMAT "-9":U
            WIDTH 3.6
      Fac_header.nom_Administrador COLUMN-LABEL "Administrador" FORMAT "X(40)":U
            WIDTH 26.2
      Fac_header.fecha_2_impresion COLUMN-LABEL "FRecibo" FORMAT "99/99/9999":U
      Fac_header.nro_usuario_2_impresion COLUMN-LABEL "URecibo" FORMAT "x(8)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN NO-COLUMN-SCROLLING SEPARATORS SIZE 196 BY 23.76 FIT-LAST-COLUMN.
...
DEFINE FRAME F-Main
     ...
     btn_cancelaciones AT ROW 1.24 COL 134 WIDGET-ID 26
     fblabel AT ROW 1.48 COL 2 NO-LABEL WIDGET-ID 24 NO-TAB-STOP 
     fblogico AT ROW 1.48 COL 18 NO-LABEL WIDGET-ID 20
     b-query AT ROW 1.24 COL 123 WIDGET-ID 16 NO-TAB-STOP 
     fbdesde AT ROW 1.33 COL 16.2 COLON-ALIGNED NO-LABEL WIDGET-ID 10
     fbusqueda AT ROW 1.33 COL 16.2 COLON-ALIGNED NO-LABEL WIDGET-ID 2
     b-nueva AT ROW 1.24 COL 111.2 WIDGET-ID 4 NO-TAB-STOP 
     fbhasta AT ROW 1.33 COL 41.2 COLON-ALIGNED NO-LABEL WIDGET-ID 12
     Btn-excel AT ROW 1.24 COL 148 WIDGET-ID 30
...
"Fac_header.impreso" "I" "X" "character" ? ? ? ? ? ? no ? no no "1.4" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > sic.Fac_header.estado_2_impresion
"Fac_header.estado_2_impresion" "R" ? "character" ? ? ? ? ? ? no ? no no "2.8" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > "_<CALC>"
"aplicado() @ aplicado" "AP" "X" ? ? ? ? ? ? ? no "Si esta aplicado a otro comprobante" no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > "_<CALC>"
"pagado() @ paga" "P" "X" ? ? ? ? ? ? ? no "Si se encuentra cancelado o pagado" no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   = sic.Fac_header.fecha
     _FldNameList[7]   > sic.Fac_header.tip_comprob
"Fac_header.tip_comprob" "Ti!po" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > sic.Fac_header.prf_comprob
"Fac_header.prf_comprob" "Pre!fijo" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > sic.Fac_header.nro_comprob
"Fac_header.nro_comprob" "Número!Comprob" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > "_<CALC>"
"imp_total * signo(fac_header.tip_comprob) @ imp_total" "Importe!Total" "->,>>>,>>>,>>9.99" ? ? ? ? ? ? ? no "Importe Total Signado (incluyendo impuestos)" no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > "_<CALC>"
"listaP() @ listaP" "Lista" "X(8)" ? ? ? ? ? ? ? no ? no no "8.6" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[12]   = sic.Fac_header.nro_contrato
     _FldNameList[13]   = sic.Fac_header.codigo_cliente
     _FldNameList[14]   > sic.Fac_header.nombre
"Fac_header.nombre" ? ? "character" ? ? ? ? ? ? no ? no no "39.6" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[15]   > sic.Fac_header.direccion
"Fac_header.direccion" ? ? "character" ? ? ? ? ? ? no ? no no "25.2" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[16]   > "_<CALC>"
"month(fecha) @ mes" "ME" "99" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[17]   > "_<CALC>"
"signo(fac_header.tip_comprob) @ signo" "SG" "-9" ? ? ? ? ? ? ? no ? no no "3.6" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[18]   > sic.Fac_header.nom_Administrador
"Fac_header.nom_Administrador" "Administrador" ? "character" ? ? ? ? ? ? no ? no no "26.2" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[19]   > sic.Fac_header.fecha_2_impresion
"Fac_header.fecha_2_impresion" "FRecibo" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[20]   > sic.Fac_header.nro_usuario_2_impresion
"Fac_header.nro_usuario_2_impresion" "URecibo" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
OR "E" OF br_table IN FRAME {&FRAME-NAME}
DO:
    DEFINE VAR rok AS LOGICAL.
    EMPTY TEMP-TABLE facturas.
    CREATE facturas.
    ASSIGN facturas.rfac = ROWID(fac_header).
FIND cliente WHERE fac_header.nro_admin = cliente.nro_admin NO-LOCK NO-ERROR.
IF NOT AVAILABLE cliente THEN DO:
    MESSAGE "Cliente Desconocido"  VIEW-AS ALERT-BOX ERROR.
    RETURN NO-APPLY.
END.

FIND FIRST domicilio NO-LOCK OF cliente.
FOR each Cliente-contacto OF Domicilio , Persona OF Cliente-contacto WHERE persona.email <> "" AND  can-do(Cliente-contacto.canal-email,"*") :
        LEAVE.
END.
IF NOT AVAILABLE persona THEN DO:
    MESSAGE "El administrador no tiene un email donde enviar"
...
IF num-entries(persona.email,"@") <> 2 THEN DO:
  MESSAGE "ERROR EMAIL Facturas la persona " + persona.nombre + " email invalido"
  VIEW-AS ALERT-BOX ERROR.
  RETURN NO-APPLY.
END.
IF NUM-ENTRIES( entry( 2 , persona.email, "@" ) , ".") < 2 THEN DO:
  MESSAGE "ERROR EMAIL Certificado la persona " + persona.nombre + " email invalido"
  VIEW-AS ALERT-BOX ERROR.
  RETURN NO-APPLY.
END.
MESSAGE "Se enviara factura a " persona.email VIEW-AS alert-box BUTTON OK-CANCEL SET rok.
IF rok THEN RUN factuemail.p ( INPUT persona.email, INPUT cliente.nom_cliente, INPUT TABLE facturas,OUTPUT rok  ).
...
DO:
    run excel-export ( br_table:HANDLE ).
END.
...
&Scoped-define SELF-NAME btn_cancelaciones
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btn_cancelaciones B-table-Win
ON CHOOSE OF btn_cancelaciones IN FRAME F-Main /* Cancelación */
DO:
  RUN d-ver_cancelacion_factura.w ( INPUT Fac_header.cdg_empresa,
...
ON LEAVE OF fbdesde IN FRAME F-Main
DO:
  IF fbhasta:SCREEN-VALUE = "?" THEN
      fbhasta:SCREEN-VALUE = fbdesde:SCREEN-VALUE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fbdesde B-table-Win
ON RETURN OF fbdesde IN FRAME F-Main
DO:
  APPLY "entry" TO fbhasta.
END.
...
ON LEAVE OF fbusqueda IN FRAME F-Main
DO:
DEF VAR i AS INT NO-UNDO.
DEF VAR dd AS DECIMAL NO-UNDO.
DEF VAR qh AS HANDLE NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fbusqueda B-table-Win
ON RETURN OF fbusqueda IN FRAME F-Main
DO:
  APPLY "leave" TO SELF.
END.
...
DEF VAR qb AS HANDLE NO-UNDO.
  ...
  DO WHILE valid-handle(qc):
    ...
    qc = qc:NEXT-COLUMN.
  END.
  qstr = qstr + " BY Fac_header.fecha DESCENDING by Fac_header.nro_comprob".
  qh:QUERY-PREPARE(qstr).
  qh:QUERY-OPEN.
  qb:PRIVATE-DATA = ?.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION Aplicado B-table-Win 
FUNCTION Aplicado RETURNS CHARACTER
...
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
DEF VAR importe_aplicado AS DECIMAL NO-UNDO.  
importe_aplicado = 0.
aplicado = "N".
IF fac_header.tip_comprob BEGINS "F" THEN DO:
    FOR each Aplicacion_pagos WHERE
          Aplicacion_pagos.cdg_empresa = fac_header.cdg_empresa AND
          Aplicacion_pagos.tip_cancela = fac_header.tip_comprob AND
          Aplicacion_pagos.prf_cancela = fac_header.prf_comprob AND
          Aplicacion_pagos.nro_cancela = fac_header.nro_comprob AND
          Aplicacion_pagos.tip_comprob  BEGINS "C" NO-LOCK :
          importe_aplicado = importe_aplicado + Aplicacion_pagos.importe.
    END.
END.
IF fac_header.tip_comprob BEGINS "C" THEN DO:
    FOR each Aplicacion_pagos WHERE
          ...
          Aplicacion_pagos.nro_comprob = fac_header.nro_comprob AND
          Aplicacion_pagos.tip_cancela  BEGINS "F" NO-LOCK :
          importe_aplicado = importe_aplicado + Aplicacion_pagos.importe.
    END.
END.
    IF importe_aplicado <> 0 THEN aplicado = "P".
    IF importe_aplicado = sic.Fac_header.imp_total THEN aplicado = "T".
  RETURN aplicado.
END FUNCTION.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION signo B-table-Win 
FUNCTION signo RETURNS INTEGER
  ( tipo AS CHAR ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
  IF LOOKUP(tipo,str_debitan) <> 0 THEN RETURN 1.
