...
PROCEDURE forma:
    ...
    FOR EACH Fac_detalle OF Fac_header, Articulo OF Fac_detalle:
            ...
            DO j = 1 TO NUM-ENTRIES(v-detallada, "|"):
               ...
               RUN escribir ( INPUT TRIM(STRING(linea0,">>9")) + ",30", INPUT ENTRY(j,v-detallada, "|"), INPUT 8, INPUT NO).
                IF j = NUM-ENTRIES(v-detallada, "|")
                   THEN RUN escribenumero ( INPUT TRIM(STRING(linea0,">>9")) + ",187", INPUT STRING(Fac_detalle.subtotal_neto_cf,"-ZZZ,ZZ9.99"), INPUT 8, INPUT NO).
        ...
        ELSE DO:
           ...
           RUN escribenumero ( INPUT TRIM(STRING(linea0,">>9")) + ",140", INPUT STRING(Fac_detalle.precio_cf,"-ZZZ,ZZ9.99"), INPUT 8, INPUT NO).
           RUN escribenumero ( INPUT TRIM(STRING(linea0,">>9")) + ",160", INPUT v-bonifs, INPUT 8, INPUT NO).
           RUN escribenumero ( INPUT TRIM(STRING(linea0,">>9")) + ",187", INPUT STRING(Fac_detalle.subtotal_bruto_cf,"-ZZZ,ZZ9.99"), INPUT 8, INPUT NO).
           linea0 = linea0 + 4.
