...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD muestracli W-Win 
FUNCTION muestracli RETURNS LOGICAL
  (  )  FORWARD.
...
DEFINE FRAME F-Main
    WITH DOWN NO-BOX NO-HIDE KEEP-TAB-ORDER OVERLAY NO-HELP 
         ...
         AT COL 1 ROW 1
         SIZE 119 BY 18.81 WIDGET-ID 100.
...
OCXFile = SEARCH( "w-geocli-adm.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-geocli-adm.wrx":U SKIP(1)
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE restric W-Win 
PROCEDURE restric :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
FOR EACH cliente:
 ...
 FOR EACH cliente_restriccion OF cliente:
    IF NOT senal THEN DO:
        senal = TRUE.
        CREATE tt.
        ASSIGN tt.nro_cliente = cliente.nro_cliente.
    END.
    ...
    restriccion1 = REPLACE(restriccion1,'"','').
    restriccion2 = REPLACE(restriccion2,'"','').
 END.
...
FOR EACH tt:
        FIND cliente WHERE cliente.nro_cliente =  tt.nro_cliente NO-LOCK.
        IF cliente.geolat <> 0 THEN DO:
            ...
            'map.addOverlay(createMarker (point,"' cliente.cdg_cliente '-' replace(Cliente.nom_cliente,'"','') 
                '<BR>Dir:' replace(cliente.direccion,'"','')
                '<BR>Rest1:' tt.restriccion1
                '<BR>Rest2:' tt.restriccion2 
               '","' (IF tt.restriccion1<>"" THEN "1" ELSE "2" ) '"));' skip.
        END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION muestracli W-Win 
FUNCTION muestracli RETURNS LOGICAL
  (  ) :
/*------------------------------------------------------------------------------
  Purpose:  
...
------------------------------------------------------------------------------*/
  ...
  chMundo:WebBrowser:Navigate( genehtml() ).
    ...
    w-win:MOVE-TO-TOP().
  RETURN AVAILABLE cliente.   /* Function return value. */
