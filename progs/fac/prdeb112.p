...
DEFINE VARIABLE v-comprobante   AS CHARACTER FORMAT "X(16)" INITIAL "NOTA DE DEBITO".
DEFINE VARIABLE que_articulo    AS CHARACTER FORMAT "X(8)".
DEFINE VARIABLE que_descripcion LIKE Articulo.descripcion FORMAT "X(40)".
...
DEFINE VARIABLE que_texto       AS CHARACTER FORMAT "X(12)".
...
FORM
    SKIP(3)
    "NOTA DE DEBITO" AT 72
    Fac_header.fecha       AT 100
    SKIP(6)
    Fac_header.nombre      AT 18  
    Cliente.cdg_cliente    AT 80
    "(" Vendedor.cdg_vendedor ")"
    SKIP
    Fac_header.direccion   AT 18 FORMAT "X(30)"
    Fac_header.localidad   AT 80
    SKIP
    Provincia.nombre       AT 18
    Fac_header.cdg_postal  AT 80 FORMAT "X(12)"
    SKIP(2)
    "IVA" AT 13
    Condicion_impos.texto  AT 18
    "C.U.I.T." AT 70
    Fac_header.cuit        AT 80 FORMAT "X(15)"
    SKIP(1)
    Condicion_venta.descripcion  AT 22 FORMAT "X(15)"
...
/*    Fac_header.leyenda VIEW-AS EDITOR SIZE 65 BY 3 AT 7*/
    Fac_header.imp_bruto AT 103 FORMAT "Z,ZZZ,ZZ9.99-"

    "IMPORTANTE=========================================" AT 7
    v-descuentos AT 65 
    v-desc AT 103 FORMAT "Z,ZZZ,ZZ9.99-"

    "Este importe equivale a dolares estadounidenses" AT 7
    Fac_header.imp_neto  AT 103 FORMAT "Z,ZZZ,ZZ9.99-"

    "y se cancelara el dia de su efectivo pago." AT 7
    prciva AT 98
    importe_iva AT 103 FORMAT "Z,ZZZ,ZZ9.99-"

    "Los cheques el dia de su acreditacion" AT 7
    prcnoi AT 98
    importe_noi AT 103 FORMAT "Z,ZZZ,ZZ9.99-"

    SKIP

    Fac_header.imp_total AT 103 FORMAT "Z,ZZZ,ZZ9.99-"
...
FORM
/*
    Fac_detalle.cantidad AT 2 FORMAT "->>,>>9"
    Articulo.cdg_articulo AT 16 FORMAT "X(6)" 
    Articulo.descripcion
    v-bonifs  AT 66
    Fac_detalle.precio_cf  AT 89
    Fac_detalle.subtotal_bruto_cf AT 103 FORMAT "Z,ZZZ,ZZ9.99-"
*/
    que_cantidad AT 2
    que_articulo AT 16 FORMAT "X(6)"   
    que_descripcion
    v-bonifs  AT 66
    que_precio      AT 89
    que_subtotal AT 105
...
OUTPUT TO VALUE("c:\sic-temp\prfab112.txt") PAGE-SIZE 72.
...
END.    
v-descuentos = "Descuentos " + SUBSTRING(v-descuentos,2).
...
FOR EACH Fac_detalle OF Fac_header 
    ...
    THEN DO:
            ...
            ASSIGN
                   ...
                   que_cantidad    = STRING(v-cantidad,"ZZZZ9")
                   que_precio      = STRING(Fac_detalle.precio_cf,"ZZZZZZ9.99")
                   que_subtotal    = STRING(v-subtotal, "ZZZZZZ9.99")
...
ELSE DO:
    DISPLAY
        Fac_header.imp_total @ Fac_header.imp_bruto
        Fac_header.imp_total
        Fac_header.prf_comprob
        Fac_header.nro_comprob
        WITH FRAME frm-pie.
...
OUTPUT CLOSE.
   ...
   RUN _osprint.p ( INPUT  CURRENT-WINDOW:HANDLE, /* HANDLE de la WINDOW    */
                    INPUT  "c:\sic-temp\prfab112.txt",           /* Archivo a imprimir     */
