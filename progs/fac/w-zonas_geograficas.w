...
DEFINE VARIABLE h_b-zonas_geograficas AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-zona_geografica AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-zonas_geograficas.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-zonas_geograficas ).
       RUN set-position IN h_b-zonas_geograficas ( 1.54 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 20.46 , 14.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-zona_geografica.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-zona_geografica ).
       RUN set-position IN h_v-zona_geografica ( 1.81 , 23.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.77 , 59.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 7.19 , 23.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 13.19 , 59.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-zona_geografica. */
       RUN add-link IN adm-broker-hdl ( h_b-zonas_geograficas , 'Record':U , h_v-zona_geografica ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-zona_geografica ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-zona_geografica ,
             h_b-zonas_geograficas , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-zona_geografica , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Mantenimiento de Zonas Geográficas"}
