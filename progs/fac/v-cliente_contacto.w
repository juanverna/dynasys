...
Persona.cdg_postal Persona.localidad Persona.direccion Persona.provincia ~
Persona.email Cliente-contacto.Canal-email Cliente-contacto.cdg_cargo ~
Persona.observacion Persona.palabras Cliente-contacto.observaciones ~
Persona.fecha_grab 
&Scoped-define ENABLED-TABLES Persona Cliente-contacto
...
Persona.observacion Persona.palabras Cliente-contacto.observaciones ~
Persona.fecha_grab 
&Scoped-define DISPLAYED-TABLES Persona Cliente-contacto
...
DEFINE FRAME F-Main
     Persona.tratamiento AT ROW 1.24 COL 10.4 COLON-ALIGNED WIDGET-ID 20
          LABEL "Trat."
          VIEW-AS FILL-IN NATIVE 
          SIZE 15.6 BY 1 TOOLTIP "Sr.,Sra.,Ing,Dr....."
          BGCOLOR 15 FGCOLOR 9 
     Persona.nombre AT ROW 1.24 COL 36 COLON-ALIGNED
          VIEW-AS FILL-IN NATIVE 
          SIZE 91 BY 1 TOOLTIP "Nombre del contacto"
          BGCOLOR 15 FGCOLOR 9 
     Persona.cdg_postal AT ROW 2.43 COL 10 COLON-ALIGNED
          VIEW-AS FILL-IN NATIVE 
          SIZE 14 BY 1 TOOLTIP "Codigo Postal"
          BGCOLOR 15 FGCOLOR 9 
     Persona.localidad AT ROW 2.43 COL 36 COLON-ALIGNED
          VIEW-AS FILL-IN NATIVE 
          SIZE 26 BY 1
          BGCOLOR 15 FGCOLOR 9 
     Persona.direccion AT ROW 2.43 COL 74 COLON-ALIGNED
          VIEW-AS FILL-IN NATIVE 
          SIZE 53 BY 1 TOOLTIP "Direccion Particular"
          BGCOLOR 15 FGCOLOR 9 
     BUTTON-6 AT ROW 3.62 COL 58.6 WIDGET-ID 96
     Persona.provincia AT ROW 3.62 COL 74 COLON-ALIGNED FORMAT "X(25)"
          VIEW-AS FILL-IN NATIVE 
     ...
     Persona.email AT ROW 4.91 COL 74 COLON-ALIGNED
     ...
     Cliente-contacto.Canal-email AT ROW 6 COL 74 COLON-ALIGNED WIDGET-ID 112
          VIEW-AS FILL-IN 
          SIZE 53 BY 1 TOOLTIP "Tipos de email que acepta *-todos !-ninguno"
     ...
     Cliente-contacto.cdg_cargo AT ROW 7.05 COL 74.6 COLON-ALIGNED
          VIEW-AS COMBO-BOX INNER-LINES 5
          LIST-ITEM-PAIRS "Item 1","Item 1"
          DROP-DOWN-LIST
          SIZE 25 BY 1
          BGCOLOR 15 FGCOLOR 9 
     Persona.observacion AT ROW 8.14 COL 15 COLON-ALIGNED
          LABEL "Obs.Persona" FORMAT "X(256)"
          VIEW-AS FILL-IN NATIVE 
          SIZE 113 BY .95
          BGCOLOR 15 FGCOLOR 9 
     Persona.palabras AT ROW 9.52 COL 15 COLON-ALIGNED WIDGET-ID 26 FORMAT "X(256)"
          VIEW-AS FILL-IN 
          SIZE 113 BY 1
     Cliente-contacto.observaciones AT ROW 10.91 COL 3 WIDGET-ID 24
          ...
          SIZE 113 BY 1
          BGCOLOR 15 FGCOLOR 9 
     Persona.fecha_grab AT ROW 7.19 COL 110 COLON-ALIGNED
          LABEL "Grab."
           VIEW-AS TEXT 
          SIZE 17 BY .62
...
/* SETTINGS FOR FILL-IN Persona.email IN FRAME F-Main
   EXP-LABEL EXP-FORMAT                                                 */
/* SETTINGS FOR FILL-IN Persona.fecha_grab IN FRAME F-Main
   EXP-LABEL                                                            */
/* SETTINGS FOR FILL-IN Persona.observacion IN FRAME F-Main
   EXP-LABEL EXP-FORMAT                                                 */
/* SETTINGS FOR FILL-IN Cliente-contacto.observaciones IN FRAME F-Main
   ALIGN-L EXP-LABEL EXP-FORMAT                                         */
/* SETTINGS FOR FILL-IN Persona.palabras IN FRAME F-Main
   EXP-FORMAT                                                           */
/* SETTINGS FOR FILL-IN Persona.provincia IN FRAME F-Main
   EXP-FORMAT                                                           */
/* SETTINGS FOR FILL-IN Persona.tratamiento IN FRAME F-Main
   EXP-LABEL                                                            */
...
&Scoped-define SELF-NAME Persona.email
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Persona.email V-table-Win
ON LEAVE OF Persona.email IN FRAME F-Main /* E-mail */
DO:
  IF persona.email:INPUT-VALUE <> "" THEN do:
      IF num-entries(persona.email:INPUT-VALUE,"@") <> 2 THEN DO:
          RUN ponmensj.p ( INPUT "EMAIL01" ).
          RETURN NO-APPLY.
      END.
      IF NUM-ENTRIES( entry( 2 , persona.email:INPUT-VALUE , "@" ) , ".") < 2 THEN DO:
...
&Scoped-define SELF-NAME Persona.nombre
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Persona.nombre V-table-Win
ON HELP OF Persona.nombre IN FRAME F-Main /* Nombre */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Persona.nombre V-table-Win
ON MOUSE-MENU-DOWN OF Persona.nombre IN FRAME F-Main /* Nombre */
DO:
    /*
  ...
  THEN DO:
      ...
      IF AVAILABLE persona THEN DO:
        BUFFER-COPY Persona TO persona.
        RUN mostrar_persona.
      END.
   END.*/
END.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
     ...
     IF AVAILABLE cargo_persona THEN
      ...
      ASSIGN persona.tratamiento:SCREEN-VALUE = ""
             persona.nombre:SCREEN-VALUE = ""
             persona.direccion:SCREEN-VALUE = Domicilio.direccion
             persona.localidad:SCREEN-VALUE = Domicilio.localidad
             persona.cdg_postal:SCREEN-VALUE = Domicilio.cdg_postal
             persona.observacion:SCREEN-VALUE = ""
             persona.email:SCREEN-VALUE = ""
             persona.provincia:SCREEN-VALUE = IF AVAILABLE provincia THEN provincia.nombre ELSE ""
             vte-1:SCREEN-VALUE = ""
             vte-2:SCREEN-VALUE = ""
             vte-3:SCREEN-VALUE = ""
             vte-4:SCREEN-VALUE = "".
...
------------------------------------------------------------------------------*/
  ...
  IF persona.nombre:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "" THEN DO:
...
DO i = 1 TO 4:
    persona.numeros_telefono = persona.numeros_telefono + "|" +
                                 STRING(thandle("te-" + STRING(i,"9")):SCREEN-VALUE ) +
                                 "!" +
                                 STRING(thandle("vte-" + STRING(i,"9")):SCREEN-VALUE ).
END.
persona.numeros_telefono = SUBSTRING(persona.numeros_telefono,2 ).
...
DEF VAR i AS INT NO-UNDO.
DEF VAR aux AS CHAR NO-UNDO.
DEFINE VAR auxte AS CHAR NO-UNDO.

/*elimina los posible errores de migraciones y/o cargas*/
IF NOT AVAILABLE persona THEN RETURN.
auxte = persona.numeros_telefono.
IF INDEX(persona.numeros_telefono,"!") = 0 THEN DO:
    RUN getparametro_c.p ("CDG-TE1",OUTPUT aux).
    auxte = replace(aux + "!" + persona.numeros_telefono,"|","%").
END.

DO i = 1 TO 4:
     IF i <= NUM-ENTRIES(auxte,"|") THEN DO:
          thandle("te-" + STRING(i,"9")):SCREEN-VALUE = entry(1,ENTRY(i,auxte,"|"),"!").
          thandle("vte-" + STRING(i,"9")):SCREEN-VALUE = entry(2,ENTRY(i,auxte,"|"),"!").
     END.
     ELSE DO:
          RUN getparametro_c.p ("CDG-TE" + STRING(i,"9"),OUTPUT aux).
          thandle("te-" + STRING(i,"9")):SCREEN-VALUE = aux.
