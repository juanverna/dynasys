...
&Scoped-Define ENABLED-OBJECTS btn-buscar 
...
DO:
  ...
  FOR EACH punto-venta WHERE punto-venta.habil AND
      punto-venta.tp = "E"  AND impresor = "E" :
...
OCXFile = SEARCH( "w-CRprinfacturas1.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-CRprinfacturas1.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  IF NOT hay_error  THEN DO:
      ...
      FIND FIRST fac_header 
              ...
              AND Fac_header.prf_comprob  = v-prf_comprob
              AND Fac_header.nro_comprob  <= v-has_nrocomprob
              AND Fac_header.nro_comprob  >= v-des_nrocomprob NO-LOCK NO-ERROR.
