...
DEFINE VARIABLE h_v-rng-archivo AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-rng-fechas AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-rng-usuario AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 18.62 COL 8 COLON-ALIGNED NO-LABEL
     "     Lista de empresas a considerar en el proceso" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 10.29 COL 10
          BGCOLOR 5 FGCOLOR 15 
     "     Archivo en que se genera la salida" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 5.76 COL 10
          BGCOLOR 5 FGCOLOR 15 
     "     Período a Listar" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 1.48 COL 10
          BGCOLOR 5 FGCOLOR 15 
     "     Rango de usuarios a considerar" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 14.33 COL 10
          BGCOLOR 5 FGCOLOR 15 
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Listar Auditoría de Accesos"
         HEIGHT             = 22.62
         WIDTH              = 115.8
         MAX-HEIGHT         = 22.95
         MAX-WIDTH          = 116.8
         VIRTUAL-HEIGHT     = 22.95
         VIRTUAL-WIDTH      = 116.8
         RESIZE             = no
         SCROLL-BARS        = no
         STATUS-AREA        = no
...
ON END-ERROR OF W-Win /* Listar Auditoría de Accesos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Listar Auditoría de Accesos */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-fechas ( 2.76 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng-archivo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-archivo ).
       RUN set-position IN h_v-rng-archivo ( 6.95 , 10.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-lst-empresas ( 11.48 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng-usuario.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-usuario ).
       RUN set-position IN h_v-rng-usuario ( 15.52 , 10.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 20.05 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.62 , 100.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-fechas ,
             v-estado:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-archivo ,
             h_v-rng-fechas , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-lst-empresas ,
             h_v-rng-archivo , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-usuario ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/SEG" "Listar Auditoría de Accesos"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE des_usuario      LIKE Usuario.cdg_usuario INITIAL "000000".
  DEFINE VARIABLE has_usuario      LIKE Usuario.cdg_usuario INITIAL "999999".
  ...
  RUN dar_rango IN h_v-rng-usuario   ( OUTPUT des_usuario, 
                                       OUTPUT has_usuario, 
                                       OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Usuarios" ).
       hay_error = error_rango.
  END.     
  ...
  RUN dar_rango IN h_v-rng-archivo ( OUTPUT v-archivo, 
                                     OUTPUT error_rango ).

  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Archivo" ).
       hay_error = error_rango.
  END.     
    ...
    lRegistros = FALSE.
    FOR EACH Usuario  
     WHERE Usuario.cdg_usuario <= has_usuario 
       AND Usuario.cdg_usuario >= des_usuario NO-LOCK,
      EACH logmenu OF Usuario
         WHERE Logmenu.fch_desde <= has_fecha 
           AND Logmenu.fch_desde >= des_fecha 
           AND LOOKUP(Logmenu.cdg_empresa, lista_empresas) <> 0 
           NO-LOCK:
        lRegistros = TRUE.
    END.
    IF NOT lRegistros THEN DO:
        MESSAGE "No existen registros para ese período de esos usuarios"
            VIEW-AS ALERT-BOX.
        hay_error = TRUE.
    END.
  ...
  THEN DO:
       ...
       RUN listar_auditoria.p ( INPUT des_fecha,
                                INPUT has_fecha,
                                INPUT des_usuario,
                                INPUT has_usuario,
                                INPUT lista_empresas, 
                                INPUT v-archivo
                            ).
...
------------------------------------------------------------------------------*/
    ...
    RUN veresult.w ( INPUT  "c:\sic-temp\genuniversal.txt",  /* Archivo */
