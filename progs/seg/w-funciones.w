...
DEFINE VARIABLE h_b-funciones AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-funcion AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Mantenimiento de Funciones"
         HEIGHT             = 21.24
         WIDTH              = 114.2
         MAX-HEIGHT         = 21.24
         MAX-WIDTH          = 114.2
...
ON END-ERROR OF W-Win /* Mantenimiento de Funciones */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Mantenimiento de Funciones */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Mantenimiento de Funciones */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-funciones.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-funciones ).
       RUN set-position IN h_b-funciones ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-funciones ( 20.95 , 15.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-funcion ).
       RUN set-position IN h_v-funcion ( 1.00 , 19.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.33 , 71.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 5.76 , 19.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.43 , 71.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-funcion. */
       RUN add-link IN adm-broker-hdl ( h_b-funciones , 'Record':U , h_v-funcion ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-funcion ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/SEG" "Mantenimiento de Funciones"}
...
*/
    ...
    IF p-operacion = "HABILITAR" 
    THEN DO: 
         IF VALID-HANDLE(h_b-usuarios)          THEN RUN set-sensitivo IN h_b-usuarios          ( INPUT YES ).
    ...
    END.     
    ELSE DO: 
         IF VALID-HANDLE(h_b-usuarios)          THEN RUN set-sensitivo IN h_b-usuarios          ( INPUT NO ).
    ...
    END.     
*/
    IF VALID-HANDLE(h_b-funciones)          THEN RUN set-sensitivo IN h_b-funciones         ( INPUT p-operacion = "HABILITAR" ).
