...
PROCEDURE calcular_saldo_presup:
   ...
   FOR EACH Aps_detalle OF Ctapsp 
       WHERE /*Aps_detalle.cdg_empresa = Empresa.cdg_empresa
         AND   Aps_detalle.fecha_mayor >= DATE(MONTH(des_fecha),1,YEAR(des_fecha)) 
...
PROCEDURE calcular_saldo_real:
   ...
   FOR EACH Asn_detalle OF Cuenta 
       WHERE Asn_detalle.cdg_empresa = Empresa.cdg_empresa /*
         AND Asn_detalle.fecha_mayor >= DATE(MONTH(des_fecha),1,YEAR(des_fecha)) */
