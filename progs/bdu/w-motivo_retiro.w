...
/* External Tables                                                      */
&Scoped-define EXTERNAL-TABLES Motivo_retiro
&Scoped-define FIRST-EXTERNAL-TABLE Motivo_retiro


/* Need to scope the external tables to this procedure                  */
DEFINE QUERY external_tables FOR Motivo_retiro.
...
DEFINE VARIABLE h_b-motivos_retiro AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-motivo_retiro AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     Btn_Done AT ROW 1.48 COL 144
     RECT-1 AT ROW 1.24 COL 143
...
/* Settings for THIS-PROCEDURE
   Type: SmartWindow
   External Tables: sic.Motivo_retiro
...
ON END-ERROR OF W-Win /* motivos de retiro */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* motivos de retiro */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* motivos de retiro */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-motivo_retiro.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-motivo_retiro ).
       RUN set-position IN h_v-motivo_retiro ( 1.24 , 28.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.43 , 109.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-motivos_retiro.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-motivos_retiro ).
       RUN set-position IN h_b-motivos_retiro ( 1.29 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-motivos_retiro ( 23.33 , 22.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 8.38 , 28.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.14 , 109.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-motivo_retiro. */
       RUN add-link IN adm-broker-hdl ( h_b-motivos_retiro , 'Record':U , h_v-motivo_retiro ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-motivo_retiro ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-motivo_retiro ,
             Btn_Done:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-motivos_retiro ,
             h_v-motivo_retiro , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/row-list.i "Motivo_retiro"}

  /* Get the record ROWID's from the RECORD-SOURCE.                  */
  {src/adm/template/row-get.i}

  /* FIND each record specified by the RECORD-SOURCE.                */
  {src/adm/template/row-find.i "Motivo_retiro"}
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/BDU" "Mantenimiento de Motivos de Retiros"}
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "Motivo_retiro"}
...
/*     END.                                                                      */
   ...
   IF VALID-HANDLE(h_b-motivos_retiro) THEN RUN set-sensitivo IN h_b-motivos_retiro ( INPUT p-operacion = "HABILITAR" ).
