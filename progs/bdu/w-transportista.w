...
DEFINE VARIABLE h_b-transportistas AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-transportista AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-transportistas.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-transportistas ).
       RUN set-position IN h_b-transportistas ( 1.24 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-transportistas ( 23.33 , 22.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-transportista.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-transportista ).
       RUN set-position IN h_v-transportista ( 1.24 , 28.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.76 , 107.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 13.14 , 28.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.14 , 105.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-transportista. */
       RUN add-link IN adm-broker-hdl ( h_b-transportistas , 'Record':U , h_v-transportista ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-transportista ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-transportistas ,
             Btn_Done:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-transportista ,
             h_b-transportistas , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/BDU" "Mantenimiento de Transportistas"}
...
/*     END.                                                                      */
   ...
   IF VALID-HANDLE(h_b-transportistas) THEN RUN set-sensitivo IN h_b-transportistas ( INPUT p-operacion = "HABILITAR" ).
