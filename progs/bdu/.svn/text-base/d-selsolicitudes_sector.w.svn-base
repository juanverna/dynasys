...
&Scoped-define OPEN-QUERY-BROWSE-2 OPEN QUERY {&BROWSE-NAME}    FOR EACH Sre_header                   WHERE Sre_header.cdg_empresa = que_empresa                     AND Sre_header.nro_area    = que_area                     AND Sre_header.tip_comprob MATCHES lista_tipos                     AND Sre_header.fecha_ingreso >= v-des_fecha                     AND Sre_header.fecha_ingreso <= v-has_fecha                     AND CAN-DO(lista_estados, ~
...
DEFINE FRAME D-Dialog
     ...
     v-des_fecha AT ROW 1.43 COL 6 COLON-ALIGNED
     v-has_fecha AT ROW 1.43 COL 24 COLON-ALIGNED
...
OPEN QUERY {&BROWSE-NAME}
   FOR EACH Sre_header
                  WHERE Sre_header.cdg_empresa = que_empresa
                    ...
                    AND Sre_header.tip_comprob MATCHES lista_tipos
                    AND Sre_header.fecha_ingreso >= v-des_fecha
                    AND Sre_header.fecha_ingreso <= v-has_fecha
                    AND CAN-DO(lista_estados,Sre_header.cdg_estado) /**/,
                        FIRST Area OF Sre_header.
     _END_FREEFORM
     _Query            is OPENED
*/  /* BROWSE BROWSE-2 */
