...
DEFINE VARIABLE h_v-rng-motivo AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 18.38 COL 22 COLON-ALIGNED NO-LABEL
     "  Rango de motivos de retiro:" VIEW-AS TEXT
          SIZE 109 BY .81 AT ROW 3.62 COL 24
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Listado de Motivos de Retiro"
         HEIGHT             = 27.38
         WIDTH              = 160
         MAX-HEIGHT         = 27.38
         MAX-WIDTH          = 160
...
ON END-ERROR OF W-Win /* Listado de Motivos de Retiro */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Listado de Motivos de Retiro */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng-motivo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-motivo ).
       RUN set-position IN h_v-rng-motivo ( 5.05 , 24.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.57 , 109.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 20.05 , 24.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.62 , 100.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-motivo ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/BDU" "Listado de Motivos de Retiro"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE des_motivo       LIKE Motivo_retiro.cdg_motivo.
  DEFINE VARIABLE has_motivo       LIKE Motivo_retiro.cdg_motivo.
  DEFINE VARIABLE error_rango      AS LOGICAL.
  DEFINE VARIABLE hay_error        AS LOGICAL.
  

  RUN dar_rango IN h_v-rng-motivo    ( OUTPUT des_motivo, 
                                       OUTPUT has_motivo, 
                                       OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Motivos de retiro" ).
  ...
  THEN DO:
       ...
       RUN lsmotivos_retiro.p (INPUT  des_motivo,
                               INPUT  has_motivo).
