...
&Scoped-define INTERNAL-TABLES Sre_header

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table Sre_header.nro_comprob 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Sre_header WHERE ~{&KEY-PHRASE} ~
      AND Sre_header.cdg_empresa  = que_empresa ~
AND Sre_header.nro_area = que_area NO-LOCK ~
    BY Sre_header.nro_solicitud DESCENDING INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br_table Sre_header
&Scoped-define FIRST-TABLE-IN-QUERY-br_table Sre_header
...
RUN set-attribute-list (
    'Keys-Accepted = "cdg_grupoemp",
     Keys-Supplied = "cdg_grupoemp,cdg_cliente,nro_cliente,cdg_condiva,cdg_postal,nro_entidad,cdg_estado,cdg_famclie,nro_proveedor,cdg_provincia,num_sucursal,cdg_tipoclie,nro_vendedor"':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Sre_header.nro_comprob FORMAT "ZZZZZZZ9":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 14 BY 22.24
         BGCOLOR 11 FGCOLOR 9 FONT 4 ROW-HEIGHT-CHARS .52.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "sic.Sre_header"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE"
     _TblOptList       = ", FIRST"
     _OrdList          = "sic.Sre_header.nro_solicitud|no"
     _Where[1]         = "Sre_header.cdg_empresa  = que_empresa
AND Sre_header.nro_area = que_area"
     _FldNameList[1]   = sic.Sre_header.nro_comprob
...
DEFINE VARIABLE es_header AS LOGICAL INITIAL YES.
{hab_panel_folder.i}
...
DO:
    ...
    RUN d-selsolicitudes_sector.w (INPUT "Seleccion de solicitudes de retiro", 
                                   INPUT "*", 
                                   INPUT "SR", 
                                   INPUT-OUTPUT rid_factura).
    
    IF rid_factura <> ?
    THEN DO:
        FIND Sre_header WHERE ROWID(sre_header) = rid_factura NO-LOCK .
        REPOSITION {&BROWSE-NAME} TO ROWID rid_factura.
        DISPLAY Sre_header.nro_solicitud @ v-codigo 
...
DO:
  ...
  &SCOPED-DEFINE CODIGO nro_comprob
  ...
  THEN DO:
    ...
    FIND FIRST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}
         WHERE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.{&CODIGO}  >= v-codigo
           AND {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.prf_comprob = 0
           AND {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.tip_comprob = "SR"
           AND {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.cdg_empresa = Usuario.cdg_empresa
           AND {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.nro_area    = que_area
                NO-LOCK NO-ERROR.
    IF NOT AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}
       THEN FIND LAST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}
             WHERE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.prf_comprob = 0
               AND {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.tip_comprob = "SR"
               AND {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.cdg_empresa = Usuario.cdg_empresa
               AND {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.nro_area    = que_area
                       NO-LOCK.
