...
&THEN
DEFINE VARIABLE p-que_area LIKE Area.nro_area.
...
&ELSE
DEFINE INPUT  PARAMETER p-que_area   LIKE Area.nro_area.
...
&Scoped-define OPEN-QUERY-br_articulos     CASE buscar_por:          WHEN "NUMERO"          THEN DO:               que_numero = clave_buscar.               OPEN QUERY br_grupos                    FOR EACH Articulo WHERE Articulo.cdg_articulo BEGINS que_numero                                        AND Articulo.cdg_estado = v-estado                                        AND CAN-DO(Articulo.lista_sectores, ~
       que_area)                                        AND CAN-DO(Articulo.lista_empresas, ~
       que_empresa)                                            NO-LOCK.     /*          BROWSE {&BROWSE-NAME}:TITLE = "Artículos que satisfacen la condición de búsqueda: POR NRO.AFILIADO=" + que_numero.*/               que_numero = "".           END.           WHEN "NOMBRE"          THEN DO:               que_nombre = clave_buscar.               IF SUBSTRING(que_nombre, ~
      LENGTH(que_nombre), ~
      1) <> "*"                  THEN que_nombre = que_nombre + "*".               OPEN QUERY {&BROWSE-NAME}                    FOR EACH Articulo WHERE Articulo.descripcion CONTAINS que_nombre                                        AND Articulo.cdg_estado = v-estado                                        AND CAN-DO(Articulo.lista_sectores, ~
       que_area)                                        AND CAN-DO(Articulo.lista_empresas, ~
       que_empresa)                                            NO-LOCK.     /*          BROWSE {&BROWSE-NAME}:TITLE = "Artículos que satisfacen la condición de búsqueda: POR NOMBRE=" + que_nombre.*/               que_nombre = "".            END.           OTHERWISE          DO:                OPEN QUERY {&BROWSE-NAME}                    FOR EACH Articulo NO-LOCK WHERE Articulo.cdg_estado = v-estado                                        AND CAN-DO(Articulo.lista_sectores, ~
       que_area)                                        AND CAN-DO(Articulo.lista_empresas, ~
       que_empresa)                                            INDEXED-REPOSITION.           END.      END CASE.
...
&Scoped-Define ENABLED-OBJECTS Btn_Done-2 que_nombre v-estado Btn_Salir ~
que_numero btn_todos br_articulos 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_articulos C-Win _FREEFORM
  QUERY br_articulos NO-LOCK DISPLAY
      ...
      Articulo.descripcion      COLUMN-LABEL "Descripcion!Asociada" FORMAT "X(80)"
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 136 BY 18.33
         FONT 4
...
DEFINE FRAME DEFAULT-FRAME
     Btn_Done-2 AT ROW 1.24 COL 115
     que_nombre AT ROW 1.29 COL 12 COLON-ALIGNED
     v-estado AT ROW 2.38 COL 97 NO-LABEL
     Btn_Salir AT ROW 2.57 COL 115
     que_numero AT ROW 2.62 COL 12 COLON-ALIGNED
     btn_todos AT ROW 2.62 COL 83
     br_articulos AT ROW 3.95 COL 2
     "  Ver Productos" VIEW-AS TEXT
          SIZE 17 BY .81 AT ROW 1.24 COL 96
...
ASSIGN 
       br_articulos:POPUP-MENU IN FRAME DEFAULT-FRAME             = MENU POPUP-MENU-br_afiliados:HANDLE.
    ...
    CASE buscar_por:
         ...
         THEN DO:
              ...
              OPEN QUERY br_grupos
                   FOR EACH Articulo WHERE Articulo.cdg_articulo BEGINS que_numero
                                       AND Articulo.cdg_estado = v-estado
                                       AND CAN-DO(Articulo.lista_sectores, que_area)
    ...
    /*          BROWSE {&BROWSE-NAME}:TITLE = "Artículos que satisfacen la condición de búsqueda: POR NRO.AFILIADO=" + que_numero.*/
         ...
         THEN DO:
              ...
              OPEN QUERY {&BROWSE-NAME}
                   FOR EACH Articulo WHERE Articulo.descripcion CONTAINS que_nombre
                                       AND Articulo.cdg_estado = v-estado
                                       AND CAN-DO(Articulo.lista_sectores, que_area)
                                       AND CAN-DO(Articulo.lista_empresas, que_empresa)
                                           NO-LOCK.
...
FIND Area WHERE Area.nro_area = p-que_area NO-LOCK.
...
------------------------------------------------------------------------------*/
    ...
    CASE buscar_por:
         ...
         THEN DO:
              OPEN QUERY {&BROWSE-NAME}
                   FOR EACH Articulo WHERE Articulo.cdg_articulo BEGINS clave_buscar
                                       AND Articulo.cdg_estado = v-estado
                                       AND CAN-DO(Articulo.lista_sectores, que_area)
                                       AND CAN-DO(Articulo.lista_empresas, que_empresa)
                                           NO-LOCK.
         ...
         THEN DO:
              OPEN QUERY {&BROWSE-NAME}
                   FOR EACH Articulo WHERE Articulo.descripcion CONTAINS clave_buscar
                                       AND Articulo.cdg_estado = v-estado
                                       AND CAN-DO(Articulo.lista_sectores, que_area)
                                       AND CAN-DO(Articulo.lista_empresas, que_empresa)
                                           NO-LOCK.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE Btn_Done-2 que_nombre v-estado Btn_Salir que_numero btn_todos 
         br_articulos 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
