...
&Scoped-Define DISPLAYED-OBJECTS des_nombre des_registro has_registro ~
has_nombre 
...
ON + OF des_registro IN FRAME F-Main /* Desde Sector */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON MOUSE-MENU-DOWN OF des_registro IN FRAME F-Main /* Desde Sector */
DO:
  &SCOPED-DEFINE ROWID_TABLA        rid_dessector
  &SCOPED-DEFINE SELECCION          SELSECTR.P
  &SCOPED-DEFINE TABLA              Area
  &SCOPED-DEFINE CDG_TABLA          cdg_area
  &SCOPED-DEFINE DSC_TABLA          denominacion
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON RETURN OF des_registro IN FRAME F-Main /* Desde Sector */
DO:

    ASSIGN FRAME {&FRAME-NAME} des_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND Area WHERE Area.cdg_area = SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
        ELSE FIND FIRST Area NO-LOCK NO-ERROR.
    
    IF AVAILABLE Area 
    THEN DO:
         des_nombre   = Area.denominacion.
         des_registro = Area.cdg_area.
    END.
    ELSE DO:
         des_nombre = "???".
    END.
...
ON + OF has_registro IN FRAME F-Main /* Hasta Sector */
DO:
  APPLY "MOUSE-MENU-DOWN" TO SELF.
  RETURN NO-APPLY.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL has_registro V-table-Win
ON MOUSE-MENU-DOWN OF has_registro IN FRAME F-Main /* Hasta Sector */
DO:
  &SCOPED-DEFINE ROWID_TABLA        rid_hassector
  &SCOPED-DEFINE SELECCION          SELSECTR.P
  &SCOPED-DEFINE TABLA              Area
  &SCOPED-DEFINE CDG_TABLA          cdg_area
  &SCOPED-DEFINE DSC_TABLA          denominacion
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL has_registro V-table-Win
ON RETURN OF has_registro IN FRAME F-Main /* Hasta Sector */
DO:

    ASSIGN FRAME {&FRAME-NAME} has_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND Area WHERE Area.cdg_area = SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
        ELSE FIND LAST Area NO-LOCK NO-ERROR.
    
    IF AVAILABLE Area 
    THEN DO:
         has_nombre = Area.denominacion.
         has_registro = Area.cdg_area.
    END.
    ELSE DO:
         has_nombre = "???".
    END.
