...
FIND area WHERE cdg_area = "A.GA.CR".
p-que_area = Area.nro_area.
&ELSE
...
&Scoped-define OPEN-QUERY-br_articulos     CASE buscar_por:          WHEN "NUMERO"          THEN DO:                que_numero = clave_buscar.               OPEN QUERY br_grupos                    FOR EACH Articulo WHERE Articulo.cdg_articulo BEGINS que_numero                                        AND Articulo.cdg_estado = v-estado                                        AND Articulo.lista_sectores CONTAINS que_area                                        AND Articulo.lista_empresas CONTAINS que_empresa NO-LOCK.     /*          BROWSE {&BROWSE-NAME}:TITLE = "Artículos que satisfacen la condición de búsqueda: POR NRO.AFILIADO=" + que_numero.*/               que_numero = "".           END.           WHEN "NOMBRE"          THEN DO:                que_nombre = clave_buscar.               IF SUBSTRING(que_nombre, ~
      LENGTH(que_nombre), ~
      1) <> "*"                  THEN que_nombre = que_nombre + "*".               OPEN QUERY {&BROWSE-NAME}                    FOR EACH Articulo WHERE Articulo.descripcion CONTAINS que_nombre                                        AND Articulo.cdg_estado = v-estado                                        AND Articulo.lista_sectores CONTAINS que_area                                        AND Articulo.lista_empresas CONTAINS que_empresa                                            NO-LOCK.     /*          BROWSE {&BROWSE-NAME}:TITLE = "Artículos que satisfacen la condición de búsqueda: POR NOMBRE=" + que_nombre.*/               que_nombre = "".            END.           OTHERWISE          DO:                OPEN QUERY {&BROWSE-NAME}                    FOR EACH Articulo NO-LOCK WHERE Articulo.cdg_estado = v-estado                                        AND Articulo.lista_sectores CONTAINS que_area                                        AND Articulo.lista_empresas CONTAINS que_empresa.           END.      END CASE.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  {&WINDOW-NAME}:TITLE = "Búsqueda de Artículos para el sector " + Area.cdg_area.
  RUN enable_UI.    
