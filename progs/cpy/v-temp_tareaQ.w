...
DEFINE VAR cnro_tipo_evento LIKE tarea.nro_tipo_evento NO-UNDO.
...
&Scoped-Define ENABLED-FIELDS Tarea.fecha_prevista Tarea.accion ~
Tarea.hora_prevista Tarea.horas_estimadas Tarea.informa 
...
&Scoped-Define DISPLAYED-FIELDS Tarea.fecha_prevista Tarea.accion ~
Tarea.hora_prevista Tarea.horas_estimadas Tarea.informa 
&Scoped-define DISPLAYED-TABLES Tarea
&Scoped-define FIRST-DISPLAYED-TABLE Tarea
&Scoped-Define DISPLAYED-OBJECTS articulo imp_total sin_cargo factura ~
frecursos hora_fin texto_adic avisar impreOT prf v-texto 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 82 BY 4.05 ROW-HEIGHT-CHARS .68.
...
DEFINE FRAME F-Main
     ...
     articulo AT ROW 1.24 COL 19.2 COLON-ALIGNED WIDGET-ID 80
     imp_total AT ROW 1.24 COL 38.2 COLON-ALIGNED WIDGET-ID 4
     Tarea.accion AT ROW 1.24 COL 62 COLON-ALIGNED WIDGET-ID 116
          VIEW-AS COMBO-BOX INNER-LINES 5
          LIST-ITEM-PAIRS "0","0"
          DROP-DOWN-LIST
          SIZE 30 BY 1
     ...
     sin_cargo AT ROW 1.33 COL 4 WIDGET-ID 72
     factura AT ROW 1.33 COL 11.6 WIDGET-ID 74
     ...
     texto_adic AT ROW 3.14 COL 3 NO-LABEL WIDGET-ID 70
     BLeyenda AT ROW 3.62 COL 113 WIDGET-ID 94
     Tarea.horas_estimadas AT ROW 3.71 COL 139.8 COLON-ALIGNED WIDGET-ID 24
          LABEL "Durac." FORMAT ">>9"
          VIEW-AS FILL-IN 
          SIZE 9 BY 1
     avisar AT ROW 3.91 COL 99 WIDGET-ID 128
     impreOT AT ROW 4.76 COL 99 WIDGET-ID 132
     Bresuelto AT ROW 5.05 COL 113 WIDGET-ID 84
     prf AT ROW 5.05 COL 141 COLON-ALIGNED WIDGET-ID 78
     Tarea.informa AT ROW 5.62 COL 99 WIDGET-ID 134
          LABEL "Inf."
          VIEW-AS TOGGLE-BOX
          SIZE 7 BY .81 TOOLTIP "Si se informa en la agenda diaria"
     b-agrega AT ROW 6.48 COL 146 WIDGET-ID 52
     BROWSE-7 AT ROW 6.95 COL 3 WIDGET-ID 200
     v-texto AT ROW 6.95 COL 86 NO-LABEL WIDGET-ID 50
     "       Descripcion adicional" VIEW-AS TEXT
          SIZE 90 BY .52 AT ROW 2.43 COL 3.4 WIDGET-ID 86
...
/* BROWSE-TAB BROWSE-7 b-agrega F-Main */
ASSIGN 
...
DO:
   ...
   IF articulo <> "" THEN DO:
       ...
       IF tipo_evento.cdg_tipo_evento <> "LT" AND  tipo_evento.cdg_tipo_evento <> "RT" THEN DO:
                MESSAGE "El articulo no es valido para el tipo de tarea que quiere realizar"
                        VIEW-AS ALERT-BOX ERROR.
       END.
    
       /*tarea.nro_tipo_evento:SCREEN-VALUE IN FRAME {&FRAME-NAME}= string(tipo_evento.nro_tipo_evento).*/
   END.
END.
...
DO:
   ...
   IF articulo <> "" THEN DO:
       ...
       /*tarea.nro_tipo_evento:SCREEN-VALUE IN FRAME {&FRAME-NAME}= tipo_evento.cdg_tipo_evento.*/
   END.
END.
...
DO:
  ...
  IF sin_cargo THEN DO:
            ...
            /*tarea.nro_tipo_evento:SENSITIVE = TRUE.*/
  END.
  ELSE do:
        ...
        /*tarea.nro_tipo_evento:SENSITIVE = FALSE.*/
  END.
END.
...
FIND articulo WHERE articulo.cdg_articulo = articulo:INPUT-VALUE NO-LOCK NO-ERROR.
...
DO k = 1 TO num-entries(frecursos):
  ...
  FIND FIRST recurso_habilidad OF Recurso 
       WHERE can-do(string(tarea.nro_tipo_evento),string( recurso_habilidad.nro_tipo_evento)) NO-LOCK NO-ERROR.
...
vartemplate = "articulo|prf|frecursos|imp_servicio|cant_periodos|contrato|texto_adic|sin_cargo|impreOT|factura|hora_fin|avisar".
...
IF AVAILABLE tarea THEN DO: 
  ...
  FIND canal WHERE canal.nro_cliente = tarea.nro_administrador AND
      canal.nro_tipo_evento = tarea.nro_tipo_evento NO-LOCK NO-ERROR.
...
cnro_tipo_evento = int(ENTRY( 2 , SUBSTRING( lista , 2 ))).
IF AVAILABLE tarea THEN DISPLAY tarea.accion WITH FRAME {&FRAME-NAME}.
FOR EACH punto-venta NO-LOCK WHERE Punto-venta.Habilitado AND Punto-venta.cdg_empresa = Empresa.cdg_empresa:
