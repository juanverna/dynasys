...
&Scoped-define QUERY-STRING-Query-Main FOR EACH Contrato_hd OF Cliente WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH Contrato_hd OF Cliente WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main Contrato_hd
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "sic.Contrato_hd OF sic.Cliente"
...
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
  FIND contrato_hd WHERE contrato_hd.nro_contrato = pp NO-LOCK NO-ERROR.
  IF AVAILABLE contrato_hd THEN DO:
      FIND cliente OF contrato_hd.
        {&OPEN-QUERY-{&query-NAME}}
        
        RETURN  contrato_hd.nro_contrato.
  END.
  ELSE RETURN ?.
