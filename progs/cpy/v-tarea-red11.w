...
&Scoped-Define ENABLED-OBJECTS bVer Bhistoria 
&Scoped-Define DISPLAYED-FIELDS Tarea.nro_persona Tarea.email ~
Tarea.cdg_cargo Tarea.comunicarsepor Tarea.Visualizar Tarea.cdg_recurso ~
...
DEFINE VARIABLE v-cdg_administrador AS CHARACTER 
     ...
     LIST-ITEMS "---" 
     DROP-DOWN
     SIZE 16 BY 1 TOOLTIP "Seleccione administrador o dbl-click para buscar"
     BGCOLOR 15 FGCOLOR 9  NO-UNDO.
...
DEFINE FRAME sF-Main
     ...
     Tarea.Visualizar AT ROW 5.43 COL 111 COLON-ALIGNED WIDGET-ID 90
          VIEW-AS FILL-IN NATIVE 
...
RUN inicia_recursos(p).
/*es el 4 parametro los tres primeros estan fijos en el programa*/
tarea.cdg_recurso:SCREEN-VALUE IN FRAME {&FRAME-NAME}= ENTRY(4,tarea.cdg_recurso:LIST-ITEM-PAIRS ).
END PROCEDURE.
...
OCXFile = SEARCH( "v-tarea-red11.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "v-tarea-red11.wrx":U SKIP(1)
...
/*    IF AVAILABLE tarea THEN
       v-cdg_administrador:SENSITIVE = tarea.nro_administradoristrador = 0 AND yy.
    ELSE 
       v-cdg_administrador:SENSITIVE = yy.*/
END.
END PROCEDURE.
...
IF pvisualizar <> tarea.visualizar THEN DO:
    ...
    /*tarea.descripcion = saveAdvTexto("Cambio Visualizar de " + STRING( pvisualizar )+ " a " + STRING( tarea.visualizar ),TABLE tttexto).*/
END.
...
IF AVAILABLE cliente THEN DO:
  ...
  IF AVAILABLE badm THEN
    ...
    v-dsc_administrador:SCREEN-VALUE = badm.nom_cliente.
...
/*limpiar todo lo existe*/
vte-1:SCREEN-VALUE IN FRAME {&FRAME-NAME}= "".
vte-2:SCREEN-VALUE = "".
vte-3:SCREEN-VALUE = "".
vte-4:SCREEN-VALUE = "".
tarea.email:SCREEN-VALUE = "".
cdg_cargo:SCREEN-VALUE = "".
comunicarsepor:SCREEN-VALUE = "T".
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    IF AVAILABLE tarea THEN DO:
         IF tarea.nro_persona <> 0  THEN DO:
             FIND persona WHERE persona.nro_persona = tarea.nro_persona.
...
RUN get-link-handle IN adm-broker-hdl
  ...
  RUN dametipo IN hproc ( OUTPUT aux ) NO-ERROR.
...
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
  IF AVAILABLE administrador THEN RETURN administrador.nro_cliente.
