...
DEFINE VARIABLE NRO_RELEASE AS CHARACTER INITIAL "Rel. 1.0.0".
DEFINE BUFFER B-Clase_de_proyecto FOR Clase_de_proyecto.

DEFINE VARIABLE f-que_clase LIKE Clase_de_proyecto.cdg_subclasepry.
...
FORM           
   SKIP(0.2)   
   Clase_de_proyecto.nombre_subclasepry  COLON 16 FGCOLOR 9 BGCOLOR 15
   SKIP(0.2)   
   Clase_de_proyecto.rotulo_siguiente COLON 16 FGCOLOR 9 BGCOLOR 15
   SKIP(0.2)   
   Clase_de_proyecto.longitud_siguiente COLON 16 FGCOLOR 9 BGCOLOR 15
   Clase_de_proyecto.tipo_siguiente COLON 16 FGCOLOR 9
   SKIP(0.2)   
...
&Scoped-define INTERNAL-TABLES Clase_de_proyecto

/* Definitions for BROWSE brw_clasificacion                             */
&Scoped-define FIELDS-IN-QUERY-brw_clasificacion ~
SUBSTRING(Clase_de_proyecto.cdg_subclasepry,LENGTH(que_clase) + 2) ~
Clase_de_proyecto.nombre_subclasepry 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brw_clasificacion 
&Scoped-define QUERY-STRING-brw_clasificacion FOR EACH Clase_de_proyecto ~
      WHERE Clase_de_proyecto.cdg_clasepry = que_clase NO-LOCK
&Scoped-define OPEN-QUERY-brw_clasificacion OPEN QUERY brw_clasificacion FOR EACH Clase_de_proyecto ~
      WHERE Clase_de_proyecto.cdg_clasepry = que_clase NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brw_clasificacion Clase_de_proyecto
&Scoped-define FIRST-TABLE-IN-QUERY-brw_clasificacion Clase_de_proyecto
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brw_clasificacion C-Win _STRUCTURED
  QUERY brw_clasificacion DISPLAY
      SUBSTRING(Clase_de_proyecto.cdg_subclasepry,LENGTH(que_clase) + 2) COLUMN-LABEL "Código!Clase" FORMAT "X(8)":U
      Clase_de_proyecto.nombre_subclasepry COLUMN-LABEL "Denominacion!Clasificación" FORMAT "X(40)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 53 BY 19.12
         FONT 4
         TITLE "Clasificación".
...
DEFINE FRAME DEFAULT-FRAME
     que_subclase AT ROW 1.27 COL 2 NO-LABEL
     que_nombre AT ROW 1.27 COL 10 COLON-ALIGNED NO-LABEL
     que_clase AT ROW 1.27 COL 56 COLON-ALIGNED NO-LABEL
     brw_clasificacion AT ROW 2.35 COL 2
     camino AT ROW 2.35 COL 58 NO-LABEL
     btn_modificar AT ROW 21.73 COL 2
     btn_eliminar AT ROW 21.73 COL 29
     Btn_Elegir AT ROW 21.73 COL 58
     Btn_Salir AT ROW 21.73 COL 87
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Mantenimiento de Clases de Proyectos"
         HEIGHT             = 21.85
         WIDTH              = 113
         MAX-HEIGHT         = 22.19
         MAX-WIDTH          = 114.29
...
/* Query rebuild information for BROWSE brw_clasificacion
     _TblList          = "sic.Clase_de_proyecto"
     _Where[1]         = "Clase_de_proyecto.cdg_clasepry = que_clase"
     _FldNameList[1]   > "_<CALC>"
"SUBSTRING(Clase_de_proyecto.cdg_subclasepry,LENGTH(que_clase) + 2)" "Código!Clase" "X(8)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > sic.Clase_de_proyecto.nombre_subclasepry
"Clase_de_proyecto.nombre_subclasepry" "Denominacion!Clasificación" "X(40)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _Query            is OPENED
...
ON END-ERROR OF C-Win /* Mantenimiento de Clases de Proyectos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Mantenimiento de Clases de Proyectos */
DO:
...
DO:
   ...
   que_subclase = SUBSTRING(Clase_de_proyecto.cdg_subclasepry,LENGTH(que_clase) + 2).
...
DO:
   ...
   IF CAN-FIND(FIRST B-Clase_de_proyecto WHERE B-Clase_de_proyecto.cdg_clasepry = Clase_de_proyecto.cdg_subclasepry)
   THEN DO:
       RUN PONMENSJ.P (INPUT "CLAS001" ).
   END.    
   ELSE DO:                                                 
       DO TRANSACTION:
          FIND CURRENT Clase_de_proyecto EXCLUSIVE-LOCK.   
          DELETE Clase_de_proyecto.
       END.
       RUN ABRE_QUERY.
   END.    
...
DO:
   DO TRANSACTION:
        ...
        DISPLAY Clase_de_proyecto.nombre_subclasepry
...
DO:
   ...
   THEN DO:
      ...
      ELSE DO:
         ...
         THEN DO:
            p_punto = p_punto - 1.
            que_clase = SUBSTRING(que_clase,1,p_punto).
            FIND FIRST Clase_de_proyecto WHERE Clase_de_proyecto.cdg_subclasepry = que_clase.
            que_nombre = Clase_de_proyecto.nombre.
            RUN armar_rotulo.
         END.
         ELSE DO:
            que_clase = "".
   ...
   ELSE DO:
      ...
      FIND FIRST Clase_de_proyecto WHERE Clase_de_proyecto.cdg_clasepry = que_clase 
                         AND Clase_de_proyecto.cdg_subclasepry = que_clase + "." + que_subclase NO-ERROR.

      IF NOT AVAILABLE Clase_de_proyecto
      THEN DO:
         que_nombre = "".
         RUN crear_clasificacion.         
      END.
      ELSE DO:
         que_nombre = Clase_de_proyecto.nombre_subclasepry.
...
------------------------------------------------------------------------------*/
/*
  IF AVAILABLE Clase_de_proyecto
     THEN rotulo = FILL(" ",l_rotulo - LENGTH(Clase_de_proyecto.rotulo_siguiente) - 1) + Clase_de_proyecto.rotulo_siguiente + ":".           ELSE rotulo = FILL(" ",l_rotulo - 1) + ":".
*/
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE:
         ...
         CREATE Clase_de_proyecto.
         ASSIGN 
                Clase_de_proyecto.cdg_clasepry = que_clase                                 
                Clase_de_proyecto.cdg_subclasepry = que_clase + "." + que_subclase.
         RUN MODIFICAR_CLASIFICACION.       
...
------------------------------------------------------------------------------*/
   ...
   UPDATE Clase_de_proyecto.nombre_subclasepry
          Clase_de_proyecto.rotulo_siguiente
          Clase_de_proyecto.longitud_siguiente
          Clase_de_proyecto.tipo_siguiente
