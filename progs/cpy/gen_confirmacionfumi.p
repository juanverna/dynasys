...
{stavisado.i}
{tiempo.i}
OUTPUT TO c:\dynasys10\logs\gen_confirmacionfumi.LOG APPEND.
...
FIND restriccion WHERE restriccion.cdg_restriccion = "CONFFU" NO-LOCK NO-ERROR.
FIND tipo_evento WHERE tipo_evento.cdg_tipo_evento = "FU" NO-LOCK.
FIND FIRST recurso WHERE CAN-DO(recurso.habilidades,"Z@1") NO-LOCK NO-ERROR.
...
FOR EACH evento WHERE evento.nro_tipo_evento = tipo_evento.nro_tipo_evento AND
  ...
  FIND FIRST tarea OF evento WHERE tarea.cdg_tipotarea = "Z" AND 
  ...
  RUN crea_tarea.p ( evento.nro_evento , evento.nro_cliente , "Z" , "Confirmar Fumigacion " , "Confirmar FU:" + string(evento.fasignado)  , resta_dia_habil(evento.fasignado,3,"23456") ,"ENC", OUTPUT rok ).
END.
