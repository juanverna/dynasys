...
&Scoped-define QUERY-STRING-br_table FOR EACH Cta_cte OF Contrato_hd WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Imputacion OF Contrato_hd NO-LOCK
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Cta_cte OF Contrato_hd WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Imputacion OF Contrato_hd NO-LOCK.
...
&Scoped-Define ENABLED-FIELDS Contrato_hd.nro_contrato 
...
&Scoped-Define DISPLAYED-FIELDS Contrato_hd.nro_contrato 
...
nro_cobrador|y|y|sic.Cta_cte.nro_cobrador
nro_contrato||y|sic.Cta_cte.nro_contrato
cdg_imputacion||y|sic.Cta_cte.cdg_imputacion
nro_moneda||y|sic.Cta_cte.nro_moneda
nro_proveedor||y|sic.Cta_cte.nro_proveedor
nro_vendedor|y|y|sic.Cta_cte.nro_vendedor
tip_comprob||y|sic.Cta_cte.tip_comprob
mes||y|sic.Cta_cte.mes
cdg_empresa||y|sic.Cta_cte.cdg_empresa
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
RUN set-attribute-list (
    'Keys-Accepted = "nro_cobrador,nro_vendedor",
     Keys-Supplied = "nro_cliente,nro_cobrador,nro_contrato,cdg_imputacion,nro_moneda,nro_proveedor,nro_vendedor,tip_comprob,mes,cdg_empresa"':U).
...
DEFINE FRAME F-Main
     ...
     Contrato_hd.nro_contrato AT ROW 2.91 COL 23 COLON-ALIGNED WIDGET-ID 2
          VIEW-AS FILL-IN 
          SIZE 14 BY 1
...
/* BROWSE-TAB br_table nro_contrato F-Main */
ASSIGN 
       FRAME F-Main:SCROLLABLE       = FALSE
       FRAME F-Main:HIDDEN           = TRUE.

ASSIGN 
       des_fecha:READ-ONLY IN FRAME F-Main        = TRUE.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "sic.Cta_cte OF sic.Contrato_hd,sic.Imputacion OF sic.Contrato_hd"
...
"Cta_cte.cambio_dolar" "Valor!Dólar" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[12]   > sic.Cta_cte.clausula_dolar
"Cta_cte.clausula_dolar" ? "Si/No" "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[13]   > sic.Cta_cte.es_difcambio
"Cta_cte.es_difcambio" ? "Si/No" "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[14]   > sic.Cta_cte.leyenda
"Cta_cte.leyenda" ? "X(24)" "character" ? ? ? ? ? ? no ? no no "4.2" yes no no "U" "" ""
...
DO:
    ...
    DO TRANSACTION:
       CREATE Cta_cte.
       ASSIGN Cta_cte.nro_cliente = contrato_hd.nro_cliente
              Cta_cte.cdg_empresa = Empresa.cdg_empresa
              cta_cte.nro_contrato = contrato_hd.nro_contrato.
       DISPLAY Cta_cte WITH 2 COLUMNS SIDE-LABELS THREE-D VIEW-AS DIALOG-BOX
               FRAME f-actualiza.
       UPDATE  Cta_cte EXCEPT Cta_cte.nro_cliente Cta_cte.cdg_empresa cta_cte.nro_contrato
               WITH 2 COLUMNS SIDE-LABELS THREE-D VIEW-AS DIALOG-BOX
               FRAME f-actualiza.
       CREATE Logmenu.
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'nro_cobrador':U THEN DO:
       &Scope KEY-PHRASE Cta_cte.nro_cobrador eq INTEGER(key-value)
       {&OPEN-QUERY-{&BROWSE-NAME}}
    END. /* nro_cobrador */
    WHEN 'nro_vendedor':U THEN DO:
       &Scope KEY-PHRASE Cta_cte.nro_vendedor eq INTEGER(key-value)
       {&OPEN-QUERY-{&BROWSE-NAME}}
    END. /* nro_vendedor */
    OTHERWISE DO:
...
MESSAGE "KKKKKKKKKKKKKKKKKKKKKK"
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "nro_contrato" "Cta_cte" "nro_contrato"}
  ...
  {src/adm/template/sndkycas.i "tip_comprob" "Cta_cte" "tip_comprob"}
  {src/adm/template/sndkycas.i "mes" "Cta_cte" "mes"}
  {src/adm/template/sndkycas.i "cdg_empresa" "Cta_cte" "cdg_empresa"}
