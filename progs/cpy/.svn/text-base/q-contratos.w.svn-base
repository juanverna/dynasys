...
&Scoped-define QUERY-STRING-Query-Main FOR EACH Contrato_hd OF Empresa WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Cliente OF Contrato_hd NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH Contrato_hd OF Empresa WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Cliente OF Contrato_hd NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "sic.Contrato_hd OF sic.Empresa,sic.Cliente OF sic.Contrato_hd"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _Design-Parent    is WINDOW q-tables @ ( 1 , 10 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
