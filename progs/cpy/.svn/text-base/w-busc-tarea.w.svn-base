...
DEFINE VAR temp_tarea AS CHAR INITIAL "".
...
DEFINE FRAME F-Main
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 155.4 BY 17.14 WIDGET-ID 100.
...
ON END-ERROR OF W-Win /* Buscador Tareas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Buscador Tareas */
DO:
...
PROCEDURE adm-create-objects PRIVATE :
/*------------------------------------------------------------------------------
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     Key-Name = nom_cliente,
                     Layout = ,
                     Create-On-Add = ?':U ,
             OUTPUT h_b-busc-tarea ).
       RUN set-position IN h_b-busc-tarea ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-busc-tarea ( 5.24 , 149.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-tarea-red2 ( 6.48 , 1.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-temp_tarea1 ( 7.91 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.00 , 150.60 ) */

       /* Links to SmartBrowser h_b-busc-tarea. */
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'Record':U , h_b-busc-tarea ).

       /* Links to SmartViewer h_v-tarea-red2. */
       RUN add-link IN adm-broker-hdl ( h_b-busc-tarea , 'Record':U , h_v-tarea-red2 ).

       /* Links to SmartViewer h_v-temp_tarea1. */
       RUN add-link IN adm-broker-hdl ( h_b-busc-tarea , 'Record':U , h_v-temp_tarea1 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-tarea-red2 ,
