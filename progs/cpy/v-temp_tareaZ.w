...
&Scoped-Define ENABLED-OBJECTS BUTTON-12 BROWSE-7 tturno 
...
&Scoped-Define DISPLAYED-OBJECTS v-texto tturno 
...
DEFINE VARIABLE tturno AS CHARACTER FORMAT "XXX":U 
     LABEL "T" 
      VIEW-AS TEXT 
     SIZE 6 BY .62 TOOLTIP "Turno" NO-UNDO.
...
DEFINE FRAME sF-Main
     BUTTON-12 AT ROW 1.14 COL 45.4 WIDGET-ID 28
     Bresuelto AT ROW 1.19 COL 53 WIDGET-ID 64
     Tarea.nro_evento AT ROW 1.24 COL 16.6 COLON-ALIGNED WIDGET-ID 26
          LABEL "Ev."
          VIEW-AS FILL-IN 
          SIZE 16 BY 1
     ...
     Tarea.estado AT ROW 1.43 COL 8.2 COLON-ALIGNED WIDGET-ID 24
           VIEW-AS TEXT 
          SIZE 4.2 BY .62
     tturno AT ROW 1.43 COL 35.6 COLON-ALIGNED WIDGET-ID 66
...
ASSIGN 
       tturno:READ-ONLY IN FRAME sF-Main        = TRUE.
...
IF AVAILABLE tarea THEN DO:
  ...
  DISPLAY v-texto WITH FRAME {&FRAME-NAME}.
 FIND evento WHERE evento.nro_evento = tarea.nro_evento NO-LOCK NO-ERROR.
 IF AVAILABLE evento THEN tturno:SCREEN-VALUE = evento.turno.
 ELSE tturno:SCREEN-VALUE = "ND".
