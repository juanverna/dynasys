...
&Scoped-Define DISPLAYED-OBJECTS v-certif v-proto frecursos v-texto 
...
DEFINE BUTTON BLeyenda 
     LABEL "Leyenda" 
     SIZE 11 BY 1.14 TOOLTIP "Es publica se trasladara al Evento".
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 82 BY 7.86 ROW-HEIGHT-CHARS .68.
...
DEFINE FRAME F-Main
     ...
     Tarea.Origen AT ROW 1.48 COL 13 COLON-ALIGNED WIDGET-ID 132
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
     Tarea.nro_identificacion AT ROW 1.48 COL 35 COLON-ALIGNED NO-LABEL WIDGET-ID 134
          VIEW-AS FILL-IN 
          SIZE 14 BY 1
     v-certif AT ROW 1.48 COL 58 COLON-ALIGNED WIDGET-ID 136
     v-proto AT ROW 1.48 COL 79 COLON-ALIGNED WIDGET-ID 138
     Bresuelto AT ROW 1.71 COL 133 WIDGET-ID 64
     frecursos AT ROW 2.52 COL 105 COLON-ALIGNED WIDGET-ID 66
     BRECURSOS AT ROW 2.52 COL 124.2 WIDGET-ID 36
     BROWSE-7 AT ROW 3.14 COL 3 WIDGET-ID 200
     BLeyenda AT ROW 3.86 COL 107 WIDGET-ID 94
     Bvecino AT ROW 5.05 COL 107 WIDGET-ID 124
...
/* BROWSE-TAB BROWSE-7 BRECURSOS F-Main */
ASSIGN 
...
/* CtrlFrame OCXINFO:CREATE-CONTROL from: {D7053240-CE69-11CD-A777-00DD01143C57} type: CommandButton */
      CtrlFrame:MOVE-AFTER(BLeyenda:HANDLE IN FRAME F-Main).
...
PROCEDURE CtrlFrame.CommandButton.Click .
/*------------------------------------------------------------------------------
  Purpose:     
...
RUN d-zoom-evento.w ( int(extrae("EVENTOLT",tarea.datos-template )),"Evento Limpieza" ).
...
OCXFile = SEARCH( "v-temp_tareaH.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "v-temp_tareaH.wrx":U SKIP(1)
...
ASSIGN FRAME {&FRAME-NAME} frecursos.
recno = TRUE.
...
sal = extrae("EVENTOLT",tarea.datos-template).
IF sal = ? THEN sal = "".
ELSE sal = "EVENTOLT|" + sal.
vartemplate = "frecursos|texto_adic".
...
hWidget = phFrame:CURRENT-ITERATION:FIRST-CHILD.
  ...
  chctrlFrame:CommandButton:ENABLED = FALSE.
...
END.
 ...
 FIND evento WHERE evento.nro_evento = int(extrae("EVENTOLT",tarea.datos-template)) NO-LOCK NO-ERROR.
 FIND evento_protocolo OF evento NO-ERROR.


 IF AVAILABLE evento THEN DO:
     chctrlFrame:CommandButton:Caption = "Evento:" + string(evento.nro_evento) + CHR(10) + "Realz:" + STRING(evento.frealizado).
     chctrlFrame:CommandButton:ENABLED = TRUE.
 END.
ELSE DO:
     chctrlFrame:CommandButton:Caption = "Evento:" + chr(10) + "NO DISP".
     chctrlFrame:CommandButton:ENABLED = FALSE.
 END.
...
FIND tipo_evento WHERE tipo_evento.cdg_tipo_evento = "EL" NO-LOCK NO-ERROR.
