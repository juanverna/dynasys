...
&Scoped-define FIELDS-IN-QUERY-br_table Tarea.nro_tarea Tarea.nro_predecesora Tarea.prioridad Tarea.cdg_tipotarea Tipo_tarea.dsc_tipotarea durmiendo() @ diasabierto Tarea.fecha_alta Tarea.hora_alta Tarea.reportado_por Tarea.fecha_prevista Tarea.hora_prevista Tarea.estado quiencargo() @ quiencargo quienrecurso( tarea.cdg_recurso ) @ quienrecurso   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table   
&Scoped-define SELF-NAME br_table
&Scoped-define QUERY-STRING-br_table FOR EACH Tarea WHERE ~{&KEY-PHRASE} NO-LOCK, ~
           FIRST tipo_tarea OF tarea BY Tarea.Visualizar DESCENDING     ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY {&SELF-NAME} FOR EACH Tarea WHERE ~{&KEY-PHRASE} NO-LOCK, ~
           FIRST tipo_tarea OF tarea BY Tarea.Visualizar DESCENDING     ~{&SORTBY-PHRASE}.
...
nom_cliente|y|y|sic.Tarea.nom_cliente
nro_cliente|y|y|sic.Tarea.nro_cliente
accion||y|sic.Tarea.accion
...
RUN set-attribute-list (
    'Keys-Accepted = "descripcion,cdg_proyecto,nom_cliente,nro_cliente,cdg_recurso",
     Keys-Supplied = "descripcion,cdg_proyecto,nom_cliente,nro_cliente,accion,cdg_recurso,cdg_cargo,nro_cliente,cdg_postal,nro_evento,nro_persona,cdg_tipotarea,cdg_tarea,cdg_usuario,nro_usuario"':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _FREEFORM
  QUERY br_table NO-LOCK DISPLAY
      ...
      Tarea.nro_predecesora FORMAT ">>>>>9":U COLUMN-LABEL "Tarea!Predec"
      Tarea.prioridad COLUMN-LABEL "PR" FORMAT "9":U WIDTH 3.2
      Tarea.cdg_tipotarea COLUMN-LABEL "T" FORMAT "X"
      Tipo_tarea.dsc_tipotarea COLUMN-LABEL "Tipo!Tarea" FORMAT "X(12)"
      durmiendo() @ diasabierto COLUMN-LABEL "Hace" WIDTH 5
      Tarea.fecha_alta FORMAT "99/99/99":U
      Tarea.hora_alta FORMAT "X(5)":U WIDTH 6
      Tarea.reportado_por FORMAT "X(20)":U
      Tarea.fecha_prevista FORMAT "99/99/99":U
      Tarea.hora_prevista FORMAT "x(5)":U WIDTH 6
      Tarea.estado FORMAT "X(1)":U COLUMN-LABEL "E"
      quiencargo() @ quiencargo COLUMN-LABEL "Cargo" FORMAT "x(15)":U
                WIDTH 20
      quienrecurso( tarea.cdg_recurso ) @ quienrecurso COLUMN-LABEL "Responsable" FORMAT "x(15)":U
                WIDTH 20
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 103 BY 6.71 FIT-LAST-COLUMN.
...
/* Query rebuild information for BROWSE br_table
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH Tarea WHERE ~{&KEY-PHRASE} NO-LOCK,
    FIRST tipo_tarea OF tarea BY Tarea.Visualizar DESCENDING
    ~{&SORTBY-PHRASE}.
     _END_FREEFORM
...
DO:
    ...
    IF VALID-HANDLE(hProc) THEN
          RUN predecesora IN hproc (tarea.nro_tarea) NO-ERROR.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    ...
    WHEN 'cdg_proyecto':U THEN DO:
       ...
       {&OPEN-QUERY-{&BROWSE-NAME}}
    END. /* cdg_proyecto */
    WHEN 'nom_cliente':U THEN DO:
       &Scope KEY-PHRASE Tarea.nom_cliente eq key-value
       {&OPEN-QUERY-{&BROWSE-NAME}}
    END. /* nom_cliente */
    WHEN 'nro_cliente':U THEN DO:
       &Scope KEY-PHRASE Tarea.nro_cliente eq INTEGER(key-value)
       {&OPEN-QUERY-{&BROWSE-NAME}}
    END. /* nro_cliente */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE cambia_template B-table-Win 
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "accion" "Tarea" "accion"}
