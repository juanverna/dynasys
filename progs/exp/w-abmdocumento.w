...
DEFINE VARIABLE h_b-documento AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-documento AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Documento */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Documento */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Documento */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-documento.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-documento ).
       RUN set-position IN h_b-documento ( 1.54 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 19.92 , 45.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-documento.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-documento ).
       RUN set-position IN h_v-documento ( 1.54 , 50.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.50 , 61.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 5.31 , 50.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.42 , 64.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-documento. */
       RUN add-link IN adm-broker-hdl ( h_b-documento , 'Record':U , h_v-documento ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-documento ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-documento ,
             h_b-documento , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-documento , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Manetenimiento de Documento"}
