...
DEFINE VARIABLE h_b-producto AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-producto AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Producto */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Producto */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Producto */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-producto.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-producto ).
       RUN set-position IN h_b-producto ( 1.81 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 19.92 , 46.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-producto.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-producto ).
       RUN set-position IN h_v-producto ( 1.81 , 50.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.12 , 64.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 7.73 , 50.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.42 , 64.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-producto. */
       RUN add-link IN adm-broker-hdl ( h_b-producto , 'Record':U , h_v-producto ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-producto ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-producto ,
             h_b-producto , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-producto , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "Mantenimiento de Producto"}
