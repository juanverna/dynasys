...
DEFINE VARIABLE det0           AS INTEGER INITIAL 80.
...
DO TRANSACTION:
    ...
    OUTPUT STREAM Formulario TO "c:\sic-temp\prfex114.xpr" CONVERT TARGET "iso8859-1".
    ...
    FILE-INFO:File-NAME = "c:\sic-temp\prfex114.xpr".
...
PROCEDURE inicia_hoja:
 ...
 /* PUT STREAM Formulario CONTROL '<TOOLBAR=!PRINT>'. */
    ...
    PUT STREAM Formulario CONTROL "<OPORTRAIT><Title=Commercial Invoice><UNITS=mm><|2><PREVIEW=40>".
    PUT STREAM Formulario CONTROL "<FORMAT=A4>".
    PUT STREAM Formulario CONTROL "<#1>".    
    PUT STREAM Formulario CONTROL "<R172><C1><#2>".    

    PUT STREAM Formulario UNFORMATTED "<=#1><BGCOLOR=WHITE><AT=,+5><FROM><AT=+75,+190><FILLRECT)>".
    PUT STREAM Formulario UNFORMATTED "<=#1><BGCOLOR=WHITE><AT=+75,+5><FROM><AT=+160,+190><FILLRECT)>". 

    RUN escribir ( INPUT "07,150", INPUT "<B>COMMERCIAL INVOICE</B>", INPUT 9, INPUT NO). 
    ...
    RUN escribir (INPUT "30,20",INPUT "INVOICE Nº : " + STRING(Ped_header.nro_comprob,"999999"),INPUT 8,INPUT NO).
    RUN escribir (INPUT "33,20",INPUT "INVOICE DATE : " + STRING(Ped_header.fecha_comex,"99/99/9999"),INPUT 8,INPUT NO).
    
    RUN escribir (INPUT "21,90",INPUT "Importer",INPUT 8,YES).
    RUN escribir (INPUT "24,90",INPUT Cliente.nom_cliente,INPUT 7,INPUT NO).
    RUN escribir (INPUT "27,90",INPUT Cliente.direccion,INPUT 7,INPUT NO).
    RUN escribir (INPUT "31,90",INPUT Cliente.localidad,INPUT 7,INPUT NO).
    RUN escribir (INPUT "34,90",INPUT "CNPJ : ", INPUT 7,INPUT NO).
    
    RUN escribir (INPUT "40,20",INPUT "ORDER BY",INPUT 8,INPUT YES).
    RUN escribir (INPUT "43,20",INPUT Domicilio.nombre,INPUT 7,INPUT NO).
    RUN escribir (INPUT "46,20",INPUT Domicilio.direccion,INPUT 7,INPUT NO).
    RUN escribir (INPUT "49,20",INPUT "CNPJ : ",INPUT 7,INPUT NO).
    ...
    RUN escribir (INPUT "58,95",INPUT "Condition of Payment : " + Condicion_venta.descripcion,INPUT 7,INPUT NO).
    RUN escribir (INPUT "61,20",INPUT "Country of Origin : ARGENTINA",INPUT 7,INPUT NO).
    RUN escribir (INPUT "64,20",INPUT "Package : ",INPUT 7,INPUT NO).
    RUN escribir (INPUT "64,95",INPUT "Container Number : ",INPUT 7,INPUT NO).
    RUN escribir (INPUT "67,20",INPUT "Order Number : " + Ped_header.nro_ocm,INPUT 7,INPUT NO).
    RUN escribir (INPUT "67,95",INPUT "EUR/U$S",INPUT 7, INPUT NO).
    RUN escribir (INPUT "70,20",INPUT "GROSS WEIGHT : ",INPUT 7,INPUT NO).
    RUN escribir (INPUT "70,95",INPUT "NET WEIGHT : ",INPUT 7,INPUT NO).

    RUN escribir (INPUT "76,9",  INPUT "Position", INPUT 8, INPUT NO).
    RUN escribir (INPUT "76,27", INPUT "Article", INPUT 8, INPUT NO).
    RUN escribir (INPUT "76,70",INPUT "Descripcion of Goods",INPUT 8,INPUT NO).
    RUN escribir (INPUT "76,135",INPUT "Quantity", INPUT 8, INPUT NO).
    RUN escribir (INPUT "76,155",INPUT "Unit Price", INPUT 8, INPUT NO).
    RUN escribir (INPUT "76,180",INPUT "Amount", INPUT 8, INPUT NO).

    RUN linea ("081,005","190","H").
    RUN linea ("075,022","152","V").
    RUN linea ("075,040","152","V").
    RUN linea ("075,130","152","V").
    RUN linea ("075,150","152","V").
    RUN linea ("075,170","152","V").
    RUN linea ("227,005","190","H").
    ...
    RUN escribir (INPUT "254,159",INPUT "signatory's name",INPUT 8,INPUT YES).

    RUN escribir (INPUT "228,130",INPUT "AMOUNT EUR/U$S",INPUT 10,INPUT YES).
    RUN escribenumero(INPUT "228,170",INPUT STRING(Ped_header.imp_neto,"->>>>9.99"),INPUT 9, INPUT YES).
...
PROCEDURE forma:
    ...
    FOR EACH Ped_detalle OF Ped_header, Articulo OF Ped_detalle,
        ...
        THEN DO:
             ...
             RUN inicia_hoja.             
        END.
        RUN escribenumero (ch_linea(det0,linea0) + ",005", INPUT STRING(linea0), INPUT 8,INPUT NO).
        RUN escribir      (ch_linea(det0,linea0) + ",027", INPUT Articulo.cdg_articulo, INPUT 8, INPUT NO).
        RUN escribir      (ch_linea(det0,linea0) + ",045", INPUT Articulo.descripcion, INPUT 8, INPUT NO). 
