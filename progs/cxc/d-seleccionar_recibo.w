...
&THEN
DEFINE VARIABLE         titulo_w      AS CHARACTER INITIAL "Selección de Notas de Débito Emitidas".
...
&Scoped-define FIELDS-IN-QUERY-BROWSE-2 Rec_header.tip_comprob Rec_header.prf_comprob Rec_header.nro_comprob Rec_header.estado Rec_header.fecha Rec_header.mes Rec_header.ano Rec_header.imp_total Cliente.cdg_cliente Cliente.nom_cliente   
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-2   
&Scoped-define SELF-NAME BROWSE-2
&Scoped-define OPEN-QUERY-BROWSE-2    IF por_cliente    THEN DO:       OPEN QUERY {&SELF-NAME}            FOR EACH Rec_header                           WHERE Rec_header.cdg_empresa = que_empresa                             AND Rec_header.nro_cliente = B-Cliente.nro_cliente                             AND Rec_header.fecha >= v-des_fecha                             AND Rec_header.fecha <= v-has_fecha                             AND LOOKUP(Rec_header.estado, ~
      lista_estados) <> 0, ~
                                       FIRST Cliente OF Rec_header                                       WHERE LOOKUP(que_sector, ~
       Cliente.lista_sectores) <> 0.    END.    ELSE DO:       OPEN QUERY {&SELF-NAME}            FOR EACH Rec_header                           WHERE Rec_header.cdg_empresa = que_empresa                             AND Rec_header.fecha >= v-des_fecha                             AND Rec_header.fecha <= v-has_fecha                             AND LOOKUP(Rec_header.estado, ~
      lista_estados) <> 0, ~
                                       FIRST Cliente OF Rec_header                                       WHERE LOOKUP(que_sector, ~
       Cliente.lista_sectores) <> 0.    END.
&Scoped-define TABLES-IN-QUERY-BROWSE-2 Rec_header Cliente
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-2 Rec_header
...
por_cliente v-cdg_cliente BROWSE-2 RECT-1 RECT-9 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-2 D-Dialog _FREEFORM
  QUERY BROWSE-2 DISPLAY
      ...
      Rec_header.prf_comprob COLUMN-LABEL "Pre-!fijo" FORMAT "9999"
      Rec_header.nro_comprob COLUMN-LABEL "Número!Compbte" FORMAT "99999999"
      Rec_header.estado      COLUMN-LABEL "S!T"
      Rec_header.fecha       COLUMN-LABEL "Fecha!Emisión"
      Rec_header.mes         COLUMN-LABEL "Per!Mes" 
      Rec_header.ano         COLUMN-LABEL "Per!Año"
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 101 BY 9.48.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME D-Dialog
     Btn_OK AT ROW 1.24 COL 72
     Btn_Cancel AT ROW 1.24 COL 88
     v-des_fecha AT ROW 1.48 COL 35 COLON-ALIGNED
     v-has_fecha AT ROW 1.48 COL 54 COLON-ALIGNED
     por_cliente AT ROW 1.52 COL 17 NO-LABEL
     v-cdg_cliente AT ROW 3.38 COL 13 COLON-ALIGNED
     v-nombre AT ROW 3.38 COL 28 COLON-ALIGNED NO-LABEL
     BROWSE-2 AT ROW 5.29 COL 2
     RECT-1 AT ROW 3.14 COL 2
     RECT-9 AT ROW 1.24 COL 2
     "Por Cliente:" VIEW-AS TEXT
          SIZE 12 BY 1.19 AT ROW 1.48 COL 5
     SPACE(89.39) SKIP(12.27)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Seleccion de órdenes de pago"
...
/* Query rebuild information for BROWSE BROWSE-2
   ...
   THEN DO:
      OPEN QUERY {&SELF-NAME}
           FOR EACH Rec_header
                          WHERE Rec_header.cdg_empresa = que_empresa
                            AND Rec_header.nro_cliente = B-Cliente.nro_cliente
                            AND Rec_header.fecha >= v-des_fecha
                            AND Rec_header.fecha <= v-has_fecha
                            AND LOOKUP(Rec_header.estado,lista_estados) <> 0,
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* Seleccion de órdenes de pago */
DO:  
...
DO:
  ...
  FIND B-Cliente WHERE B-Cliente.cdg_cliente = INPUT FRAME {&FRAME-NAME} v-cdg_cliente 
                   AND LOOKUP(que_sector, B-Cliente.lista_sectores) <> 0 NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  DISPLAY v-des_fecha v-has_fecha por_cliente v-cdg_cliente v-nombre 
      WITH FRAME D-Dialog.
  ENABLE Btn_OK Btn_Cancel v-des_fecha v-has_fecha por_cliente v-cdg_cliente 
