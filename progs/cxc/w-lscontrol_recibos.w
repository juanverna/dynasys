...
DEFINE VARIABLE h_v-rng_numeros AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 16.24 COL 17 COLON-ALIGNED NO-LABEL
     "  Rango de fechas de recibo a considerar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 7.19 COL 19
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Indique la empresa a considerar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 11.95 COL 19
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de números de recibo a considerar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 3.14 COL 19
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Cobranzas Efectuadas por Fecha"
         HEIGHT             = 22.14
         WIDTH              = 132.8
         MAX-HEIGHT         = 22.14
         MAX-WIDTH          = 132.8
...
ON END-ERROR OF W-Win /* Cobranzas Efectuadas por Fecha */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Cobranzas Efectuadas por Fecha */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng_numeros.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng_numeros ).
       RUN set-position IN h_v-rng_numeros ( 4.43 , 19.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 8.62 , 19.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN set-position IN h_v-lst-empresas ( 13.05 , 19.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 17.62 , 19.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.62 , 100.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng_numeros ,
             v-estado:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-fechas ,
             h_v-rng_numeros , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/CXC" "Cobranzas Efectuadas por Fecha"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE que_ptovta         AS INTEGER.
  DEFINE VARIABLE des_numero         AS INTEGER.
  DEFINE VARIABLE has_numero         AS INTEGER.
  ...
  RUN dar_rango IN h_v-rng_numeros    ( OUTPUT que_ptovta,
                                        OUTPUT des_numero, 
                                        OUTPUT has_numero, 
                                        OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "numeros" ).
  ...
  THEN DO:
       ...
       RUN lscontrol_recibos.p ( INPUT v-lista_empresas,
                                 INPUT que_ptovta,
                                 INPUT des_numero,
                                 INPUT has_numero,
                                 INPUT des_fecha,
