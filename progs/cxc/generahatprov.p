...
FOR EACH caj_detalle OF rubro NO-LOCK:
    FIND caj_header OF caj_detalle NO-LOCK.
...
/*    FIND proveedor WHERE proveedor.nombre = cliente.nom_cliente NO-ERROR.
    IF AVAILABLE proveedor THEN DO: 
    ...
    IF NOT AVAILABLE proveedor THEN DO:
        ...
        END.
    END.
    FIND fac_header_prv WHERE fac_header_prv.cdg_empresa = empresa.cdg_empresa AND
        fac_header_prv.tip_comprob = "FH" AND
        fac_header_prv.prf_comprob = 0 AND
        fac_header_prv.nro_comprob = rendicion_hd.nro_rendicion NO-LOCK NO-ERROR.
    IF AVAILABLE fac_header_prv THEN next.
    ...
    DISPLAY rendicion_hd.nro_rendicion.
