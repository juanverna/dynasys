...
&ANALYZE-RESUME
&Scoped-define WINDOW-NAME p
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CUSTOM _DEFINITIONS p 
/*------------------------------------------------------------------------
...
DEFINE VAR p AS WIDGET-HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-que-fecha AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-que-vencimiento AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 17.95 COL 12 COLON-ALIGNED NO-LABEL
     "  Lista de puntos de venta a considerar" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 14.1 COL 14 WIDGET-ID 4
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "Movimientos con vencimiento hasta" VIEW-AS TEXT
          SIZE 43 BY 1 AT ROW 5.76 COL 14 WIDGET-ID 2
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Lista de empresas a considerar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 9.86 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Clientes a listar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 1.52 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "Movimientos emitidos hasta" VIEW-AS TEXT
          SIZE 42.8 BY 1 AT ROW 5.76 COL 70.6
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CUSTOM _INCLUDED-LIB p 
/* ************************* Included-Libraries *********************** */
...
/* SETTINGS FOR WINDOW p
  VISIBLE,,RUN-PERSISTENT                                               */
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
/* SETTINGS FOR FILL-IN v-estado IN FRAME F-Main
   NO-ENABLE                                                            */
IF SESSION:DISPLAY-TYPE = "GUI":U AND VALID-HANDLE(p)
THEN p:HIDDEN = yes.
...
&Scoped-define SELF-NAME p
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL p p
ON END-ERROR OF p /* Comprobantes impagos a fecha */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL p p
ON WINDOW-CLOSE OF p /* Comprobantes impagos a fecha */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CUSTOM _MAIN-BLOCK p 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE adm-create-objects p  _ADM-CREATE-OBJECTS
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng-cliente.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-cliente ).
       RUN set-position IN h_v-rng-cliente ( 2.62 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.86 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-que-fecha.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-que-vencimiento ).
       RUN set-position IN h_v-que-vencimiento ( 6.95 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 42.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-que-fecha.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-que-fecha ).
       RUN set-position IN h_v-que-fecha ( 6.95 , 71.60 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 42.00 ) */
       ...
       RUN set-position IN h_v-lst-empresas ( 11.19 , 14.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-lst_punto-vta ( 15.29 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 19.19 , 11.40 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-que-vencimiento ,
             h_v-rng-cliente , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-que-fecha ,
             h_v-que-vencimiento , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-lst-empresas ,
             h_v-que-fecha , 'AFTER':U ).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE adm-row-available p  _ADM-ROW-AVAILABLE
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE disable_UI p  _DEFAULT-DISABLE
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE enable_UI p  _DEFAULT-ENABLE
PROCEDURE enable_UI :
/*------------------------------------------------------------------------------
...
------------------------------------------------------------------------------*/
  DISPLAY v-estado 
      WITH FRAME F-Main IN WINDOW p.
  VIEW FRAME F-Main IN WINDOW p.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE local-exit p 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE local-initialize p 
...
------------------------------------------------------------------------------*/
  ...
  RUN habilitar_verpor IN h_v-rng-cliente ( INPUT YES ).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE local-view p 
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/CXC" "Aplicacion de Cobranzas por Fecha"}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE lst-ejecutar p 
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE que_vencimiento    AS DATE.
 ...
 IF error_rango 
  ...
  RUN dar_rango IN h_v-que-fecha      ( OUTPUT has_fecha, 
                                       OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       ...
       hay_error = error_rango.
  END.     
  RUN dar_rango IN h_v-que-vencimiento    ( OUTPUT que_vencimiento,
                                            OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
      RUN ver-error-rango ( "Fechas Vencimiento" ).
      hay_error = error_rango.
  END.     
  ...
  THEN DO:
       ...
       RUN pendiente_cob ( INPUT v-lista_empresas , 
                           INPUT ( IF ver_por = 1 THEN des_cliente ELSE des_nombre ), 
                           INPUT ( IF ver_por = 1 THEN has_cliente ELSE has_nombre ),
                           INPUT has_fecha ,
                           INPUT que_vencimiento,
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pendiente_cob p 
...
DEF VAR exportFileName AS CHAR NO-UNDO.
  ...
  RUN prinresumenes-email.p ( INPUT p-cdg_empresa,
                             INPUT p-cdg_dadm,
                             INPUT p-cdg_hadm,
                             INPUT p-hfecha ,
                             INPUT p-vencimiento,
...
RUN crearReporte(chReport,"xls",/*ViewReport*/ TRUE,/*PrinterName*/ "",
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE send-records p  _ADM-SEND-RECORDS
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setear-botones p 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE state-changed p 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE ver-error-rango p 
