...
PROCEDURE imprimir_orden:
    ...
    OUTPUT STREAM Formulario TO "c:\sic-temp\prrca194.xpr" CONVERT TARGET "iso8859-1".
    ...
    DO ncopia = 1 TO 2:
       RUN escribir     ( INPUT "120,15", INPUT "<B>Son " + Moneda.descripcion + ":</B>" + letras, 8, NO).
       RUN escribenumero( INPUT "112,53", INPUT STRING(Rec_header.imp_pesos + Rec_header.imp_difcambio,"->,>>>,>>9.99"), 8, YES, 1.0).
       RUN escribenumero( INPUT "112,176", INPUT STRING(Caj_header.importe,"->,>>>,>>9.99"), 8, YES, 1.0).
    END.
    
    OUTPUT STREAM Formulario CLOSE.
    FILE-INFO:File-NAME = "c:\sic-temp\prrca194.xpr".
    ...
    DO WHILE AVAILABLE Rec_detalle OR AVAILABLE Caj_detalle:
      ...
      THEN DO:
            ...
            DO ncopia = 1 TO 2:
               ...
               RUN escribenumero( INPUT chr_linea + ",53", INPUT STRING(Rec_detalle.imp_pesos + Rec_detalle.difcambio,"->,>>>,>>9.99"), 8, NO, 1.0).
            END.                
      END.
...
/*     PUT STREAM Formulario UNFORMATTED f-ref(ncopia) + "<AT=+17,+13><#3><AT=+8,+60><IMAGE#3=.\imagenes\logo.bmp>". */
    ...
    PUT STREAM Formulario UNFORMATTED f-ref(ncopia) + "<AT=+53,+76><FROM><AT=+65><LINE>". 
    PUT STREAM Formulario UNFORMATTED f-ref(ncopia) + "<AT=+61,+55><FROM><AT=+57><LINE>".  /* /// */
    ...
    RUN escribir ( INPUT "55,118", INPUT "VALORES RECIBIDOS", 10, YES).

    RUN escribir ( INPUT "18,80",  INPUT "RECIBO DE PAGO", 14, YES).
    RUN escribir ( INPUT "30,160", INPUT "Nro:<B>" + Rec_header.tip_comprob + " " +
    ...
    RUN escribir ( INPUT "34,160", INPUT "Fecha:" + STRING(Rec_header.fecha,"99/99/9999"), INPUT 10, INPUT NO). 
    ...
    RUN escribir ( INPUT "46,40",  INPUT "CUIT:" + Cliente.cuit + " - " + Condicion_impos.texto, 10, NO). 
