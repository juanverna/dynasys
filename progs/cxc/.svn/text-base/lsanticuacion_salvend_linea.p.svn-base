/*=================================================================================*/
/*      CLASIFICACION DE LOS DERECHOS SEGUN ANTIGUEDAD y POR LINEA DE PRODUCTOS    */
/*=================================================================================*/
...
DEFINE VARIABLE x-factor_importe        AS DECIMAL DECIMALS 4.
DEFINE VARIABLE x-fraccion_impaga       AS DECIMAL DECIMALS 4.
...
DEFINE VARIABLE primera_familia         AS LOGICAL.
...
DEFINE TEMP-TABLE Acumulado
   ...
   INDEX por_fila_columna IS UNIQUE PRIMARY nro_cliente cdg_familia n-columna.
...
FORM 
   header_sr1 
   WITH FRAME frm-subraya WIDTH 300 USE-TEXT STREAM-IO NO-LABEL NO-UNDERLINE.

FORM
   Cliente.cdg_cliente          COLUMN-LABEL "Código!Cliente"
   Cliente.nom_cliente          COLUMN-LABEL "Razón!Social" 
   Familia_articulo.dsc_familia COLUMN-LABEL "Familia!Artículo" FORMAT "X(15)"
   columnas
   WITH FRAME frm-listado DOWN WIDTH 300 USE-TEXT STREAM-IO NO-LABEL NO-UNDERLINE.
...
PROCEDURE listar_todo:
    ...
    header_tt1 = "Identificación                                    " + "        Familia " + "         Saldo " + "         Saldo " + "         Saldo ".
    header_tt2 = "del Cliente                                       " + "       Articulo " + "         Saldo " + "       Vencido " + "      A Vencer ".
    header_sr1 = "------------------------------------------------- " + "--------------- " + "-------------- " + "-------------- " + "-------------- ".
    ...
    /* Crea los registros de acumulado para el total de los clientes. Recorre todas las   */
    /* Familias de articulos habilitadas para ventas.                                     */
  
    FOR EACH Def-columna NO-LOCK
        WHERE Def-columna.cdg_empresa = Empresa.cdg_empresa
              BREAK BY Def-columna.n-columna:

        FOR EACH Familia_articulo WHERE Familia_articulo.ventas_sino:
    ...
    FOR EACH Cliente NO-LOCK
         ...
         /* Crea los registros de acumulado para el cliente a razon de uno por cada columna */
          ...
          FOR EACH Cta_cte OF Cliente 
              ...
              FIND Fac_header WHERE Fac_header.cdg_empresa = Cta_cte.cdg_empresa
                                AND Fac_header.tip_comprob = Cta_cte.tip_comprob
                                AND Fac_header.prf_comprob = Cta_cte.prf_comprob
                                AND Fac_header.nro_comprob = Cta_cte.nro_comprob
                                    NO-LOCK.

              x-factor_importe = Fac_header.imp_total / Fac_header.imp_neto.
              x-fraccion_impaga = Cta_cte.debito - Cta_cte.credito / Fac_header.imp_total.
              
              FOR EACH Fac_detalle OF Fac_header, Articulo OF Fac_detalle, Familia_articulo OF Articulo:
                  ...
                  x-importe = ROUND(Fac_detalle.subtotal_neto * x-factor_importe * x-fraccion_impaga,2).
          ...
          THEN DO:
               ...
               FOR EACH Acumulado NO-LOCK OF Cliente, Familia_articulo OF Acumulado BREAK BY Acumulado.cdg_familia BY Acumulado.n-columna:
         
                   FIND B-Acumulado OF Cliente 
                       WHERE B-Acumulado.cdg_familia = "" 
                         AND B-Acumulado.n-columna = Acumulado.n-columna
                             EXCLUSIVE-LOCK NO-ERROR.
                   IF NOT AVAILABLE B-Acumulado
                   THEN DO:
                       CREATE B-Acumulado.
                       BUFFER-COPY Acumulado TO B-Acumulado
                           ASSIGN B-Acumulado.cdg_familia = "" 
                                  B-Acumulado.importe = 0.
                   END.
                   ...
                   B-Acumulado.importe = B-Acumulado.importe + Acumulado.importe.
                   ...
                   IF LAST-OF(Acumulado.cdg_familia)
                   THEN DO:
                       ...
                       */
                       ...
                       DISPLAY Cliente.cdg_cliente WHEN primera_familia
                               Cliente.nom_cliente WHEN primera_familia
                               Familia_articulo.dsc_familia 
                               columnas
                               WITH FRAME frm-listado.
                       DOWN WITH FRAME frm-listado.        

                       ASSIGN saldo_cliente_vencido = 0
                              saldo_cliente_a_vencer = 0
                              columnas = ""
                              primera_familia = NO.
               ...
               */
               ...
               columnas = "".
               FOR EACH Acumulado NO-LOCK OF Cliente WHERE Acumulado.cdg_familia = "" BY Acumulado.n-columna:
...
*/
               UNDERLINE columnas
                   WITH FRAME  frm-listado.
               DISPLAY "Total Cliente" @ Cliente.nom_cliente
                       columnas
                       WITH FRAME frm-listado.
