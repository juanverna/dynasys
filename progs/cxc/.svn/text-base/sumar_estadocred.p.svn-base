...
PROCEDURE SUMAR_VALORES:
    ...
    FOR EACH Valor OF Cliente 
            WHERE Valor.estado <> stchq_acredit
              ...
              AND ( Valor.cdg_empresa = Empresa.cdg_empresa OR v-consolidado )
                  NO-LOCK:
...
PROCEDURE SUMAR_REMITOS:
    ...
    FOR EACH Rem_header OF Cliente 
        WHERE Rem_header.estado = "E"
          ...
          AND ( Rem_header.cdg_empresa = Empresa.cdg_empresa  OR v-consolidado ),
              Tipocomprobante OF Rem_header
              NO-LOCK:
        ...
        IF Moneda.es_local
         ...
         IF Tipocomprobante.debita
             THEN tot_remitos = tot_remitos + x-importe_movimiento.
             ELSE tot_remitos = tot_remitos - x-importe_movimiento.
    ...
    */
    ...
    FOR EACH Ped_header NO-LOCK OF Cliente 
        WHERE LOOKUP(Ped_header.cdg_estado,"AA,SI,AM",",") <> 0
          AND ( Ped_header.cdg_empresa = Empresa.cdg_empresa  OR v-consolidado ),
        Tipocomprobante OF Ped_header:
        ...
        IF Tipocomprobante.debita
            THEN tot_pedidos = tot_pedidos + x-importe_movimiento.
            ELSE tot_pedidos = tot_pedidos - x-importe_movimiento.

        PUT STREAM Valuacion T-Ped_header.tip_comprob " " 
                             T-Ped_header.prf_comprob " "
                             T-Ped_header.nro_comprob " "
                             Tipocomprobante.debita " "
