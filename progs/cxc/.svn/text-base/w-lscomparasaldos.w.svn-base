...
DEFINE VARIABLE h_v-sinmovimientos AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     "  Indique si desea listar clientes sin movimientos:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 10.52 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Moneda de los movimientos a comparar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 5.57 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Clientes a comparar:" VIEW-AS TEXT
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Comparación de Saldos Históricos y Analíticos"
         HEIGHT             = 22.71
         WIDTH              = 123
         MAX-HEIGHT         = 22.71
         MAX-WIDTH          = 123
...
ON END-ERROR OF W-Win /* Comparación de Saldos Históricos y Analíticos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Comparación de Saldos Históricos y Analíticos */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-sinmovimientos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-sinmovimientos ).
       RUN set-position IN h_v-sinmovimientos ( 11.71 , 14.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-sinmovimientos ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/CXC" "Comparación de Saldos Históricos y Analíticos"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE v-sinmovimientos   AS LOGICAL.
  ...
  RUN dar_rango IN h_v-sinmovimientos ( OUTPUT v-sinmovimientos, 
                                        OUTPUT error_rango ).
  ...
  THEN DO:
       ...
       RUN lscomparasaldos.p ( INPUT des_cliente,
                               INPUT has_cliente,
                               INPUT des_nombre,
                               INPUT has_nombre,
                               INPUT ver_por,
                               INPUT que_moneda, 
                               INPUT v-sinmovimientos).
