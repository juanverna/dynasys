...
&Scoped-define INTERNAL-TABLES Rec_header
...
Rec_header.prf_comprob Rec_header.nro_comprob Rec_header.estado ~
Rec_header.fecha Rec_header.fecha_grab Rec_header.imp_total ~
Rec_header.anulado Rec_header.tipo_pago 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH Rec_header OF Cliente WHERE ~{&KEY-PHRASE} ~
      AND Rec_header.cdg_empresa = que_empresa AND  ~
Rec_header.anulado = TRUE ~
 NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Rec_header OF Cliente WHERE ~{&KEY-PHRASE} ~
      AND Rec_header.cdg_empresa = que_empresa AND  ~
Rec_header.anulado = TRUE ~
 NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table Rec_header
...
tip_comprob||y|sic.Rec_header.tip_comprob
...
RUN set-attribute-list (
    'Keys-Accepted = ,
     Keys-Supplied = "nombre,nro_transaccion,nro_cliente,nro_cobrador,tip_comprob,cdg_condiva,nro_cndventa,mes,cdg_postal,cdg_empresa,nro_entidad,fecha,cdg_imputacion,nro_moneda,cdg_provincia,nro_recibo,nro_rendicion,nro_resumen,num_sucursal,nro_usuario,nro_vendedor,cdg_zonag"':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Rec_header.tip_comprob COLUMN-LABEL "Ti-!po" FORMAT "X(3)":U
      Rec_header.prf_comprob COLUMN-LABEL "Pre-!fijo" FORMAT "ZZZ9":U
      Rec_header.nro_comprob COLUMN-LABEL "Número!Recibo" FORMAT "ZZZZZZZ9":U
      Rec_header.estado COLUMN-LABEL "Es-!tado" FORMAT "X(1)":U
      Rec_header.fecha COLUMN-LABEL "Fecha!Recibo" FORMAT "99/99/9999":U
      Rec_header.fecha_grab COLUMN-LABEL "Fecha!Carga" FORMAT "99/99/9999":U
      Rec_header.imp_total COLUMN-LABEL "Importe!Total" FORMAT "->,>>>,>>9.99":U
      Rec_header.anulado COLUMN-LABEL "Anu-!lado" FORMAT "Si/No":U
      Rec_header.tipo_pago COLUMN-LABEL "Tipo!Pago" FORMAT "9":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 83 BY 6.71
         TITLE "Recibos asociado al cliente actual".
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "sic.Rec_header OF sic.Cliente"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "Rec_header.cdg_empresa = que_empresa AND 
Rec_header.anulado = TRUE
"
     _FldNameList[1]   > sic.Rec_header.tip_comprob
"Rec_header.tip_comprob" "Ti-!po" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > sic.Rec_header.prf_comprob
"Rec_header.prf_comprob" "Pre-!fijo" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > sic.Rec_header.nro_comprob
"Rec_header.nro_comprob" "Número!Recibo" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[4]   > sic.Rec_header.estado
"Rec_header.estado" "Es-!tado" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   > sic.Rec_header.fecha
"Rec_header.fecha" "Fecha!Recibo" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[6]   > sic.Rec_header.fecha_grab
"Rec_header.fecha_grab" "Fecha!Carga" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[7]   > sic.Rec_header.imp_total
"Rec_header.imp_total" "Importe!Total" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[8]   > sic.Rec_header.anulado
"Rec_header.anulado" "Anu-!lado" ? "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[9]   > sic.Rec_header.tipo_pago
"Rec_header.tipo_pago" "Tipo!Pago" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* Recibos asociado al cliente actual */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* Recibos asociado al cliente actual */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* Recibos asociado al cliente actual */
DO:
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "tip_comprob" "Rec_header" "tip_comprob"}
