...
&Scoped-Define ENABLED-OBJECTS rd_opcion 
&Scoped-Define DISPLAYED-OBJECTS rd_opcion v-estado 
...
DEFINE VARIABLE rd_opcion AS INTEGER INITIAL 1 
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS 
          "Movimientos que son de Cuenta Corriente", 1,
"Movimientos que NO son de Cuenta Corriente", 2,
"Ambos", 3
     SIZE 69 BY 3 NO-UNDO.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME F-Main
     rd_opcion AT ROW 19.1 COL 48 NO-LABEL
     v-estado AT ROW 22.91 COL 27 COLON-ALIGNED NO-LABEL
     "  Lista de empresas a considerar:" VIEW-AS TEXT
          SIZE 100 BY .81 AT ROW 14.76 COL 29
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Moneda de los movimientos a listar:" VIEW-AS TEXT
          SIZE 100 BY .81 AT ROW 6.95 COL 29
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Clientes a listar:" VIEW-AS TEXT
          SIZE 100 BY .81 AT ROW 2.19 COL 29
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de fechas de los movimientos:" VIEW-AS TEXT
          SIZE 100 BY .81 AT ROW 10.71 COL 29
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
ON END-ERROR OF W-Win /* Subdiario de Cobranzas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Subdiario de Cobranzas */
DO:
...
&Scoped-define SELF-NAME rd_opcion
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL rd_opcion W-Win
ON VALUE-CHANGED OF rd_opcion IN FRAME F-Main
DO:
  ASSIGN rd_opcion.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-cliente ( 3.29 , 29.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-esp-moneda ( 8.05 , 29.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 11.81 , 29.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN set-position IN h_v-lst-empresas ( 15.86 , 29.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 24.10 , 29.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.62 , 100.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-cliente ,
             rd_opcion:HANDLE IN FRAME F-Main , 'BEFORE':U ).
...
------------------------------------------------------------------------------*/
  ...
  RUN habilitar_verpor IN h_v-rng-cliente ( INPUT NO ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/CXC" "Subdiario de Cobranzas"}
...
------------------------------------------------------------------------------*/
  ...
  THEN DO:
       ...
       RUN lssubdiario_cobranzas_mhsa.p ( INPUT v-lista_empresas,
                                          INPUT des_cliente,
                                          INPUT has_cliente,
                                          INPUT que_moneda, 
                                          INPUT des_fecha,
