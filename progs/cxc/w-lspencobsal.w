...
DEFINE VARIABLE p-estado_cliente AS INTEGER.
...
&Scoped-Define ENABLED-OBJECTS Estado_cliente RECT-1 
&Scoped-Define DISPLAYED-OBJECTS Estado_cliente v-estado 
...
DEFINE VARIABLE h_v-que-moneda AS HANDLE NO-UNDO.
...
DEFINE VARIABLE Estado_cliente AS INTEGER INITIAL 1 
...
DEFINE FRAME F-Main
     Estado_cliente AT ROW 16.71 COL 65 NO-LABEL
     v-estado AT ROW 19.1 COL 12 COLON-ALIGNED NO-LABEL
     RECT-1 AT ROW 16.48 COL 62
     "  Moneda de los movimientos:" VIEW-AS TEXT
          SIZE 100 BY .81 AT ROW 10.05 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de Cobradores a Considerar:" VIEW-AS TEXT
          SIZE 100 BY .81 AT ROW 2.33 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Indique si desea el listado consolidado o solo para la empresa actual:" VIEW-AS TEXT
          SIZE 100 BY .81 AT ROW 13.91 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de Fechas de Transacciones:" VIEW-AS TEXT
          SIZE 100 BY .81 AT ROW 6.43 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Estado de Clientes a ser listados:" VIEW-AS TEXT
          SIZE 52.2 BY 1 AT ROW 15.43 COL 61.8
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Documentos pendientes x Cobrador"
         HEIGHT             = 24.24
         WIDTH              = 126.8
         MAX-HEIGHT         = 24.24
         MAX-WIDTH          = 126.8
...
ON END-ERROR OF W-Win /* Documentos pendientes x Cobrador */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Documentos pendientes x Cobrador */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-cobrador ( 3.43 , 14.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 7.43 , 13.40 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-que-moneda.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-que-moneda ).
       RUN set-position IN h_v-que-moneda ( 11.05 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN set-position IN h_v-consolidado ( 15.38 , 14.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  DISPLAY Estado_cliente v-estado 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE Estado_cliente RECT-1 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/CXC" "Documentos Pendientes por Cobrador"}
...
------------------------------------------------------------------------------*/
  ...
  RUN dar_rango IN h_v-que-moneda ( OUTPUT que_moneda, 
                                    OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
  ...
  THEN DO:
       ...
       p-estado_cliente = 1.
       IF Estado_cliente:SCREEN-VALUE IN FRAME {&FRAME-NAME}= '1' THEN
       p-estado_cliente = 1.
       IF Estado_cliente:SCREEN-VALUE IN FRAME {&FRAME-NAME}= '2' THEN
       p-estado_cliente = 2.
       IF Estado_cliente:SCREEN-VALUE IN FRAME {&FRAME-NAME}= '3' THEN
       p-estado_cliente = 3.
       RUN lspencobsal.p ( INPUT v-consolidado,
                           INPUT des_cobrador,
                           INPUT has_cobrador,
                           INPUT des_fecha,
                           INPUT has_fecha,
                           INPUT que_moneda,
                           INPUT p-estado_cliente).
