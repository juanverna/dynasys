...
DEFINE VARIABLE h_v-comprobantes AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-comprobantes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-comprobantes ).
       RUN set-position IN h_v-comprobantes ( 10.69 , 14.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-consolidado ( 15.00 , 14.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-comprobantes ,
             h_v-rng-fechas , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-consolidado ,
             h_v-comprobantes , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  RUN dar_rango IN h_v-comprobantes (  OUTPUT  lista_tipos,
                                       OUTPUT  error_rango).

  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Comprobantes" ).
       hay_error = error_rango.
  END.     
  ...
  THEN DO:
       ...
       RUN lsdocxcobrador.p ( INPUT des_cobrador,
                              INPUT has_cobrador,
                              INPUT des_fecha, 
                              INPUT has_fecha,
                              INPUT lista_tipos,
