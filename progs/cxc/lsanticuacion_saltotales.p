...
DEFINE FRAME frm-titulo HEADER
   que_empresa
   "Resumen de Saldos por Edades Por Vendedor" AT 75
...
PROCEDURE listar_todo:
    ...
    desc_moneda = Moneda.descripcion + IF v-consolidado THEN " CONSOLIDADO" ELSE "" + " - " + 
                  STRING(des_fecha,"99/99/99") + " al " + STRING(has_fecha,"99/99/99").
  
                        /* se arma el titulo con las fechas */

    header_tt1 = "Identificación                               " + "         Saldo " + "         Saldo " + "         Saldo ".
    header_tt2 = "del Vendedor                                 " + "         Total " + "       Vencido " + "      A Vencer ".
    header_sr1 = "-------------------------------------------- " + "-------------- " + "-------------- " + "-------------- ".
    ...
    FOR EACH Acumulado NO-LOCK WHERE Acumulado.nro_cliente = 0 BY Acumulado.n-columna:
        ...
        IF Acumulado.n-columna <= ncol_vencidas 
           THEN saldo_total_vencido  = saldo_total_vencido  + t-importe [ Acumulado.n-columna ].
           ELSE saldo_total_a_vencer = saldo_total_a_vencer + t-importe [ Acumulado.n-columna ].

      
    END.
    saldo_total = saldo_total_vencido + saldo_total_a_vencer.
    columnas = STRING(saldo_total,">>,>>>,>>9.99-") + " " + 
               STRING(saldo_total_vencido,">>,>>>,>>9.99-") + " " + 
               STRING(saldo_total_a_vencer,">>,>>>,>>9.99-") + " " +                            
               columnas.
