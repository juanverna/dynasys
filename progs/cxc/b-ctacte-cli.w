...
&Scoped-define QUERY-STRING-br_table FOR EACH Cta_cte OF Cliente WHERE ~{&KEY-PHRASE} ~
      AND Cta_cte.cdg_empresa = que_empresa NO-LOCK, ~
...
&Scoped-Define ENABLED-OBJECTS RECT-1 ficha des_fecha has_fecha bemail ~
btn_credito btn_imprimir btn_integral id_moneda btn_consolidado ~
btn_verificar br_table 
...
DEFINE BUTTON bemail 
     LABEL "Email" 
     SIZE 22 BY 1.14 TOOLTIP "Envia por email las facturas seleccionadas".
...
DEFINE VARIABLE id_moneda AS INTEGER FORMAT "->,>>>,>>9":U INITIAL 0 
     LABEL "Moneda" 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 145 BY 12.14
         BGCOLOR 15 FGCOLOR 9 FONT 4
         TITLE BGCOLOR 15 FGCOLOR 9 "Movimientos de Cuenta Corriente del Cliente" FIT-LAST-COLUMN.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME F-Main
     ficha AT ROW 1.24 COL 2 NO-LABEL
     des_fecha AT ROW 1.24 COL 26 COLON-ALIGNED
     has_fecha AT ROW 1.24 COL 44 COLON-ALIGNED
     bemail AT ROW 1.24 COL 63 WIDGET-ID 2
     btn_credito AT ROW 1.24 COL 87
     btn_imprimir AT ROW 1.24 COL 107
     btn_integral AT ROW 1.24 COL 127
     id_moneda AT ROW 2.43 COL 26 COLON-ALIGNED
     v-saldo AT ROW 2.43 COL 61 COLON-ALIGNED
     btn_consolidado AT ROW 2.43 COL 107
     btn_verificar AT ROW 2.43 COL 127
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "Cta_cte.cdg_empresa = que_empresa"
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bemail B-table-Win
...
DO:
    ...
    FIND fac_header WHERE 
        cta_cte.cdg_emp = fac_header.cdg_emp AND
        cta_cte.prf_comprob = fac_header.prf_comprob AND
        cta_cte.tip_comprob = fac_header.tip_comprob AND
        cta_cte.nro_comprob = fac_header.nro_comprob NO-LOCK.
        FIND cliente WHERE fac_header.nro_admin = cliente.nro_cliente NO-LOCK NO-ERROR.
...
IF RETURN-VALUE <> "" OR NOT rok THEN MESSAGE RETURN-VALUE VIEW-AS ALERT-BOX INFORMATION.
END.
...
DO:
   ...
   DO TRANSACTION:
  /*     DEFINE BUTTON b-dupli LABEL "Duplica".
       DISPLAY b-dupli WITH FRAME f-actualiza.
       ON CHOOSE OF b-dupli 
       DO:
            FIND CURRENT Cta_cte EXCLUSIVE-LOCK.
            BUFFER-COPY cta_cte EXCEPT cta_cte.nro_comprob TO b-cta_cte.
       END.
   */    
       FIND CURRENT Cta_cte EXCLUSIVE-LOCK.
       DISPLAY Cta_cte WITH 2 COLUMNS SIDE-LABELS THREE-D VIEW-AS DIALOG-BOX
           WITH FRAME f-actualiza.
   /*    UPDATE  b-dupli WITH FRAME f-actualiza.*/
       UPDATE  Cta_cte EXCEPT Cta_cte.nro_cliente Cta_cte.cdg_empresa
...
DO:
    ...
    DO TRANSACTION:
       CREATE Cta_cte.
       ASSIGN Cta_cte.nro_cliente = Cliente.nro_cliente
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON MOUSE-MENU-CLICK OF br_table IN FRAME F-Main /* Movimientos de Cuenta Corriente del Cliente */
DO:
  DEFINE VAR opt AS LOGICAL NO-UNDO.
  {findempresa.i}
  FIND usuario_funcion OF usuario WHERE usuario_Funcion.cdg_funcion = "HAB" and
     sic.Usuario_funcion.permiso = "A" NO-LOCK.
  IF NOT AVAILABLE usuario_funcion THEN RETURN NO-APPLY.
  ...
  ELSE DO:
     ...
     THEN DO:                  
         MESSAGE "Actualmente el comprobante " (IF Fac_header.mostrar_cc THEN "" ELSE "NO ")
             "se muestra en las cobranzas, quiere cambiarlo" VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO SET opt.
         IF opt THEN Fac_header.mostrar_cc = NOT Fac_header.mostrar_cc.
     END.
  END.
END.
...
DO:
    RUN listar_cuenta_corriente.p ( INPUT ROWID(Cliente), 
                                    INPUT des_fecha, 
                                    INPUT has_fecha, 
                                    INPUT ficha,
                                    INPUT que_moneda ).
...
DO:
     RUN fintegral_cliente.p (INPUT ROWID(Cliente), 
                              INPUT que_moneda ).
END.
...
DO:
     RUN verificar_aplicacion_cc.p (INPUT ROWID(Cliente), 
                                    INPUT que_moneda ).
END.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE xx AS CHAR NO-UNDO.
    xx = "".
  FOR EACH Moneda:
  ...
  IF VALID-HANDLE( hproc )THEN DO:
      ...
      ficha     = DYNAMIC-FUNCTION("get-ficha" IN hproc ).
      id_moneda = DYNAMIC-FUNCTION("get-moneda" IN hproc ).
      has_fecha:HIDDEN IN FRAME {&FRAME-NAME} = TRUE.
      des_fecha:HIDDEN IN FRAME {&FRAME-NAME} = true.
      ficha:HIDDEN IN FRAME {&FRAME-NAME} = TRUE.
      id_moneda:HIDDEN IN FRAME {&FRAME-NAME} = TRUE.
  END.
  ELSE do: 
      has_fecha = TODAY.
...
/*  RUN dispatch IN THIS-PROCEDURE ( INPUT 'open-query':U ) .*/
  ...
  IF VALID-HANDLE( hproc )THEN DO:
    ...
    des_fecha = DYNAMIC-FUNCTION("get-des_fecha" IN hproc ).
    ficha     = DYNAMIC-FUNCTION("get-ficha" IN hproc ).
    que_moneda = DYNAMIC-FUNCTION("get-moneda" IN hproc ).
  END.
  ELSE DO:
      CASE ficha:
       WHEN his 
    ...
    has_fecha:HIDDEN IN FRAME {&FRAME-NAME} = NOT des_fecha:SENSITIVE IN FRAME {&FRAME-NAME}.
  END.
