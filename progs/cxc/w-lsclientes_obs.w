...
DEFINE FRAME F-Main
     ...
     "  Rango de Fechas de las Observaciones:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 6.24 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Indique si desea el listado consolidado o solo para la empresa actual:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 14.33 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de Vendedores a Considerar:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 1.95 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Indique usuario:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 10.05 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Observaciones de Clientes"
         HEIGHT             = 22.76
         WIDTH              = 121
         MAX-HEIGHT         = 25.91
         MAX-WIDTH          = 160
...
ON END-ERROR OF W-Win /* Observaciones de Clientes */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Observaciones de Clientes */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '//server_merino/dynasys/r3.5/progs_user/v-rng-usuario.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-usuario ).
       RUN set-position IN h_v-rng-usuario ( 11.24 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN set-position IN h_v-lst-empresas ( 15.29 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 20.52 , 14.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/CXC" "Observaciones de Clientes"}
...
/*   END.                                                     */
  ...
  IF NOT hay_error
  THEN DO:
       RUN dar_rango IN h_v-rng-usuario( OUTPUT usuario,                                         
                                       OUTPUT error_rango ).
      ...
      THEN DO:
       ...
       RUN lsobsclc.p ( INPUT v-lista_empresas,
                             INPUT des_vendedor,
                             INPUT has_vendedor,
                             INPUT des_fecha,
                             INPUT has_fecha,                             
