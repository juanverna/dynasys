...
DEFINE VARIABLE h_v-fecha_operaciones AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Anticuación de Operaciones de Clientes por Vendedor"
         HEIGHT             = 22.14
         WIDTH              = 123.8
         MAX-HEIGHT         = 22.33
         MAX-WIDTH          = 127.2
...
ON END-ERROR OF W-Win /* Anticuación de Operaciones de Clientes por Vendedor */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Anticuación de Operaciones de Clientes por Vendedor */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-vendedor ( 4.00 , 14.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-esp-moneda ( 8.76 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-fecha_operaciones.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-fecha_operaciones ).
       RUN set-position IN h_v-fecha_operaciones ( 13.86 , 14.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 19.19 , 14.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-fecha_operaciones ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/CXC" "Anticuación de Operaciones de Clientes por Vendedor"}
...
------------------------------------------------------------------------------*/
  ...
  RUN dar_rango IN h_v-fecha_operaciones ( OUTPUT ref_fecha, 
                                           OUTPUT consolidado, 
                                           OUTPUT error_rango ).
  ...
  THEN DO:
       ...
       RUN lsanticuacion_operaciones.p ( INPUT  des_vendedor,
                                         INPUT  has_vendedor,
                                         INPUT  que_moneda,
                                         INPUT  ref_fecha,
                                         INPUT  consolidado).
