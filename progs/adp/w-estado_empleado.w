...
DEFINE VARIABLE h_b-estado_empleado AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-estado_empleado AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/b-estado_empleado.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-estado_empleado ).
       RUN set-position IN h_b-estado_empleado ( 3.86 , 12.00 ) NO-ERROR.
       RUN set-size IN h_b-estado_empleado ( 12.38 , 13.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/v-estado_empleado.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-estado_empleado ).
       RUN set-position IN h_v-estado_empleado ( 3.86 , 37.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.48 , 73.40 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 14.57 , 37.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 1.76 , 73.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-estado_empleado. */
       RUN add-link IN adm-broker-hdl ( h_b-estado_empleado , 'Record':U , h_v-estado_empleado ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-estado_empleado ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-estado_empleado ,
             h_b-estado_empleado , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-estado_empleado , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/ADP" "Mantenimiento de Estados de Empleados -"}
