...
DEFINE VARIABLE h_b-novedades_empleado AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Consulta de Novedades por Empleado"
         HEIGHT             = 26.76
         WIDTH              = 160
         MAX-HEIGHT         = 27.67
         MAX-WIDTH          = 160
...
ON END-ERROR OF W-Win /* Consulta de Novedades por Empleado */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Consulta de Novedades por Empleado */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Consulta de Novedades por Empleado */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-id_empleado-2 ( 1.00 , 21.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.67 , 102.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/b-novedades_empleado.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-novedades_empleado ).
       RUN set-position IN h_b-novedades_empleado ( 2.91 , 21.00 ) NO-ERROR.
       RUN set-size IN h_b-novedades_empleado ( 22.86 , 102.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-id_empleado-2. */
       RUN add-link IN adm-broker-hdl ( h_b-empleados , 'Record':U , h_v-id_empleado-2 ).

       /* Links to SmartBrowser h_b-novedades_empleado. */
       RUN add-link IN adm-broker-hdl ( h_b-empleados , 'Record':U , h_b-novedades_empleado ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-id_empleado-2 ,
             h_b-empleados , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-novedades_empleado ,
             h_v-id_empleado-2 , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/ADP" "Consulta de novedades por empleado"}
