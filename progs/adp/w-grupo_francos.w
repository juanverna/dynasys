...
DEFINE VARIABLE h_b-grupo_francos AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-grupo_francos AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/b-grupo_francos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-grupo_francos ).
       RUN set-position IN h_b-grupo_francos ( 3.86 , 13.00 ) NO-ERROR.
       RUN set-size IN h_b-grupo_francos ( 12.38 , 13.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adp/v-grupo_francos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-grupo_francos ).
       RUN set-position IN h_v-grupo_francos ( 3.86 , 36.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.91 , 74.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 14.33 , 36.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 1.76 , 74.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-grupo_francos. */
       RUN add-link IN adm-broker-hdl ( h_b-grupo_francos , 'Record':U , h_v-grupo_francos ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-grupo_francos ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-grupo_francos ,
             h_b-grupo_francos , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-grupo_francos , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/ADP" "Mantenimiento de Grupos de Francos -"}
