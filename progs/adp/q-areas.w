...
&Scoped-define QUERY-STRING-Query-Main FOR EACH Area WHERE ~{&KEY-PHRASE} ~
      AND Area.cdg_empresa = que_empresa NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH Area WHERE ~{&KEY-PHRASE} ~
      AND Area.cdg_empresa = que_empresa NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main Area
...
cdg_empresa||y|sic.Area.cdg_empresa
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Where[1]         = "Area.cdg_empresa = que_empresa"
...
------------------------------------------------------------------------------*/
  ...
  DEFINE INPUT PARAMETER rid-registro AS ROWID.
  
  REPOSITION {&QUERY-NAME} TO ROWID rid-registro.
  GET NEXT {&QUERY-NAME}.
