...
/*------------------------------------------------------------------------
  ...
  Description: Dynamic SmartDataBrowser Object
...
DO:
  ...
  DEFINE VARIABLE hModify        AS HANDLE    NO-UNDO.
  ...
  hField = {&BROWSE-NAME}:FIRST-COLUMN. 
  DO WHILE VALID-HANDLE(hField):
    IF hField = {&BROWSE-NAME}:CURRENT-COLUMN AND 
       LOOKUP(hField:NAME,cSearchFields) NE 0 THEN
      ASSIGN hModify = hField.
    ASSIGN cFields = (IF cFields = '' THEN STRING(hField) 
                     ELSE cFields + ',':U + STRING(hField)) WHEN hModify NE hField
           hField  = hField:NEXT-COLUMN.
  END.
  IF VALID-HANDLE(hModify) THEN
  DO:
    SESSION:SET-WAIT-STATE('general':U).
    DO i = 1 TO NUM-ENTRIES(cFields):
     ASSIGN 
        hField                = WIDGET-HANDLE(ENTRY(i,cFields))
        hField:COLUMN-BGCOLOR = 15
        hField:COLUMN-FGCOLOR = 0.
    END.
    ASSIGN hModify:COLUMN-BGCOLOR = 9 
           hModify:COLUMN-FGCOLOR = 10. 
    DYNAMIC-FUNCTION('ModifySearchfield':U,hModify:NAME).
