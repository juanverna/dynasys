...
DEFINE VARIABLE h_b-agendas AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-agenda_telefonica AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Mantenimiento de Agendas Telefónicas"
         HEIGHT             = 8.33
         WIDTH              = 96.6
         MAX-HEIGHT         = 26.62
         MAX-WIDTH          = 159.6
...
ON END-ERROR OF W-Win /* Mantenimiento de Agendas Telefónicas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Mantenimiento de Agendas Telefónicas */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Mantenimiento de Agendas Telefónicas */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-agendas ).
       RUN set-position IN h_b-agendas ( 1.48 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-agendas ( 7.38 , 15.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-agenda_telefonica.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-agenda_telefonica ).
       RUN set-position IN h_v-agenda_telefonica ( 1.48 , 19.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 6.48 , 19.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.38 , 77.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-agenda_telefonica. */
       RUN add-link IN adm-broker-hdl ( h_b-agendas , 'Record':U , h_v-agenda_telefonica ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-agenda_telefonica ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-agenda_telefonica ,
             h_b-agendas , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-agenda_telefonica , 'AFTER':U ).
    END. /* Page 0 */
