...
&Scoped-define QUERY-STRING-br_table FOR EACH Caj_header OF Empresa WHERE ~{&KEY-PHRASE} NO-LOCK ~
    BY Caj_header.cdg_empresa ~
       BY Caj_header.tip_comprob ~
        BY Caj_header.prf_comprob ~
         BY Caj_header.nro_comprob INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Caj_header OF Empresa WHERE ~{&KEY-PHRASE} NO-LOCK ~
    BY Caj_header.cdg_empresa ~
       BY Caj_header.tip_comprob ~
        BY Caj_header.prf_comprob ~
         BY Caj_header.nro_comprob INDEXED-REPOSITION.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Caj_header.tip_comprob COLUMN-LABEL "Tipo!Doc" FORMAT "X(3)":U
            WIDTH 7.2
...
/* Query rebuild information for BROWSE br_table
     ...
     _OrdList          = "sic.Caj_header.cdg_empresa|yes,sic.Caj_header.tip_comprob|yes,sic.Caj_header.prf_comprob|yes,sic.Caj_header.nro_comprob|yes"
     _FldNameList[1]   > sic.Caj_header.tip_comprob
"Caj_header.tip_comprob" "Tipo!Doc" ? "character" ? ? ? ? ? ? no ? no no "7.2" yes no no "U" "" ""
     _FldNameList[2]   > sic.Caj_header.prf_comprob
"Caj_header.prf_comprob" "Pre-!fijo" ? "integer" ? ? ? ? ? ? no ? no no "7.2" yes no no "U" "" ""
     _FldNameList[3]   > sic.Caj_header.nro_comprob
"Caj_header.nro_comprob" "Número!Comprobante" ? "integer" ? ? ? ? ? ? no ? no no "13.2" yes no no "U" "" ""
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* Movimientos */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* Movimientos */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* Movimientos */
DO:
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute PRIOR to standard behavior. */
    ...
    BROWSE {&BROWSE-NAME}:TITLE = "Recuperando Datos ...".
  ...
  /* Code placed here will execute AFTER standard behavior.    */
    ...
    BROWSE {&BROWSE-NAME}:TITLE = "Movimientos".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE reposicionar_query B-table-Win 
PROCEDURE reposicionar_query :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    DEFINE INPUT PARAMETER p-prefijo AS INTEGER.
    DEFINE INPUT PARAMETER p-numero  AS INTEGER.

    BROWSE {&BROWSE-NAME}:TITLE = "Reposicionando ...".

    FIND FIRST Caj_header 
         WHERE Caj_header.cdg_empresa  = Empresa.cdg_empresa
           AND Caj_header.tip_comprob  = p-tipo
           AND Caj_header.prf_comprob  = p-prefijo
           AND Caj_header.nro_comprob >= p-numero NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Caj_header 
       THEN FIND LAST Caj_header NO-LOCK.
    REPOSITION {&BROWSE-NAME} TO ROWID ROWID(Caj_header).
