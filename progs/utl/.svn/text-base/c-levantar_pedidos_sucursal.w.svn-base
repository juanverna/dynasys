...
&Scoped-Define ENABLED-OBJECTS v-fecha_proceso v-arch_entrada btn_elegir ~
...
ON CHOOSE OF btn_elegir IN FRAME DEFAULT-FRAME /* Archivos */
DO:
  DEFINE VARIABLE ok AS LOGICAL.
  ASSIGN FRAME {&FRAME-NAME} v-arch_entrada.
  RUN seleccionar_archivo ( INPUT "hd", INPUT-OUTPUT v-arch_entrada, OUTPUT ok ).
  IF ok 
  THEN DO:
      v-arch_entrada-2 = REPLACE(v-arch_entrada,"hd","dt").
      v-arch_salida = REPLACE(SUBSTRING(v-arch_entrada,3),".txt","(BAJADOS).txt").
      DISPLAY v-arch_entrada
              v-arch_entrada-2
              v-arch_salida
...
DO:
    ...
    ELSE DO:
        ...
        ELSE DO: 
                 ...
                 RUN levantar_pedidos_sucursal.p ( INPUT v-arch_entrada,
                                                   INPUT v-arch_entrada-2,
                                                   INPUT v-arch_salida,
                                                   INPUT v-arch_copia1,
                                                   INPUT v-arch_copia2,
                                                   INPUT NO /* No es prueba */).
...
------------------------------------------------------------------------------*/
  ...
  ENABLE v-fecha_proceso v-arch_entrada btn_elegir v-arch_entrada-2 
         v-arch_salida btn_hacer-2 btn_hacer RECT-1 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE seleccionar_archivo C-Win 
PROCEDURE seleccionar_archivo :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE INPUT        PARAMETER v-comienzo AS CHARACTER.
...
SYSTEM-DIALOG GET-FILE v-archivo
      FILTERS "Texto Plano (*.TXT)" "*.TXT"
      INITIAL-FILTER 1
      DEFAULT-EXTENSION v-comienzo + "*.txt"
      MUST-EXIST
      INITIAL-DIR "\\Server\work\PROGRESS\PedidosInternet" 
      RETURN-TO-START-DIR 
