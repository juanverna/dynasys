...
DEFINE VARIABLE h_b-error AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-error AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Mantenimiento de tabla de errores y mensajes"
         HEIGHT             = 25.43
         WIDTH              = 160
         MAX-HEIGHT         = 26.48
         MAX-WIDTH          = 160
...
ON END-ERROR OF W-Win /* Mantenimiento de tabla de errores y mensajes */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Mantenimiento de tabla de errores y mensajes */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Mantenimiento de tabla de errores y mensajes */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-error ).
       RUN set-position IN h_b-error ( 1.00 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-error ( 15.71 , 154.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-error ).
       RUN set-position IN h_v-error ( 17.67 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.95 , 153.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 23.86 , 3.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.24 , 154.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-error. */
       RUN add-link IN adm-broker-hdl ( h_b-error , 'Record':U , h_v-error ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-error ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-error ,
             h_b-error , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/BAS" "Mantenimiento de Tabla de Errores y Mensajes"}
