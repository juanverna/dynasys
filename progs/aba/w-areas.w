...
DEFINE VARIABLE h_b-area AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-area AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     Btn_Done AT ROW 1.48 COL 92
     RECT-1 AT ROW 1.24 COL 90
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-area ).
       RUN set-position IN h_b-area ( 1.29 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-area ( 17.57 , 15.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-area ).
       RUN set-position IN h_v-area ( 3.86 , 23.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.38 , 84.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 11.95 , 23.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.14 , 85.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-area. */
       RUN add-link IN adm-broker-hdl ( h_b-area , 'Record':U , h_v-area ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-area ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-area ,
             Btn_Done:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-area ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/ABA" "Mantenimiento de Areas"}
