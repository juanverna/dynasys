...
DEFINE VARIABLE h_b-solicitantes AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-solicitante AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-solicitantes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-solicitantes ).
       RUN set-position IN h_b-solicitantes ( 1.27 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 19.12 , 15.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-solicitante.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-solicitante ).
       RUN set-position IN h_v-solicitante ( 1.27 , 19.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.77 , 52.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 5.31 , 19.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 8.35 , 52.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-solicitante. */
       RUN add-link IN adm-broker-hdl ( h_b-solicitantes , 'Record':U , h_v-solicitante ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-solicitante ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-solicitantes ,
             Btn_Done:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-solicitante ,
             h_b-solicitantes , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/ABA" "Mantenimiento de Solicitantes"}
