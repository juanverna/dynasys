...
&Scoped-Define ENABLED-OBJECTS RECT-4 Btn_Done-2 
...
DEFINE VARIABLE h_b-asignar_existencias AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-asignar_existencias AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     Btn_Done-2 AT ROW 20.35 COL 94
     RECT-4 AT ROW 20.12 COL 93
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 113.43 BY 22.15
         DEFAULT-BUTTON Btn_Done-2.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "SIC/ABA - Asignación de existencias"
         HEIGHT             = 21.23
         WIDTH              = 114.29
         MAX-HEIGHT         = 22.15
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF W-Win /* SIC/ABA - Asignación de existencias */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* SIC/ABA - Asignación de existencias */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-asignar_existencias.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-asignar_existencias ).
       RUN set-position IN h_b-asignar_existencias ( 2.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.04 , 111.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-asignar_existencias.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-asignar_existencias ).
       RUN set-position IN h_v-asignar_existencias ( 13.92 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.19 , 90.00 ) */
       ...
       RUN set-position IN h_v-remanente ( 13.92 , 93.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.19 , 21.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-editar ).
       RUN set-position IN h_p-editar ( 20.12 , 3.00 ) NO-ERROR.
       RUN set-size IN h_p-editar ( 1.77 , 90.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-deposito ).
       RUN set-position IN h_q-deposito ( 1.00 , 57.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 6.43 ) */
       ...
       RUN set-position IN h_q-arti-depo ( 1.00 , 65.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.15 , 5.00 ) */

       /* Links to SmartBrowser h_b-asignar_existencias. */
       RUN add-link IN adm-broker-hdl ( h_q-deposito , 'Record':U , h_b-asignar_existencias ).

       /* Links to SmartViewer h_v-asignar_existencias. */
       RUN add-link IN adm-broker-hdl ( h_b-asignar_existencias , 'Record':U , h_v-asignar_existencias ).
       RUN add-link IN adm-broker-hdl ( h_p-editar , 'TableIO':U , h_v-asignar_existencias ).
       ...
       RUN add-link IN adm-broker-hdl ( h_b-asignar_existencias , 'Record':U , h_q-arti-depo ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-id-deposito ,
             Btn_Done-2:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-asignar_existencias ,
             h_v-id-deposito , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-asignar_existencias ,
             h_b-asignar_existencias , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-remanente ,
             h_v-asignar_existencias , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-editar ,
             h_v-remanente , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-deposito ,
             Btn_Done-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-arti-depo ,
             h_q-deposito , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/ABA" "Asignación de existencias"}
