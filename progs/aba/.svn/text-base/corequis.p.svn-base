...
FORM 
     ...
     B-Rqs_header.fecha FGCOLOR fe_c BGCOLOR be_c 
     SKIP(0.5)
     btn_COPIAR AT 1
     btn_CANCEL AT 36
     btn_SALIR AT 69
...
DO:
   ...
   IF LOOKUP(INPUT FRAME frm-copiar B-Rqs_header.tip_comprob, 
                                                  tipos_validos) = 0 
   THEN DO:
...
DO:
   ...
   IF LOOKUP(INPUT FRAME frm-copiar B-Rqs_header.tip_comprob, tipos_validos) = 0 
   THEN DO:
      RUN PONMENSJ.P (INPUT "DOCS010").
      RETURN NO-APPLY.
   END.
...
DO:
  ...
  RUN SELREQUI.P (OUTPUT ult_Rqs_head).
  IF ult_Rqs_head <> ?
  THEN DO:
