...
DEFINE VARIABLE v-autorizante  AS CHARACTER FORMAT "X(75)".
DEFINE VARIABLE prciva         LIKE Impuesto.tasa FORMAT "ZZ9.99".
DEFINE VARIABLE prcnoi         LIKE Impuesto.tasa FORMAT "ZZ9.99".
DEFINE VARIABLE importe_iva    LIKE Rqs_header.imp_neto FORMAT "Z,ZZZ,ZZ9.99-".
DEFINE VARIABLE importe_noi    LIKE Rqs_header.imp_neto FORMAT "Z,ZZZ,ZZ9.99-".
...
DO TRANSACTION:
    ...
    FIND Rqs_header WHERE ROWID(Rqs_header) = rid_pedido EXCLUSIVE-LOCK.

    FIND Solicitante     OF Rqs_header NO-LOCK NO-ERROR.
    FIND Deposito        OF Rqs_header NO-LOCK.
    
    /*FIND Estado   OF Rqs_header NO-LOCK.*/
    v-autorizante = "".
    
    que_mes = STRING(MONTH(Rqs_header.fecha),"99").
    que_ano = STRING(YEAR(Rqs_header.fecha),"9999").
    que_dia = STRING(DAY(Rqs_header.fecha),"99").
    str_fecha = que_dia + " de " + nom_mes [ MONTH(Rqs_header.fecha) ] + " de " + que_ano.
...
PROCEDURE inicia_hoja:
  ...
  /*PUT STREAM Formulario CONTROL '<TOOLBAR=!PRINT>'.                   */
    ...
    PUT STREAM Formulario CONTROL "<OPORTRAIT><Title=Impresión de Pedido Nro.:" + STRING(Rqs_header.nro_comprob,'999999')
...
*/
    ...
    RUN escribir ( INPUT "07,50", INPUT "REQUERIMIENTO DE MERCADERIA", INPUT 14, INPUT YES). 
    RUN escribir ( INPUT "07,165", INPUT "<B>NRO:</B>" + STRING(Rqs_header.nro_comprob), INPUT 12, INPUT NO). 
    ...
    RUN escribir ( INPUT "20,186", INPUT Rqs_header.cdg_empresa, INPUT 12, INPUT YES). 
/*
    RUN escribir ( INPUT "33,20", INPUT "[" + Proveedor.cdg_Proveedor + "] " + Proveedor.nombre, INPUT 10, INPUT YES).
    RUN escribir ( INPUT "33,115", INPUT "<B>Alta:</B>" + STRING(Rqs_header.fecha,"99/99/99") , INPUT 10, INPUT NO). 

    RUN escribir ( INPUT "37,20", INPUT Domicilio_prv.direccion, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "37,115", INPUT "<B>Estado:</B>" + Rqs_header.cdg_estado, INPUT 10, INPUT NO).
*/
    IF AVAILABLE Solicitante
        THEN RUN escribir ( INPUT "41,115", INPUT "<B>Solicita:</B>" + Solicitante.nombre, INPUT 10, INPUT NO).
    RUN escribir ( INPUT "45,115", INPUT "<B>Autoriza:</B>" + v-autorizante, INPUT 10, INPUT NO).
/*
    RUN escribir ( INPUT "41,20", INPUT "(" + Domicilio_prv.cdg_postal + ") " + Domicilio_prv.localidad, INPUT 8, INPUT NO).
    
    RUN escribir ( INPUT "41,115", INPUT "<B>Entrega en:</B>" + Deposito.nombre, INPUT 10, INPUT NO).
...
*/
  ...
  /*RUN escribir ( ch_linea(det0 + 1,-1)  + ",146", INPUT "Precio sin IVA", INPUT 8, INPUT NO).*/
    ...
    /*
    RUN escribenumero ( INPUT "267,141", INPUT Rqs_header.imp_neto, INPUT 12, INPUT YES).
    RUN escribenumero ( INPUT "267,169", INPUT Rqs_header.imp_total, INPUT 12, INPUT YES).
    */
...
PROCEDURE forma:
    ...
    FOR EACH Rqs_detalle OF Rqs_header NO-LOCK, Articulo OF Rqs_detalle NO-LOCK,
        Partida OF Rqs_detalle NO-LOCK
        ...
        RUN escribenumero ( ch_linea(det0,linea0)  + ",117", INPUT STRING(Rqs_detalle.cantidad,"ZZZZZZ9"), INPUT 8, INPUT NO).
      /*RUN escribenumero ( ch_linea(det0,linea0)  + ",145", INPUT STRING(Rqs_detalle.precio,"->>>>9.9999"), INPUT 8, INPUT NO). */
        RUN escribenumero ( ch_linea(det0,linea0)  + ",170", INPUT STRING(Rqs_detalle.fecha_temprana,"99/99/99"), INPUT 8, INPUT NO). 
        /*RUN linea         ( ch_linea(det0 + 3,linea0)  + ",170", INPUT "23", "H" ).*/
        linea0 = linea0 + 1.

        total_articulo = total_articulo + Rqs_detalle.cantidad.    
    ...
    RUN escribir ( ch_linea(det0,linea0 + 1)  + ",30", INPUT "FIN DE LOS ITEMS", INPUT 12, INPUT YES).
    
    IF Rqs_header.leyenda <> ""
    THEN DO:    
        RUN RENGLONS.P (INPUT  Rqs_header.leyenda, 
