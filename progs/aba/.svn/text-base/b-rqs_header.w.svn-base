...
&Scoped-define QUERY-STRING-br_table FOR EACH Rqs_header OF Area WHERE ~{&KEY-PHRASE} ~
      AND Rqs_header.cdg_estado = v-estado NO-LOCK ~
...
&Scoped-Define ENABLED-OBJECTS v-estado v-numero br_table RECT-6 
&Scoped-Define DISPLAYED-OBJECTS v-estado v-numero 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Rqs_header.nro_comprob COLUMN-LABEL "Número" FORMAT "ZZZZZZZ9":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 15 BY 20.71
...
DEFINE FRAME F-Main
     v-estado AT ROW 1.48 COL 2 NO-LABEL
     v-numero AT ROW 3.38 COL 2 NO-LABEL
...
/* BROWSE-TAB br_table v-numero F-Main */
ASSIGN 
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "sic.Rqs_header OF sic.Area"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "Rqs_header.cdg_estado = v-estado"
     _FldNameList[1]   > sic.Rqs_header.nro_comprob
"Rqs_header.nro_comprob" "Número" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
DO:
  ASSIGN FRAME {&FRAME-NAME} v-estado.
...
DO:
    ASSIGN v-numero.
    FIND FIRST Rqs_header WHERE Rqs_header.nro_comprob >= v-numero NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Rqs_header 
        THEN FIND LAST Rqs_header NO-LOCK.
    REPOSITION {&BROWSE-NAME} TO ROWID ROWID(Rqs_header).
