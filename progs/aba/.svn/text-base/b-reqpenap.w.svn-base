...
&Scoped-define INTERNAL-TABLES Rqs_detalle Rqs_header Articulo Unidad Area
...
Rqs_header.nro_comprob Rqs_header.fecha Area.cdg_area ~
Rqs_detalle.cdg_estado Rqs_detalle.fecha_temprana Rqs_detalle.nro_linea ~
Articulo.cdg_articulo Articulo.descripcion 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Rqs_detalle WHERE ~{&KEY-PHRASE} ~
    ...
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table Rqs_detalle Rqs_header Articulo ~
Unidad Area
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Rqs_header.tip_comprob COLUMN-LABEL "Tipo!Comp."
      Rqs_header.nro_comprob COLUMN-LABEL "Número!Requisición"
      Rqs_header.fecha COLUMN-LABEL "Fecha!Requis."
      Area.cdg_area
      ...
      Articulo.cdg_articulo COLUMN-LABEL "Artículo!Código"
      Articulo.descripcion COLUMN-LABEL "Artículo!Descripción"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 110 BY 7.5
         BGCOLOR 11 FGCOLOR 9 FONT 4
         TITLE BGCOLOR 11 FGCOLOR 9 "Requisiciones pendientes de resolución".
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "sic.Rqs_detalle,sic.Rqs_header OF sic.Rqs_detalle,sic.Articulo OF sic.Rqs_detalle,sic.Unidad OF sic.Articulo,sic.Area OF sic.Rqs_header"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "Rqs_detalle.cdg_estado = ""AC"""
     _FldNameList[1]   > sic.Rqs_header.tip_comprob
"Rqs_header.tip_comprob" "Tipo!Comp." ? "character" ? ? ? ? ? ? no ?
     _FldNameList[2]   > sic.Rqs_header.nro_comprob
"Rqs_header.nro_comprob" "Número!Requisición" ? "integer" ? ? ? ? ? ? no ?
     _FldNameList[3]   > sic.Rqs_header.fecha
"Rqs_header.fecha" "Fecha!Requis." ? "date" ? ? ? ? ? ? no ?
     _FldNameList[4]   = sic.Area.cdg_area
     _FldNameList[5]   > sic.Rqs_detalle.cdg_estado
...
"Rqs_detalle.nro_linea" "Nro.!Linea" ? "integer" ? ? ? ? ? ? no ?
     _FldNameList[8]   > sic.Articulo.cdg_articulo
"Articulo.cdg_articulo" "Artículo!Código" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[9]   > sic.Articulo.descripcion
"Articulo.descripcion" "Artículo!Descripción" ? "character" ? ? ? ? ? ? no ?
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* Requisiciones pendientes de resolución */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* Requisiciones pendientes de resolución */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* Requisiciones pendientes de resolución */
DO:
