...
DEFINE FRAME F-Main
     v-estado AT ROW 14.95 COL 2 NO-LABEL
     "  Lista de empresas a considerar en el proceso:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 9.05 COL 2
          BGCOLOR 5 FGCOLOR 15 
     "  Rango de cobradores a listar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 1.24 COL 2
          BGCOLOR 5 FGCOLOR 15 
     "  Rango de fecha de las rendiciones a listar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 5.29 COL 2
          BGCOLOR 5 FGCOLOR 15 
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Resumen de Rendiciones por Cobrador"
         HEIGHT             = 18.57
         WIDTH              = 103
         MAX-HEIGHT         = 22.33
         MAX-WIDTH          = 114.2
...
ON END-ERROR OF W-Win /* Resumen de Rendiciones por Cobrador */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Resumen de Rendiciones por Cobrador */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-cobrador ( 2.29 , 2.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 6.33 , 2.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-lst-empresas ( 10.38 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN set-position IN h_v-rng-impresora ( 12.91 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 16.57 , 2.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/AFI" "Resumen de Rendiciones por Cobrador"}
...
/*
       RUN  cargoycobrado.p ( INPUT lista_empresas, 
                              INPUT des_cobrador, 
                              INPUT has_cobrador, 
                              INPUT des_fecha, 
                              INPUT has_fecha, 
...
*/ 
       ...
       RUN  resurendcob.p ( INPUT lista_empresas, 
                            INPUT des_cobrador, 
                            INPUT has_cobrador, 
                            INPUT des_fecha, 
                            INPUT has_fecha 
...
------------------------------------------------------------------------------*/
    ...
    RUN veresult.w ( INPUT  "cargoycobrado.txt" ,  /* Archivo */
