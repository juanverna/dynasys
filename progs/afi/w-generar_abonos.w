...
DEFINE VARIABLE h_v-rng-parametros_facturacion AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-rng-punto_venta AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 16.48 COL 13 COLON-ALIGNED NO-LABEL
     "  Rango de puntos de venta a incluir en el proceso" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 2.43 COL 15
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de fechas de vigencias de los contratos a considerar" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 7.19 COL 15
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Parámetros generales del listado" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 11.71 COL 15
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Facturación periódica de contratos"
         HEIGHT             = 21.52
         WIDTH              = 122.4
         MAX-HEIGHT         = 22.33
         MAX-WIDTH          = 122.4
...
ON END-ERROR OF W-Win /* Facturación periódica de contratos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Facturación periódica de contratos */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng-punto_venta.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-punto_venta ).
       RUN set-position IN h_v-rng-punto_venta ( 3.86 , 15.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 8.38 , 15.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng-parametros_facturacion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-parametros_facturacion ).
       RUN set-position IN h_v-rng-parametros_facturacion ( 12.91 , 15.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.95 , 100.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 17.67 , 15.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.14 , 103.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-punto_venta ,
             v-estado:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-fechas ,
             h_v-rng-punto_venta , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-parametros_facturacion ,
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE VARIABLE des_punto        AS INTEGER.
  DEFINE VARIABLE has_punto        AS INTEGER.
  ...
  ERROR_rango = FALSE.
  RUN dar_rango IN h_v-rng-punto_venta ( OUTPUT des_punto, 
                                         OUTPUT has_punto, 
                                         OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "puntoes" ).
  ...
  RUN dar_rango IN h_v-rng-parametros_facturacion
        ( OUTPUT mes_imputa, 
          OUTPUT ano_imputa, 
          OUTPUT fecha_emision, 
          OUTPUT generar_cupones, 
          OUTPUT error_rango ).
  /*IF MONTH(fecha_emision) <> mes_imputa OR 
     YEAR(fecha_emision) <> ano_imputa 
     THEN error_rango = TRUE.
  IF fecha_emision < TODAY THEN error_rango = TRUE.*/
  IF error_rango 
  THEN DO:
  ...
  THEN DO:
      ...
      ReportePath = "AFI/generar_abonos".
       ...
       /*a Imprimir*/
        ...
        RUN crearReporte(chReport,"pdf",/*ViewReport*/ TRUE, /*impresora*/ "" , 
           ...
           RUN borra_temp ( INPUT xFullPath, OUTPUT ERROR_nro ).*/
           v-estado:SCREEN-VALUE in FRAME {&FRAME-NAME} = "Proceso Terminado. Ingresando Parametros ...".
      END.
