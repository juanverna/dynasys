...
DEFINE VARIABLE h_v-rng-emision AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     "     Fechas de Alta" VIEW-AS TEXT
          SIZE 17 BY .62 AT ROW 8 COL 44
     "   Datos de la Emisión" VIEW-AS TEXT
          SIZE 20 BY .62 AT ROW 10.42 COL 43
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Cobranzas de Telemarketing"
         HEIGHT             = 22.35
         WIDTH              = 114.29
         MAX-HEIGHT         = 22.35
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF W-Win /* Cobranzas de Telemarketing */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Cobranzas de Telemarketing */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-promotor ( 1.81 , 10.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-cobrador ( 4.77 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */
       ...
       RUN set-position IN h_v-rng-fechas ( 8.27 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng-emision.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-emision ).
       RUN set-position IN h_v-rng-emision ( 10.96 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.23 , 89.00 ) */
       ...
       RUN set-position IN h_v-lst-empresas ( 14.46 , 10.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-emision ,
             h_v-rng-fechas , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-lst-empresas ,
             h_v-rng-emision , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/AFI" "Cobranzas de Telemarketing"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE desde_emision    AS DATE.
  DEFINE VARIABLE hasta_emision    AS DATE.
  ...
  RUN dar_rango IN h_v-rng-emision  ( OUTPUT desde_emision, 
                                      OUTPUT hasta_emision,
                                      OUTPUT lista_tipos,
                                      OUTPUT v-id_lote,
                                      OUTPUT error_rango ).
  ...
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Período de Emisión" ).
       hay_error = error_rango.
  END.     
  ...
  THEN DO:
       ...
       RUN cobrostel.p ( INPUT lista_empresas, 
                         INPUT des_promotor,
                         INPUT has_promotor,
                         INPUT des_cobrador,
                         INPUT has_cobrador,
...
------------------------------------------------------------------------------*/
    ...
    RUN veresult.w ( INPUT  "cobrosxpromotor.txt" ,  /* Archivo */
