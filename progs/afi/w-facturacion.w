...
DEFINE VARIABLE h_v-rng-periodo AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 18.5 COL 13 COLON-ALIGNED NO-LABEL
     "  Parámetros generales del listado" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 8.81 COL 15
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de cobradores a incluir en el proceso" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 1.27 COL 15
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de fechas de ingreso de los grupos familiares" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 5.04 COL 15
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Indique la empresa para cual desea generar los cupones" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 13.12 COL 15
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Servicios por Motivo de Llamado"
         HEIGHT             = 22.15
         WIDTH              = 114.29
         MAX-HEIGHT         = 22.35
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF W-Win /* Servicios por Motivo de Llamado */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Servicios por Motivo de Llamado */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-cobrador ( 2.35 , 15.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 6.12 , 15.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng-periodo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-periodo ).
       RUN set-position IN h_v-rng-periodo ( 9.88 , 15.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.96 , 89.00 ) */
       ...
       RUN set-position IN h_v-lst-empresas ( 14.19 , 15.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 20.12 , 15.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-periodo ,
             h_v-rng-fechas , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-lst-empresas ,
             h_v-rng-periodo , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
   ...
   RUN cantidad_empresas IN h_v-lst-empresas ( INPUT 1 ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/FAC" "facturación de Servicios Mensualizados"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE listar_grupos    AS LOGICAL.
  DEFINE VARIABLE lista_tipos      AS CHARACTER.
  DEFINE VARIABLE generar_cupones  AS LOGICAL.
  ...
  RUN dar_rango IN h_v-rng-periodo  ( OUTPUT mes_imputa, 
                                      OUTPUT ano_imputa, 
                                      OUTPUT fecha_emision, 
                                      OUTPUT listar_grupos,
                                      OUTPUT generar_cupones, 
                                      OUTPUT lista_tipos,
                                      OUTPUT v-pto_venta,
                                      OUTPUT v-id_lote,
                                      OUTPUT error_rango ).
  ...
  END.     
     ...
     message string(v-pto_venta) view-as alert-box message title "lst-ejecutar".
  ...
  THEN DO:
       ...
       RUN genfacturacion.p ( INPUT lista_empresas, 
                              INPUT des_cobrador, 
                              INPUT has_cobrador, 
                              INPUT des_fecha, 
                              INPUT has_fecha, 
...
------------------------------------------------------------------------------*/
    ...
    RUN veresult.w ( INPUT  "genfacturacion.txt" ,  /* Archivo */
