...
ON + OF des_registro IN FRAME F-Main /* Desde Plan */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON MOUSE-MENU-DOWN OF des_registro IN FRAME F-Main /* Desde Plan */
DO:
  &SCOPED-DEFINE ROWID_TABLA        rid_descobrador
  &SCOPED-DEFINE SELECCION          SELPLAN.P
  &SCOPED-DEFINE TABLA              Plan
  &SCOPED-DEFINE CDG_TABLA          cdg_plan
  &SCOPED-DEFINE DSC_TABLA          dsc_plan
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON RETURN OF des_registro IN FRAME F-Main /* Desde Plan */
DO:

    ASSIGN FRAME {&FRAME-NAME} des_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND Plan WHERE Plan.cdg_plan = SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
        ELSE FIND FIRST Plan NO-LOCK NO-ERROR.
    
    IF AVAILABLE Plan 
    THEN DO:
         des_nombre = Plan.dsc_plan.
         des_registro = Plan.cdg_plan.
    END.
    ELSE DO:
         des_nombre = "???".
    END.
...
ON + OF has_registro IN FRAME F-Main /* Hasta Plan */
DO:
  APPLY "MOUSE-MENU-DOWN" TO SELF.
  RETURN NO-APPLY.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL has_registro V-table-Win
ON MOUSE-MENU-DOWN OF has_registro IN FRAME F-Main /* Hasta Plan */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL has_registro V-table-Win
ON RETURN OF has_registro IN FRAME F-Main /* Hasta Plan */
DO:

    ASSIGN FRAME {&FRAME-NAME} has_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND Plan WHERE Plan.cdg_plan = SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
        ELSE FIND LAST Plan NO-LOCK NO-ERROR.
    
    IF AVAILABLE Plan 
    THEN DO:
         has_nombre = Plan.dsc_plan.
         has_registro = Plan.cdg_plan.
    END.
    ELSE DO:
         has_nombre = "???".
    END.
