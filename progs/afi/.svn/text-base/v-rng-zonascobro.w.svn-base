...
ON + OF des_registro IN FRAME F-Main /* Desde Zona */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON MOUSE-MENU-DOWN OF des_registro IN FRAME F-Main /* Desde Zona */
DO:
  &SCOPED-DEFINE ROWID_TABLA        rid_deszonag
  &SCOPED-DEFINE SELECCION          SELZONACOB.P
  &SCOPED-DEFINE TABLA              Zona_cobranza
  &SCOPED-DEFINE CDG_TABLA          cdg_zonag
  &SCOPED-DEFINE DSC_TABLA          dsc_zonag
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON RETURN OF des_registro IN FRAME F-Main /* Desde Zona */
DO:

    ASSIGN FRAME {&FRAME-NAME} des_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND Zona_cobranza WHERE Zona_cobranza.cdg_zonag = SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
        ELSE FIND FIRST Zona_cobranza NO-LOCK NO-ERROR.
    
    IF AVAILABLE Zona_cobranza 
    THEN DO:
         des_nombre = Zona_cobranza.dsc_zonag.
         des_registro = Zona_cobranza.cdg_zonag.
    END.
    ELSE DO:
         des_nombre = "???".
    END.
...
ON + OF has_registro IN FRAME F-Main /* Hasta Zona */
DO:
  APPLY "MOUSE-MENU-DOWN" TO SELF.
  RETURN NO-APPLY.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL has_registro V-table-Win
ON MOUSE-MENU-DOWN OF has_registro IN FRAME F-Main /* Hasta Zona */
DO:
  &SCOPED-DEFINE ROWID_TABLA        rid_haszonag
  &SCOPED-DEFINE SELECCION          SELZONACOB.P
  &SCOPED-DEFINE TABLA              Zona_cobranza
  &SCOPED-DEFINE CDG_TABLA          cdg_zonag
  &SCOPED-DEFINE DSC_TABLA          dsc_zonag
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL has_registro V-table-Win
ON RETURN OF has_registro IN FRAME F-Main /* Hasta Zona */
DO:

    ASSIGN FRAME {&FRAME-NAME} has_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND Zona_cobranza WHERE Zona_cobranza.cdg_zonag = SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
        ELSE FIND LAST Zona_cobranza NO-LOCK NO-ERROR.
    
    IF AVAILABLE Zona_cobranza 
    THEN DO:
         has_nombre = Zona_cobranza.dsc_zonag.
         has_registro = Zona_cobranza.cdg_zonag.
    END.
    ELSE DO:
         has_nombre = "???".
    END.
