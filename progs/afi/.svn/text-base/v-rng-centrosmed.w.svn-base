...
ON + OF des_registro IN FRAME F-Main /* Desde Centro */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON MOUSE-MENU-DOWN OF des_registro IN FRAME F-Main /* Desde Centro */
DO:
  &SCOPED-DEFINE ROWID_TABLA        rid_descobrador
  &SCOPED-DEFINE SELECCION          SELCENTROMED.P
  &SCOPED-DEFINE TABLA              Centromedico
  &SCOPED-DEFINE CDG_TABLA          centro_medico
  &SCOPED-DEFINE DSC_TABLA          nombre_centro
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON RETURN OF des_registro IN FRAME F-Main /* Desde Centro */
DO:

    ASSIGN FRAME {&FRAME-NAME} des_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND Centromedico WHERE Centromedico.centro_medico = SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
        ELSE FIND FIRST Centromedico NO-LOCK NO-ERROR.
    
    IF AVAILABLE Centromedico 
    THEN DO:
         des_nombre = Centromedico.nombre.
         des_registro = Centromedico.centro_medico.
    END.
    ELSE DO:
         des_nombre = "???".
    END.
...
ON + OF has_registro IN FRAME F-Main /* Hasta Centro */
DO:
  APPLY "MOUSE-MENU-DOWN" TO SELF.
  RETURN NO-APPLY.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL has_registro V-table-Win
ON MOUSE-MENU-DOWN OF has_registro IN FRAME F-Main /* Hasta Centro */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL has_registro V-table-Win
ON RETURN OF has_registro IN FRAME F-Main /* Hasta Centro */
DO:

    ASSIGN FRAME {&FRAME-NAME} has_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND Centromedico WHERE Centromedico.centro_medico = SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
        ELSE FIND LAST Centromedico NO-LOCK NO-ERROR.
    
    IF AVAILABLE Centromedico 
    THEN DO:
         has_nombre = Centromedico.nombre.
         has_registro = Centromedico.centro_medico.
    END.
    ELSE DO:
         has_nombre = "???".
    END.
