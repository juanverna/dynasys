...
&Scoped-Define ENABLED-OBJECTS RECT-1 des_fecha des_hora has_fecha has_hora 
&Scoped-Define DISPLAYED-OBJECTS des_fecha des_hora has_fecha has_hora 
...
DEFINE VARIABLE des_hora AS INTEGER FORMAT "99":U INITIAL 0 
     VIEW-AS FILL-IN 
     SIZE 4 BY .77
     BGCOLOR 15 FGCOLOR 9  NO-UNDO.
...
DEFINE VARIABLE has_hora AS INTEGER FORMAT "99":U INITIAL 0 
     VIEW-AS FILL-IN 
     SIZE 4 BY .77
     BGCOLOR 15 FGCOLOR 9  NO-UNDO.
...
DEFINE FRAME F-Main
     des_fecha AT ROW 1.27 COL 16 COLON-ALIGNED
     des_hora AT ROW 1.27 COL 29 COLON-ALIGNED NO-LABEL
     has_fecha AT ROW 1.27 COL 65 COLON-ALIGNED
     has_hora AT ROW 1.27 COL 78 COLON-ALIGNED NO-LABEL
     "Hs." VIEW-AS TEXT
          SIZE 3 BY .88 AT ROW 1.27 COL 36
     "Hs." VIEW-AS TEXT
          SIZE 3 BY .88 AT ROW 1.27 COL 85
...
------------------------------------------------------------------------------*/
  ...
  DEFINE OUTPUT PARAMETER p-des_hora    AS INTEGER.
  DEFINE OUTPUT PARAMETER p-has_fecha   AS DATE.
  DEFINE OUTPUT PARAMETER p-has_hora    AS INTEGER.
  ...
  DO WITH FRAME {&FRAME-NAME}:
        ...
        IF ERROR-STATUS:ERROR THEN error_rango = YES.
...
------------------------------------------------------------------------------*/
  ...
  has_hora = INTEGER(SUBSTRING(STRING(TIME,"HH:MM:SS"),1,2)).
  has_fecha = TODAY.
  des_fecha = has_fecha.
    
  des_hora = has_hora - 12. /* Ultimas 12 horas */
  IF des_hora < 0 
  THEN DO:
       des_hora = des_hora + 24.
       des_fecha = des_fecha - 1.
  END.     

  DISPLAY des_fecha des_hora has_fecha has_hora WITH FRAME {&FRAME-NAME}.
