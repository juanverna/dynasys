...
DEFINE VARIABLE h_v-lst-estados AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     "   Lista de estados a verificar (0,2):" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 11.5 COL 10
          BGCOLOR 5 FGCOLOR 14 FONT 6
     "   Impresora por defecto:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 14.19 COL 10
          BGCOLOR 5 FGCOLOR 14 FONT 6
     "   Período de emisión de los cupones:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 5.31 COL 10
          BGCOLOR 5 FGCOLOR 14 FONT 6
     "   Lista de empresas a considerar:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 8.27 COL 10
          BGCOLOR 5 FGCOLOR 14 FONT 6
     "   Rango de cobradores a verificar:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 1.54 COL 10
          BGCOLOR 5 FGCOLOR 14 FONT 6
...
ON END-ERROR OF W-Win /* Verificación de Arqueo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Verificación de Arqueo */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-cobrador ( 2.62 , 10.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 6.38 , 10.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-lst-empresas ( 9.35 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.88 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-lst-estados.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-lst-estados ).
       RUN set-position IN h_v-lst-estados ( 12.58 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.35 , 46.00 ) */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/AFI" "Verificación de Arqueo"}
...
------------------------------------------------------------------------------*/
  ...
  IF NUM-ENTRIES(lista_empresas,",") <> 1
  THEN DO:
       MESSAGE "Indique una y solo una empresa" VIEW-AS ALERT-BOX ERROR.
       hay_error = YES.
  END.     
  ...
  RUN dar_rango IN h_v-lst-estados  ( OUTPUT lista_estados, 
                                       OUTPUT error_rango ).

  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Estados" ).
       hay_error = error_rango.
  END.     
  ...
  THEN DO:
       ...
       RUN verarqueo.p ( INPUT lista_empresas,
                         INPUT des_cobrador, 
                         INPUT has_cobrador, 
                         INPUT des_fecha, 
                         INPUT has_fecha,
