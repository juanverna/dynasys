...
DEFINE VARIABLE h_v-lst-planes AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     "     Rango de Fechas de Alta" VIEW-AS TEXT
          SIZE 89 BY .62 AT ROW 1.81 COL 10
          BGCOLOR 5 FGCOLOR 14 
     "     Lista de empresas a considerar" VIEW-AS TEXT
          SIZE 89 BY .62 AT ROW 12.04 COL 10
          BGCOLOR 5 FGCOLOR 14 
     "     Lista de planes a considerar" VIEW-AS TEXT
          SIZE 89 BY .62 AT ROW 8.81 COL 10
          BGCOLOR 5 FGCOLOR 14 
     "     Archivo de salida para interface" VIEW-AS TEXT
          SIZE 89 BY .62 AT ROW 5.58 COL 10
          BGCOLOR 5 FGCOLOR 14 
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Listado de Afiliados x Plan"
         HEIGHT             = 22.35
         WIDTH              = 114.29
         MAX-HEIGHT         = 22.35
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF W-Win /* Listado de Afiliados x Plan */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Listado de Afiliados x Plan */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-fechas ( 2.62 , 10.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-archivo ( 6.38 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.88 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-lst-planes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-lst-planes ).
       RUN set-position IN h_v-lst-planes ( 9.62 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.88 , 89.00 ) */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/AFI" "Afiliados x Plan"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE lista_planes     AS CHARACTER.
  ...
  RUN dar_rango IN h_v-lst-planes  ( OUTPUT lista_planes, 
                                       OUTPUT error_rango ).

  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Planes" ).
       hay_error = error_rango.
  END.     
  ...
  THEN DO:
       ...
       RUN afiliadosxplan.p ( INPUT des_fecha,
                              INPUT has_fecha,
...
------------------------------------------------------------------------------*/
    ...
    RUN veresult.w ( INPUT  "c:\sic-temp\afiliadosxplan.txt",  /* Archivo */
