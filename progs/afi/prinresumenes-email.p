...
DEFINE INPUT PARAMETER p-fmoroso AS DATE.
...
DEFINE TEMP-TABLE T-param
        FIELD ver_por AS INT
        FIELD has_fecha AS DATE
        FIELD fmoroso AS DATE.
...
DEFINE TEMP-TABLE T-Administrador 
        ...
        FIELD observacion              LIKE persona.observacion
...
DEFINE DATASET dset FOR T-Administrador,T-Cta_cte,T-param
...
/*=========================================================================================*/
CREATE t-param.
t-param.has_fecha = p-has_fecha.
t-param.ver_por = p-ver_por.
t-param.fmoroso = p-fmoroso.
...
/*ver contacto con email*/
       ...
       FOR EACH Cliente-contacto OF Domicilio WHERE can-do(Cliente-contacto.canal-email,"COB"), first Persona OF Cliente-contacto WHERE persona.email <> "" NO-LOCK :
            T-Administrador.email = persona.email.
            t-Administrador.observacion = persona.observacion.
            LEAVE.
       END.
       IF t-administrador.email = "" THEN
           FOR EACH Cliente-contacto OF Domicilio WHERE cliente-contacto.cdg_cargo BEGINS "ADM" , first Persona OF Cliente-contacto WHERE persona.email <> "" and  can-do(Cliente-contacto.canal-email,"COB") NO-LOCK :
                T-Administrador.email = persona.email.
                T-Administrador.observacion = persona.observacion.
                LEAVE.
           END.
       IF t-administrador.email = "" THEN
           FOR EACH Cliente-contacto OF Domicilio , first Persona OF Cliente-contacto WHERE persona.email <> "" and  can-do(Cliente-contacto.canal-email,"COB") NO-LOCK :
                T-Administrador.email = persona.email.
                T-Administrador.observacion = persona.observacion.
                LEAVE.
           END.
       ...
       senal = FALSE.
           ...
           FOR EACH Cta_cte NO-LOCK
                ...
                IF NOT cta_cte.tip_comprob BEGINS "F" THEN NEXT.
                ...
                BUFFER-COPY Cta_cte TO T-Cta_cte
                    ...
                    ASSIGN T-Cta_cte.fnComprobante = fnComprobante(cta_cte.tip_comprob,cta_cte.prf_comprob,cta_cte.nro_comprob)
                           ...
                           T-Cta_cte.tip_comprob = substring(fac_header.tip_comprob,1,1)
                           T-Cta_cte.nro_comprob = fac_header.nro_comprob.

                T-Cta_cte.ListaArticulo = "".
                FOR EACH fac_detalle OF fac_header,articulo OF fac_detalle BY fac_detalle.precio DESC:
                    T-Cta_cte.ListaArticulo = Articulo.cdg_tipoart.
                    FIND tipo_articulo OF articulo.
                    T-Cta_cte.desc_ListaArticulo = Tipo_articulo.dsc_tipoart.
                    LEAVE.
                END.
...
END.
pxfile = ?.
FIND FIRST t-administrador NO-LOCK.
IF NOT AVAILABLE t-administrador THEN RETURN.
pxfile =TempFile("") + ".xml".
