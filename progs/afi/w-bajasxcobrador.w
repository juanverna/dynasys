...
DEFINE VARIABLE h_v-lst-motivos AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 17.96 COL 8 COLON-ALIGNED NO-LABEL
     "     Fechas de Alta" VIEW-AS TEXT
          SIZE 17 BY .62 AT ROW 4.23 COL 44
     "   Fechas de Baja" VIEW-AS TEXT
          SIZE 17 BY .62 AT ROW 7.19 COL 45
...
ON END-ERROR OF W-Win /* Bajas por Cobrador */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Bajas por Cobrador */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-cobrador ( 1.54 , 10.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 4.50 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */
       ...
       RUN set-position IN h_v-rng-emision ( 7.46 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.23 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-lst-motivos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-lst-motivos ).
       RUN set-position IN h_v-lst-motivos ( 11.23 , 10.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-lst-empresas ( 13.38 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.88 , 89.00 ) */
       ...
       RUN set-position IN h_v-rng-impresora ( 15.54 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.88 , 89.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 19.31 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/AFI" "Bajas por Cobrador"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE lista_motivos    AS CHARACTER.
  ...
  RUN dar_rango IN h_v-lst-motivos  ( OUTPUT lista_motivos, 
                                       OUTPUT error_rango ).

  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Motivos de Baja" ).
       hay_error = error_rango.
  END.     
  ...
  THEN DO:
       ...
       RUN bajasxcobrador.p ( INPUT lista_empresas, 
                              INPUT des_cobrador,
                              INPUT has_cobrador,
                              INPUT des_fecha,
                              INPUT has_fecha,
...
------------------------------------------------------------------------------*/
    ...
    RUN veresult.w ( INPUT  "bajasxcobrador.txt" ,  /* Archivo */
