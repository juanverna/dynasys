...
DEFINE VARIABLE h_v-que-printer AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 9.19 COL 3.4 NO-LABEL
     "  Clientes a listar::" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 1 COL 3.4
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "Movimientos con fecha de emision hasta el" VIEW-AS TEXT
          SIZE 99.8 BY 1 AT ROW 5.29 COL 3 WIDGET-ID 4
          BGCOLOR 5 FGCOLOR 15 FONT 6
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng-cliente-incl.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-cliente ).
       RUN set-position IN h_v-rng-cliente ( 2.19 , 2.80 ) NO-ERROR.
       /* Size in UIB:  ( 2.86 , 102.00 ) */
       ...
       RUN set-position IN h_v-que-fecha ( 6.52 , 9.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 42.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-que-printer.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-que-printer ).
       RUN set-position IN h_v-que-printer ( 6.52 , 62.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 42.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 10.38 , 1.80 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-que-printer ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/CXC" "Reporte de Movimientos de Cuenta Corriente"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE que_moneda       AS CHARACTER.
  DEFINE VAR timpre AS INT NO-UNDO.
DEFINE VAR incl1 AS LOGICAL.
DEFINE VAR incl2 AS LOGICAL.
  ...
  THEN DO:
       ...
       hay_error = error_rango.
  END.     
  RUN dar_rango IN h_v-que-printer   ( OUTPUT timpre, 
                                       OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
...
IF ver_por = 1 THEN DO:
 FOR EACH Administrador  
    WHERE (( Administrador.cdg_cliente >= des_cliente AND incl1 ) OR ( Administrador.cdg_cliente > des_cliente AND NOT incl1 ))
      AND (( Administrador.cdg_cliente <= has_cliente AND incl2 ) OR ( Administrador.cdg_cliente < has_cliente AND NOT incl2 ))
      AND CAN-DO(Administrador.lista_empresas,empresa.cdg_empresa)
           USE-INDEX cdg_cliente NO-LOCK:
            v-estado:SCREEN-VALUE in FRAME {&FRAME-NAME} = "Procesando " + administrador.nom_cliente.
            RUN imprerec0.p(rowid(administrador),
                          has_fecha,
                          timpre).
 END.
END.
ELSE DO:
 FOR EACH Administrador  
    WHERE (( Administrador.nom_cliente >= des_nombre AND incl1 ) OR ( Administrador.nom_cliente > des_nombre AND NOT incl1 ))
      AND (( Administrador.nom_cliente <= has_nombre AND incl2 ) OR ( Administrador.nom_cliente < has_nombre AND NOT incl2 ))
      AND CAN-DO(Administrador.lista_empresas,empresa.cdg_empresa)
