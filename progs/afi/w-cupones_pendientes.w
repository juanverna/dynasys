...
DEFINE VARIABLE h_b-cupones_pendientes AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Impresión de Cupones Pendientes"
         HEIGHT             = 21.23
         WIDTH              = 114.29
         MAX-HEIGHT         = 21.62
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF W-Win /* Impresión de Cupones Pendientes */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Impresión de Cupones Pendientes */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Impresión de Cupones Pendientes */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-cupones_pendientes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cupones_pendientes ).
       RUN set-position IN h_b-cupones_pendientes ( 2.62 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 18.31 , 109.00 ) */
       ...
       /* Links to SmartBrowser h_b-cupones_pendientes. */
       RUN add-link IN adm-broker-hdl ( h_q-empresa , 'Record':U , h_b-cupones_pendientes ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-id-empresa-2 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cupones_pendientes ,
             h_v-id-empresa-2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-empresa ,
             h_b-cupones_pendientes , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/AFI" "Impresión de Cupones de Cobranza"}
