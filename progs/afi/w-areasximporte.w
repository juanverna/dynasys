...
DEFINE VARIABLE h_v-rng-importes AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 19.04 COL 12 COLON-ALIGNED NO-LABEL
     "  Rango de Importes:" VIEW-AS TEXT
          SIZE 89 BY .62 AT ROW 12.85 COL 14
          BGCOLOR 5 FGCOLOR 14 
     "  Rango de Cobradores a Considerar:" VIEW-AS TEXT
          SIZE 89 BY .62 AT ROW 1.54 COL 14
          BGCOLOR 5 FGCOLOR 14 
     "  Lista de Empresas:" VIEW-AS TEXT
          SIZE 89 BY .62 AT ROW 9.62 COL 14
          BGCOLOR 5 FGCOLOR 14 
     "  Rango de Fechas de Alta:" VIEW-AS TEXT
          SIZE 89 BY .62 AT ROW 5.58 COL 14
          BGCOLOR 5 FGCOLOR 14 
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-cobrador ( 2.62 , 14.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 6.65 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */
       ...
       RUN set-position IN h_v-lst-empresas ( 10.42 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.88 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng-importes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-importes ).
       RUN set-position IN h_v-rng-importes ( 13.65 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.88 , 89.00 ) */
       ...
       RUN set-position IN h_v-rng-impresora ( 16.62 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.88 , 89.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 20.12 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/AFI" "Areas Protegidas por Importe"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE des_importe      AS DECIMAL.
  DEFINE VARIABLE has_importe      AS DECIMAL.
  ...
  RUN dar_rango IN h_v-rng-importes    ( OUTPUT des_importe, 
                                         OUTPUT has_importe, 
                                         OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Importes" ).
       hay_error = error_rango.
  END.     
  ...
  THEN DO:
       ...
       RUN areasximporte.p ( INPUT lista_empresas,
                             INPUT des_cobrador, 
                             INPUT has_cobrador, 
                             INPUT des_fecha, 
                             INPUT has_fecha,
...
------------------------------------------------------------------------------*/
    ...
    RUN veresult.w ( INPUT  "c:\tempdsp\areasximporte.txt" ,  /* Archivo */
