...
DEFINE VARIABLE h_b-pedido-credencial AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-empresa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-id-empresa-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-obs-pedido-cred AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Generación de Credenciales */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Generación de Credenciales */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Generación de Credenciales */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 1.00 , 24.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-id-empresa-2 ( 1.27 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 0.81 , 21.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-pedido-credencial.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_b-pedido-credencial ).
       /* Position in UIB:  ( 2.62 , 2.00 ) */
       /* Size in UIB:  ( 15.62 , 109.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-obs-pedido-cred.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-obs-pedido-cred ).
       RUN set-position IN h_v-obs-pedido-cred ( 18.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.23 , 109.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-empresa ).
       RUN set-position IN h_q-empresa ( 1.00 , 103.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.77 , 8.29 ) */

       /* Links to SmartViewer h_v-id-empresa-2. */
       RUN add-link IN adm-broker-hdl ( h_q-empresa , 'Record':U , h_v-id-empresa-2 ).

       /* Links to  h_b-pedido-credencial. */
       RUN add-link IN adm-broker-hdl ( h_q-empresa , 'Record':U , h_b-pedido-credencial ).

       /* Links to SmartViewer h_v-obs-pedido-cred. */
       RUN add-link IN adm-broker-hdl ( h_b-pedido-credencial , 'Record':U , h_v-obs-pedido-cred ).

       /* Links to SmartQuery h_q-empresa. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-empresa ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-id-empresa-2 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pedido-credencial ,
             h_v-id-empresa-2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-obs-pedido-cred ,
             h_b-pedido-credencial , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-empresa ,
             h_v-obs-pedido-cred , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/AFI" "Generación de Credenciales"}
