...
&Scoped-define QUERY-STRING-F-Main FOR EACH contrato_hd WHERE ROWID(contrato_hd) = rrowid, ~
           FIRST cliente OF contrato_hd
&Scoped-define OPEN-QUERY-F-Main OPEN QUERY {&SELF-NAME} FOR EACH contrato_hd WHERE ROWID(contrato_hd) = rrowid, ~
           FIRST cliente OF contrato_hd.
&Scoped-define TABLES-IN-QUERY-F-Main contrato_hd cliente
&Scoped-define FIRST-TABLE-IN-QUERY-F-Main contrato_hd
...
DEFINE VARIABLE h_b-detalle_contrato-2 AS HANDLE NO-UNDO.
...
/* Query rebuild information for FRAME F-Main
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH contrato_hd WHERE ROWID(contrato_hd) = rrowid,
    FIRST cliente OF contrato_hd.
     _END_FREEFORM
     _Query            is OPENED
*/  /* FRAME F-Main */
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-dsc_cliente ( 1.24 , 32.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-contrato ).
       RUN set-position IN h_v-contrato ( 3.14 , 20.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.95 , 124.00 ) */
       ...
       RUN set-position IN h_v-detalle_contrato1linea ( 16.95 , 44.20 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-detalle_contrato-2 ).
       RUN set-position IN h_b-detalle_contrato-2 ( 17.43 , 6.00 ) NO-ERROR.
       RUN set-size IN h_b-detalle_contrato-2 ( 8.10 , 36.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_p-soloedita , 'TableIO':U , h_v-contrato ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'Record':U , h_v-contrato ).

       /* Links to SmartViewer h_v-detalle_contrato1linea. */
       RUN add-link IN adm-broker-hdl ( h_b-detalle_contrato-2 , 'Record':U , h_v-detalle_contrato1linea ).
       RUN add-link IN adm-broker-hdl ( h_p-soloedita-2 , 'TableIO':U , h_v-detalle_contrato1linea ).

       /* Links to SmartBrowser h_b-detalle_contrato-2. */
       RUN add-link IN adm-broker-hdl ( h_v-contrato , 'Record':U , h_b-detalle_contrato-2 ).
       RUN add-link IN adm-broker-hdl ( h_v-detalle_contrato1linea , 'refresco':U , h_b-detalle_contrato-2 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-detalle_contrato-2 ,
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-position IN h_p-soloedita ( 14.57 , 50.00 ) NO-ERROR.
       RUN set-size IN h_p-soloedita ( 1.76 , 58.00 ) NO-ERROR.
       ...
       RUN set-position IN h_p-soloedita-2 ( 24.33 , 61.00 ) NO-ERROR.
       RUN set-size IN h_p-soloedita-2 ( 1.76 , 68.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-soloedita ,
             h_v-contrato , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-soloedita-2 ,
             h_b-detalle_contrato-2 , 'AFTER':U ).
    END. /* Page 2 */
...
RUN adm-row-available.
  MESSAGE "Esto no aundo aun, paciencia....".
END PROCEDURE.
