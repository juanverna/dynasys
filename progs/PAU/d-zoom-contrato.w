...
DEFINE INPUT PARAMETER pcontrato LIKE contrato_hd.nro_contrato.
...
DEFINE VARIABLE h_q-contrato AS HANDLE NO-UNDO.
...
DEFINE FRAME D-Dialog
     SPACE(149.63) SKIP(22.27)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Zoom Contrato" WIDGET-ID 100.
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* Zoom Contrato */
DO:  
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-dsc_cliente-2 ( 1.48 , 18.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-contrato_consorcios ( 3.38 , 17.00 ) NO-ERROR.
       ...
       RUN set-position IN h_b-detalle_contrato ( 16.00 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-detalle_contrato ( 6.91 , 35.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-detalle_contrato1linea ( 16.00 , 38.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.91 , 112.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'q-contrato.w':U ,
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-contrato ).
       RUN set-position IN h_q-contrato ( 2.19 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.86 , 10.80 ) */

       /* Links to SmartViewer h_v-dsc_cliente-2. */
       RUN add-link IN adm-broker-hdl ( h_q-contrato , 'Record':U , h_v-dsc_cliente-2 ).

       /* Links to SmartViewer h_v-contrato_consorcios. */
       RUN add-link IN adm-broker-hdl ( h_q-contrato , 'Record':U , h_v-contrato_consorcios ).

       /* Links to SmartBrowser h_b-detalle_contrato. */
       RUN add-link IN adm-broker-hdl ( h_q-contrato , 'Record':U , h_b-detalle_contrato ).
...
DYNAMIC-FUNCTION("pcontrato" IN   h_q-contrato, pcontrato).
END PROCEDURE.
