...
&Scoped-Define ENABLED-OBJECTS b-resumen BUTTON-12 
&Scoped-Define DISPLAYED-FIELDS Cliente.nro_cliente 
...
DEFINE FRAME F-Main
     b-resumen AT ROW 1 COL 1 WIDGET-ID 8
     BUTTON-12 AT ROW 1 COL 14 WIDGET-ID 10
     Cliente.nro_cliente AT ROW 1.24 COL 4 COLON-ALIGNED NO-LABEL WIDGET-ID 2
          VIEW-AS FILL-IN 
          SIZE 4 BY .48
...
/* SETTINGS FOR FILL-IN Cliente.nro_cliente IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       Cliente.nro_cliente:HIDDEN IN FRAME F-Main           = TRUE.
...
ON CHOOSE OF BUTTON-12 IN FRAME F-Main /* Geo */
DO:
  RUN w-geocli-adm.w.
END.
...
END.
IF xfile = ? THEN DO:
        MESSAGE "No registra movimientos" VIEW-AS ALERT-BOX INFORMATION.
        RETURN NO-APPLY.
END.
