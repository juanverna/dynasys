...
DEFINE VARIABLE h_b-tapa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-tapas_inspeccion AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tapa ).
       RUN set-position IN h_b-tapa ( 1.95 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-tapa ( 11.67 , 17.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-tapas_inspeccion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-tapas_inspeccion ).
       RUN set-position IN h_v-tapas_inspeccion ( 2.19 , 23.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.95 , 69.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 8.38 , 23.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 1.76 , 69.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-tapas_inspeccion. */
       RUN add-link IN adm-broker-hdl ( h_b-tapa , 'Record':U , h_v-tapas_inspeccion ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-tapas_inspeccion ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-tapas_inspeccion ,
             h_b-tapa , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updspa ,
             h_v-tapas_inspeccion , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/USR" "Mantenimiento de Tapas de Inspecciñon"}
