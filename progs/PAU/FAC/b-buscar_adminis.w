...
&Scoped-define QUERY-STRING-br_table FOR EACH Cliente WHERE cliente.nro_cliente = cliente.nro_administrador AND CAN-DO(CLIENTE.LISTA_EMPRESAS, ~
       que_empresa) NO-LOCK         ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY {&SELF-NAME} FOR EACH Cliente WHERE cliente.nro_cliente = cliente.nro_administrador AND CAN-DO(CLIENTE.LISTA_EMPRESAS, ~
       que_empresa) NO-LOCK         ~{&SORTBY-PHRASE}.
...
&Scoped-Define ENABLED-OBJECTS RECT-9 que_nombre v-fantasia Bok v-acti ~
br_table 
&Scoped-Define DISPLAYED-OBJECTS que_nombre v-fantasia v-acti 
...
DEFINE VARIABLE v-acti AS CHARACTER FORMAT "X(256)":U INITIAL "*" 
     LABEL "Estado" 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 147 BY 20.71
         TITLE "Maestro de Administraciones" FIT-LAST-COLUMN.
...
DEFINE FRAME F-Main
     ...
     Bok AT ROW 1.24 COL 81
     v-acti AT ROW 1.24 COL 99 COLON-ALIGNED WIDGET-ID 2
...
/* BROWSE-TAB br_table v-acti F-Main */
ASSIGN 
...
/* Query rebuild information for BROWSE br_table
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH Cliente WHERE cliente.nro_cliente = cliente.nro_administrador AND CAN-DO(CLIENTE.LISTA_EMPRESAS, que_empresa) NO-LOCK
        ~{&SORTBY-PHRASE}.
     _END_FREEFORM
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Bok B-table-Win
ON MOUSE-SELECT-DBLCLICK OF Bok IN FRAME F-Main /* OK */
DO:
  DEF VAR c_window AS CHAR NO-UNDO.
  ...
  RUN get-link-handle IN adm-broker-hdl
    ...
    IF VALID-HANDLE(h_window) THEN RUN salir IN h_window.
END.
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* Maestro de Administraciones */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* Maestro de Administraciones */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON START-SEARCH OF br_table IN FRAME F-Main /* Maestro de Administraciones */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* Maestro de Administraciones */
DO:
...
DO:
    ASSIGN que_nombre v-fantasia v-acti.

IF que_nombre = "" THEN
    OPEN QUERY {&BROWSE-NAME} FOR EACH Cliente WHERE cdg_cliente BEGINS "A" 
    AND CAN-DO(v-acti , cliente.cdg_estado)                  
    AND CAN-DO(CLIENTE.LISTA_EMPRESAS, que_empresa) NO-LOCK.
else
        ...
        WHEN 'direccion' THEN 
         ...
         WHEN 'cuit' THEN 
              DO WITH FRAME {&FRAME-NAME}:
                 ...
                 AND LOOKUP(que_sector, Cliente.lista_sectores) <> 0
                                    NO-LOCK.
                  {&BROWSE-NAME}:TITLE = "Clientes que satisfacen la condición de búsqueda: POR CUIT=" + que_nombre.
...
&Scoped-define SELF-NAME v-acti
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL v-acti B-table-Win
ON VALUE-CHANGED OF v-acti IN FRAME F-Main /* Estado */
DO:
    ASSIGN v-acti.
    APPLY "RETURN" TO que_nombre.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE seleccionado B-table-Win 
PROCEDURE seleccionado :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT-OUTPUT PARAM p-table AS ROWID NO-UNDO.
p-table = ROWID(  sic.cliente ).
END PROCEDURE.
