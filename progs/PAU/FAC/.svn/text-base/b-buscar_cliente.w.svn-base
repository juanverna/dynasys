...
&Scoped-define QUERY-STRING-br_table FOR EACH Cliente NO-LOCK     ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY {&SELF-NAME} FOR EACH Cliente NO-LOCK     ~{&SORTBY-PHRASE}.
...
&Scoped-Define ENABLED-OBJECTS RECT-9 que_nombre v-fantasia Bok ~
v-cdg_estado BUTTON-1 br_table 
&Scoped-Define DISPLAYED-OBJECTS que_nombre v-fantasia v-cdg_estado 
...
DEFINE VARIABLE v-cdg_estado AS CHARACTER FORMAT "X(256)":U INITIAL "*" 
     LABEL "Estado" 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 147 BY 14.52
         TITLE "Maestro de Clientes" ROW-HEIGHT-CHARS .52 FIT-LAST-COLUMN.
...
DEFINE FRAME F-Main
     ...
     v-cdg_estado AT ROW 1.24 COL 102 COLON-ALIGNED WIDGET-ID 2
     BUTTON-1 AT ROW 1.24 COL 138
...
/* Query rebuild information for BROWSE br_table
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH Cliente NO-LOCK
    ~{&SORTBY-PHRASE}.
     _END_FREEFORM
...
DO:
ASSIGN que_nombre v-fantasia v-cdg_estado.
             ...
             THEN DO:
                  OPEN QUERY {&BROWSE-NAME} 
                      FOR EACH Cliente WHERE Cliente.nom_cliente CONTAINS que_nombre 
                                        ...
                                        AND CAN-DO(v-cdg_estado,Cliente.cdg_estado) NO-LOCK.
...
&Scoped-define SELF-NAME v-cdg_estado
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL v-cdg_estado B-table-Win
ON VALUE-CHANGED OF v-cdg_estado IN FRAME F-Main /* Estado */
DO:
  APPLY "RETURN" TO que_nombre.
END.
