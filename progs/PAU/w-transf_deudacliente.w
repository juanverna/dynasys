...
&Scoped-define INTERNAL-TABLES fac_header cta_cte Administrador ~
Condicion_venta Cliente

/* Definitions for BROWSE BROWSE-1                                      */
&Scoped-define FIELDS-IN-QUERY-BROWSE-1 administrador.cdg_cliente administrador.nom_cliente fac_header.tip_comprob fac_header.prf_comprob fac_header.nro_comprob fac_header.fecha Condicion_venta.cdg_cndventa Condicion_venta.descripcion pagado() @ paga fac_header.imp_total fac_header.imp_total   
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-1   
&Scoped-define SELF-NAME BROWSE-1
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH fac_header OF cliente NO-LOCK, ~
       ...
       FIRST Administrador WHERE Administrador.nro_cliente = fac_header.nro_admin NO-LOCK, ~
       FIRST Condicion_venta OF fac_header NO-LOCK BY fac_header.fecha DESC
&Scoped-define OPEN-QUERY-BROWSE-1 OPEN QUERY {&SELF-NAME} FOR EACH fac_header OF cliente NO-LOCK, ~
             FIRST cta_cte WHERE       cta_cte.cdg_empresa = fac_header.cdg_empresa AND       cta_cte.tip_comprob = fac_header.tip_comprob AND       cta_cte.prf_comprob = fac_header.prf_comprob AND       cta_cte.nro_comprob = fac_header.nro_comprob AND       cta_cte.credito <> cta_cte.debito , ~
       FIRST Administrador WHERE Administrador.nro_cliente = fac_header.nro_admin NO-LOCK, ~
       FIRST Condicion_venta OF fac_header NO-LOCK BY fac_header.fecha DESC.
...
&Scoped-define FOURTH-TABLE-IN-QUERY-BROWSE-1 Condicion_venta
...
&Scoped-define QUERY-STRING-F-Main FOR EACH Administrador SHARE-LOCK, ~
      EACH Cliente WHERE TRUE /* Join to Administrador incomplete */ SHARE-LOCK
&Scoped-define OPEN-QUERY-F-Main OPEN QUERY F-Main FOR EACH Administrador SHARE-LOCK, ~
      EACH Cliente WHERE TRUE /* Join to Administrador incomplete */ SHARE-LOCK.
&Scoped-define TABLES-IN-QUERY-F-Main Administrador Cliente
&Scoped-define FIRST-TABLE-IN-QUERY-F-Main Administrador
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-1 W-Win _FREEFORM
  QUERY BROWSE-1 NO-LOCK DISPLAY
      administrador.cdg_cliente FORMAT "x(8)" COLUMN-LABEL "Paga!Cod"
      administrador.nom_cliente FORMAT "X(25)" COLUMN-LABEL "Paga!Nombre"
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH SEPARATORS MULTIPLE SIZE 128 BY 10.24
...
DEFINE FRAME F-Main
     ...
     Cliente.nom_cliente AT ROW 1.24 COL 28 COLON-ALIGNED NO-LABEL WIDGET-ID 54
          VIEW-AS FILL-IN 
          SIZE 81 BY 1
     B-tr AT ROW 1.33 COL 117 WIDGET-ID 48
     v-cdg_administrador AT ROW 2.43 COL 11.8 COLON-ALIGNED WIDGET-ID 36 NO-TAB-STOP 
     Administrador.nom_cliente AT ROW 2.43 COL 28 COLON-ALIGNED NO-LABEL WIDGET-ID 56
          VIEW-AS FILL-IN 
          SIZE 81 BY 1
          BGCOLOR 9 FGCOLOR 15 
     B-tr-2 AT ROW 2.43 COL 117 WIDGET-ID 52
...
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-1 B-tr-2 F-Main */
ASSIGN 
...
/* Query rebuild information for BROWSE BROWSE-1
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH fac_header OF cliente NO-LOCK,
      FIRST cta_cte WHERE
...
FIRST Administrador WHERE Administrador.nro_cliente = fac_header.nro_admin NO-LOCK,
FIRST Condicion_venta OF fac_header NO-LOCK
BY fac_header.fecha DESC.
     _END_FREEFORM
...
/* Query rebuild information for FRAME F-Main
     _TblList          = "Temp-Tables.Administrador,sic.Cliente WHERE Temp-Tables.Administrador ..."
     _Query            is OPENED
*/  /* FRAME F-Main */
&ANALYZE-RESUME
...
ON CHOOSE OF B-tr IN FRAME F-Main /* Paga Cliente */
DO:
  RUN barrer("C").
...
&Scoped-define SELF-NAME B-tr-2
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-tr-2 W-Win
ON CHOOSE OF B-tr-2 IN FRAME F-Main /* Paga Admin */
DO:
  RUN barrer("A").
  {&OPEN-QUERY-{&BROWSE-NAME}}
END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE v-cdg_cliente Cliente.nom_cliente B-tr v-cdg_administrador 
         Administrador.nom_cliente B-tr-2 BROWSE-1 
      WITH FRAME F-Main IN WINDOW W-Win.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT PARAMETER qp AS CHAR.
DEFINE VAR nuevoadm AS INT NO-UNDO.
DEFINE VAR anterioradm AS INT NO-UNDO.
ASSIGN FRAME {&FRAME-NAME} v-cdg_cliente.
FIND cliente OF fac_header NO-LOCK.
IF qp = "A" THEN nuevoadm = cliente.nro_admin.
ELSE nuevoadm = cliente.nro_cliente.
...
fac_header.tip_comprob + "-" + STRING(fac_header.prf_comprob,"9999") + "-" + STRING(fac_header.nro_comprob) + 
    ...
    RETURN ERROR.
END.
anterioradm = fac_header.nro_administrador.
/*genera evento o tarea aun sin no hay deuda vencida*/
FIND CURRENT fac_header EXCLUSIVE-LOCK.
ASSIGN 
            fac_header.nro_administrador = nuevoadm
            fac_header.estado_2_impresion = IF fac_header.estado_2_impresion = "I" THEN "" ELSE fac_header.estado_2_impresion
            Cta_cte.imp_comision = IF qp = "C" THEN 0 ELSE Administrador.HAT
            Cta_cte.nro_administrador = nuevoadm.
RELEASE fac_header.
/*{debug.i}*/
RUN gen_tarea_cobranzasH.p(nuevoadm,"Cambio de Administracion",TODAY).
RUN gen_tarea_cobranzasH.p(anterioradm,"Recalculo por cambio de Administracion",TODAY). 
/*genera evento o tarea aun sin no hay deuda vencida*/
END PROCEDURE.
