...
DEFINE VARIABLE h_b-tipoproveedor AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-tipoproveedor AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Tipos de Proveedores */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Tipos de Proveedores */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Tipos de Proveedores */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa-2 ).
       RUN set-position IN h_p-updspa-2 ( 6.71 , 20.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-tipoproveedor.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tipoproveedor ).
       RUN set-position IN h_b-tipoproveedor ( 1.71 , 5.00 ) NO-ERROR.
       RUN set-size IN h_b-tipoproveedor ( 24.05 , 12.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-tipoproveedor.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-tipoproveedor ).
       RUN set-position IN h_v-tipoproveedor ( 1.71 , 20.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.52 , 70.00 ) */

       /* Links to SmartViewer h_v-tipoproveedor. */
       RUN add-link IN adm-broker-hdl ( h_b-tipoproveedor , 'Record':U , h_v-tipoproveedor ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa-2 , 'TableIO':U , h_v-tipoproveedor ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/CXP" "Tipos de Proveedores"}
...
*/
 ...
 IF VALID-HANDLE(h_b-tipoproveedor) THEN RUN set-sensitivo IN h_b-tipoproveedor ( INPUT p-operacion = "HABILITAR" ).
