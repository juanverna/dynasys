...
&Scoped-define INTERNAL-TABLES Certificado_iva Tipo_retiva Proveedor

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table Certificado_iva.nro_certifiva ~
Proveedor.cdg_proveedor Proveedor.nombre Tipo_retiva.cdg_tiporetiva ~
Certificado_iva.fecha_deposito Certificado_iva.fecha_emision ~
Certificado_iva.imp_pagado Certificado_iva.imp_retenido 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH Certificado_iva WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Tipo_retiva OF Certificado_iva NO-LOCK, ~
      EACH Proveedor OF Certificado_iva NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Certificado_iva WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Tipo_retiva OF Certificado_iva NO-LOCK, ~
      EACH Proveedor OF Certificado_iva NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table Certificado_iva Tipo_retiva ~
Proveedor
&Scoped-define FIRST-TABLE-IN-QUERY-br_table Certificado_iva
&Scoped-define SECOND-TABLE-IN-QUERY-br_table Tipo_retiva
...
&Scoped-Define ENABLED-OBJECTS v-des_fecha v-has_fecha ver_pagos ~
btn_comprobte btn_emitir btn_listado br_table RECT-1 RECT-2 
&Scoped-Define DISPLAYED-OBJECTS v-des_fecha v-has_fecha ver_pagos 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Certificado_iva.nro_certifiva FORMAT ">>>>9":U
      Proveedor.cdg_proveedor FORMAT "X(8)":U WIDTH 12.4
      Proveedor.nombre FORMAT "X(35)":U
      Tipo_retiva.cdg_tiporetiva FORMAT "X(8)":U
      Certificado_iva.fecha_deposito COLUMN-LABEL "Fecha!Depósito" FORMAT "99/99/99":U
      Certificado_iva.fecha_emision COLUMN-LABEL "Fecha!Retención" FORMAT "99/99/99":U
      Certificado_iva.imp_pagado FORMAT "->,>>>,>>9.99":U
      Certificado_iva.imp_retenido FORMAT "->,>>>,>>9.99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 124 BY 21.95
...
DEFINE FRAME F-Main
     v-des_fecha AT ROW 1.24 COL 13 COLON-ALIGNED
     v-has_fecha AT ROW 1.24 COL 31 COLON-ALIGNED
     ver_pagos AT ROW 1.24 COL 66 NO-LABEL
     btn_comprobte AT ROW 2.86 COL 4
     btn_emitir AT ROW 2.91 COL 27
     btn_listado AT ROW 2.91 COL 104
     br_table AT ROW 4.52 COL 1
     RECT-1 AT ROW 1 COL 1
     RECT-2 AT ROW 2.62 COL 1
     "Mostrar:" VIEW-AS TEXT
          SIZE 7 BY .81 AT ROW 1.24 COL 58
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "sic.Certificado_iva,sic.Tipo_retiva OF sic.Certificado_iva,sic.Proveedor OF sic.Certificado_iva"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   = sic.Certificado_iva.nro_certifiva
     _FldNameList[2]   > sic.Proveedor.cdg_proveedor
"Proveedor.cdg_proveedor" ? ? "character" ? ? ? ? ? ? no ? no no "12.4" yes no no "U" "" ""
     _FldNameList[3]   > sic.Proveedor.nombre
"Proveedor.nombre" ? "X(35)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[4]   = sic.Tipo_retiva.cdg_tiporetiva
     _FldNameList[5]   > sic.Certificado_iva.fecha_deposito
"Certificado_iva.fecha_deposito" "Fecha!Depósito" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[6]   > sic.Certificado_iva.fecha_emision
"Certificado_iva.fecha_emision" "Fecha!Retención" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[7]   = sic.Certificado_iva.imp_pagado
     _FldNameList[8]   = sic.Certificado_iva.imp_retenido
...
DO:
  IF AVAILABLE Certificado_iva
  THEN DO:
       RUN d-ver_certificado_iva.w ( INPUT ROWID(Certificado_iva)).
  END.
  ELSE DO:       
       BELL.
  END.
...
DO:
  IF AVAILABLE Certificado_iva
  THEN DO:
       RUN imprimir_Certificado_ivautos.p ( INPUT ROWID(Certificado_iva)).
    ...
    THEN DO:
        OPEN QUERY {&BROWSE-NAME}
             FOR EACH Certificado_iva
                WHERE NOT Certificado_iva.anulado
                  AND Certificado_iva.cdg_empresa = que_empresa
                  AND Certificado_iva.fecha_emision <= v-has_fecha
                  AND Certificado_iva.fecha_emision >= v-des_fecha,
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "Certificado_iva"}
  {src/adm/template/snd-list.i "Tipo_retiva"}
