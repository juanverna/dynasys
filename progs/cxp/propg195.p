...
PROCEDURE imprimir_orden:
    ...
    OUTPUT STREAM Formulario TO "c:\sic-temp\propg195.xpr" CONVERT TARGET "iso8859-1".
    ...
    DO WHILE AVAILABLE Opg_detalle OR AVAILABLE Caj_detalle:
      ...
      THEN DO:
            ...
            DO ncopia = 1 TO 2:
               ...
               RUN escribenumero( INPUT chr_linea + ",53", INPUT STRING(Opg_detalle.importe,"->,>>>,>>9.99"), 8, NO, 1.0).
            END.                
      END.
...
PROCEDURE encabezado:
    ...
    RUN escribir ( INPUT "127,27", INPUT "ASIENTO MFG", 8, YES).
    RUN escribir ( INPUT "127,74", INPUT "PREPARO", 8, YES).
    RUN escribir ( INPUT "127,120", INPUT "AUTORIZO", 8, YES).
    RUN escribir ( INPUT "127,161", INPUT "RECIBIO CONFORME", 8, YES).
...
PROCEDURE pie:
    ...
    RUN escribircentrado( INPUT "143,158", INPUT "Por " + Proveedor.nombre, 7, NO, 1.0).
...
PROCEDURE copiar_impresion:

    OS-COPY VALUE("c:\sic-temp\propg195.xpr")
                    VALUE("f:\watch\in\" + CAPS(Caj_header.tip_comprob) + "-" +
