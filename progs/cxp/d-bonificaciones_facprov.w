...
&Scoped-define INTERNAL-TABLES T-Fac_header_prv_bon Bonificacion

/* Definitions for BROWSE BROWSE-8                                      */
&Scoped-define FIELDS-IN-QUERY-BROWSE-8 T-Fac_header_prv_bon.cdg_bonificacion ~
Bonificacion.descripcion T-Fac_header_prv_bon.porcentaje ~
T-Fac_header_prv_bon.importe 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-8 
&Scoped-define QUERY-STRING-BROWSE-8 FOR EACH T-Fac_header_prv_bon NO-LOCK, ~
      EACH Bonificacion OF T-Fac_header_prv_bon NO-LOCK
&Scoped-define OPEN-QUERY-BROWSE-8 OPEN QUERY BROWSE-8 FOR EACH T-Fac_header_prv_bon NO-LOCK, ~
      EACH Bonificacion OF T-Fac_header_prv_bon NO-LOCK.
&Scoped-define TABLES-IN-QUERY-BROWSE-8 T-Fac_header_prv_bon Bonificacion
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-8 T-Fac_header_prv_bon
...
&Scoped-Define ENABLED-OBJECTS RECT-10 v-cdg_bonificacion v-porcentaje ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-8 Dialog-Frame _STRUCTURED
  QUERY BROWSE-8 NO-LOCK DISPLAY
      T-Fac_header_prv_bon.cdg_bonificacion COLUMN-LABEL "Có-!digo" FORMAT "ZZ9":U
      Bonificacion.descripcion COLUMN-LABEL "Descripción!Bonificación" FORMAT "X(30)":U
      T-Fac_header_prv_bon.porcentaje COLUMN-LABEL "%.!Bonificado" FORMAT "->>9.99":U
      T-Fac_header_prv_bon.importe COLUMN-LABEL "Importe!Bonificado" FORMAT "->,>>>,>>9.99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 65 BY 7.52.
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-8 AT ROW 3.14 COL 4
     btn_crear AT ROW 10.95 COL 4
     btn_eliminar AT ROW 10.95 COL 27
     btn_salir AT ROW 10.95 COL 50
     RECT-10 AT ROW 1.29 COL 2
     SPACE(1.59) SKIP(0.46)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Bonificaciones de la presente factura".
...
/* Query rebuild information for BROWSE BROWSE-8
     _TblList          = "Temp-Tables.T-Fac_header_prv_bon,sic.Bonificacion OF Temp-Tables.T-Fac_header_prv_bon"
     _Options          = "NO-LOCK"
     _FldNameList[1]   > Temp-Tables.T-Fac_header_prv_bon.cdg_bonificacion
"T-Fac_header_prv_bon.cdg_bonificacion" "Có-!digo" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
"Bonificacion.descripcion" "Descripción!Bonificación" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > Temp-Tables.T-Fac_header_prv_bon.porcentaje
"T-Fac_header_prv_bon.porcentaje" "%.!Bonificado" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[4]   > Temp-Tables.T-Fac_header_prv_bon.importe
"T-Fac_header_prv_bon.importe" "Importe!Bonificado" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-8 */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Bonificaciones de la presente factura */
DO:
...
DO:
  ...
  THEN DO:
       FIND T-Fac_header_prv_bon 
            WHERE T-Fac_header_prv_bon.cdg_bonificacion = v-cdg_bonificacion NO-LOCK NO-ERROR.
       IF AVAILABLE T-Fac_header_prv_bon 
       THEN DO:
       ...
       ELSE DO:
            DO TRANSACTION:
               ...
               ASSIGN T-Fac_header_prv_bon.cdg_bonificacion = v-cdg_bonificacion
                      T-Fac_header_prv_bon.porcentaje       = v-porcentaje. 
            END.
...
DO:
    ...
    IF NOT AVAILABLE T-Fac_header_prv_bon
    THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-10 v-cdg_bonificacion v-porcentaje BROWSE-8 btn_crear 
         btn_eliminar 
      WITH FRAME Dialog-Frame.
