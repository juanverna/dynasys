...
&Scoped-define INTERNAL-TABLES Fac_header_prv Proveedor Moneda

/* Definitions for BROWSE BROWSE-2                                      */
&Scoped-define FIELDS-IN-QUERY-BROWSE-2 Fac_header_prv.tip_comprob Fac_header_prv.prf_comprob Fac_header_prv.nro_comprob Fac_header_prv.estado Fac_header_prv.fecha Fac_header_prv.mes Fac_header_prv.ano Fac_header_prv.imp_total Moneda.cdg_moneda Proveedor.cdg_proveedor Proveedor.nombre   
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-2   
&Scoped-define SELF-NAME BROWSE-2
&Scoped-define OPEN-QUERY-BROWSE-2    IF por_proveedor    THEN DO:       OPEN QUERY {&SELF-NAME}            FOR EACH Fac_header_prv                           WHERE Fac_header_prv.cdg_empresa = que_empresa                             AND CAN-DO(Tipocomprobante.tip_comprob, ~
       Fac_header_prv.tip_comprob)                             AND Fac_header_prv.nro_proveedor = que_proveedor                             AND Fac_header_prv.fecha >= v-des_fecha                             AND Fac_header_prv.fecha <= v-has_fecha, ~
                                       FIRST Proveedor OF Fac_header_prv, ~
                                       FIRST Moneda OF Fac_header_prv.    END.    ELSE DO:       OPEN QUERY {&SELF-NAME}            FOR EACH Fac_header_prv                           WHERE Fac_header_prv.cdg_empresa = que_empresa                             AND CAN-DO(Tipocomprobante.tip_comprob, ~
       Fac_header_prv.tip_comprob)                             AND Fac_header_prv.fecha >= v-des_fecha                             AND Fac_header_prv.fecha <= v-has_fecha, ~
                                       FIRST Proveedor OF Fac_header_prv                                       WHERE Proveedor.titular_oxp_sino = FALSE, ~
                                             FIRST Moneda OF Fac_header_prv.    END.
&Scoped-define TABLES-IN-QUERY-BROWSE-2 Fac_header_prv Proveedor Moneda
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-2 Fac_header_prv
&Scoped-define SECOND-TABLE-IN-QUERY-BROWSE-2 Proveedor
&Scoped-define THIRD-TABLE-IN-QUERY-BROWSE-2 Moneda
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-2 D-Dialog _FREEFORM
  QUERY BROWSE-2 DISPLAY
      Fac_header_prv.tip_comprob COLUMN-LABEL "Ti-!po"
      Fac_header_prv.prf_comprob COLUMN-LABEL "Pre-!fijo"
      Fac_header_prv.nro_comprob COLUMN-LABEL "Número!Compbte"
      Fac_header_prv.estado COLUMN-LABEL "S!T"
      Fac_header_prv.fecha COLUMN-LABEL "Fecha!Emisión"
      Fac_header_prv.mes
      Fac_header_prv.ano
      Fac_header_prv.imp_total COLUMN-LABEL "Importe!Total"
      Moneda.cdg_moneda        COLUMN-LABEL "Mo-!neda" 
      Proveedor.cdg_proveedor  COLUMN-LABEL "Código!Proveedor"
      Proveedor.nombre         COLUMN-LABEL "Razón!Social" FORMAT "X(29)"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 103 BY 9.95.
      ...
      OPEN QUERY {&SELF-NAME}
           FOR EACH Fac_header_prv
                          WHERE Fac_header_prv.cdg_empresa = que_empresa
                            AND CAN-DO(Tipocomprobante.tip_comprob, Fac_header_prv.tip_comprob)
                            AND Fac_header_prv.nro_proveedor = que_proveedor
                            AND Fac_header_prv.fecha >= v-des_fecha
                            AND Fac_header_prv.fecha <= v-has_fecha,
                                FIRST Proveedor OF Fac_header_prv,
                                FIRST Moneda OF Fac_header_prv.
   END.
   ELSE DO:
      OPEN QUERY {&SELF-NAME}
           FOR EACH Fac_header_prv
...
------------------------------------------------------------------------------*/
  DISPLAY v-des_fecha v-has_fecha por_proveedor v-cdg_proveedor v-nombre 
      WITH FRAME D-Dialog.
  ENABLE Btn_OK Btn_Cancel v-des_fecha v-has_fecha por_proveedor 
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute PRIOR to standard behavior. */
   ...
   FIND Tipocomprobante WHERE Tipocomprobante.cdg_comprobante = que_comprobante
