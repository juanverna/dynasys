...
FIND Certificado_ibr WHERE ROWID(Certificado_ibr) = act_certificado EXCLUSIVE-LOCK.

FIND Empresa             OF Certificado_ibr NO-LOCK.
...
*/
    ...
    RUN escribir ( INPUT "18,70",  INPUT "RETENCION DE IMPUESTO A LOS INGRESOS BRUTOS", 10, YES).
    RUN escribir ( INPUT "19,171", INPUT "Cert. Nro:<B>" + STRING(Certificado_ibr.nro_certifibr,"999999") + "</B>", INPUT 10, INPUT NO). 
    RUN escribir ( INPUT "30,175", INPUT STRING(Certificado_ibr.fecha_emision,"99/99/9999"), INPUT 10, INPUT NO). 
    ...
    RUN escribir ( INPUT "58,105",  INPUT "Régimen:" + Tipo_retibr.nom_retibr, 10, NO). 
    ...
    */
    ...
    FOR EACH Cert_ibr-detalle OF Certificado_ibr NO-LOCK:
        ...
        FIND FIRST Cta_cte_prv 
             WHERE Cta_cte_prv.cdg_empresa      = Certificado_ibr.cdg_empresa
               AND Cta_cte_prv.nro_comprob      = Cert_ibr-detalle.nro_comprob 
        ...
        RUN escribir      ( INPUT ch_linea  + ",20" , INPUT Cert_ibr-detalle.tip_comprob + " " + 
                                                  STRING(Cert_ibr-detalle.prf_comprob,"9999") + " " +
                                                  STRING(Cert_ibr-detalle.nro_comprob,"99999999"), 10, NO).
        RUN escribir      ( INPUT ch_linea  + ",80" , INPUT STRING(Cta_cte_prv.fecha_emision,"99/99/99"), 10, NO).
        RUN escribenumero ( INPUT ch_linea  + ",140", INPUT STRING(Cert_ibr-detalle.base_imponible,"->,>>>,>>9.99"), 10, NO).
        RUN escribenumero ( INPUT ch_linea  + ",162", INPUT STRING(Cert_ibr-detalle.alicuota,"% ->,>>>,>>9.99"), 10, NO).
        RUN escribenumero ( INPUT ch_linea  + ",192", INPUT STRING(Cert_ibr-detalle.imp_retenido,"Z,ZZZ,ZZ9.99-"), 10, NO).

        linea0 = linea0 + 4.

    END.

    RUN escribenumero ( INPUT "125,192", INPUT STRING(Certificado_ibr.imp_retenido,"->,>>>,>>9.99"), 10, YES).
