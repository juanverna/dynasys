...
DEFINE VARIABLE h_v-par-vermovyresum AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Imputación Contable de las Obligaciones Canceladas por Período"
         HEIGHT             = 22.15
         WIDTH              = 114.29
         MAX-HEIGHT         = 22.15
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF W-Win /* Imputación Contable de las Obligaciones Canceladas por Período */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Imputación Contable de las Obligaciones Canceladas por Período */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-fechas ( 5.04 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-par-vermovyresum.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-par-vermovyresum ).
       RUN set-position IN h_v-par-vermovyresum ( 9.88 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 89.00 ) */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-par-vermovyresum ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/CXP" "Imputación Contable de las Obligaciones Canceladas por Período"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE ver_movim          AS LOGICAL INITIAL YES.
  DEFINE VARIABLE ver_resum          AS LOGICAL INITIAL YES.
  ...
  RUN dar_rango IN h_v-par-vermovyresum    ( OUTPUT ver_movim, 
                                             OUTPUT ver_resum, 
                                             OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
  ...
  THEN DO:
       ...
       RUN lsimputacion_pagos.p ( INPUT des_fecha,
                                  INPUT has_fecha,
                                  INPUT ver_movim,
                                  INPUT ver_resum).
