...
EMPTY TEMP-TABLE T-Pagos_x_retsus.
EMPTY TEMP-TABLE T-Pagos_x_retsus_det.
EMPTY TEMP-TABLE T-Certificado_sus.     
EMPTY TEMP-TABLE T-Cert_sus-detalle. 
...
FOR EACH T-Opg_detalle   OF T-Opg_header NO-LOCK,
    ...
    FIRST Fac_header_prv OF Proveedor
         ...
         FIRST Familia_retsus OF Articulo, 
         FIRST Famretsuss_regimen OF Familia_retsus
               WHERE Famretsuss_regimen.cdg_condiva = T-Opg_header.cdg_condiva
                     BREAK BY Famretsuss_regimen.cdg_tiporetsus:

    /* Halla el total pagado para esta actividad a efectos de acumular esta retención */
           
    FIND FIRST T-Pagos_x_retsus 
         WHERE T-Pagos_x_retsus.cdg_tiporetsus = Famretsuss_regimen.cdg_tiporetsus NO-ERROR.
  
    IF NOT AVAILABLE T-Pagos_x_retsus
    THEN DO:
       CREATE T-Pagos_x_retsus.
       ASSIGN T-Pagos_x_retsus.cdg_tiporetsus =  Famretsuss_regimen.cdg_tiporetsus.
    END.           
    ...
    T-Pagos_x_retsus.imp_estepago = T-Pagos_x_retsus.imp_estepago + x-pago_neto.

    FIND FIRST T-Pagos_x_retsus_det
         WHERE T-Pagos_x_retsus_det.tip_comprob     = Fac_header_prv.tip_comprob
           AND T-Pagos_x_retsus_det.prf_comprob     = Fac_header_prv.prf_comprob
           AND T-Pagos_x_retsus_det.nro_comprob     = Fac_header_prv.nro_comprob
           AND T-Pagos_x_retsus_det.nro_vencimiento = T-Opg_detalle.nro_cancela
           AND T-Pagos_x_retsus_det.cdg_tiporetsus  = T-Pagos_x_retsus.cdg_tiporetsus
               NO-ERROR.

    IF NOT AVAILABLE T-Pagos_x_retsus_det
    THEN DO:
        CREATE T-Pagos_x_retsus_det.
        BUFFER-COPY Fac_header_prv TO T-Pagos_x_retsus_det
            ASSIGN T-Pagos_x_retsus_det.cdg_tiporetsus = T-Pagos_x_retsus.cdg_tiporetsus
                   T-Pagos_x_retsus_det.nro_vencimiento = T-Opg_detalle.nro_vencimiento.
    END.

    ASSIGN T-Pagos_x_retsus_det.base_imponible = T-Pagos_x_retsus_det.base_imponible + x-pago_neto.
...
FOR EACH T-Pagos_x_retsus, Tipo_retsus OF T-Pagos_x_retsus  
         BY T-Pagos_x_retsus.cdg_tiporetsus:

    RUN retener_sus_monto.p (INPUT  T-Pagos_x_retsus.cdg_tiporetsus,
                             INPUT  T-Opg_header.fecha,
                             INPUT  T-Pagos_x_retsus.imp_estepago,
                             INPUT  Proveedor.convenio_sino,
                             INPUT  YES, /* activa importe minimo */
                             OUTPUT T-Pagos_x_retsus.imp_retenido,
                             OUTPUT T-Pagos_x_retsus.alicuota).


    FIND FIRST Proveedor_excen_sus OF Proveedor
         WHERE Proveedor_excen_sus.cdg_empresa     = T-Opg_header.cdg_empresa 
           AND Proveedor_excen_sus.cdg_tiporetsus  = T-Pagos_x_retsus.cdg_tiporetsus
           AND Proveedor_excen_sus.fch_desde       <= T-Opg_header.fecha
           AND Proveedor_excen_sus.fch_hasta       >= T-Opg_header.fecha
               NO-LOCK NO-ERROR.

    IF AVAILABLE Proveedor_excen_sus 
        THEN ASSIGN T-Pagos_x_retsus.prc_excencion = Proveedor_excen_sus.prc_excencion
                    T-Pagos_x_retsus.imp_retenido = T-Pagos_x_retsus.imp_retenido *
                                            ( 1 - Proveedor_excen_sus.prc_excencion / 100 ).

    FOR EACH T-Pagos_x_retsus_det WHERE T-Pagos_x_retsus_det.cdg_tiporetsus = T-Pagos_x_retsus.cdg_tiporetsus:
        ASSIGN T-Pagos_x_retsus_det.alicuota = T-Pagos_x_retsus.alicuota
               T-Pagos_x_retsus_det.tot_retenido = T-Pagos_x_retsus_det.base_imponible * T-Pagos_x_retsus_det.alicuota / 100.
    END.

         /* Chequea que exista el item de caja */
             
    FIND FIRST T-Caj_detalle OF T-Caj_header 
         WHERE T-Caj_detalle.cdg_rubro = Tipo_retsus.cdg_rubro 
               EXCLUSIVE-LOCK NO-ERROR.

    IF T-Pagos_x_retsus.imp_retenido > 0 
    THEN DO:
       ...
       THEN DO:
           ...
           ASSIGN T-Caj_header.ultima_linea      = T-Caj_header.ultima_linea + 1
                  ...
                  T-Caj_detalle.cdg_rubro        = Tipo_retsus.cdg_rubro.
                
           CREATE T-Certificado_sus.
           ASSIGN T-Certificado_sus.cdg_empresa      = T-Opg_header.cdg_empresa
                  T-Certificado_sus.cdg_tiporetsus   = T-Pagos_x_retsus.cdg_tiporetsus
                  T-Certificado_sus.fecha_emision    = T-Opg_header.fecha
                  T-Certificado_sus.imp_retenido     = T-Pagos_x_retsus.imp_retenido
                  T-Certificado_sus.nro_linea        = T-Caj_detalle.nro_linea
                  T-Certificado_sus.nro_proveedor    = T-Opg_header.nro_proveedor
                  T-Certificado_sus.nro_transaccion  = T-Caj_detalle.nro_transaccion
                  T-Certificado_sus.nro_certifsus    = T-Caj_detalle.nro_linea
                  T-Caj_detalle.observacion          = "Nro.Cert.:" + STRING(T-Certificado_sus.nro_certifsus,"999999").
            
           FOR EACH T-Pagos_x_retsus_det WHERE T-Pagos_x_retsus_det.nro_ordpago = T-Pagos_x_retsus.nro_ordpago NO-LOCK:
                        
               CREATE T-Cert_sus-detalle.
               ASSIGN T-Cert_sus-detalle.imp_retenido    = T-Pagos_x_retsus_det.tot_retenido
                      T-Cert_sus-detalle.base_imponible  = T-Pagos_x_retsus_det.base_imponible
                      T-Cert_sus-detalle.alicuota        = T-Pagos_x_retsus_det.alicuota
                      T-Cert_sus-detalle.nro_certifsus   = T-Certificado_sus.nro_certifsus
                      T-Cert_sus-detalle.nro_comprob     = T-Pagos_x_retsus_det.nro_comprob
                      T-Cert_sus-detalle.prf_comprob     = T-Pagos_x_retsus_det.prf_comprob
                      T-Cert_sus-detalle.tip_comprob     = T-Pagos_x_retsus_det.tip_comprob
                      T-Cert_sus-detalle.nro_vencimiento = T-Pagos_x_retsus_det.nro_vencimiento.
                    /* Cert_sus-detalle.nro_secuencia   = 0.*/
       ...
       ASSIGN T-Caj_header.ingreso = T-Caj_header.ingreso - T-Caj_detalle.importe.        /* Resta importe anterior */
              T-Caj_detalle.importe = T-Pagos_x_retsus.imp_retenido.               /* Asigna nuevo importe   */
