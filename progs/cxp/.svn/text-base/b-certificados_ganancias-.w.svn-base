...
&Scoped-define INTERNAL-TABLES Certificado_gan Tipo_actividad Proveedor

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table Certificado_gan.nro_certifgan ~
Proveedor.cdg_proveedor Proveedor.nombre Tipo_actividad.cdg_tiporetgan ~
Certificado_gan.fecha_emision Certificado_gan.imp_retenido ~
Certificado_gan.imp_pagado Certificado_gan.emitido 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH Certificado_gan WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Tipo_actividad OF Certificado_gan NO-LOCK, ~
      EACH Proveedor OF Certificado_gan NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Certificado_gan WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Tipo_actividad OF Certificado_gan NO-LOCK, ~
      EACH Proveedor OF Certificado_gan NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table Certificado_gan Tipo_actividad ~
Proveedor
&Scoped-define FIRST-TABLE-IN-QUERY-br_table Certificado_gan
...
&Scoped-Define ENABLED-OBJECTS RECT-1 RECT-2 v-des_fecha v-has_fecha ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Certificado_gan.nro_certifgan COLUMN-LABEL "Numero!Certif." FORMAT ">>>>9":U
      Proveedor.cdg_proveedor FORMAT "X(8)":U
      Proveedor.nombre FORMAT "X(40)":U
      Tipo_actividad.cdg_tiporetgan FORMAT "X(8)":U
      Certificado_gan.fecha_emision FORMAT "99/99/99":U
      Certificado_gan.imp_retenido FORMAT "->,>>>,>>9.99":U
      Certificado_gan.imp_pagado FORMAT "->,>>>,>>9.99":U
      Certificado_gan.emitido FORMAT "Si/No":U WIDTH 8.2
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 122 BY 16.95
...
DEFINE FRAME F-Main
     v-des_fecha AT ROW 1.24 COL 16 COLON-ALIGNED
     v-has_fecha AT ROW 1.24 COL 34 COLON-ALIGNED
     ver_pagos AT ROW 1.48 COL 65 NO-LABEL
     btn_comprobte AT ROW 2.86 COL 4
     btn_emitir AT ROW 2.86 COL 26
     btn_listado AT ROW 2.86 COL 92
     br_table AT ROW 4.52 COL 1
     "Mostrar:" VIEW-AS TEXT
          SIZE 7 BY .81 AT ROW 1.48 COL 57
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "sic.Certificado_gan,sic.Tipo_actividad OF sic.Certificado_gan,sic.Proveedor OF sic.Certificado_gan"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > sic.Certificado_gan.nro_certifgan
"Certificado_gan.nro_certifgan" "Numero!Certif." ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   = sic.Proveedor.cdg_proveedor
     _FldNameList[3]   = sic.Proveedor.nombre
     _FldNameList[4]   = sic.Tipo_actividad.cdg_tiporetgan
     _FldNameList[5]   = sic.Certificado_gan.fecha_emision
     _FldNameList[6]   = sic.Certificado_gan.imp_retenido
     _FldNameList[7]   = sic.Certificado_gan.imp_pagado
     _FldNameList[8]   > sic.Certificado_gan.emitido
"Certificado_gan.emitido" ? ? "logical" ? ? ? ? ? ? no ? no no "8.2" yes no no "U" "" ""
...
DO:
  IF AVAILABLE Certificado_gan
     THEN RUN d-ver_certificado_ganancias.w ( INPUT ROWID(Certificado_gan)).
     ELSE BELL.
END.
...
DO:
   RUN  listar_certificados_ganancias.p (INPUT v-des_fecha,
                                         INPUT v-has_fecha,
                                         INPUT ver_pagos).
        ...
        OPEN QUERY {&BROWSE-NAME}
             FOR EACH Certificado_gan
                WHERE NOT Certificado_gan.anulado
                  AND Certificado_gan.cdg_empresa = que_empresa
                  AND Certificado_gan.fecha_emision <= v-has_fecha
                  AND Certificado_gan.fecha_emision >= v-des_fecha,
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "Certificado_gan"}
