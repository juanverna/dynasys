...
&Scoped-define INTERNAL-TABLES Opg_header Moneda Proveedor

/* Definitions for BROWSE BROWSE-2                                      */
&Scoped-define FIELDS-IN-QUERY-BROWSE-2 Opg_header.tip_comprob Opg_header.prf_comprob Opg_header.nro_comprob Opg_header.anulado Opg_header.fecha Opg_header.mes Opg_header.ano Opg_header.imp_total Moneda.abrevia Proveedor.cdg_proveedor Proveedor.nombre   
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-2   
&Scoped-define SELF-NAME BROWSE-2
&Scoped-define OPEN-QUERY-BROWSE-2    IF por_proveedor    THEN DO:       OPEN QUERY {&SELF-NAME}            FOR EACH Opg_header                           WHERE Opg_header.cdg_empresa = que_empresa                             AND Opg_header.tip_comprob = "OP"                             AND Opg_header.nro_proveedor = que_proveedor                             AND Opg_header.fecha >= v-des_fecha                             AND Opg_header.fecha <= v-has_fecha                             AND LOOKUP(Opg_header.estado, ~
      lista_estados) <> 0 NO-LOCK, ~
                                       FIRST Moneda OF Opg_header NO-LOCK, ~
                                       FIRST Proveedor OF Opg_header NO-LOCK.    END.    ELSE DO:       OPEN QUERY {&SELF-NAME}            FOR EACH Opg_header                           WHERE Opg_header.cdg_empresa = que_empresa                             AND Opg_header.tip_comprob = "OP"                             AND Opg_header.fecha >= v-des_fecha                             AND Opg_header.fecha <= v-has_fecha                             AND LOOKUP(Opg_header.estado, ~
      lista_estados) <> 0 NO-LOCK, ~
                                       FIRST Moneda OF Opg_header NO-LOCK, ~
                                       FIRST Proveedor OF Opg_header                                       WHERE Proveedor.titular_oxp_sino = FALSE NO-LOCK.    END.
&Scoped-define TABLES-IN-QUERY-BROWSE-2 Opg_header Moneda Proveedor
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-2 Opg_header
&Scoped-define SECOND-TABLE-IN-QUERY-BROWSE-2 Moneda
&Scoped-define THIRD-TABLE-IN-QUERY-BROWSE-2 Proveedor
...
&Scoped-Define ENABLED-OBJECTS RECT-1 RECT-9 v-des_fecha v-has_fecha Btn_OK ~
Btn_Cancel por_proveedor v-cdg_proveedor BROWSE-2 
&Scoped-Define DISPLAYED-OBJECTS v-des_fecha v-has_fecha por_proveedor ~
...
DEFINE VARIABLE por_proveedor AS LOGICAL INITIAL no 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-2 D-Dialog _FREEFORM
  QUERY BROWSE-2 DISPLAY
      Opg_header.tip_comprob    COLUMN-LABEL "Ti-!po"
      Opg_header.prf_comprob    COLUMN-LABEL "Pre-!fijo"
      Opg_header.nro_comprob    COLUMN-LABEL "Número!Compbte"
      Opg_header.anulado        COLUMN-LABEL "Anu-!lada"
      ...
      Proveedor.nombre          COLUMN-LABEL "Razón!Social" FORMAT "X(35)"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 116 BY 9.67 EXPANDABLE.
...
DEFINE FRAME D-Dialog
     ...
     v-has_fecha AT ROW 1.48 COL 59 COLON-ALIGNED
     Btn_OK AT ROW 1.48 COL 85
     Btn_Cancel AT ROW 1.48 COL 101
     por_proveedor AT ROW 1.71 COL 4
     v-cdg_proveedor AT ROW 3.14 COL 13 COLON-ALIGNED
     v-nombre AT ROW 3.14 COL 29 COLON-ALIGNED NO-LABEL
     BROWSE-2 AT ROW 4.81 COL 2
     RECT-1 AT ROW 2.86 COL 2
     RECT-9 AT ROW 1.29 COL 2
     SPACE(1.79) SKIP(12.27)
...
/* Query rebuild information for BROWSE BROWSE-2
   ...
   THEN DO:
      OPEN QUERY {&SELF-NAME}
           FOR EACH Opg_header
                          WHERE Opg_header.cdg_empresa = que_empresa
                            AND Opg_header.tip_comprob = "OP"
                            AND Opg_header.nro_proveedor = que_proveedor
                            AND Opg_header.fecha >= v-des_fecha
                            AND Opg_header.fecha <= v-has_fecha
                            AND LOOKUP(Opg_header.estado,lista_estados) <> 0 NO-LOCK,
                                FIRST Moneda OF Opg_header NO-LOCK,
                                FIRST Proveedor OF Opg_header NO-LOCK.
   END.
   ELSE DO:
      OPEN QUERY {&SELF-NAME}
           FOR EACH Opg_header
                          WHERE Opg_header.cdg_empresa = que_empresa
                            ...
                            AND LOOKUP(Opg_header.estado,lista_estados) <> 0 NO-LOCK,
                                ...
                                FIRST Proveedor OF Opg_header
                                      WHERE Proveedor.titular_oxp_sino = FALSE NO-LOCK.
   END.
     _END_FREEFORM
     _Query            is OPENED
*/  /* BROWSE BROWSE-2 */
...
ON VALUE-CHANGED OF por_proveedor IN FRAME D-Dialog /* Filtrar por Proveedor */
DO:
...
------------------------------------------------------------------------------*/
  DISPLAY v-des_fecha v-has_fecha por_proveedor v-cdg_proveedor v-nombre 
      WITH FRAME D-Dialog.
  ENABLE RECT-1 RECT-9 v-des_fecha v-has_fecha Btn_OK Btn_Cancel por_proveedor 
         v-cdg_proveedor BROWSE-2 
      WITH FRAME D-Dialog.
