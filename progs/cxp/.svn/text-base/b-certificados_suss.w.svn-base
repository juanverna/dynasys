...
&Scoped-define INTERNAL-TABLES Certificado_sus Tipo_retsus Proveedor

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table Certificado_sus.nro_certifsus ~
Proveedor.cdg_proveedor Proveedor.nombre Tipo_retsus.cdg_tiporetsus ~
Certificado_sus.fecha_emision Certificado_sus.imp_retenido ~
Certificado_sus.imp_pagado Certificado_sus.fecha_deposito 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH Certificado_sus WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Tipo_retsus OF Certificado_sus NO-LOCK, ~
      EACH Proveedor OF Certificado_sus NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Certificado_sus WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Tipo_retsus OF Certificado_sus NO-LOCK, ~
      EACH Proveedor OF Certificado_sus NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table Certificado_sus Tipo_retsus ~
Proveedor
&Scoped-define FIRST-TABLE-IN-QUERY-br_table Certificado_sus
&Scoped-define SECOND-TABLE-IN-QUERY-br_table Tipo_retsus
...
&Scoped-Define ENABLED-OBJECTS v-has_fecha v-des_fecha ver_pagos ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Certificado_sus.nro_certifsus FORMAT ">>>>9":U
      ...
      Certificado_sus.fecha_emision FORMAT "99/99/99":U
      Certificado_sus.imp_retenido FORMAT "->,>>>,>>9.99":U
      Certificado_sus.imp_pagado FORMAT "->,>>>,>>9.99":U
      Certificado_sus.fecha_deposito FORMAT "99/99/99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "sic.Certificado_sus,sic.Tipo_retsus OF sic.Certificado_sus,sic.Proveedor OF sic.Certificado_sus"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   = sic.Certificado_sus.nro_certifsus
...
"Proveedor.nombre" ? "X(34)" "character" ? ? ? ? ? ? no ? no no "36.8" yes no no "U" "" ""
     _FldNameList[4]   = sic.Tipo_retsus.cdg_tiporetsus
     _FldNameList[5]   = sic.Certificado_sus.fecha_emision
     _FldNameList[6]   = sic.Certificado_sus.imp_retenido
     _FldNameList[7]   = sic.Certificado_sus.imp_pagado
     _FldNameList[8]   = sic.Certificado_sus.fecha_deposito
...
DO:
  IF AVAILABLE Certificado_sus
  THEN DO:
       RUN d-ver_certificado_susutos.w ( INPUT ROWID(Certificado_sus)).
  END.
  ELSE DO:       
       BELL.
  END.
...
DO:
  IF AVAILABLE Certificado_sus
  THEN DO:
       RUN imprimir_certificado_susutos.p ( INPUT ROWID(Certificado_sus)).
...
DO:
   RUN  listar_certificados_susutos.p (INPUT v-des_fecha,
                                       INPUT v-has_fecha,
                                       INPUT ver_pagos).
...
------------------------------------------------------------------------------*/
  ...
  CASE ver_pagos:
    ...
    THEN DO:
        OPEN QUERY {&BROWSE-NAME}
             FOR EACH Certificado_sus
                WHERE NOT Certificado_sus.anulado
                  AND Certificado_sus.cdg_empresa = que_empresa
                  AND Certificado_sus.fecha_emision <= v-has_fecha
                  AND Certificado_sus.fecha_emision >= v-des_fecha,
                FIRST Tipo_retsus OF Certificado_sus, FIRST Proveedor OF Certificado_sus
                   BY Certificado_sus.fecha_emision.
    END.
    WHEN ya_emi
    THEN DO:
        OPEN QUERY {&BROWSE-NAME}
        ...
        OPEN QUERY {&BROWSE-NAME}
             FOR EACH Certificado_sus
                WHERE NOT Certificado_sus.anulado
                  ...
                  AND Certificado_sus.fecha_emision <= v-has_fecha
                  AND Certificado_sus.fecha_emision >= v-des_fecha
                  AND NOT Certificado_sus.emitido,
                FIRST Tipo_retsus OF Certificado_sus, FIRST Proveedor OF Certificado_sus
                   BY Certificado_sus.fecha_emision.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "Certificado_sus"}
