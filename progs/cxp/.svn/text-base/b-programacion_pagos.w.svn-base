...
&Scoped-define QUERY-STRING-br_table FOR EACH Cta_cte_prv OF Moneda WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      FIRST Imputacion OF Cta_cte_prv NO-LOCK, ~
      FIRST Proveedor OF Cta_cte_prv NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Cta_cte_prv OF Moneda WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      FIRST Imputacion OF Cta_cte_prv NO-LOCK, ~
      FIRST Proveedor OF Cta_cte_prv NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
&Scoped-Define ENABLED-OBJECTS btn_comprobte v-des_fecha ver_pagos ~
id_moneda v-has_fecha btn_listado br_table RECT-1 
&Scoped-Define DISPLAYED-OBJECTS v-des_fecha ver_pagos id_moneda ~
v-has_fecha 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 153 BY 15.48
...
DEFINE FRAME F-Main
     btn_comprobte AT ROW 1.24 COL 128
     v-des_fecha AT ROW 1.29 COL 9 COLON-ALIGNED
     ver_pagos AT ROW 1.29 COL 35 NO-LABEL
     id_moneda AT ROW 1.95 COL 67 COLON-ALIGNED
     v-has_fecha AT ROW 2.33 COL 9 COLON-ALIGNED
     btn_listado AT ROW 2.43 COL 128
...
"Cta_cte_prv.programada" "Progra-!mada" ? "logical" ? ? ? ? ? ? no ? no no "10" yes no no "U" "" ""
...
DO:
  ...
  ELSE DO:
     ...
     THEN DO:                  
        ...
        RUN VALUE("run_" + Fac_header_prv.cdg_comprobante + ".p" ) ( INPUT-OUTPUT act_fpr_head , INPUT 2 ).
        /*
        CASE Cta_cte_prv.tip_comprob:
            WHEN "FA" THEN RUN c-factura_proveedor.w (INPUT-OUTPUT act_fpr_head, INPUT 2).
            WHEN "FB" THEN RUN c-factura_proveedor.w (INPUT-OUTPUT act_fpr_head, INPUT 2).
            WHEN "FC" THEN RUN c-factura_proveedor.w (INPUT-OUTPUT act_fpr_head, INPUT 2).
  
            WHEN "CA" THEN RUN c-creditoart_proveedor.w (INPUT-OUTPUT act_fpr_head, INPUT 2).
            WHEN "CB" THEN RUN c-creditoart_proveedor.w (INPUT-OUTPUT act_fpr_head, INPUT 2).
            WHEN "CC" THEN RUN c-creditoart_proveedor.w (INPUT-OUTPUT act_fpr_head, INPUT 2).
            WHEN "CI" THEN RUN c-credintart_proveedor.w (INPUT-OUTPUT act_fpr_head, INPUT 2).
  
            WHEN "DA" THEN RUN c-debitoart_proveedor.w (INPUT-OUTPUT act_fpr_head, INPUT 2).
            WHEN "DB" THEN RUN c-debitoart_proveedor.w (INPUT-OUTPUT act_fpr_head, INPUT 2).
            WHEN "DC" THEN RUN c-debitoart_proveedor.w (INPUT-OUTPUT act_fpr_head, INPUT 2).
            WHEN "DI" THEN RUN c-debintart_proveedor.w (INPUT-OUTPUT act_fpr_head, INPUT 2).
...
------------------------------------------------------------------------------*/
  ...
  CASE ver_pagos:
    WHEN todos
    THEN DO:
        {&BROWSE-NAME}:TITLE IN FRAME {&FRAME-NAME} = "Pagos programados o no por fecha de vencimiento".
        OPEN QUERY {&BROWSE-NAME}
    ...
    WHEN con_p
    THEN DO:
        {&BROWSE-NAME}:TITLE IN FRAME {&FRAME-NAME} = "Pagos programados por fecha de pago".
        OPEN QUERY {&BROWSE-NAME}
             FOR EACH Cta_cte_prv
                WHERE Cta_cte_prv.cdg_empresa = que_empresa
                  AND Cta_cte_prv.nro_moneda = que_moneda
                  AND Cta_cte_prv.debito < Cta_cte_prv.credito
                  AND Cta_cte_prv.fecha_programada <= v-has_fecha
                  AND Cta_cte_prv.fecha_programada >= v-des_fecha
    ...
    WHEN sin_p
    THEN DO:
        {&BROWSE-NAME}:TITLE IN FRAME {&FRAME-NAME} = "Pagos s/programar por fecha de vencimiento".
        OPEN QUERY {&BROWSE-NAME}
             FOR EACH Cta_cte_prv
                WHERE Cta_cte_prv.cdg_empresa = que_empresa
                  AND Cta_cte_prv.nro_moneda = que_moneda
