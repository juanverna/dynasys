...
DEFINE TEMP-TABLE CC_destino NO-UNDO LIKE Cta_cte_prv
       ...
       .
DEFINE TEMP-TABLE CC_origen NO-UNDO LIKE Cta_cte_prv
...
DEFINE VARIABLE saldo_origen              LIKE Cta_cte_prv.debito.
DEFINE VARIABLE saldo_destino             LIKE Cta_cte_prv.credito.
...
/*
DEFINE TEMP-TABLE T-Fac_header_prv-bon        NO-UNDO LIKE Fac_header_prv-bon.
DEFINE TEMP-TABLE T-Fac_detalle_prv-bon       NO-UNDO LIKE Fac_detalle_prv-bon.
*/
...
&Scoped-Define ENABLED-OBJECTS v-cdg_proveedor v-fecha_transferencia ~
Btn_Done v-pago_parcial BROWSE_ORIGEN BROWSE_DESTINO RECT-1 RECT-2 RECT-3 ~
RECT-4 RECT-6 RECT-7 RECT-8 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE_ORIGEN C-Win _STRUCTURED
  QUERY BROWSE_ORIGEN NO-LOCK DISPLAY
      ...
      CC_origen.fecha_emision FORMAT "99/99/99":U
...
DEFINE FRAME DEFAULT-FRAME
     v-cdg_proveedor AT ROW 3.14 COL 16 COLON-ALIGNED
     v-dsc_proveedor AT ROW 3.14 COL 34 COLON-ALIGNED NO-LABEL
     ...
     "        Moneda de destino de los movimientos y cambio actual" VIEW-AS TEXT
          ...
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "   Proveedor cuyos movimientos se desea transferir" VIEW-AS TEXT
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: CC_destino T "?" NO-UNDO sic Cta_cte_prv
      ADDITIONAL-FIELDS:
          FIELD saldo_comprobante LIKE CC_Destino.credito
          FIELD saldo_equivalente LIKE CC_Destino.credito
          FIELD cambio_destino    LIKE CC_Destino.cambio
      ...
      END-FIELDS.
      TABLE: CC_origen T "?" NO-UNDO sic Cta_cte_prv
      ADDITIONAL-FIELDS:
          FIELD saldo_comprobante LIKE CC_Origen.credito
          FIELD saldo_original LIKE CC_Origen.credito
      END-FIELDS.
...
DO:
    DEFINE VARIABLE x-cambio LIKE Cta_cte_prv.cambio.
   ...
   /*  v-estado:SENSITIVE = YES. */
    ...
    THEN DO:
        ...
        THEN DO:
            ...
            ASSIGN FRAME {&FRAME-NAME} v-cdg_proveedor v-fecha_transferencia v-pto_venta.
            RUN transferir_movimientos.
            v-estado:HIDDEN = YES.
            APPLY "RETURN" TO v-cdg_proveedor.
        END.
    END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  FIND FIRST Proveedor WHERE CAN-DO(Proveedor.lista_empresas, que_empresa) NO-LOCK NO-ERROR.
  IF AVAILABLE Proveedor
  THEN DO:
      ASSIGN v-cdg_proveedor = Proveedor.cdg_proveedor 
             v-dsc_proveedor = Proveedor.nombre.
      DISPLAY v-cdg_proveedor v-dsc_proveedor WITH FRAME {&FRAME-NAME}.
  END.

  APPLY "RETURN" TO v-cdg_proveedor IN FRAME {&FRAME-NAME}.
  APPLY "ENTRY" TO v-cdg_proveedor IN FRAME {&FRAME-NAME}.
...
------------------------------------------------------------------------------*/
    ...
    RUN getparametro_n.p (  INPUT  "DFDEPOSI", OUTPUT v-valor_c ).
    ...
    FIND Vendedor OF Proveedor NO-LOCK.
    FIND Obra WHERE Obra.cdg_obra = Vendedor.cdg_vendedor NO-LOCK.
    
    CREATE T-Fac_header_prv.
    ASSIGN T-Fac_header_prv.origen            = "R"
           T-Fac_header_prv.cdg_empresa       = que_empresa
           T-Fac_header_prv.cdg_comprobante   = "DBCAMBIO"
           T-Fac_header_prv.estado            = "P"
           T-Fac_header_prv.cta_cte           = YES /* OJO !!!!!! */
           T-Fac_header_prv.nro_facprov       = 1
           T-Fac_header_prv.fecha             = v-fecha_transferencia
           ...
           T-Fac_header_prv.cdg_lista         = Proveedor.dfl_lista
    ...
    FOR EACH CC_Destino WHERE CC_Destino.diferencia_cambio <> 0:
        T-Fac_header_prv.imp_total = T-Fac_header_prv.imp_total + CC_Destino.diferencia_cambio.    
    END.
    
    IF T-Fac_header_prv.imp_total < 0
    THEN DO:
       ASSIGN
           T-Fac_header_prv.cdg_comprobante = "CRCAMBIO"
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN T-Fac_header_prv.origen            = "R"
           T-Fac_header_prv.cdg_empresa       = que_empresa
           T-Fac_header_prv.cdg_comprobante   = "CRTRAMON"
           T-Fac_header_prv.estado            = "P"
           T-Fac_header_prv.Cta_cte           = YES /* OJO !!!!!! */
           T-Fac_header_prv.nro_facprov       = 1
           T-Fac_header_prv.fecha             = v-fecha_transferencia
           ...
           T-Fac_header_prv.cdg_imputacion    = p-cdg_concepto
           T-Fac_header_prv.impreso           = ""
           T-Fac_header_prv.cambio            = v-cambio_origen
           T-Fac_header_prv.prf_comprob       = v-pto_venta
           T-Fac_header_prv.nro_cndventa      = Condicion_venta.nro_cndventa
           T-Fac_header_prv.nro_moneda        = v-nro_monedaorigen
           T-Fac_header_prv.nro_deposito      = Deposito.nro_deposito
           T-Fac_header_prv.nro_obra          = Obra.nro_obra
           T-Fac_header_prv.nro_proveedor       = Proveedor.nro_proveedor
           T-Fac_header_prv.nombre            = Proveedor.nombre
    ...
    FOR EACH CC_Destino:
        T-Fac_header_prv.imp_total = T-Fac_header_prv.imp_total + CC_Destino.saldo_comprobante.    
    END.
    
    IF T-Fac_header_prv.imp_total < 0
    THEN DO:
       ASSIGN
           T-Fac_header_prv.cdg_comprobante = "DBTRAMON"
           T-Fac_header_prv.imp_total =  T-Fac_header_prv.imp_total * ( - 1 ).
    END.
    ...
    FOR EACH Cta_cte_prv WHERE Cta_cte_prv.cdg_empresa     = T-Fac_header_prv.cdg_empresa    
                       AND Cta_cte_prv.tip_comprob     = T-Fac_header_prv.tip_comprob    
                       AND Cta_cte_prv.prf_comprob     = T-Fac_header_prv.prf_comprob    
                       AND Cta_cte_prv.nro_comprob     = T-Fac_header_prv.nro_comprob
                           EXCLUSIVE-LOCK:

        IF CAN-DO(str_debitan,Cta_cte_prv.tip_comprob)
            THEN Cta_cte_prv.credito = Cta_cte_prv.debito.
            ELSE Cta_cte_prv.debito  = Cta_cte_prv.credito.

    END.

    FOR EACH CC_Destino:

        FIND Cta_cte_prv WHERE Cta_cte_prv.cdg_empresa     = CC_Destino.cdg_empresa    
                       AND Cta_cte_prv.tip_comprob     = CC_Destino.tip_comprob    
                       AND Cta_cte_prv.prf_comprob     = CC_Destino.prf_comprob    
                       AND Cta_cte_prv.nro_comprob     = CC_Destino.nro_comprob    
                       AND Cta_cte_prv.nro_vencimiento = CC_Destino.nro_vencimiento
                           EXCLUSIVE-LOCK.

        IF CAN-DO(str_debitan,Cta_cte_prv.tip_comprob)
            THEN Cta_cte_prv.credito = Cta_cte_prv.credito + CC_Destino.saldo_comprobante.
            ELSE Cta_cte_prv.debito = Cta_cte_prv.debito - CC_Destino.saldo_comprobante. /* CC_Destino.saldo_comprobante. es < 0 */
        ...
        ASSIGN Aplicacion_pagos.cdg_empresa      = T-Fac_header_prv.cdg_empresa
               ...
               Aplicacion_pagos.tip_comprob      = T-Fac_header_prv.tip_comprob
               Aplicacion_pagos.prf_comprob      = T-Fac_header_prv.prf_comprob
               Aplicacion_pagos.nro_comprob      = T-Fac_header_prv.nro_comprob           
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH CC_destino:
        T-Fac_header_prv.imp_total = T-Fac_header_prv.imp_total + CC_destino.saldo_equivalente.    
    END.
...
------------------------------------------------------------------------------*/
  DISPLAY v-cdg_proveedor v-dsc_proveedor v-fecha_transferencia v-pto_venta 
  ...
  ENABLE v-cdg_proveedor v-fecha_transferencia Btn_Done v-pago_parcial 
         BROWSE_ORIGEN BROWSE_DESTINO RECT-1 RECT-2 RECT-3 RECT-4 RECT-6 RECT-7 
         RECT-8 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH Cta_cte_prv OF Proveedor 
      WHERE Cta_cte_prv.nro_moneda = v-nro_monedaorigen 
        AND Cta_cte_prv.cdg_empresa = que_empresa
        AND Cta_cte_prv.debito <> Cta_cte_prv.credito NO-LOCK:

        CREATE CC_Origen.
        BUFFER-COPY Cta_cte_prv TO CC_origen
        ...
        THEN DO:
            ...
            RUN hallar_cambio ( INPUT v-nro_monedadestino,
                                INPUT Cta_cte_prv.fecha_emision,
                                OUTPUT x-cambio).

            /* Asigna la tasa de cambio entre una moneda y otra */

            ASSIGN CC_Origen.cambio = x-cambio / Cta_cte_prv.cambio.
    ...
    ELSE DO:
        ...
        ELSE DO:
            ...
            ELSE DO:
                ...
                IF CAN-FIND(FIRST Fac_header_prv
                                  WHERE Fac_header_prv.cdg_empresa = que_empresa
                                    AND Fac_header_prv.tip_comprob = v-tip_comprob
                                    AND Fac_header_prv.prf_comprob = v-pto_venta
                                    AND Fac_header_prv.fecha > v-fecha_transferencia)
                THEN DO:
