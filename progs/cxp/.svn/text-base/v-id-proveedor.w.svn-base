...
&Scoped-Define ENABLED-OBJECTS v-cdg_proveedor RECT-1 
&Scoped-Define DISPLAYED-OBJECTS v-nombre v-cdg_proveedor 
...
DEFINE FRAME F-Main
     v-nombre AT ROW 1.24 COL 30 COLON-ALIGNED NO-LABEL
     v-cdg_proveedor AT ROW 1.29 COL 12 COLON-ALIGNED
...
ON LEAVE OF v-cdg_proveedor IN FRAME F-Main /* Proveedor */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL v-cdg_proveedor V-table-Win
ON LEFT-MOUSE-DBLCLICK OF v-cdg_proveedor IN FRAME F-Main /* Proveedor */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL v-cdg_proveedor V-table-Win
ON MOUSE-MENU-DOWN OF v-cdg_proveedor IN FRAME F-Main /* Proveedor */
DO:
...
DO:
  ...
  FIND Proveedor WHERE Proveedor.cdg_proveedor = INPUT FRAME F-Main v-cdg_proveedor NO-LOCK NO-ERROR.
  ...
  IF NUM-ENTRIES (c) eq 1 THEN DO:
    h = WIDGET-HANDLE (c).
    RUN posicionar_query IN h (INPUT ROWID(Proveedor)).
  END.
...
------------------------------------------------------------------------------*/
  ...
  v-cdg_proveedor:SENSITIVE IN FRAME {&FRAME-NAME} = YES.
