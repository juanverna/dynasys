...
DEFINE VARIABLE h_v-par-ivacompras AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Exportacion Archivo CITI"
         HEIGHT             = 24
         WIDTH              = 124.4
         MAX-HEIGHT         = 24
         MAX-WIDTH          = 124.4
...
ON END-ERROR OF W-Win /* Exportacion Archivo CITI */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Exportacion Archivo CITI */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-par-ivacompras.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-par-ivacompras ).
       RUN set-position IN h_v-par-ivacompras ( 8.52 , 14.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-par-ivacompras ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/CXP" "Subdiario de IVA Compras"}
...
------------------------------------------------------------------------------*/
  ...
  RUN dar_rango IN h_v-par-ivacompras (  OUTPUT  lista_tipos,
                                         OUTPUT  lis_fecha,
                                         OUTPUT  error_rango).
  ...
  THEN DO:
       ...
       RUN lsexportaciti.p ( INPUT des_fecha, 
                             INPUT has_fecha, 
                             INPUT lista_tipos).
