...
DO:
    ...
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND FIRST Impuesto 
             WHERE Impuesto.nombre >= SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} 
      /*       AND CAN-DO(Impuesto.lista_empresas,Empresa.cdg_empresa)*/
                   NO-LOCK NO-ERROR.
        ELSE FIND FIRST Impuesto NO-LOCK NO-ERROR.
    
    IF AVAILABLE Impuesto 
    THEN DO:
         des_nombre = Impuesto.nombre.
         des_registro = Impuesto.cdg_Impuesto.
    END.
    ELSE DO:
         des_nombre = "???".
    END.
...
ON + OF des_registro IN FRAME F-Main /* Impuesto */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON MOUSE-MENU-DOWN OF des_registro IN FRAME F-Main /* Impuesto */
DO:
  &SCOPED-DEFINE ROWID_TABLA        rid_desregistro
  &SCOPED-DEFINE SELECCION          SELIMPUE.P
  &SCOPED-DEFINE TABLA              Impuesto
  &SCOPED-DEFINE CDG_TABLA          cdg_impuesto
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON RETURN OF des_registro IN FRAME F-Main /* Impuesto */
DO:

    ASSIGN FRAME {&FRAME-NAME} des_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND FIRST Impuesto WHERE Impuesto.cdg_impuesto >= INTEGER(SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME}) NO-LOCK NO-ERROR.
        ELSE FIND FIRST Impuesto USE-INDEX cdg_impuesto NO-LOCK NO-ERROR.
