...
DEFINE FRAME F-Main
     des_registro AT ROW 1.29 COL 19.6 COLON-ALIGNED
     des_nombre AT ROW 1.29 COL 35.6 COLON-ALIGNED NO-LABEL
     has_registro AT ROW 2.33 COL 19.6 COLON-ALIGNED
     has_nombre AT ROW 2.33 COL 35.6 COLON-ALIGNED NO-LABEL
...
ON + OF des_registro IN FRAME F-Main /* Desde Provincia */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON MOUSE-MENU-DOWN OF des_registro IN FRAME F-Main /* Desde Provincia */
DO:
  &SCOPED-DEFINE ROWID_TABLA        rid_desprovincia
  &SCOPED-DEFINE SELECCION          SELPROV.P
  &SCOPED-DEFINE TABLA              Provincia
  &SCOPED-DEFINE CDG_TABLA          cdg_provincia
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL des_registro V-table-Win
ON RETURN OF des_registro IN FRAME F-Main /* Desde Provincia */
DO:

    ASSIGN FRAME {&FRAME-NAME} des_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND Provincia WHERE Provincia.cdg_provincia = SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
        ELSE FIND FIRST Provincia NO-LOCK NO-ERROR.
    
    IF AVAILABLE Provincia 
    THEN DO:
         des_nombre   = Provincia.nombre.
         des_registro = Provincia.cdg_provincia.
    END.
    ELSE DO:
         des_nombre = "???".
    END.
...
ON + OF has_registro IN FRAME F-Main /* Hasta Provincia */
DO:
  APPLY "MOUSE-MENU-DOWN" TO SELF.
  RETURN NO-APPLY.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL has_registro V-table-Win
ON MOUSE-MENU-DOWN OF has_registro IN FRAME F-Main /* Hasta Provincia */
DO:
   &SCOPED-DEFINE ROWID_TABLA       rid_hasprovincia
  &SCOPED-DEFINE SELECCION          SELPROV.P
  &SCOPED-DEFINE TABLA              Provincia
  &SCOPED-DEFINE CDG_TABLA          cdg_provincia
  &SCOPED-DEFINE DSC_TABLA          nombre
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL has_registro V-table-Win
ON RETURN OF has_registro IN FRAME F-Main /* Hasta Provincia */
DO:

    ASSIGN FRAME {&FRAME-NAME} des_registro.
    IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> ""
        THEN FIND Provincia WHERE Provincia.cdg_provincia = SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
        ELSE FIND LAST Provincia NO-LOCK NO-ERROR.
    
    IF AVAILABLE Provincia 
    THEN DO:
         has_nombre   = Provincia.nombre.
         has_registro = Provincia.cdg_provincia.
    END.
    ELSE DO:
         has_nombre = "???".
    END.
