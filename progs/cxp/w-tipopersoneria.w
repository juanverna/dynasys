...
DEFINE VARIABLE h_b-tipopersoneria AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-tipopersoneria AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Tipos de Personería */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Tipos de Personería */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Tipos de Personería */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa-2 ).
       RUN set-position IN h_p-updspa-2 ( 6.00 , 22.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa-2 ( 2.14 , 96.00 ) NO-ERROR.

    END. /* Page 0 */

    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-tipopersoneria.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tipopersoneria ).
       RUN set-position IN h_b-tipopersoneria ( 1.95 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-tipopersoneria ( 24.76 , 16.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-tipopersoneria.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-tipopersoneria ).
       RUN set-position IN h_v-tipopersoneria ( 1.95 , 22.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.57 , 96.00 ) */

       /* Links to SmartViewer h_v-tipopersoneria. */
       RUN add-link IN adm-broker-hdl ( h_b-tipopersoneria , 'Record':U , h_v-tipopersoneria ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa-2 , 'TableIO':U , h_v-tipopersoneria ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/CXP" "Tipos de Personería"}
...
*/
 ...
 IF VALID-HANDLE(h_b-tipopersoneria) THEN RUN set-sensitivo IN h_b-tipopersoneria ( INPUT p-operacion = "HABILITAR" ).
