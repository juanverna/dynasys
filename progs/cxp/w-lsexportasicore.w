...
DEFINE FRAME F-Main
     v-estado AT ROW 17.91 COL 27 COLON-ALIGNED NO-LABEL
     "  Rango de fechas de los movimientos a exportar:" VIEW-AS TEXT
          SIZE 100 BY .81 AT ROW 7.67 COL 29
...
ON END-ERROR OF W-Win /* Exportacion a Sicore */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Exportacion a Sicore */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-fechas ( 8.76 , 29.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 19.29 , 29.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/CXP" "Exportación de Retenciones a SICORE"}
...
------------------------------------------------------------------------------*/
  ...
  THEN DO:
       ...
       RUN lsexportasicore.p (INPUT des_fecha,
                              INPUT has_fecha).
