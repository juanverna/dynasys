...
DEFINE VARIABLE nt-copias      AS INTEGER INITIAL 1.
...
DEFINE VARIABLE c-linea_detalle_prv AS INTEGER.
...
DEFINE VARIABLE v-leng_detalle_prv AS INTEGER INITIAL 60. /* Ancho en chars del detalle    */
...
DO TRANSACTION:
    ...
    FIND Proveedor   OF Fac_header_prv NO-LOCK NO-ERROR.
    /*FIND Provlegal WHERE Provlegal.cdg_provincia = Fac_header_prv.cdg_provincia_leg NO-LOCK.    */
    ...
    FIND FIRST Cta_cte_prv WHERE Cta_cte_prv.tip_comprob   = Fac_header_prv.tip_comprob
                             ...
                             AND Cta_cte_prv.nro_proveedor = Fac_header_prv.nro_proveedor
                                 NO-LOCK NO-ERROR.
    
    v-remitos = "".
    FOR EACH Rem_header_prv WHERE Rem_header_prv.nro_facprov = Fac_header_prv.nro_facprov NO-LOCK:
        v-remitos = v-remitos + "," + fnComprobante(Rem_header_prv.tip_comprob,Rem_header_prv.prf_comprob,Rem_header_prv.nro_comprob).
    END.
...
PROCEDURE cabecera_forma:
    ...
    RUN escribir ( INPUT "28,10", INPUT Empresa.direccion, INPUT 8, INPUT NO, INPUT "BLACK"). 
    RUN escribir ( INPUT "28,105", INPUT "C.U.I.T.: " + Empresa.cuit, INPUT 8, INPUT NO, INPUT "BLACK"). 

    RUN escribir ( INPUT "32,10", INPUT Empresa.localidad + " - " + Empresa.provincia, INPUT 8, INPUT NO, INPUT "BLACK"). 
    RUN escribir ( INPUT "32,105", INPUT "Ingresos Brutos(Conv.Multil.): 902-871143-5", INPUT 8, INPUT NO, INPUT "BLACK"). 

    RUN escribir ( INPUT "36,10", INPUT Empresa.telefono, INPUT 8, INPUT NO, INPUT "BLACK"). 
...
PROCEDURE cabecera_datos:
  ...
  /*RUN escribir ( INPUT "16,180", INPUT TRIM(STRING(n-hoja,">>9")) + "/" + TRIM(STRING(nt-hojas,">>9")), INPUT 12, INPUT YES, INPUT "BLACK").*/
    ...
    RUN escribir ( INPUT "51,10", INPUT Fac_header_prv.nombre, INPUT 8, INPUT YES, INPUT "BLUE").
    RUN escribir ( INPUT "51,85", INPUT "[" + Proveedor.cdg_Proveedor + "]", INPUT 8, INPUT NO, INPUT "BLUE").
    /*
    RUN escribir ( INPUT "55,10", INPUT Fac_header_prv.direccion_leg, INPUT 8, INPUT NO, INPUT "BLUE").
    RUN escribir ( INPUT "59,10", INPUT "(" + Fac_header_prv.cdg_postal_leg + ") " + Fac_header_prv.localidad_leg, INPUT 8, INPUT NO, INPUT "BLUE").
    ...
    */
    RUN escribir ( INPUT "67,21", INPUT Fac_header_prv.cuit, INPUT 8, INPUT NO, INPUT "BLUE").
    RUN escribir ( INPUT "67,70", INPUT Proveedor.numero_ibr, INPUT 8, INPUT NO, INPUT "BLUE").
    ...
    RUN escribir ( INPUT "55,105", INPUT Fac_header_prv.direccion, INPUT 8, INPUT NO, INPUT "BLUE").
    RUN escribir ( INPUT "59,105", INPUT "(" + Fac_header_prv.cdg_postal + ") " + Fac_header_prv.localidad, INPUT 8, INPUT NO, INPUT "BLUE").
    ...
    RUN escribir ( INPUT "84,170", INPUT Fac_header_prv.nro_ocm, INPUT 8, INPUT NO, INPUT "BLUE").
     
    RUN escribir ( INPUT "88,25", INPUT v-remitos, INPUT 8, INPUT NO, INPUT "BLUE").

    IF AVAILABLE Cta_cte_prv
       THEN RUN escribir ( INPUT "84,32", INPUT STRING(Cta_cte_prv.fecha_vencimiento,"99/99/9999"), INPUT 8, INPUT NO, INPUT "BLUE").
...
PROCEDURE detalle_datos:
    ...
    FOR EACH Fac_detalle_prv OF Fac_header_prv,
        ...
        IF NOT Articulo.extendida OR TRUE
        THEN DO:
            
            ch_linea = STRING(linea0,">>9").  
            RUN detalle-numr ( ch_linea  + ",127", INPUT STRING(v-cantidad,"ZZZZZZ9.99"), INPUT 8, INPUT NO, INPUT "BLUE").
            RUN detalle-char ( ch_linea  + ",147", INPUT Unidad.abrevia, INPUT 8, INPUT NO, INPUT "BLUE").
            ...
            THEN DO:
                IF Fac_detalle_prv.precio <> 0
                THEN DO:
                    IF Imputacion.afecta_stock 
                        THEN RUN detalle-numr ( ch_linea  + ",147", INPUT STRING(Fac_detalle_prv.precio,"-ZZZ,ZZ9.9999"), INPUT 8, INPUT NO, INPUT "BLUE").
                    RUN detalle-numr ( ch_linea  + ",166", INPUT STRING(Fac_detalle_prv.subtotal_bruto,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT NO, INPUT "BLUE").
                END.
                ELSE DO:
            ...
            c-linea_detalle_prv = c-linea_detalle_prv + 1.
    
        END.
        ELSE DO:
            
            RUN RENGLONS.P (INPUT  Fac_detalle_prv.detallada, 
                            INPUT  v-leng_detalle_prv,
                            OUTPUT v-detallada,
                            INPUT  "|").
                ...
                THEN DO:
                    ...
                    THEN DO:
                        ...
                        THEN DO:
                            ...
                            RUN detalle-numr ( ch_linea  + ",145", INPUT STRING(Fac_detalle_prv.precio,"-ZZZ,ZZ9.99"), INPUT 8, INPUT NO, INPUT "BLUE").
                        END.
                        RUN detalle-numr ( ch_linea  + ",166", INPUT STRING(Fac_detalle_prv.subtotal_bruto,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT NO, INPUT "BLUE").
                    END.
                END.
    ...
    IF NUM-ENTRIES(ventas_terceros,",") > 0
        THEN c-linea_detalle_prv = c-linea_detalle_prv + 1.
    ...
    THEN DO:
        c-linea_detalle_prv = c-linea_detalle_prv + 1.
        DO j-tomo = 1 TO NUM-ENTRIES(Fac_header_prv.leyenda,chr(10)):

            v-tomo_leyenda = ENTRY(j-tomo,Fac_header_prv.leyenda,chr(10)).
            IF v-tomo_leyenda <> ""
            THEN DO:
    ...
    IF CAN-FIND(FIRST Fac_header_prv_impuesto OF Fac_header_prv)
    THEN DO:

        lineas_impuestos = 0.
        FOR EACH Fac_header_prv_impuesto OF Fac_header_prv, FIRST Impuesto OF Fac_header_prv_impuesto WHERE NOT Impuesto.es_iva BY Impuesto.cdg_impuesto:
            lineas_impuestos = lineas_impuestos + 1.
        END.

        cantidad_hojas = TRUNC(( c-linea_detalle_prv / nmax_det ),0) + IF c-linea_detalle_prv MOD nmax_det = 0 THEN 0 ELSE 1.
        IF c-linea_detalle_prv + lineas_impuestos > cantidad_hojas * nmax_det THEN cantidad_hojas = cantidad_hojas + 1.
        comienzo_impuestos = cantidad_hojas * nmax_det - lineas_impuestos + 1.

        nreng = c-linea_detalle_prv + 1.
        ...
        FOR EACH Fac_header_prv_impuesto OF Fac_header_prv, FIRST Impuesto OF Fac_header_prv_impuesto WHERE NOT Impuesto.es_iva BY Impuesto.cdg_impuesto:
            ...
            RUN detalle-numr ( INPUT TRIM(STRING(linea0,">>9")) + ",166", INPUT STRING(Fac_header_prv_impuesto.importe,"-ZZ,ZZZ,ZZ9.99"), INPUT 8, INPUT NO, INPUT "BLUE").        
            c-linea_detalle_prv = c-linea_detalle_prv + 1.
        END.
...
PROCEDURE pie_forma:
    ...
    RUN escribir ( INPUT "25,102", INPUT Fac_header_prv.cai, INPUT 8, INPUT NO, INPUT "BLACK"). 
    RUN escribir ( INPUT "24,175", INPUT STRING(Fac_header_prv.rige_hasta,"99/99/9999"), INPUT 10, INPUT YES, INPUT "BLACK"). 
...
PROCEDURE poner_monto_letras:
    ...
    RUN RENGLONS.P (INPUT  Fac_header_prv.monto_letras, 
                    INPUT  v-leng_monto,
    ...
    IF Moneda.es_local
        ...
        ELSE RUN escribir ( INPUT TRIM(STRING(linea0,">>9")) + ",10", INPUT "SON " + Moneda.descripcion /*+ " a una razón de cambio de $" + STRING(Fac_header_prv.cambio,">>>9.9999")*/, INPUT 8, INPUT YES, INPUT "BLACK").
...
PROCEDURE detalle-char:
    ...
    ASSIGN T-Detalle.n-orden = c-orden
           T-Detalle.n-linea = c-linea_detalle_prv
...
PROCEDURE bajar_copias:
   ...
   */
   ...
   DO ncopia = 1 TO nt-copias:    
        ...
        c-linea_detalle_prv = 0.
        FOR EACH T-Detalle BREAK BY T-Detalle.n-linea:

            IF FIRST-OF(T-Detalle.n-linea)
            THEN DO:
                c-linea_detalle_prv = T-Detalle.n-linea - ( n-hoja - 1 ) * nmax_det.
                IF c-linea_detalle_prv > nmax_det
                THEN DO:
            ...
            RUN grabar_linea ( INPUT REPLACE(T-Detalle.l-texto,"&&",TRIM(STRING(linea0 + c-linea_detalle_prv * 4 ,">>>9")))).
