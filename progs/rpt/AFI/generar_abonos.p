...
FUNCTION fac_bloqueo RETURNS LOGICAL ( nro AS INT , cant AS INT ) :
    ...
    FOR EACH fac WHERE fac.nro_cliente = nro BY fac.fecha DESC:
        IF pagado(fac.nro_factura) <> "S" THEN
            ii = ii  + 1.
        IF ii >= cant THEN RETURN FALSE.
    END.
    RETURN TRUE.
    END.
...
/*RUN carparam.p.*/
    ...
    FOR EACH Contrato_hd 
        ...
        END.
     /*
        IF fac_bloqueo( cliente.nro_cliente , 3 ) THEN DO:
                OUTPUT STREAM errores TO value(filebloqueo) append.
                put STREAM errores "Cliente " cliente.cdg_cliente "-" Cliente.nom_cliente contrato_hd.prf_contrato "-" contrato_hd.num_contrato 
                    " Tiene facturas impagas" SKIP.
                hayerror = TRUE.
                OUTPUT STREAM errores CLOSE.
                NEXT.
        END.
        ...
        THEN DO:
            ...
            IF NOT CAN-FIND(FIRST   Cta_cte OF Cliente 
                              WHERE Cta_cte.ano = p-ano_imputa
                                ...
                                AND Cta_cte.tip_comprob begins "F"
...
PROCEDURE generar_facturacion:
    ...
    DEFINE OUTPUT PARAMETER plista AS CHAR NO-UNDO.
    ...
    ASSIGN  T-Fac_header.prf_comprob        = Contrato_hd.prf_contrato
       ...
       FOR EACH Contrato_dt OF Contrato_hd NO-LOCK, 
        ...
        IF LOOKUP(Articulo.cdg_tipoart , substring( plista , 2 ) ," " ) = 0 THEN
                         ASSIGN plista = plista + " " +  Articulo.cdg_tipoart.
    END.
