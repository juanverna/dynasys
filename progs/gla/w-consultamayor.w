...
DEFINE VARIABLE h_b-mayor-cuenta AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-cuenta AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-que-cuenta AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Consulta de Mayor por Cuenta"
         HEIGHT             = 26.76
         WIDTH              = 160
         MAX-HEIGHT         = 26.76
         MAX-WIDTH          = 160
...
ON END-ERROR OF W-Win /* Consulta de Mayor por Cuenta */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Consulta de Mayor por Cuenta */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Consulta de Mayor por Cuenta */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-que-cuenta.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-que-cuenta ).
       RUN set-position IN h_v-que-cuenta ( 1.24 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.67 , 81.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-mayor-cuenta.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-mayor-cuenta ).
       RUN set-position IN h_b-mayor-cuenta ( 3.38 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-mayor-cuenta ( 23.81 , 157.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-cuenta ).
       RUN set-position IN h_q-cuenta ( 1.29 , 67.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.76 , 8.20 ) */

       /* Links to SmartBrowser h_b-mayor-cuenta. */
       RUN add-link IN adm-broker-hdl ( h_q-cuenta , 'Record':U , h_b-mayor-cuenta ).

       /* Links to SmartQuery h_q-cuenta. */
       RUN add-link IN adm-broker-hdl ( h_v-que-cuenta , 'Posicionar':U , h_q-cuenta ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-mayor-cuenta ,
             h_v-que-cuenta , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/GLA" "Consulta de Mayor por Cuenta"}
