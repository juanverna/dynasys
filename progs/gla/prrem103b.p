/*=================================================================================*/
/*              IMPRESION DE REMITO DE EGRESOS DE BIENES A CLIENTES                */
/*=================================================================================*/
...
DO TRANSACTION:
    ...
    FIND Empleado WHERE Empleado.nro_legajo = Rem_header.cdg_solicitante NO-LOCK .
...
/*     PUT STREAM Formulario CONTROL "<AT=3,0><#2>". */
    ...
    RUN escribir ( INPUT "26,1", INPUT Cliente.nom_cliente, INPUT 8, INPUT YES).  
/*     RUN escribir ( INPUT "51,85", INPUT "[" + Cliente.cdg_proveedor + "]", INPUT 8, INPUT NO).  */
    RUN escribir ( INPUT "30,1", INPUT Cliente.direccion, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "34,1", INPUT "(" + Cliente.cdg_postal + ") " + Cliente.localidad, INPUT 8, INPUT NO).
...
/*     RUN escribir ( INPUT "63,85", INPUT "[" + Provlegal.cdg_provincia + "]", INPUT 8, INPUT NO).                                                                                          */
    RUN escribir ( INPUT "38,12", INPUT Cliente.cuit, INPUT 8, INPUT NO).
...
/*     PUT STREAM Formulario CONTROL "<AT=235,8><#1>". */
    ...
    RUN escribir ( INPUT "159,28", INPUT Rem_header.observacion, INPUT 8, INPUT NO).
    ...
    RUN escribir ( INPUT "196,122", INPUT Rem_header.cai, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "199,127", INPUT STRING(Rem_header.rige_hasta,"99/99/9999"), INPUT 14, INPUT NO).

    RUN escribebarra ( INPUT "197,50", INPUT STRING(Rem_header.prf_comprob,"9999") + STRING(Rem_header.nro_comprob,"99999999"), INPUT 9, INPUT no).
