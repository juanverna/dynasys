...
DEFINE VARIABLE h_v-par-renasn AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     "  Rango de fechas de los asientos a renumerar:" VIEW-AS TEXT
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Renumeración de Asientos Contables"
         HEIGHT             = 22.91
         WIDTH              = 124.2
         MAX-HEIGHT         = 25.33
         MAX-WIDTH          = 138.6
...
ON END-ERROR OF W-Win /* Renumeración de Asientos Contables */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Renumeración de Asientos Contables */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-fechas ( 3.86 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-par-renasn.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-par-renasn ).
       RUN set-position IN h_v-par-renasn ( 8.81 , 14.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-par-renasn ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/GLA" "Renumeración de Asientos Contables"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE ult_asiento      AS INTEGER.
  ...
  RUN dar_rango IN h_v-par-renasn (  OUTPUT ult_asiento,
                                     OUTPUT error_rango).
  ...
  THEN DO:
       ...
       RUN lsrenasn.P ( INPUT  des_fecha,
                        INPUT  has_fecha,
                        INPUT  ult_asiento).   
