...
DEFINE VARIABLE h_v-par-refundir AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 16.95 COL 12 COLON-ALIGNED NO-LABEL
     "  Parámetros Generales del Listado:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 2.91 COL 15
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Moneda en la que se va a exhibir el asiento:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 10.29 COL 15
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Asiento de Cierre de Cuentas Patrimoniales"
         HEIGHT             = 22.14
         WIDTH              = 132.6
         MAX-HEIGHT         = 34.24
         MAX-WIDTH          = 204.8
...
ON END-ERROR OF W-Win /* Asiento de Cierre de Cuentas Patrimoniales */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Asiento de Cierre de Cuentas Patrimoniales */
DO:
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Layout = ,
                     Create-On-Add = No':U ,
             OUTPUT h_v-par-refundir ).
       RUN set-position IN h_v-par-refundir ( 4.10 , 15.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.33 , 100.00 ) */
       ...
       RUN set-position IN h_v-que-expresion ( 11.71 , 15.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 18.57 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.62 , 100.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-par-refundir ,
             v-estado:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-que-expresion ,
             h_v-par-refundir , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/GLA" "Asiento de Cierre de Cuentas Patrimoniales"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE v-cdg_cuenta     LIKE Cuenta.cdg_cuenta.
  ...
  DEFINE VARIABLE ult_mes          AS INTEGER.
  DEFINE VARIABLE ult_ano          AS INTEGER.
  DEFINE VARIABLE generar          AS LOGICAL.
  DEFINE VARIABLE v-tip_comprob    AS CHARACTER.
  DEFINE VARIABLE v-nro_comprob    AS CHARACTER.

  DEFINE VARIABLE error_rango      AS LOGICAL.
  DEFINE VARIABLE hay_error        AS LOGICAL.

  RUN dar_rango IN h_v-par-refundir (  OUTPUT ult_ano,
                                       OUTPUT ult_mes,
                                       OUTPUT generar,
  ...
  THEN DO:
       ...
       RUN lsasiento_cierre_patrimonial.p (INPUT generar, 
                                           INPUT ult_mes,
                                           INPUT ult_ano,
                                           INPUT v-cdg_moneda,
                                           INPUT v-reexpresion).   
