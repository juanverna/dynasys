...
PROCEDURE inicia_formulario:
 ...
 /* RUN grabar_linea ( '<TOOLBAR=!PRINT>'. */
    ...
    RUN grabar_linea ( "<OLANDSCAPE><Title=Impresion de remitos><UNITS=mm><|2>").
    RUN grabar_linea ( "<FORMAT=Legal>").
    RUN grabar_linea ( "<AT=5,7><#1>").   
...
/*     RUN grabar_linea ( "<AT=1,1><#2>"). */
    ...
    RUN grabar_linea ("<=#1><BGCOLOR=WHITE><AT=+01,+01><#3><AT=+5,+15><IMAGE#3=.\imagenes\logo.bmp>").
...
/*     RUN grabar_linea ( "<AT=14,8><#1>"). */
    ...
    RUN linea    ( "46,113", INPUT "87", "V" ).
    RUN escribir ( "48,116", INPUT "CANT.", INPUT 7, INPUT YES).

    RUN linea    ( "46,126", INPUT "87", "V" ).
    RUN escribir ( "48,127", INPUT "Nº SERIE", INPUT 7, INPUT YES).

    RUN linea    ( "46,139", INPUT "87", "V" ).
    RUN escribir ( "48,140", INPUT "RETOR", INPUT 7, INPUT YES).
...
/*     RUN grabar_linea ( "<AT=3,0><#2>"). */
    ...
    RUN escribir ( INPUT "26,20", INPUT Proveedor.nombre, INPUT 8, INPUT YES).  
/*     RUN escribir ( INPUT "51,85", INPUT "[" + Proveedor.cdg_proveedor + "]", INPUT 8, INPUT NO).  */
    RUN escribir ( INPUT "30,20", INPUT Proveedor.direccion, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "34,21", INPUT "(" + Proveedor.cdg_postal + ") " + Proveedor.localidad, INPUT 8, INPUT NO).
...
/*     RUN grabar_linea ( "<AT=14,8><#1>"). */
    ...
    FOR EACH Rem_detalle_prv OF Rem_header_prv 
        ...
        THEN DO:
            ...
            RUN escribir      ( ch_linea  + ",16",  INPUT /*Articulo.descripcion*/ FILL("m",80), INPUT 5, INPUT NO).
            RUN escribenumero ( ch_linea  + ",105",  INPUT STRING(v-cantidad,"ZZZZZZ9.99"), INPUT 6, INPUT NO).
            RUN escribir      ( ch_linea  + ",123", INPUT Unidad.abrevia, INPUT 6, INPUT NO).
            ...
            THEN DO:
                    ...
                    ch_linea = STRING(52 + linea0 * 3,">>9").
                    RUN escribenumero ( ch_linea  + ",143",  INPUT STRING(Etiqueta.num_etiqueta,"99999999"), INPUT 5, INPUT NO).
                    RUN escribir ( ch_linea  + ",127",  INPUT Registrable.nro_serie, INPUT 5, INPUT NO).

                    FIND Sre_detalle WHERE Sre_detalle.nro_solicitud = Sre_header.nro_solicitud AND Sre_detalle.nro_articulo = Articulo.nro_articulo NO-LOCK.
          IF AVAILABLE Sre_detalle THEN
          RUN escribir      ( ch_linea  + ",141",   INPUT Sre_detalle.fecha_retorno, INPUT 6, INPUT NO).
...
/*     RUN grabar_linea ( "<AT=235,8><#1>"). */
    ...
    */
    ...
    RUN escribir ( "196,100", INPUT "C.A.I nº:", INPUT 8, INPUT YES).
    RUN escribir ( "200,100", INPUT "Fecha Vto:", INPUT 8, INPUT YES).
...
/*     RUN grabar_linea ( "<AT=235,8><#1>"). */
    ...
    RUN escribir ( INPUT "196,112", INPUT Rem_header_prv.cai, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "199,115", INPUT STRING(Rem_header_prv.rige_hasta,"99/99/9999"), INPUT 14, INPUT NO).
...
PROCEDURE escribebarra:
    ...
    RUN grabar_linea ("<=#1><AT=+" + TRIM(STRING(i-linea,">>9")) + ",+" + TRIM(STRING(i-columna,'>>9')) + "><#2>").
    RUN grabar_linea ("<=#2><AT=+7,+55><#3>").
    RUN grabar_linea ("<=#2><FROM><BARCODE#3,TYPE=128A,CHECKSUM=TRUE,VALUE=" + texto  + ">").
