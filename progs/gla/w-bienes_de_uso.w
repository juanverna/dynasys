...
DEFINE VARIABLE h_b-bien_de_uso AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updspa AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-bien_de_uso AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Bienes de Uso */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Bienes de Uso */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-MINIMIZED OF W-Win /* Bienes de Uso */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Bien de Uso|Valores' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 1.48 , 22.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 25.95 , 138.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-bien_de_uso.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-bien_de_uso ).
       RUN set-position IN h_b-bien_de_uso ( 1.57 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-bien_de_uso ( 25.71 , 17.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-bien_de_uso.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-bien_de_uso ).
       RUN set-position IN h_v-bien_de_uso ( 5.05 , 26.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.95 , 109.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updspa ).
       RUN set-position IN h_p-updspa ( 16.48 , 26.00 ) NO-ERROR.
       RUN set-size IN h_p-updspa ( 2.33 , 109.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-bien_de_uso. */
       RUN add-link IN adm-broker-hdl ( h_b-bien_de_uso , 'Record':U , h_v-bien_de_uso ).
       RUN add-link IN adm-broker-hdl ( h_p-updspa , 'TableIO':U , h_v-bien_de_uso ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/GLA" "Mantenimiento de Bienes de Uso"}
...
*/
    ...
    IF p-operacion = "HABILITAR" 
    THEN DO:  
         IF VALID-HANDLE(h_b-bien_de_uso) THEN RUN set-sensitivo IN h_b-bien_de_uso  ( INPUT YES ).
    ...
    END.     
    ELSE DO: 
         IF VALID-HANDLE(h_b-bien_de_uso) THEN RUN set-sensitivo IN h_b-bien_de_uso  ( INPUT NO ).
