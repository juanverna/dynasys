...
DEFINE VARIABLE h_v-que-coeficiente AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     "  Rango de fechas de los asientos:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 8.62 COL 16
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Juego de coeficientes a utilizar en el cálculo:" VIEW-AS TEXT
...
ON END-ERROR OF W-Win /* Ajuste por Inflación */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Ajuste por Inflación */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-que-coeficiente.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-que-coeficiente ).
       RUN set-position IN h_v-que-coeficiente ( 4.81 , 16.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 10.05 , 16.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/GLA" "Ajuste por Inflación"}

  v-estado:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "Ingresando Parametros ...".

  RUN habilitar_botones IN h_p-ejecutar ( INPUT "salir=SI,ejecutar=SI,imprimir=SI,verlistado=SI" ).
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE v-cdg_tipocoeficiente     LIKE tipocoeficiente.cdg_tipocoeficiente.
  DEFINE VARIABLE v-generar                 AS LOGICAL.
  DEFINE VARIABLE error_rango               AS LOGICAL.
  DEFINE VARIABLE hay_error                 AS LOGICAL.

  RUN dar_rango IN h_v-que-coeficiente ( OUTPUT v-cdg_tipocoeficiente,
                                         OUTPUT v-generar, 
                                         OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
  ...
  THEN DO:
       ...
       RUN lsajuste_inflacion.p ( INPUT  des_fecha,
                                  INPUT  has_fecha,
                                  INPUT  v-cdg_tipocoeficiente,
                                  INPUT  v-generar).   
