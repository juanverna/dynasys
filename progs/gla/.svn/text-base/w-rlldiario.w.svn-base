...
DEFINE VARIABLE h_v-par-libdiario AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 16.86 COL 12 COLON-ALIGNED NO-LABEL
     "  Moneda en la que deben expresarse los movimientos:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 7.19 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Parámetros Generales del Listado:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 11.24 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de fechas de los asientos:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 2.86 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
ON END-ERROR OF W-Win /* Emisión de Libro Diario */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Emisión de Libro Diario */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-que-expresion ( 8.29 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-par-libdiario.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-par-libdiario ).
       RUN set-position IN h_v-par-libdiario ( 12.33 , 14.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 18.52 , 14.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-par-libdiario ,
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/GLA" "Emisión de Libro Diario"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE acm_debitos      AS DECIMAL.
  DEFINE VARIABLE ult_pagina       AS INTEGER.
  DEFINE VARIABLE renumerar        AS CHARACTER.
  ...
  RUN dar_rango IN h_v-par-libdiario (  OUTPUT acm_debitos,
                                        OUTPUT ult_pagina,
                                        OUTPUT renumerar,
                                        OUTPUT lis_fecha,
                                        OUTPUT error_rango).
  ...
  THEN DO:
       ...
       RUN lslibro_diario.p ( INPUT des_fecha,     
                              INPUT has_fecha,
                              INPUT v-cdg_moneda,
                              INPUT v-reexpresion,     
                              INPUT lis_fecha,   
