...
DEFINE VARIABLE h_v-que-modulo AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 16.86 COL 17 COLON-ALIGNED NO-LABEL
     "  Rango de fechas de los asientos:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 1.95 COL 19
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Archivo de salida a generarse:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 11.48 COL 19
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Módulo a exportar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 6.48 COL 19
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Generación de Interface Contable"
         HEIGHT             = 22.14
         WIDTH              = 135
         MAX-HEIGHT         = 22.14
         MAX-WIDTH          = 135
...
ON END-ERROR OF W-Win /* Generación de Interface Contable */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Generación de Interface Contable */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-rng-fechas ( 3.19 , 19.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-que-modulo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-que-modulo ).
       RUN set-position IN h_v-que-modulo ( 7.67 , 19.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN set-position IN h_v-rng-archivo ( 12.67 , 19.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/GLA" "Generación de Interface Contable"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE que_modulo       LIKE Modulo-sic.cdg_sigla-sic.
  ...
  RUN dar_rango IN h_v-que-modulo   ( OUTPUT que_modulo, 
                                      OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Modulo" ).
       hay_error = error_rango.
  END.     

  RUN dar_rango IN h_v-rng-archivo   ( OUTPUT que_archivo, 
                                       OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
  ...
  THEN DO:
       ...
       RUN lsinterface_contable.p ( INPUT des_fecha,     
                                    INPUT has_fecha,
                                    INPUT que_modulo,
                                    INPUT que_archivo).
