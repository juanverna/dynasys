...
DEFINE BUFFER B-Clase_de_Obra FOR Clase_de_Obra.

DEFINE VARIABLE f-que_clase LIKE Clase_de_Obra.cdg_subclaseobr.
...
DEFINE VARIABLE v-nombre_subclaseobr   LIKE Clase_de_Obra.nombre_subclaseobr.
DEFINE VARIABLE v-rotulo_siguiente     LIKE Clase_de_Obra.rotulo_siguiente.
DEFINE VARIABLE v-longitud_siguiente   LIKE Clase_de_Obra.longitud_siguiente.
DEFINE VARIABLE v-tipo_siguiente       LIKE Clase_de_Obra.tipo_siguiente.
...
&Scoped-define INTERNAL-TABLES Clase_de_Obra

/* Definitions for BROWSE brw_clasificacion                             */
&Scoped-define FIELDS-IN-QUERY-brw_clasificacion ~
SUBSTRING(Clase_de_Obra.cdg_subclaseobr,LENGTH(que_clase) + 2) ~
Clase_de_Obra.nombre_subclaseobr 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brw_clasificacion 
&Scoped-define QUERY-STRING-brw_clasificacion FOR EACH Clase_de_Obra ~
      WHERE Clase_de_Obra.cdg_claseobr = que_clase NO-LOCK
&Scoped-define OPEN-QUERY-brw_clasificacion OPEN QUERY brw_clasificacion FOR EACH Clase_de_Obra ~
      WHERE Clase_de_Obra.cdg_claseobr = que_clase NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brw_clasificacion Clase_de_Obra
&Scoped-define FIRST-TABLE-IN-QUERY-brw_clasificacion Clase_de_Obra
...
&Scoped-Define ENABLED-OBJECTS que_clase que_subclase que_nombre ~
brw_clasificacion camino btn_modificar btn_eliminar Btn_Elegir Btn_Salir 
&Scoped-Define DISPLAYED-OBJECTS que_clase que_subclase que_nombre camino 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brw_clasificacion C-Win _STRUCTURED
  QUERY brw_clasificacion DISPLAY
      SUBSTRING(Clase_de_Obra.cdg_subclaseobr,LENGTH(que_clase) + 2) COLUMN-LABEL "Código!Clase" FORMAT "X(8)":U
      Clase_de_Obra.nombre_subclaseobr COLUMN-LABEL "Denominacion!Clasificación" FORMAT "X(50)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 66 BY 23.57
         FONT 4
         TITLE "Clasificación" EXPANDABLE.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME DEFAULT-FRAME
     que_clase AT ROW 1.24 COL 70 COLON-ALIGNED NO-LABEL
     que_subclase AT ROW 1.29 COL 3 NO-LABEL
     que_nombre AT ROW 1.29 COL 18 COLON-ALIGNED NO-LABEL
     brw_clasificacion AT ROW 2.43 COL 3
     camino AT ROW 2.43 COL 72 NO-LABEL
     btn_modificar AT ROW 26.48 COL 3
     btn_eliminar AT ROW 26.48 COL 43
     Btn_Elegir AT ROW 26.48 COL 72
     Btn_Salir AT ROW 26.48 COL 131
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Mantenimiento de Clases de Obras Contables"
         HEIGHT             = 27.67
         WIDTH              = 160
         MAX-HEIGHT         = 27.67
         MAX-WIDTH          = 160
...
/* SETTINGS FOR FRAME DEFAULT-FRAME
                                                                        */
/* BROWSE-TAB brw_clasificacion que_nombre DEFAULT-FRAME */
...
/* Query rebuild information for BROWSE brw_clasificacion
     _TblList          = "sic.Clase_de_Obra"
     _Where[1]         = "Clase_de_Obra.cdg_claseobr = que_clase"
     _FldNameList[1]   > "_<CALC>"
"SUBSTRING(Clase_de_Obra.cdg_subclaseobr,LENGTH(que_clase) + 2)" "Código!Clase" "X(8)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > sic.Clase_de_Obra.nombre_subclaseobr
"Clase_de_Obra.nombre_subclaseobr" "Denominacion!Clasificación" "X(50)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _Query            is OPENED
...
ON END-ERROR OF C-Win /* Mantenimiento de Clases de Obras Contables */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Mantenimiento de Clases de Obras Contables */
DO:
...
DO:
   ...
   que_subclase = SUBSTRING(Clase_de_Obra.cdg_subclaseobr,LENGTH(que_clase) + 2).
...
DO:
   ...
   IF CAN-FIND(FIRST B-Clase_de_Obra WHERE B-Clase_de_Obra.cdg_claseobr = Clase_de_Obra.cdg_subclaseobr)
   THEN DO:
       RUN PONMENSJ.P (INPUT "CLAS001" ).
   END.    
   ELSE DO:                                                 
       DO TRANSACTION:
          FIND CURRENT Clase_de_Obra EXCLUSIVE-LOCK.   
          DELETE Clase_de_Obra.
       END.
       RUN ABRE_QUERY.
   END.    
...
DO:
   DO TRANSACTION:
        ...
        DISPLAY Clase_de_Obra.nombre_subclaseobr
...
DO:
   ...
   THEN DO:
      ...
      ELSE DO:
         ...
         THEN DO:
            p_punto = p_punto - 1.
            que_clase = SUBSTRING(que_clase,1,p_punto).
            FIND FIRST Clase_de_Obra WHERE Clase_de_Obra.cdg_subclaseobr = que_clase.
            que_nombre = Clase_de_Obra.nombre.
            RUN armar_rotulo.
         END.
         ELSE DO:
            que_clase = "".
   ...
   ELSE DO:
      ...
      FIND FIRST Clase_de_Obra WHERE Clase_de_Obra.cdg_claseobr = que_clase 
                         AND Clase_de_Obra.cdg_subclaseobr = que_clase + "." + que_subclase NO-ERROR.

      IF NOT AVAILABLE Clase_de_Obra
      THEN DO:
         que_nombre = "".
         RUN crear_clasificacion.         
      END.
      ELSE DO:
         que_nombre = Clase_de_Obra.nombre_subclaseobr.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  RUN enable_UI.
  {setwintit.i "SIC/GLA" "Clasificación de Obras Contables"}
...
------------------------------------------------------------------------------*/
/*
  IF AVAILABLE Clase_de_Obra
     THEN rotulo = FILL(" ",l_rotulo - LENGTH(Clase_de_Obra.rotulo_siguiente) - 1) + Clase_de_Obra.rotulo_siguiente + ":".           ELSE rotulo = FILL(" ",l_rotulo - 1) + ":".
*/
...
------------------------------------------------------------------------------*/
   ...
   RUN d-actualizar_nodo.w ( INPUT "Creación de nuevos nodos",
                             INPUT-OUTPUT v-nombre_subclaseobr,
   ...
   IF como_fue
   THEN DO TRANSACTION:
       CREATE Clase_de_Obra.
       ASSIGN Clase_de_Obra.cdg_claseobr         = que_clase                                 
              Clase_de_Obra.cdg_subclaseobr      = que_clase + "." + que_subclase
              Clase_de_Obra.nombre_subclaseobr   = v-nombre_subclaseobr
              Clase_de_Obra.rotulo_siguiente     = v-rotulo_siguiente
              Clase_de_Obra.longitud_siguiente   = v-longitud_siguiente
              Clase_de_Obra.tipo_siguiente       = v-tipo_siguiente.
       FIND CURRENT Clase_de_Obra NO-LOCK.
   END.
...
------------------------------------------------------------------------------*/
  DISPLAY que_clase que_subclase que_nombre camino 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE que_clase que_subclase que_nombre brw_clasificacion camino 
...
------------------------------------------------------------------------------*/
  ...
  FIND FIRST Clase_de_Obra NO-LOCK NO-ERROR.
  IF NOT AVAILABLE Clase_de_Obra
  THEN DO:
       DO TRANSACTION:
          CREATE Clase_de_Obra.
          ASSIGN Clase_de_Obra.cdg_claseobr    = ?
                 Clase_de_Obra.cdg_subclaseobr = "".
       END.
       RELEASE Clase_de_Obra.
  END.
...
------------------------------------------------------------------------------*/
   ...
   ASSIGN v-nombre_subclaseobr   = Clase_de_Obra.nombre_subclaseobr
          v-rotulo_siguiente     = Clase_de_Obra.rotulo_siguiente
          v-longitud_siguiente   = Clase_de_Obra.longitud_siguiente
          v-tipo_siguiente       = Clase_de_Obra.tipo_siguiente.
   ...
   THEN DO TRANSACTION:
       FIND CURRENT Clase_de_Obra EXCLUSIVE-LOCK.
       ASSIGN Clase_de_Obra.nombre_subclaseobr   = v-nombre_subclaseobr
              Clase_de_Obra.rotulo_siguiente     = v-rotulo_siguiente
              Clase_de_Obra.longitud_siguiente   = v-longitud_siguiente
              Clase_de_Obra.tipo_siguiente       = v-tipo_siguiente.
       FIND CURRENT Clase_de_Obra NO-LOCK.
