...
DEFINE VARIABLE v-imp_gravado  LIKE Rem_header.imp_neto FORMAT "Z,ZZZ,ZZ9.99-".
DEFINE VARIABLE v-imp_exento   LIKE Rem_header.imp_neto FORMAT "Z,ZZZ,ZZ9.99-".
...
/*                NO-LOCK NO-ERROR.                                 */
    ...
    FIND Provincia OF Rem_header NO-LOCK NO-ERROR.
    ...
    FIND Area OF Rem_header NO-LOCK.
    v-ingresante = Area.denominacion.
    FIND Empleado WHERE Empleado.nro_legajo = Rem_header.cdg_solicitante NO-LOCK NO-ERROR .
    FIND Area WHERE Area.cdg_area = Empleado.cdg_seccion NO-LOCK.
    FIND Sre_header WHERE Sre_header.nro_solicitud = Rem_header.nro_solicitud NO-LOCK NO-ERROR.
...
/*     PUT STREAM Formulario CONTROL "<AT=1,1><#2>". */
    ...
    RUN rectangulo ( INPUT "26,0,25,160" ).
 ...
 /* RUN escribir ( INPUT "40,10", INPUT "I.V.A. RESPONSABLE INSCRIPTO", INPUT 8, INPUT NO). */
    ...
    RUN escribir ( INPUT "47,1", INPUT "CLIENTE:" , INPUT 8, INPUT YES).
...
/*     PUT STREAM Formulario CONTROL "<AT=14,8><#1>". */
    ...
    RUN rectangulo ( INPUT "51,0,7,160" ).
    RUN rectangulo ( INPUT "58,0,80,160" ).

/*     RUN escribir ( "53,3", INPUT "CODIGO", INPUT 8, INPUT YES).        */
/*                                                                        */
/*     RUN linea    ( "51,18", INPUT "83", "V" ).                         */
/*     RUN escribir ( "53,34", INPUT "DESCRIPCION", INPUT 8, INPUT YES).  */
/*                                                                        */
/*     RUN linea    ( "51,104", INPUT "83", "V" ).                        */
/*     RUN escribir ( "53,107", INPUT "CANTIDAD", INPUT 8, INPUT YES).    */
/*                                                                        */
/*     RUN linea    ( "51,125", INPUT "83", "V" ).                        */
/*     RUN escribir ( "53,126", INPUT "Nº DE SERIE", INPUT 8, INPUT YES). */
/*                                                                        */
/*     /*                                                                 */
/*     RUN linea    ( "46,122", INPUT "87", "V" ).                        */
/*     RUN escribir ( "48,123", INPUT "RETORNO", INPUT 8, INPUT YES).     */
/*     */                                                                 */
/*                                                                        */
/*     RUN linea    ( "51,145", INPUT "83", "V" ).                        */
/*     RUN escribir ( "53,146", INPUT "ETIQUETA", INPUT 8, INPUT YES).    */

    RUN escribir ( "49,3", INPUT "CODIGO", INPUT 7, INPUT YES).

       RUN linea    ( "47,15", INPUT "87", "V" ).
       RUN escribir ( "49,16", INPUT "DESCRIPCION", INPUT 7, INPUT YES).
       RUN linea    ( "47,113", INPUT "87", "V" ).
       RUN escribir ( "49,116", INPUT "CANT.", INPUT 7, INPUT YES).

       RUN linea    ( "47,126", INPUT "87", "V" ).
       RUN escribir ( "49,127", INPUT "Nº SERIE", INPUT 7, INPUT YES).


       RUN linea    ( "47,139", INPUT "87", "V" ).
       RUN escribir ( "49,140", INPUT "RETOR", INPUT 7, INPUT YES).


       RUN linea    ( "47,150", INPUT "87", "V" ).                  
       RUN escribir ( "49,152", INPUT "IDENT", INPUT 7, INPUT YES). 
...
/*     PUT STREAM Formulario CONTROL "<AT=3,0><#2>". */
    ...
    RUN escribir ( INPUT "1,78", INPUT "X", INPUT 16, INPUT YES). 

    RUN escribir ( INPUT "4,95", INPUT STRING(Rem_header.prf_comprob,"9999") + "-" + STRING(Rem_header.nro_comprob,"99999999"), INPUT 10, INPUT YES). 
    ...
    RUN escribir ( INPUT "47,15", INPUT Cliente.nom_cliente, INPUT 8, INPUT YES).
/*     RUN escribir ( INPUT "51,85", INPUT "[" + Cliente.cdg_cliente + "]", INPUT 8, INPUT NO).  */
    RUN escribir ( INPUT "27,20", INPUT Cliente.direccion, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "32,21", INPUT "(" + Cliente.cdg_postal + ") " + Cliente.localidad, INPUT 8, INPUT NO).
...
/*     RUN escribir ( INPUT "63,85", INPUT "[" + Provlegal.cdg_provincia + "]", INPUT 8, INPUT NO).                                                                                          */
    RUN escribir ( INPUT "37,12", INPUT Cliente.cuit, INPUT 8, INPUT NO).
/*     RUN escribir ( INPUT "30,113", INPUT Empleado.nombre, INPUT 8, INPUT NO).  */
...
/*     RUN escribir ( INPUT "67,65", INPUT Cliente.ing_brutos, INPUT 8, INPUT YES).                                                                                                          */
    ...
    RUN escribir ( INPUT "144,20", INPUT Rem_header.nom_transportista, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "144,118", INPUT Rem_header.cuit_transportista, INPUT 8, INPUT NO).

    RUN escribir ( INPUT "147,15", INPUT Rem_header.domicilio_transportista, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "147,115", INPUT Rem_header.Ibrutos, INPUT 8, INPUT NO).

    RUN escribir ( INPUT "150,12", INPUT Rem_header.chofer, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "150,86", INPUT Rem_header.dni_transportista, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "150,122", INPUT Rem_header.patente, INPUT 8, INPUT NO).

    RUN escribir ( INPUT "153,15", INPUT Rem_header.precintos, INPUT 8, INPUT NO).

    RUN escribir ( INPUT "159,35", INPUT Rem_header.observacion, INPUT 8, INPUT NO).

    RUN escribir ( INPUT "200,112", INPUT Rem_header.cai, INPUT 8, INPUT NO).
    ...
    RUN escribir ( INPUT "40,113", INPUT  Rem_header.nro_solicitud, INPUT 8, INPUT NO).
...
/*     PUT STREAM Formulario CONTROL "<AT=14,8><#1>". */
    ...
    FOR EACH Rem_detalle OF Rem_header 
        ...
        THEN DO:
            ...
            RUN escribenumero ( ch_linea  + ",98",  INPUT STRING(v-cantidad,"ZZZZZZ9.99"), INPUT 7, INPUT NO).
            RUN escribir      ( ch_linea  + ",118", INPUT Unidad.abrevia, INPUT 7, INPUT NO).

            IF Articulo.es_registrable
            THEN DO:
                FOR EACH Registrable-remito OF Rem_detalle, FIRST Registrable OF Registrable-remito:
                    ...
                    RUN escribenumero ( ch_linea  + ",122",  INPUT Registrable.nro_serie, INPUT 7, INPUT NO).
                    linea0 = linea0 + 1.
                END.
            END.
            ELSE DO:
                FIND FIRST Etiqueta 
                     WHERE Etiqueta.nro_articulo     = Articulo.nro_articulo
                       AND Etiqueta.nro_registrable  = 0
                           NO-LOCK NO-ERROR.
                RUN escribenumero ( ch_linea  + ",141",  INPUT STRING(Etiqueta.num_etiqueta,"ZZZZZZ9"), INPUT 7, INPUT NO).
                linea0 = linea0 + 1.
            END.
