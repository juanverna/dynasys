...
DEFINE VARIABLE h_v-par-cuenta AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 15.52 COL 12 COLON-ALIGNED NO-LABEL
     "  Cuenta de refundición:" VIEW-AS TEXT
          SIZE 89 BY .81 AT ROW 2.33 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Número del último asiento renumerado:" VIEW-AS TEXT
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Asiento de Refundición de Pérdidas y Ganancias"
         HEIGHT             = 22.14
         WIDTH              = 114.2
         MAX-HEIGHT         = 22.14
         MAX-WIDTH          = 114.2
...
ON END-ERROR OF W-Win /* Asiento de Refundición de Pérdidas y Ganancias */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Asiento de Refundición de Pérdidas y Ganancias */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-par-cuenta.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-par-cuenta ).
       RUN set-position IN h_v-par-cuenta ( 3.95 , 14.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-par-refundir ( 8.81 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.33 , 89.00 ) */
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/GLA" "Asiento de Refundición de Pérdidas y Ganancias"}
...
------------------------------------------------------------------------------*/
  ...
  RUN dar_rango IN h_v-par-cuenta    ( OUTPUT v-cdg_cuenta, 
                                       OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Refundidora" ).
  ...
  THEN DO:
       ...
       RUN lsasnref.p (INPUT v-cdg_cuenta,
                       INPUT generar, 
                       INPUT v-tip_comprob,
                       INPUT v-nro_comprob,
                       INPUT ult_mes,
