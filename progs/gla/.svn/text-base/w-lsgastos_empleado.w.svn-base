...
DEFINE VARIABLE h_v-par_sueldos AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-rng-empleado AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 18.14 COL 12 COLON-ALIGNED NO-LABEL
     "  Rango de fechas de los asientos:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 6.48 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Parámetros generales del listado:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 10.29 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-rng-empleado.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rng-empleado ).
       RUN set-position IN h_v-rng-empleado ( 3.38 , 14.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 7.67 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-par_sueldos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-par_sueldos ).
       RUN set-position IN h_v-par_sueldos ( 11.48 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.19 , 100.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 19.38 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.62 , 100.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-empleado ,
             v-estado:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rng-fechas ,
             h_v-rng-empleado , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-par_sueldos ,
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE des_empleado     LIKE Empleado.nro_legajo.
  DEFINE VARIABLE has_empleado     LIKE Empleado.nro_legajo.
  
  DEFINE VARIABLE v-cambio_pago    LIKE Moneda.cambio.
  DEFINE VARIABLE v-generar        AS LOGICAL.
  ...
  RUN dar_rango IN h_v-rng-empleado ( OUTPUT des_empleado,
                                      OUTPUT has_empleado, 
                                      OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Empleados" ).
       hay_error = error_rango.
  END.     

  RUN dar_rango IN h_v-par_sueldos ( OUTPUT v-cdg_moneda,
                                     OUTPUT v-cambio_pago, 
                                     OUTPUT v-generar,
                                     OUTPUT v-salida,
                                     OUTPUT error_rango ).
  IF error_rango 
  ...
  THEN DO:
       ...
       RUN lsgastos_empleado.p ( INPUT des_fecha,
                                 INPUT has_fecha,
                                 INPUT des_empleado,
                                 INPUT has_empleado,
                                 INPUT v-cdg_moneda,
