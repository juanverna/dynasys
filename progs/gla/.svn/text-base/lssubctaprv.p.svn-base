/*=================================================================================*/
/*                MOVIMIENTOS DE SUBDIARIO PROVEEDORES POR CUENTA                  */
/*=================================================================================*/
...
DEFINE FRAME frm-movimiento
    Sub_header_prv.fecha       COLUMN-LABEL "Fecha!Movimto."
    Sub_detalle_prv.tip_comprob FORMAT "X(2)" COLUMN-LABEL "Tip.!Comp."
    Sub_detalle_prv.prf_comprob FORMAT ">>>9" COLUMN-LABEL "Pto.!Vta."
    Sub_detalle_prv.nro_comprob FORMAT ">>>>>>>9" COLUMN-LABEL "Número!Compbte."
...
PROCEDURE LISTAR:
  ...
  IF NOT exportacion 
     THEN OUTPUT TO VALUE (dire_tmp + "lssubctaprv.txt") PAGED PAGE-SIZE VALUE(lin_pagina).
     ELSE OUTPUT TO VALUE (dire_tmp + "lssubctaprv.txt").
      
  IF NOT exportacion THEN RUN PONE_CODIGO ( INPUT "SET17CPI,CARTA,SET8LPI" ).

  FOR EACH Cuenta 
      WHERE Cuenta.cdg_cuenta <= has_cuenta
        AND Cuenta.cdg_cuenta >= des_cuenta, EACH Sub_detalle_prv OF Cuenta,
                     FIRST Sub_header_prv OF Sub_detalle_prv
                           WHERE Sub_header_prv.fecha >= des_fecha 
                             AND Sub_header_prv.fecha <= has_fecha
                             AND Sub_header_prv.cdg_empresa <= Empresa.cdg_empresa
                             AND NOT Sub_header_prv.anulado, 
                                 FIRST Proveedor OF Sub_header_prv
                             BREAK BY Cuenta.cdg_cuenta BY Sub_header_prv.fecha:
       ...
       FIND FIRST Tipocomprobante OF Sub_header_prv NO-LOCK.
       IF Tipocomprobante.debita
          THEN v-debito  = Sub_detalle_prv.valor.
          ELSE v-credito = Sub_detalle_prv.valor.

        v-saldo = v-saldo + v-debito - v-credito.

        FIND Entidad OF Sub_detalle_prv NO-LOCK NO-ERROR.
        FIND Obra    OF Sub_detalle_prv NO-LOCK NO-ERROR.

        IF NOT exportacion 
        THEN DISPLAY    
                Sub_header_prv.fecha       
                Sub_detalle_prv.tip_comprob FORMAT "X(2)"
                Sub_detalle_prv.prf_comprob FORMAT ">>>9"
                Sub_detalle_prv.nro_comprob FORMAT ">>>>>>>9"
        ...
        THEN DO:
                ...
                THEN DO:
                        UNDERLINE 
                               Sub_header_prv.fecha       
                               Sub_detalle_prv.tip_comprob
                               Sub_detalle_prv.prf_comprob
                               Sub_detalle_prv.nro_comprob
                               Entidad.cdg_entidad
                               Obra.cdg_obra 
                               Proveedor.cdg_proveedor
                               Proveedor.nombre
