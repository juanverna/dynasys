...
DEFINE VARIABLE h_v-que-cuadro AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     v-estado AT ROW 15.62 COL 18 COLON-ALIGNED NO-LABEL
     "  Cuadro a emitir y fecha a la que deben expresarse los saldos:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 2.43 COL 20
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Moneda en la que deben expresare los saldos:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 6.95 COL 20
          BGCOLOR 5 FGCOLOR 15 FONT 6
     "  Rango de fechas de los movimientos a considerar:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 11.24 COL 20
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
ON END-ERROR OF W-Win /* Cuadros de Resultados */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Cuadros de Resultados */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-que-cuadro.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-que-cuadro ).
       RUN set-position IN h_v-que-cuadro ( 3.76 , 20.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-que-expresion ( 8.29 , 20.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN set-position IN h_v-rng-fechas ( 12.43 , 20.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 17.24 , 20.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.62 , 100.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-que-cuadro ,
             v-estado:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-que-expresion ,
             h_v-que-cuadro , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {setwintit.i "SIC/GLA" "Cuadros de Resultados"}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE v-cdg_cuadrores      LIKE Cuadro_resultado.cdg_cuadrores.
  ...
  RUN dar_rango IN h_v-rng-fechas    ( OUTPUT v-des_fecha, 
                                       OUTPUT v-has_fecha, 
                                       OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
  ...
  RUN dar_rango IN h_v-que-cuadro    ( OUTPUT v-cdg_cuadrores, 
                                       OUTPUT error_rango ).
  IF error_rango 
  THEN DO:
       RUN ver-error-rango ( "Cuadro de Resultados" ).
       hay_error = error_rango.
  END.     
  ...
  THEN DO:
       ...
       RUN lsresultados.p ( INPUT  v-cdg_cuadrores,
                            INPUT  v-des_fecha,
                            INPUT  v-has_fecha,
                            INPUT  v-cdg_moneda,
                            INPUT  v-reexpresion).   
