...
PROCEDURE imprimir:
    ...
    DO ncopia = 1 TO 2:
        ...
        PUT STREAM Formulario CONTROL "<AT=5,185><#1>".   
...
/*     PUT STREAM Formulario CONTROL "<AT=14,8><#1>". */
    ...
    RUN escribir ( "48,3", INPUT "CODIGO", INPUT 8, INPUT YES).
        
    RUN linea    ( "46,18", INPUT "87", "V" ).
    RUN escribir ( "48,34", INPUT "DESCRIPCION", INPUT 8, INPUT YES).
    
    RUN linea    ( "46,104", INPUT "87", "V" ).
    RUN escribir ( "48,107", INPUT "CANTIDAD", INPUT 8, INPUT YES).

    RUN linea    ( "46,125", INPUT "87", "V" ).
    RUN escribir ( "48,128", INPUT "Nº DE SERIE", INPUT 8, INPUT YES).
    ...
    */
    ...
    RUN linea    ( "46,147", INPUT "87", "V" ).
    RUN escribir ( "48,149", INPUT "IDENT", INPUT 8, INPUT YES).
...
/*     PUT STREAM Formulario CONTROL "<AT=14,8><#1>". */
    ...
    FOR EACH Rem_detalle_prv OF Rem_header_prv 
        ...
        THEN DO:
            ...
            RUN escribenumero ( ch_linea  + ",99",  INPUT STRING(v-cantidad,"ZZZZZZ9.99"), INPUT 7, INPUT NO).
            RUN escribir      ( ch_linea  + ",119", INPUT Unidad.abrevia, INPUT 7, INPUT NO).
            ...
            THEN DO:
                FOR EACH Registrable-remprov OF Rem_detalle_prv, FIRST Registrable OF Registrable-remprov:
                    ...
                    RUN escribir ( ch_linea  + ",126",  INPUT Registrable.nro_serie, INPUT 5, INPUT NO).
                    linea0 = linea0 + 1.
                END.
            END.
            ELSE DO:
...
/*     PUT STREAM Formulario CONTROL "<AT=235,8><#1>". */
    ...
    */
    ...
    RUN escribir ( "198,100", INPUT "C.A.I nº:", INPUT 8, INPUT YES).
    RUN escribir ( "203,100", INPUT "Fecha Vto:", INPUT 8, INPUT YES).
...
/*     PUT STREAM Formulario CONTROL "<AT=235,8><#1>". */
    ...
    RUN escribir ( INPUT "213,35", INPUT Rem_header_prv.observacion, INPUT 8, INPUT NO).

    RUN escribir ( INPUT "198,112", INPUT Rem_header_prv.cai, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "203,115", INPUT STRING(Rem_header_prv.rige_hasta,"99/99/9999"), INPUT 10, INPUT NO).

    RUN escribebarra ( INPUT "200,50", INPUT STRING(Rem_header_prv.prf_comprob,"9999") + "/" + STRING(Rem_header_prv.nro_comprob,"99999999"), INPUT 9, INPUT no).
  /*RUN escribebarra ( INPUT "200,50", INPUT STRING(Rem_header_prv.prf_comprob,"9999") + STRING(Rem_header_prv.nro_comprob,"99999999"), INPUT 9, INPUT no).*/
...
PROCEDURE escribebarra:
    ...
    PUT STREAM Formulario UNFORMATTED "<=#2><FROM><BARCODE#3,TYPE=128A,CHECKSUM=TRUE,VALUE="   texto ">".
