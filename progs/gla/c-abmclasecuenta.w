...
DEFINE VARIABLE v-nombre_subclasecta   LIKE Clase_de_Cuenta.nombre_subclasecta.
DEFINE VARIABLE v-rotulo_siguiente     LIKE Clase_de_Cuenta.rotulo_siguiente.
DEFINE VARIABLE v-longitud_siguiente   LIKE Clase_de_Cuenta.longitud_siguiente.
DEFINE VARIABLE v-tipo_siguiente       LIKE Clase_de_Cuenta.tipo_siguiente.
...
&Scoped-define QUERY-STRING-brw_clasificacion FOR EACH Clase_de_Cuenta ~
      WHERE Clase_de_Cuenta.cdg_clasecta = que_clase ~
 AND Clase_de_Cuenta.cdg_librocontable = que_libro ~
 AND Clase_de_Cuenta.cdg_empresa = que_empresa NO-LOCK
...
&Scoped-Define ENABLED-OBJECTS v-libro que_nombre que_clase que_subclase ~
camino brw_clasificacion btn_eliminar Btn_Elegir btn_modificar Btn_Salir 
&Scoped-Define DISPLAYED-OBJECTS v-libro que_nombre que_clase que_subclase ~
camino 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brw_clasificacion C-Win _STRUCTURED
  QUERY brw_clasificacion DISPLAY
      SUBSTRING(Clase_de_cuenta.cdg_subclasecta,LENGTH(que_clase) + 2) COLUMN-LABEL "Código!Clase" FORMAT "X(8)":U
      Clase_de_Cuenta.nombre_subclasecta COLUMN-LABEL "Denominacion!Clasificación" FORMAT "X(50)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 69 BY 21.48
         FONT 4
...
DEFINE FRAME DEFAULT-FRAME
     v-libro AT ROW 2.33 COL 3 NO-LABEL
     que_nombre AT ROW 3.62 COL 12 COLON-ALIGNED NO-LABEL
     que_clase AT ROW 3.62 COL 72 COLON-ALIGNED NO-LABEL
     que_subclase AT ROW 3.67 COL 3 NO-LABEL
     camino AT ROW 4.71 COL 74 NO-LABEL
     brw_clasificacion AT ROW 4.76 COL 3
     btn_eliminar AT ROW 26.48 COL 46
     Btn_Elegir AT ROW 26.48 COL 74
     btn_modificar AT ROW 26.52 COL 3
     Btn_Salir AT ROW 26.52 COL 132
     "    Seleccione el libro contable cuya clasificación desea mantener:" VIEW-AS TEXT
          SIZE 155 BY .81 AT ROW 1.29 COL 3
          BGCOLOR 5 FGCOLOR 15 FONT 5
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Mantenimiento de Clases de Cuentas"
         HEIGHT             = 27.67
         WIDTH              = 160
         MAX-HEIGHT         = 27.67
         MAX-WIDTH          = 160
...
/* SETTINGS FOR FRAME DEFAULT-FRAME
                                                                        */
/* BROWSE-TAB brw_clasificacion camino DEFAULT-FRAME */
...
/* Query rebuild information for BROWSE brw_clasificacion
 ...
 AND Clase_de_Cuenta.cdg_librocontable = que_libro
 AND Clase_de_Cuenta.cdg_empresa = que_empresa"
     _FldNameList[1]   > "_<CALC>"
"SUBSTRING(Clase_de_cuenta.cdg_subclasecta,LENGTH(que_clase) + 2)" "Código!Clase" "X(8)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > sic.Clase_de_Cuenta.nombre_subclasecta
"nombre_subclasecta" "Denominacion!Clasificación" "X(50)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _Query            is OPENED
...
ON END-ERROR OF C-Win /* Mantenimiento de Clases de Cuentas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Mantenimiento de Clases de Cuentas */
DO:
...
DO:
   ...
   IF CAN-FIND(FIRST B-Clase_de_cuenta WHERE B-Clase_de_cuenta.cdg_clasecta = Clase_de_cuenta.cdg_subclasecta)
   THEN DO:
       RUN PONMENSJ.P (INPUT "CLAS001" ).
   END.    
   ELSE DO:                                                 
       DO TRANSACTION:
          FIND CURRENT Clase_de_cuenta EXCLUSIVE-LOCK.   
          DELETE Clase_de_cuenta.
       END.
       RUN ABRE_QUERY.
   END.    
...
DO:
   DO TRANSACTION:
        ...
        DISPLAY Clase_de_cuenta.nombre_subclasecta
...
ON VALUE-CHANGED OF v-libro IN FRAME DEFAULT-FRAME
DO:
  RUN poner_libro.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  ASSIGN v-libro:SCREEN-VALUE  = Librocontable.dsc_librocontable.
  RUN poner_libro.
  RUN enable_UI.
  {setwintit.i "SIC/GLA" "Clasificación de Cuentas Contables"}
...
------------------------------------------------------------------------------*/
   ...
   RUN d-actualizar_nodo.w ( INPUT "Creación de nuevos nodos",
                             INPUT-OUTPUT v-nombre_subclasecta,
   ...
   THEN DO TRANSACTION:
       ...
       ASSIGN Clase_de_cuenta.cdg_empresa          = que_empresa
              Clase_de_cuenta.cdg_librocontable    = que_libro                                 
              Clase_de_cuenta.cdg_clasecta         = que_clase                                 
              Clase_de_cuenta.cdg_subclasecta      = que_clase + "." + que_subclase
              Clase_de_Cuenta.nombre_subclasecta   = v-nombre_subclasecta
              Clase_de_Cuenta.rotulo_siguiente     = v-rotulo_siguiente
              Clase_de_Cuenta.longitud_siguiente   = v-longitud_siguiente
              Clase_de_Cuenta.tipo_siguiente       = v-tipo_siguiente.
       FIND CURRENT Clase_de_Cuenta NO-LOCK.
   END.
...
------------------------------------------------------------------------------*/
  DISPLAY v-libro que_nombre que_clase que_subclase camino 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE v-libro que_nombre que_clase que_subclase camino brw_clasificacion 
         btn_eliminar Btn_Elegir btn_modificar Btn_Salir 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
     ...
     ASSIGN v-libro:DELIMITER     = "!"
            v-libro:LIST-ITEMS    = "".
            
     FOR EACH Librocontable NO-LOCK BY Librocontable.dsc_librocontable:
         ok = v-libro:ADD-LAST(Librocontable.dsc_librocontable).
     END.
...
------------------------------------------------------------------------------*/
   ...
   ASSIGN v-nombre_subclasecta   = Clase_de_Cuenta.nombre_subclasecta
          v-rotulo_siguiente     = Clase_de_Cuenta.rotulo_siguiente
          v-longitud_siguiente   = Clase_de_Cuenta.longitud_siguiente
          v-tipo_siguiente       = Clase_de_Cuenta.tipo_siguiente.
   ...
   THEN DO TRANSACTION:
       FIND CURRENT Clase_de_Cuenta EXCLUSIVE-LOCK.
       ASSIGN Clase_de_Cuenta.nombre_subclasecta   = v-nombre_subclasecta
              Clase_de_Cuenta.rotulo_siguiente     = v-rotulo_siguiente
              Clase_de_Cuenta.longitud_siguiente   = v-longitud_siguiente
              Clase_de_Cuenta.tipo_siguiente       = v-tipo_siguiente.
       FIND CURRENT Clase_de_Cuenta NO-LOCK.
...
------------------------------------------------------------------------------*/
  ...
  FIND FIRST Librocontable 
       WHERE Librocontable.dsc_librocontable = 
             v-libro:SCREEN-VALUE IN FRAME {&FRAME-NAME}.
             
  que_libro = Librocontable.cdg_librocontable.
  que_clase = "". /* Cuando cambiamos de libro, siempre arrancamos de arriba */          
  que_subclase = "".
  que_nombre = "".
