/*=================================================================================*/
/*              IMPRESION DE REMITO DE EGRESOS DE BIENES A DEPOSITOS               */
/*=================================================================================*/
...
DEFINE VARIABLE v-cantidad     LIKE Transdep_dt.cantidad.
...
/* DEFINE VARIABLE importe_iva    LIKE Transdep_hd.imp_neto FORMAT "Z,ZZZ,ZZ9.99-".  */
/* DEFINE VARIABLE importe_noi    LIKE Transdep_hd.imp_neto FORMAT "Z,ZZZ,ZZ9.99-".  */

/* DEFINE VARIABLE v-imp_gravado  LIKE Transdep_hd.imp_neto FORMAT "Z,ZZZ,ZZ9.99-".  */
/* DEFINE VARIABLE v-imp_exento   LIKE Transdep_hd.imp_neto FORMAT "Z,ZZZ,ZZ9.99-".  */
...
DO TRANSACTION:
    ...
    FIND Transdep_hd WHERE ROWID(Transdep_hd) = rid_factura EXCLUSIVE-LOCK.   
    v-reimpresion = IF Transdep_hd.impreso = "S" THEN "REIMPRESION" ELSE "". 
    
    FIND Punto-venta WHERE Punto-venta.cdg_empresa = Transdep_hd.cdg_empresa  
                       AND Punto-venta.cdg_puntovta = Transdep_hd.prf_comprob 
                           NO-LOCK NO-ERROR.                                 
    
    FIND Tipocomprobante OF Transdep_hd NO-LOCK.
    FIND Empresa OF Transdep_hd NO-LOCK.
/*     FIND Condicion_impos OF Transdep_hd NO-LOCK.  */
/*     FIND Condicion_venta OF Transdep_hd NO-LOCK.  */
/*     FIND Provincia OF Transdep_hd NO-LOCK NO-ERROR.  */
   
    FIND Provlegal OF Deposito NO-LOCK NO-ERROR.    
/*     FIND Domicilio OF Transdep_hd NO-LOCK NO-ERROR.  */
    FIND Usuario OF Transdep_hd NO-LOCK.
    FIND Area OF Transdep_hd NO-LOCK.
    v-ingresante = Area.denominacion.
    
    FIND Empleado WHERE Empleado.nro_legajo = Transdep_hd.cdg_solicitante NO-LOCK .
    FIND Area WHERE Area.cdg_area = Empleado.cdg_seccion NO-LOCK.
    FIND Sre_header WHERE Sre_header.nro_solicitud = Transdep_hd.nro_solicitud NO-LOCK.
    FIND Destinatario OF Sre_header NO-LOCK NO-ERROR.
    FIND Deposito OF Destinatario NO-LOCK.
/*     FIND Sre_header OF Transdep_hd NO-LOCK.  */
    ...
    que_mes = STRING(MONTH(Transdep_hd.fecha),"99").
    que_ano = STRING(YEAR(Transdep_hd.fecha),"9999").
    que_dia = STRING(DAY(Transdep_hd.fecha),"99").
    str_fecha = que_dia + " de " + nom_mes [ MONTH(Transdep_hd.fecha) ] + " de " + que_ano.
        
    Transdep_hd.impreso = "S".    
    ...
    FOR EACH Transdep_dt OF Transdep_hd 
             WHERE Transdep_dt.cantidad <> 0 OR Transdep_dt.granel <> 0 OR TRUE:
...
/*     RUN escribir ( INPUT "17,25", INPUT Punto-venta.localidad + " - " + Punto-venta.provincia, INPUT 6, INPUT NO).  */
    ...
    RUN escribir ( INPUT "0,90", INPUT Tipocomprobante.denominacion_impresa + " EGRESO DE BIENES", INPUT 10, INPUT YES).
...
/*     PUT STREAM Formulario CONTROL "<AT=3,0><#2>". */
    ...
    RUN escribir ( INPUT "4,95", INPUT STRING(Transdep_hd.prf_comprob,"9999") + "-" + STRING(Transdep_hd.nro_comprob,"99999999"), INPUT 10, INPUT YES). 
    ...
    RUN escribir ( INPUT "26,1", INPUT Destinatario.dsc_destinatario, INPUT 8, INPUT YES).  
    RUN escribir ( INPUT "30,1", INPUT Deposito.direccion, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "34,1", INPUT "(" + Deposito.cdg_postal + ") " + Deposito.localidad, INPUT 8, INPUT NO).
    ...
    RUN escribir ( INPUT "40,113", INPUT  Transdep_hd.nro_solicitud, INPUT 8, INPUT NO).
...
/*     RUN escribir ( INPUT "8,103", INPUT que_dia + "/" + que_mes + "/" + que_ano, INPUT 10, INPUT YES).                                                                                                  */
/*     RUN escribir ( INPUT "4,150", INPUT TRIM(STRING(n-hoja,">>9")) + "/" + TRIM(STRING(nt-hojas,">>9")), INPUT 10, INPUT YES).                                                                          */
/*     RUN escribir ( INPUT "26,1", INPUT Deposito.nom_Deposito, INPUT 8, INPUT YES).                                                                                                                      */
/* /*     RUN escribir ( INPUT "51,85", INPUT "[" + Deposito.cdg_proveedor + "]", INPUT 8, INPUT NO).  */                                                                                                  */
/*     RUN escribir ( INPUT "30,1", INPUT Deposito.direccion, INPUT 8, INPUT NO).                                                                                                                          */
/*     RUN escribir ( INPUT "34,1", INPUT "(" + Deposito.cdg_postal + ") " + Deposito.localidad, INPUT 8, INPUT NO).                                                                                       */
...
/*     RUN escribir ( INPUT "38,12", INPUT Deposito.cuit, INPUT 8, INPUT NO).                                                                                                                              */
/*     RUN escribir ( INPUT "30,113", INPUT v-ingresante, INPUT 8, INPUT NO).                                                                                                                              */
/*                                                                                                                                                                                                         */
/* /*     RUN escribir ( INPUT "67,65", INPUT Deposito.ing_brutos, INPUT 8, INPUT YES).                                                                                                          */        */
...
/* /*     RUN escribir ( INPUT "84,170", INPUT Transdep_hd.nro_ocm, INPUT 8, INPUT NO).                                                                                                          */        */
/* /*                                                                                                                                                                                           */         */
...
/*  PUT STREAM Formulario CONTROL "<AT=14,8><#1>". */
    ...
    FOR EACH Transdep_dt OF Transdep_hd 
        ...
        THEN DO:
            ...
            */
            ...
            THEN DO:
                 ...
                 FOR EACH Registrable-transdep OF Transdep_dt, FIRST Registrable OF Registrable-remito:

                    FIND FIRST Etiqueta 
                         WHERE Etiqueta.nro_articulo     = Articulo.nro_articulo
                           AND Etiqueta.nro_registrable  = Registrable-transdep.nro_registrable
                               NO-LOCK.
        ...
        ELSE DO:
            ...
            RUN RENGLONS.P (INPUT  Transdep_dt.detallada, 
    ...
    IF Transdep_hd.leyenda <> ""
    THEN DO:
                      
        linea0 = linea0 + 2.

        RUN RENGLONS.P (INPUT  Transdep_hd.leyenda, 
...
/*     PUT STREAM Formulario CONTROL "<AT=235,8><#1>". */
    ...
    RUN escribir ( INPUT "144,20", INPUT Transdep_hd.nom_transportista, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "144,118", INPUT Transdep_hd.cuit_transportista, INPUT 8, INPUT NO).

    RUN escribir ( INPUT "147,15", INPUT Transdep_hd.domicilio_transportista, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "147,115", INPUT Transdep_hd.Ibrutos, INPUT 8, INPUT NO).

    RUN escribir ( INPUT "150,12", INPUT Transdep_hd.chofer, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "150,86", INPUT Transdep_hd.dni_transportista, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "150,122", INPUT Transdep_hd.patente, INPUT 8, INPUT NO).

    RUN escribir ( INPUT "153,15", INPUT Transdep_hd.precintos, INPUT 8, INPUT NO).

    RUN escribir ( INPUT "159,28", INPUT Transdep_hd.observacion, INPUT 8, INPUT NO).
    ...
    RUN escribir ( INPUT "196,122", INPUT Transdep_hd.cai, INPUT 8, INPUT NO).
    RUN escribir ( INPUT "199,127", INPUT STRING(Transdep_hd.rige_hasta,"99/99/9999"), INPUT 14, INPUT NO).

    RUN escribebarra ( INPUT "197,50", INPUT STRING(Transdep_hd.prf_comprob,"9999") + STRING(Transdep_hd.nro_comprob,"99999999"), INPUT 9, INPUT no).
