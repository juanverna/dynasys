...
DEFINE BUFFER B-Clase_de_Entidad FOR Clase_de_Entidad.

DEFINE VARIABLE f-que_clase LIKE Clase_de_Entidad.cdg_subclaseent.
...
DEFINE VARIABLE v-nombre_subclaseent   LIKE Clase_de_Entidad.nombre_subclaseent.
DEFINE VARIABLE v-rotulo_siguiente     LIKE Clase_de_Entidad.rotulo_siguiente.
DEFINE VARIABLE v-longitud_siguiente   LIKE Clase_de_Entidad.longitud_siguiente.
DEFINE VARIABLE v-tipo_siguiente       LIKE Clase_de_Entidad.tipo_siguiente.
...
&Scoped-define INTERNAL-TABLES Clase_de_Entidad

/* Definitions for BROWSE brw_clasificacion                             */
&Scoped-define FIELDS-IN-QUERY-brw_clasificacion ~
SUBSTRING(Clase_de_Entidad.cdg_subclaseent,LENGTH(que_clase) + 2) ~
Clase_de_Entidad.nombre_subclaseent 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brw_clasificacion 
&Scoped-define QUERY-STRING-brw_clasificacion FOR EACH Clase_de_Entidad ~
      WHERE Clase_de_Entidad.cdg_claseent = que_clase NO-LOCK
&Scoped-define OPEN-QUERY-brw_clasificacion OPEN QUERY brw_clasificacion FOR EACH Clase_de_Entidad ~
      WHERE Clase_de_Entidad.cdg_claseent = que_clase NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brw_clasificacion Clase_de_Entidad
&Scoped-define FIRST-TABLE-IN-QUERY-brw_clasificacion Clase_de_Entidad
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brw_clasificacion C-Win _STRUCTURED
  QUERY brw_clasificacion DISPLAY
      SUBSTRING(Clase_de_Entidad.cdg_subclaseent,LENGTH(que_clase) + 2) COLUMN-LABEL "Código!Clase" FORMAT "X(8)":U
      Clase_de_Entidad.nombre_subclaseent COLUMN-LABEL "Denominacion!Clasificación" FORMAT "X(50)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 67 BY 23.57
         FONT 4
...
DEFINE FRAME DEFAULT-FRAME
     que_nombre AT ROW 1.24 COL 12 COLON-ALIGNED NO-LABEL
     ...
     camino AT ROW 2.67 COL 72 NO-LABEL
     btn_modificar AT ROW 26.48 COL 3
     btn_eliminar AT ROW 26.48 COL 44
...
/* Query rebuild information for BROWSE brw_clasificacion
     _TblList          = "sic.Clase_de_Entidad"
     _Where[1]         = "Clase_de_Entidad.cdg_claseent = que_clase"
     _FldNameList[1]   > "_<CALC>"
"SUBSTRING(Clase_de_Entidad.cdg_subclaseent,LENGTH(que_clase) + 2)" "Código!Clase" "X(8)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > sic.Clase_de_Entidad.nombre_subclaseent
"nombre_subclaseent" "Denominacion!Clasificación" "X(50)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _Query            is OPENED
...
DO:
   ...
   que_subclase = SUBSTRING(Clase_de_Entidad.cdg_subclaseent,LENGTH(que_clase) + 2).
...
DO:
   ...
   IF CAN-FIND(FIRST B-Clase_de_Entidad WHERE B-Clase_de_Entidad.cdg_claseent = Clase_de_Entidad.cdg_subclaseent)
   THEN DO:
       RUN PONMENSJ.P (INPUT "CLAS001" ).
   END.    
   ELSE DO:                                                 
       DO TRANSACTION:
          FIND CURRENT Clase_de_Entidad EXCLUSIVE-LOCK.   
          DELETE Clase_de_Entidad.
       END.
       RUN ABRE_QUERY.
   END.    
...
DO:
   DO TRANSACTION:
        ...
        DISPLAY Clase_de_Entidad.nombre_subclaseent
...
DO:
   ...
   THEN DO:
      ...
      ELSE DO:
         ...
         THEN DO:
            p_punto = p_punto - 1.
            que_clase = SUBSTRING(que_clase,1,p_punto).
            FIND FIRST Clase_de_Entidad WHERE Clase_de_Entidad.cdg_subclaseent = que_clase.
            que_nombre = Clase_de_Entidad.nombre.
            RUN armar_rotulo.
         END.
         ELSE DO:
            que_clase = "".
   ...
   ELSE DO:
      ...
      FIND FIRST Clase_de_Entidad WHERE Clase_de_Entidad.cdg_claseent = que_clase 
                         AND Clase_de_Entidad.cdg_subclaseent = que_clase + "." + que_subclase NO-ERROR.

      IF NOT AVAILABLE Clase_de_Entidad
      THEN DO:
         que_nombre = "".
         RUN crear_clasificacion.         
      END.
      ELSE DO:
         que_nombre = Clase_de_Entidad.nombre_subclaseent.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  RUN enable_UI.
  {setwintit.i "SIC/GLA" "Clasificación de Entidades Contables"}
...
------------------------------------------------------------------------------*/
/*
  IF AVAILABLE Clase_de_Entidad
     THEN rotulo = FILL(" ",l_rotulo - LENGTH(Clase_de_Entidad.rotulo_siguiente) - 1) + Clase_de_Entidad.rotulo_siguiente + ":".           ELSE rotulo = FILL(" ",l_rotulo - 1) + ":".
*/
...
------------------------------------------------------------------------------*/
   ...
   RUN d-actualizar_nodo.w ( INPUT "Creación de nuevos nodos",
                             INPUT-OUTPUT v-nombre_subclaseent,
   ...
   IF como_fue
   THEN DO TRANSACTION:
       CREATE Clase_de_Entidad.
       ASSIGN Clase_de_Entidad.cdg_claseent         = que_clase                                 
              Clase_de_Entidad.cdg_subclaseent      = que_clase + "." + que_subclase
              Clase_de_Entidad.nombre_subclaseent   = v-nombre_subclaseent
              Clase_de_Entidad.rotulo_siguiente     = v-rotulo_siguiente
              Clase_de_Entidad.longitud_siguiente   = v-longitud_siguiente
              Clase_de_Entidad.tipo_siguiente       = v-tipo_siguiente.
       FIND CURRENT Clase_de_Entidad NO-LOCK.
   END.
...
------------------------------------------------------------------------------*/
  ...
  FIND FIRST Clase_de_entidad NO-LOCK NO-ERROR.
  IF NOT AVAILABLE Clase_de_entidad
  THEN DO:
       DO TRANSACTION:
          CREATE Clase_de_entidad.
          ASSIGN Clase_de_Entidad.cdg_claseent    = ?
                 Clase_de_Entidad.cdg_subclaseent = "".
       END.
       RELEASE Clase_de_entidad.
  END.
...
------------------------------------------------------------------------------*/
   ...
   ASSIGN v-nombre_subclaseent   = Clase_de_Entidad.nombre_subclaseent
          v-rotulo_siguiente     = Clase_de_Entidad.rotulo_siguiente
          v-longitud_siguiente   = Clase_de_Entidad.longitud_siguiente
          v-tipo_siguiente       = Clase_de_Entidad.tipo_siguiente.
   ...
   THEN DO TRANSACTION:
       FIND CURRENT Clase_de_Entidad EXCLUSIVE-LOCK.
       ASSIGN Clase_de_Entidad.nombre_subclaseent   = v-nombre_subclaseent
              Clase_de_Entidad.rotulo_siguiente     = v-rotulo_siguiente
              Clase_de_Entidad.longitud_siguiente   = v-longitud_siguiente
              Clase_de_Entidad.tipo_siguiente       = v-tipo_siguiente.
       FIND CURRENT Clase_de_Entidad NO-LOCK.
