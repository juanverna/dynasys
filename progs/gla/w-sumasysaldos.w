...
DEFINE FRAME F-Main
     v-estado AT ROW 17.05 COL 12 COLON-ALIGNED NO-LABEL
     "  Rango de fechas de los asientos:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 7.95 COL 14
     ...
     "  Moneda de expresión del listado:" VIEW-AS TEXT
          SIZE 100 BY 1 AT ROW 3.62 COL 14
          BGCOLOR 5 FGCOLOR 15 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Listado de Sumas y Saldos Clasificado"
         HEIGHT             = 23.33
         WIDTH              = 122.4
         MAX-HEIGHT         = 23.33
         MAX-WIDTH          = 122.4
...
ON END-ERROR OF W-Win /* Listado de Sumas y Saldos Clasificado */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Listado de Sumas y Saldos Clasificado */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-que-expresion ( 5.05 , 14.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-rng-fechas ( 9.29 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 100.00 ) */
       ...
       RUN set-position IN h_v-par-libmayor ( 13.38 , 14.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-ejecutar ).
       RUN set-position IN h_p-ejecutar ( 18.67 , 14.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  RUN habilitar_botones IN h_p-ejecutar ( INPUT "salir=SI,ejecutar=SI,imprimir=NO,verlistado=NO" ).
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE v-nom_balance    AS CHARACTER INITIAL "ZZZ".
  ...
  THEN DO:
       ...
       RUN lsbalance_saldos.p ( INPUT  des_fecha,
                                INPUT  has_fecha,
                                INPUT  v-cdg_moneda,
                                INPUT  v-reexpresion, 
                                INPUT  v-nom_balance, /* Inicialmente en blanco */
                                INPUT  lis_fecha,
                                INPUT  lin_pagina,
                                INPUT  ult_pagina,
                                INPUT  todas_cuent ).   
